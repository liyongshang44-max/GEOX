...
export type SkillRegistryDeps<TSkill extends RegistryRuleSkill> = {
  ...
  appendSkillBindingFact: (pool: Pool, input: {
    ...
    priority: number;
  }) => Promise<AppendedSkillBinding>;
  projectSkillRegistryReadV1: (pool: Pool, input: {
    tenant_id: string;
    project_id: string;
    group_id: string;
  }) => Promise<unknown>;
  querySkillRegistryReadV1: (pool: Pool, input: {
    tenant_id: string;
    project_id: string;
    group_id: string;
    crop_code?: string;
    fact_type?: "skill_definition_v1" | "skill_binding_v1" | "skill_run_v1";
  }) => Promise<RegistryRow[]>;
};
...
export function createSkillRegistry<TSkill extends RegistryRuleSkill>(deps: SkillRegistryDeps<TSkill>) {
  ...
  function toBindingRecord(row: RegistryRow): SkillBindingRecord {
    return {
  ...
  function configureSkillBindingsPool(pool: Pool): void {
    bindingsPool = pool;
  }

  async function listSkillBindings(input?: {
    crop_code?: string;
    tenant_id?: string;
    project_id?: string;
    group_id?: string;
    ...
    const rows = await deps.querySkillRegistryReadV1(bindingsPool, {
      tenant_id: input.tenant_id,
      project_id: input.project_id,
      group_id: input.group_id,
  ...
  async function switchSkillBinding(input: {
    skill_id: string;
    version: string;
    enabled: boolean;
    priority?: number;
    ...
    const appended = await deps.appendSkillBindingFact(bindingsPool, {
      tenant_id,
      project_id,
      group_id,
    ...
    await deps.projectSkillRegistryReadV1(bindingsPool, { tenant_id, project_id, group_id });
  ...
  async function getRuleSkills(input: {
    crop_code: string;
    tenant_id: string;
    project_id?: string;
    group_id?: string;
