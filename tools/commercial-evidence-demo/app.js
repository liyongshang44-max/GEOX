...
function renderPersistedTrace(wrapper, operatorBaseUrl, decisionCycleId) {
  ...
  if (!trace || !trace.system_derived) throw new Error("TWIN_TRACE_READ_MODEL_MISSING");
  const order = [
  ...
  byId("traceObjects").innerHTML = order.map((objectType) => {
  ...
  byId("traceStatus").className = "status-line good-text";
  byId("traceStatus").textContent = `Persisted trace loaded · read_only=${trace.read_only} · stage=${answers.current_stage ?? "—"} · forbidden_auto_writes_absent=${answers.forbidden_auto_writes_absent ?? "—"}`;
  const href = `${operatorBaseUrl}/operator/twin/traces/${encodeURIComponent(decisionCycleId)}`;
  byId("traceActions").innerHTML = `<a class="primary-link" href="${escapeHtml(href)}" target="_blank" rel="noreferrer">Open full GEOX Twin Trace ↗</a>`;
}

async function loadTrace(packet, decisionCycleId) {
  const id = String(decisionCycleId ?? "").trim();
  if (!id) {
    byId("traceStatus").className = "status-line blocked-text";
    byId("traceStatus").textContent = "decision_cycle_id is required.";
    return;
  }
  byId("traceStatus").className = "status-line muted";
  byId("traceStatus").textContent = "Loading persisted Twin Trace...";
  byId("traceObjects").innerHTML = "";
  byId("traceActions").innerHTML = "";
...
async function main() {
  ...
  byId("loadTraceButton").addEventListener("click", () => {
    loadTrace(packet, byId("decisionCycleId").value).catch((error) => {
      byId("traceStatus").className = "status-line blocked-text";
      byId("traceStatus").textContent = `Trace load failed: ${error.message}`;
    });
  });
  if (initialTraceId) await loadTrace(packet, initialTraceId);
}
