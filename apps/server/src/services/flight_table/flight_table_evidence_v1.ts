...
function updateStep(steps: FlightTableStepV1[], key: string, status: FlightTableStepV1["status"], message: string): FlightTableStepV1[] {
  ...
  return steps.map((step) => step.step_key === key ? {
    ...
    finished_at: ["PASS", "FAIL", "SKIPPED"].includes(status) ? ts : undefined,
    updated_at: ts,
  } : step);
}
...
function devArtifactsForScenario(params: {
  evidence_id: string;
  run_id: string;
  operation_id: string;
  operation_plan_id: string;
  ...
  const base = { run_id, operation_id, operation_plan_id, act_task_id, receipt_id, field_id, lane, evidence_id, evidence_status: scenario.evidence_status, sha256, source_lane: "SIMULATED_DEV_ONLY", formal_eligible: false };
  return [
...
export async function runFlightTableEvidenceAcceptanceExportV1(args: {
  ...
  const snapshot = await writeFlightTableApiSnapshotV1({
    ...
    ok: true,
    status_code: 200,
    label: "dev-only evidence export flight",
    request: { lane, operation_id, act_task_id, receipt_id, field_id },
    response: { evidence_status: scenario.evidence_status, acceptance_status, final_status, evidence_export_job_id, normalized_export_status: "DONE", sha256: evidence.sha256, source_lane: "SIMULATED_DEV_ONLY", formal_eligible: false },
  });

  const steps = updateStep(run.steps, "G", "PASS", `dev-only evidence=${scenario.evidence_status}; acceptance=${acceptance_status}; final=${final_status}; formal_eligible=false`);
