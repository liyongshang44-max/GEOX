...
function evidenceRefIdsV1(value: unknown): string[] {
  ...
  for (const ref of refs) {
    ...
    for (const key of ["ref_id", "ref", "fact_id", "artifact_id", "evidence_id"]) {
      const id = textOrNull(obj[key]);
      if (id) {
        ids.push(id);
        break;
      }
...
async function loadAcceptanceResultForMemoryV1(db: DbConn, tenant: TenantTriple, input: { operation_plan_id: string; acceptance_id: string }): Promise<AcceptanceResultForMemoryV1 | null> {
  ...
  if ((q.rows ?? []).length > 1) throw new Error("ACCEPTANCE_AMBIGUOUS");
...
async function loadEvidenceArtifactObservationPairV1(db: DbConn, tenant: TenantTriple, acceptance: AcceptanceResultForMemoryV1, operationPlanId: string): Promise<ObservationPairV1 | null> {
  const evidenceRefs = evidenceRefIdsV1(acceptance.payload?.evidence_refs);
  ...
  if (pairs.length > 1) throw new Error("OBSERVATION_PAIR_AMBIGUOUS");
  return pairs[0] ?? null;
}
...
async function loadReceiptObservationPairV1(db: DbConn, tenant: TenantTriple, acceptance: AcceptanceResultForMemoryV1, operationPlanId: string): Promise<ObservationPairV1 | null> {
  const receiptId = textOrNull(acceptance.payload?.receipt_id);
  ...
  const q = await db.query(
    `SELECT fact_id, record_json::jsonb AS record_json
      ...
      ORDER BY fact_id
      LIMIT 2`,
    [tenant.tenant_id, tenant.project_id, tenant.group_id, receiptId, operationPlanId]
  );
  if ((q.rows ?? []).length > 1) throw new Error("RECEIPT_AMBIGUOUS");
  const row = q.rows?.[0] as any;
  if (!row) return null;
  const fact = parseJsonMaybe(row.record_json) ?? {};
  return observationPairFromPayload(fact.payload ?? {}, { kind: "receipt_fact", ref: String(row.fact_id ?? "") }, "receipt_observed_parameters");
}
...
export async function recordMemoryV1(db: DbConn, tenant_id: string, input: RecordMemoryInput): Promise<FieldMemoryV1> {
  ...
  if (!project_id) throw new Error("FIELD_MEMORY_PROJECT_ID_REQUIRED");
  if (!group_id) throw new Error("FIELD_MEMORY_GROUP_ID_REQUIRED");
  ...
  const confidence = num((metrics as any).confidence);
  ...
  await db.query(
    ...
    [
      memory_id, tenant_id, project_id, group_id, input.field_id, input.season_id ?? null, null,
      memory_type, metric_key, metric_value ?? null, null, before_value ?? null, after_value ?? null, null, delta_value ?? null,
      JSON.stringify((metrics as any).target_range ?? null), confidence ?? null, source_type,
  ...
  return {
    memory_id, tenant_id, project_id, group_id, field_id: input.field_id, season_id: input.season_id, memory_type,
    metric_key, metric_value, before_value, after_value, delta_value, ...(confidence !== undefined ? { confidence } : {}),
