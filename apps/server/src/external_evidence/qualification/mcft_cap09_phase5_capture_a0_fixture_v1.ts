...
async function main():Promise<void> {
  ...
  const causalMax=new Date(Math.max(Date.parse(soilObserved),Date.parse(soilRaw.response.retrieved_at))).toISOString();
  const a0=ceilHourV1(causalMax);
  ...
  const expectedMemberCount=composed.raw_provider_object_count;
  const selectedDirectoryCount=gfsResponses.filter(row=>row.kind==="GFS_DIRECTORY").length;
  if(selectedDirectoryCount!==1) throw new Error("PHASE5_CAPTURE_EXACT_SELECTED_GFS_DIRECTORY_REQUIRED");
  if(gfsResponses.length!==expectedMemberCount) {
    throw new Error(`PHASE5_CAPTURE_GFS_MEMBER_COUNT_MISMATCH:${gfsResponses.length}:${expectedMemberCount}`);
  }
  ...
  const proof={
    schema_version:"geox_mcft_cap09_phase5_a0_live_raw_capture_v1",
    status:"PASS",
    subject_sha:subject,
