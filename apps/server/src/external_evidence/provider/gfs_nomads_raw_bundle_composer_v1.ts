...
async function tarArchiveToFileV1(
  entries: readonly TarEntryV1[],
  outputPath: string,
): Promise<{ raw_sha256: string; raw_bytes: number }> {
  const handle = await fs.promises.open(outputPath, "w", 0o600);
  ...
  const writeChunk = async (chunk: Uint8Array): Promise<void> => {
    ...
    while (offset < buffer.byteLength) {
      ...
      if (result.bytesWritten <= 0) throw new Error("PHASE3_GFS_TAR_WRITE_ZERO_PROGRESS");
      const written = buffer.subarray(offset, offset + result.bytesWritten);
      hash.update(written);
  ...
  try {
    for (const entry of entries) {
      const size = "body" in entry
        ? entry.body.byteLength
        : fs.statSync(entry.file_path).size;
      await writeChunk(tarHeaderV1(entry.name, size));
      if ("body" in entry) {
        await writeChunk(entry.body);
      } else {
        const stream = fs.createReadStream(entry.file_path, { highWaterMark: 256 * 1024 });
        for await (const chunk of stream) await writeChunk(chunk as Buffer);
      }
      const remainder = size % 512;
      if (remainder !== 0) await writeChunk(Buffer.alloc(512 - remainder, 0));
    }
...
function stageMemberFileV1(root: string, name: string, bytes: Uint8Array): string {
  const memberRoot = path.join(root, "members");
  fs.mkdirSync(memberRoot, { recursive: true, mode: 0o700 });
  const filePath = path.join(
    memberRoot,
    sha256V1(name).slice("sha256:".length) + ".bin",
  );
...
export class GfsNomadsRawBundleComposerV1 {
  ...
  async compose(input: {
    ...
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mcft-cap09-gfs-bundle-"));
    let completed = false;
    ...
    try {
      ...
      const selection = await this.deps.provider.selectLatestCompleteCycle(target, async (raw) => {
        ...
        bodyFiles.set(name, stageMemberFileV1(tempRoot, name, raw.response.bytes));
        members.push({
          name,
          kind: raw.kind,
      ...
      for (const lead of leads) {
        ...
        bodyFiles.set(idxName, stageMemberFileV1(tempRoot, idxName, idxRaw.response.bytes));
        members.push({
          name: idxName,
          kind: idxRaw.kind,
          lead,
          sha256: idxRaw.sha256,
          bytes: idxRaw.response.bytes.byteLength,
          retention_ref: idxReceipt.retention_ref,
        });
        ...
        const messageRaw = await this.deps.provider.fetchSfluxMessageRaw(
          selection.cycle,
          lead,
          target,
          selectedRange,
        ...
        bodyFiles.set(
          messageName,
          stageMemberFileV1(tempRoot, messageName, messageRaw.response.bytes),
        );
        members.push({
          name: messageName,
          kind: messageRaw.kind,
          lead,
          sha256: messageRaw.sha256,
          bytes: messageRaw.response.bytes.byteLength,
          retention_ref: messageReceipt.retention_ref,
        });
      }
      ...
      const manifest = {
        ...
        retention_before_sflux_idx_parse: true,
        retention_before_scientific_decode: true,
        file_backed_bundle_composition: true,
      };

      const tarEntries: TarEntryV1[] = [{
        name: "manifest.json",
        body: new TextEncoder().encode(canonicalJsonV1(manifest)),
      }];
      for (const member of members) {
        const filePath = bodyFiles.get(member.name);
        if (!filePath) throw new Error(`PHASE3_GFS_BUNDLE_BODY_MISSING:${member.name}`);
        tarEntries.push({ name: member.name, file_path: filePath });
      }

      const bundleFilePath = path.join(tempRoot, "gfs-raw-bundle.tar");
      const bundle = await tarArchiveToFileV1(tarEntries, bundleFilePath);
      completed = true;
      ...
      return {
        ...
        raw_bundle_sha256: bundle.raw_sha256,
        raw_bundle_bytes: bundle.raw_bytes,
        manifest,
      ...
      if (!completed) fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  }
}
