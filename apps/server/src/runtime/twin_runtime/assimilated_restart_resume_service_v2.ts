...
import type {
  ...
  RunAssimilatedContiguousRangeResultV2,
} from "./assimilated_contiguous_range_service_v2.js";
import type {
  PreparedRestartInputV1,
  TwinScopeKeyV1,
} from "./ports.js";

export type ResumeAssimilatedFromCheckpointPortV2 = {
  resumeFromCheckpointV1(
    scope: TwinScopeKeyV1,
  ): Promise<PreparedRestartInputV1>;
};

export type AssimilatedContiguousRangePortV2 = {
  runAssimilatedContiguousRangeV2(
    input: RunAssimilatedContiguousRangeInputV2,
  ): Promise<RunAssimilatedContiguousRangeResultV2>;
};

export type RunAssimilatedRestartResumeInputV2 =
  RunAssimilatedContiguousRangeInputV2;

export type RunAssimilatedBoundedBackfillInputV2 =
  RunAssimilatedContiguousRangeInputV2 & {
    evidence_intent:
...
export type AssimilatedRestartResumeResultV2 = {
  ...
  range_result: RunAssimilatedContiguousRangeResultV2;
};
...
export class AssimilatedRestartResumeServiceV2 {
  constructor(
    private readonly checkpointService:
      ResumeAssimilatedFromCheckpointPortV2,
    private readonly rangeService:
      AssimilatedContiguousRangePortV2,
  ) {}

  async resumeAssimilatedFromCheckpointV2(
    input: RunAssimilatedRestartResumeInputV2,
  ): Promise<AssimilatedRestartResumeResultV2> {
    ...
    const rangeResult =
      await this.rangeService
        .runAssimilatedContiguousRangeV2(input);
  ...
  async runAssimilatedBoundedBackfillV2(
    input: RunAssimilatedBoundedBackfillInputV2,
  ): Promise<AssimilatedRestartResumeResultV2> {
    if (
