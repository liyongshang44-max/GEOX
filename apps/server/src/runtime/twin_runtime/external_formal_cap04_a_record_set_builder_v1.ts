...
// Purpose: construct deterministic External Formal CAP04 A1/A2 eight-member canonical candidates from B5A External source members plus an Externalized canonical Forecast authority.
// Boundary: pure construction/validation only; no database, lease, persistence, provider fetch, scheduler, route, Scenario creation, Recommendation, Action, model activation, or O00 execution.
...
export const EXTERNAL_FORMAL_CAP04_A_RECORD_SET_PROFILE_ID_V1 =
  "MCFT_CAP09_EXTERNAL_FORMAL_CAP04_A_RECORD_SET_V1" as const;

export type ExternalFormalCap04ARecordSetBuilderInputV1 = {
...
export type ExternalFormalCap04ARecordSetCandidateV1 = {
  profile_id: typeof EXTERNAL_FORMAL_CAP04_A_RECORD_SET_PROFILE_ID_V1;
...
function exactExternalScopeV1(scope: ContinuationScopeV1): void {
  exactScopeV1(scope, { ...MCFT_CAP09_EXTERNAL_FORMAL_SCOPE_V1 }, "EXTERNAL_CAP04_A_SCOPE_MISMATCH");
}
...
function externalConfigPayloadV1(
  ...
  if (config.object_type !== "twin_runtime_config_v1") throw new Error("EXTERNAL_CAP04_A_RUNTIME_CONFIG_OBJECT_TYPE_REQUIRED");
  exactScopeV1(config, scope, "EXTERNAL_CAP04_A_RUNTIME_CONFIG_SCOPE_MISMATCH");
  if (config.logical_time !== logicalTime || config.as_of !== logicalTime) throw new Error("EXTERNAL_CAP04_A_RUNTIME_CONFIG_TIME_MISMATCH");
  if (computeMemberDeterminismHashV1(config as unknown as Record<string, unknown>) !== config.determinism_hash) {
    throw new Error("EXTERNAL_CAP04_A_RUNTIME_CONFIG_HASH_MISMATCH");
  }
  ...
  if (payload.config_role !== "HOURLY_CAP04" || payload.effective_logical_time !== logicalTime) {
    throw new Error("EXTERNAL_CAP04_A_HOURLY_RUNTIME_CONFIG_REQUIRED");
  }
  assertNoReplayCanonicalMarkerV1(config, "EXTERNAL_CAP04_A_RUNTIME_CONFIG_REPLAY_LEAKAGE");
  return payload;
}

function validateSourceMemberV1(
  member: CanonicalObjectEnvelopeV1,
  expectedType: SourceMemberTypeV1,
  input: ExternalFormalCap04ARecordSetBuilderInputV1,
): void {
  if (member.object_type !== expectedType) throw new Error(`EXTERNAL_CAP04_A_SOURCE_TYPE_MISMATCH:${expectedType}`);
  exactScopeV1(member, input.scope, `EXTERNAL_CAP04_A_SOURCE_SCOPE_MISMATCH:${expectedType}`);
  if (member.logical_time !== input.logical_time || member.as_of !== input.logical_time) throw new Error(`EXTERNAL_CAP04_A_SOURCE_TIME_MISMATCH:${expectedType}`);
  if (member.lineage_id !== input.lineage_id || member.revision_id !== input.revision_id) throw new Error(`EXTERNAL_CAP04_A_SOURCE_LINEAGE_MISMATCH:${expectedType}`);
  if (member.runtime_config_ref !== input.runtime_config.object_id || member.runtime_config_hash !== input.runtime_config.determinism_hash) throw new Error(`EXTERNAL_CAP04_A_SOURCE_RUNTIME_CONFIG_MISMATCH:${expectedType}`);
  if (computeMemberDeterminismHashV1(member as unknown as Record<string, unknown>) !== member.determinism_hash) throw new Error(`EXTERNAL_CAP04_A_SOURCE_HASH_MISMATCH:${expectedType}`);
  assertNoReplayCanonicalMarkerV1(member, `EXTERNAL_CAP04_A_SOURCE_REPLAY_LEAKAGE:${expectedType}`);
}

function validateSourceGraphV1(input: ExternalFormalCap04ARecordSetBuilderInputV1): void {
  ...
  if (transition.payload.evidence_window_ref !== evidence.object_id
    ...
    throw new Error("EXTERNAL_CAP04_A_SOURCE_GRAPH_MISMATCH");
  }
}
...
function buildExternalFormalCap04ARecordSetInternalV1(
  input: ExternalFormalCap04ARecordSetBuilderInputV1,
  operationVariant: Cap04AOperationVariantV1,
): ExternalFormalCap04ARecordSetCandidateV1 {
  ...
  const externalConfig = externalConfigPayloadV1(input.runtime_config, input.scope, logicalTime);
  ...
  if (input.forecast_payload.status !== expectedStatus) throw new Error("EXTERNAL_CAP04_A_FORECAST_STATUS_VARIANT_MISMATCH");
  if (input.forecast_payload.issued_at !== logicalTime
    ...
    || input.forecast_payload.crop_stage_context_ref !== externalConfig.crop_stage_context_authority.context_ref
    || input.forecast_payload.crop_stage_context_hash !== externalConfig.crop_stage_context_authority.context_hash) {
  ...
  const boundToSourceState = input.forecast_payload.source_posterior_ref === sourceState.object_id
    && input.forecast_payload.source_posterior_hash === sourceState.determinism_hash;
  if (!boundToSourceState) throw new Error("EXTERNAL_CAP04_A_FORECAST_SOURCE_STATE_TEMPLATE_MISMATCH");
  ...
  const operationLimitations = operationVariant === CAP04_A1_OPERATION_VARIANT_V1
    ? externalLimitationsV1([], ["EXTERNAL_CAP04_A1_CANDIDATE", "SCENARIO_SET_NOT_CREATED_BY_EA5B5B"])
    : externalLimitationsV1([], ["EXTERNAL_CAP04_A2_CANDIDATE", "FORECAST_BLOCKED", "SCENARIO_SET_NOT_EXPECTED"]);
  ...
  const buildMemberV1 = (
    ...
    const member: CanonicalObjectEnvelopeV1 = {
      ...
      determinism_hash: "",
      limitations: externalLimitationsV1(limitations),
  ...
  if (!state) throw new Error("EXTERNAL_CAP04_A_STATE_MEMBER_MISSING");
  ...
  return {
    profile_id: EXTERNAL_FORMAL_CAP04_A_RECORD_SET_PROFILE_ID_V1,
...
export function buildExternalFormalCap04CompletedA1RecordSetV1(
  input: ExternalFormalCap04ARecordSetBuilderInputV1,
): ExternalFormalCap04ARecordSetCandidateV1 {
  return buildExternalFormalCap04ARecordSetInternalV1(input, CAP04_A1_OPERATION_VARIANT_V1);
}

export function buildExternalFormalCap04BlockedA2RecordSetV1(
  input: ExternalFormalCap04ARecordSetBuilderInputV1,
): ExternalFormalCap04ARecordSetCandidateV1 {
  return buildExternalFormalCap04ARecordSetInternalV1(input, CAP04_A2_OPERATION_VARIANT_V1);
}
