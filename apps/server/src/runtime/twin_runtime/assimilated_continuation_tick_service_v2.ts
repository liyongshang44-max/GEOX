...
export type AssimilatedSingleTickPersistencePortV2 = AssimilatedContinuationPersistencePortV2
  & Pick<BootstrapPersistencePortV1, "acquireLease">;

export type ExecuteAssimilatedContinuationTickInputV2 = {
...
export type ExecuteAssimilatedContinuationTickResultV2 = {
...
function dynamicsConfigV1(config: AssimilatedContinuationRuntimeConfigPayloadV2): HourlyWaterBalanceConfigV1 {
  return {
...
function memberV1(recordSet: AssimilatedContinuationRecordSetV2, objectType: string) {
...
function memberRefV1(recordSet: AssimilatedContinuationRecordSetV2, objectType: string): string {
  return memberV1(recordSet, objectType).object_id;
}

function memberHashV1(recordSet: AssimilatedContinuationRecordSetV2, objectType: string): string {
  return memberV1(recordSet, objectType).determinism_hash;
}

function committedSequenceV1(recordSet: AssimilatedContinuationRecordSetV2): number {
...
export class AssimilatedContinuationTickServiceV2 {
  constructor(
    ...
    private readonly persistence: AssimilatedSingleTickPersistencePortV2,
  ) {}
  ...
  async executeOneTick(
    ...
    validateAssimilatedContinuationRuntimeConfigPayloadV2(runtimeConfig.payload);
    const config = runtimeConfig.payload as unknown as AssimilatedContinuationRuntimeConfigPayloadV2;
    if (
    ...
    const preliminaryEvidenceWindow = buildAssimilatedContinuationEvidenceWindowV2({
      scope: input.scope,
      logical_time: logicalTime,
    ...
    const assimilation = composeAssimilatedContinuationPosteriorV1({
      ...
      // V2 candidates are a strict semantic superset of the fields consumed
      // by the frozen V1 Gaussian assimilation kernel.
      selected_observation: (preliminaryEvidenceWindow.observation_selection.selected_observation as never),
      saturation_fraction: config.soil_hydraulic_snapshot.saturation_fraction,
    ...
    const evidenceWindow = finalizeAssimilatedContinuationEvidenceWindowV2({
      window: preliminaryEvidenceWindow,
      assimilation,
    });
    const candidate = buildAssimilatedContinuationRecordSetV2({
      scope: input.scope,
      logical_time: logicalTime,
    ...
    if (existingAfterBuild) {
      validateAssimilatedContinuationCrossReferencesV2(existingAfterBuild);
      if (
    ...
    validateAssimilatedContinuationCrossReferencesV2(readback);
    if (
