// Purpose: build the deterministic MCFT-CAP-08.S4 corrected T16 State, Forecast, Scenario, Tick and Checkpoint plus immutable append-forward authority.
// Boundary: pure canonical construction only; no database, persistence, projections, pointers, lease, route, scheduler, Residual commit, Calibration, Shadow, or production authority.

import { deriveSemanticObjectIdV1 } from "../../domain/twin_runtime/canonical_identity_v1.js";
...
function exactBaseV1(input: {
  ...
  if (input.object.object_type !== input.type
    || input.object.object_id !== input.ref
    || input.object.determinism_hash !== input.hash
    || input.object.logical_time !== input.time
    || input.object.as_of !== input.time
    || input.object.lineage_id !== input.lineage
    || input.object.revision_id !== input.revision) {
    throw new Error(`${input.code}_IDENTITY_MISMATCH`);
  }
  exactScopeV1(input.object, input.scope, `${input.code}_SCOPE_MISMATCH`);
  validateCap04CanonicalEnvelopeV1(input.object);
}
...
function decimalTextV1(value: number, scale: number): string {
  ...
  return (Object.is(value, -0) ? 0 : value).toFixed(scale);
}

function replaceDecimalV1(existing: unknown, value: number, scale: number): unknown {
...
function buildEnvelopeV1(input: {
  ...
  object.determinism_hash = computeCap04AMemberDeterminismHashV1(object);
  validateCap04CanonicalEnvelopeV1(object);
  return object;
}
...
export function buildCap08S4CorrectedCanonicalSetV1(
  ...
  exactBaseV1({ object: input.base_t16_state, type: "twin_state_estimate_v1", ref: identity.identity_input.base_t16_state.ref, hash: identity.identity_input.base_t16_state.hash, time, scope, lineage, revision, code: "CAP08_S4_BASE_STATE" });
  exactBaseV1({ object: input.base_t16_forecast, type: "twin_forecast_run_v1", ref: identity.identity_input.base_t16_forecast.ref, hash: identity.identity_input.base_t16_forecast.hash, time, scope, lineage, revision, code: "CAP08_S4_BASE_FORECAST" });
  exactBaseV1({ object: input.base_t16_tick, type: "twin_runtime_tick_v1", ref: identity.identity_input.base_t16_tick.ref, hash: identity.identity_input.base_t16_tick.hash, time, scope, lineage, revision, code: "CAP08_S4_BASE_TICK" });
  exactBaseV1({ object: input.base_t16_checkpoint, type: "twin_runtime_checkpoint_v1", ref: identity.identity_input.base_t16_checkpoint.ref, hash: identity.identity_input.base_t16_checkpoint.hash, time, scope, lineage, revision, code: "CAP08_S4_BASE_CHECKPOINT" });

  if (input.math_result.disposition !== "APPLIED" || input.math_input.lag_hours !== CAP08_S4_LAG_HOURS_V1) {
    throw new Error("CAP08_S4_APPLIED_FORMAL_MATH_REQUIRED");
  }
  if (input.historical_hash_manifest.manifest_digest !== identity.identity_input.historical_hash_manifest_digest) {
  ...
  validateCap04RuntimeConfigPayloadV1(config);
  if (config.effective_logical_time !== time) throw new Error("CAP08_S4_RUNTIME_CONFIG_TIME_MISMATCH");

  const baseStatePayload = payloadV1(input.base_t16_state, "CAP08_S4_BASE_STATE_PAYLOAD_REQUIRED");
  ...
  if (!baseComputation || typeof baseComputation !== "object" || Array.isArray(baseComputation)
    || !baseStorage || typeof baseStorage !== "object" || Array.isArray(baseStorage)
    || !baseVwc || typeof baseVwc !== "object" || Array.isArray(baseVwc)) {
    throw new Error("CAP08_S4_BASE_STATE_MOMENTS_REQUIRED");
  }

  const rootZoneDepth = finiteV1(config.soil_hydraulic_snapshot.root_zone_depth_mm, "CAP08_S4_ROOT_ZONE_DEPTH_INVALID");
  const correctedMean = finiteV1(input.math_result.mean, "CAP08_S4_CORRECTED_MEAN_INVALID");
  const correctedVariance = finiteV1(input.math_result.variance, "CAP08_S4_CORRECTED_VARIANCE_INVALID");
  if (rootZoneDepth <= 0 || correctedVariance < 0) throw new Error("CAP08_S4_CORRECTED_MOMENTS_INVALID");
  ...
  const availableWaterFraction = Math.max(0, Math.min(1,
  ...
  computation.storage_mean_mm_decimal = replaceDecimalV1(computation.storage_mean_mm_decimal, correctedStorage, 6);
  computation.storage_variance_mm2_decimal = replaceDecimalV1(computation.storage_variance_mm2_decimal, correctedStorageVariance, 6);
  computation.posterior_vwc_decimal = replaceDecimalV1(computation.posterior_vwc_decimal, correctedMean, 12);
  computation.posterior_vwc_variance_decimal = replaceDecimalV1(computation.posterior_vwc_variance_decimal, correctedVariance, 12);
  ...
  computation.late_append_forward_trace = {
  ...
  const baseTickPayload = payloadV1(input.base_t16_tick, "CAP08_S4_BASE_TICK_PAYLOAD_REQUIRED");
  const baseCheckpointPayload = payloadV1(input.base_t16_checkpoint, "CAP08_S4_BASE_CHECKPOINT_PAYLOAD_REQUIRED");
  const sequence = baseCheckpointPayload.tick_sequence;
  if (!Number.isInteger(sequence) || Number(sequence) < 1) throw new Error("CAP08_S4_BASE_CHECKPOINT_SEQUENCE_INVALID");
  ...
  const limitations = [
    "CONTROLLED_REPLAY", "NOT_FIELD_CALIBRATED", "S4_SLICE_ACCEPTANCE_ONLY",
    "LATE_APPEND_FORWARD", "HISTORICAL_REWRITE_FORBIDDEN",
    "HISTORICAL_REVISION_CREATION_FORBIDDEN", "LATEST_POINTER_REGRESSION_FORBIDDEN",
    "FVO16_RESIDUAL_ONLY", "NO_RECOMMENDATION", "NO_AO_ACT", "NO_DISPATCH",
    "NO_MODEL_ACTIVATION",
  ];
  ...
  const state = buildEnvelopeV1({
    ...
    source_refs: [...input.base_t16_state.source_refs, input.base_t16_state.object_id, identity.authority_ref],
    evidence_refs: evidenceRefs,
    runtime_config_ref: input.runtime_config.object_id,
    runtime_config_hash: input.runtime_config.determinism_hash,
    lineage_id: lineage,
    revision_id: revision,
    limitations: [...input.base_t16_state.limitations, ...limitations],
    authority_ref: identity.authority_ref,
    payload: {
      ...baseStatePayload,
  ...
  const forecastPayload = structuredClone(forecastMath.forecast_payload) as unknown as Cap04ForecastRunPayloadV1;
  ...
  if (forecastPayload.status !== "COMPLETED") throw new Error("CAP08_S4_CORRECTED_FORECAST_MUST_COMPLETE");
  const forecast = buildEnvelopeV1({
    ...
    evidence_refs: [...input.base_t16_forecast.evidence_refs, identity.identity_input.late_observation.ref],
    runtime_config_ref: input.runtime_config.object_id,
    runtime_config_hash: input.runtime_config.determinism_hash,
    lineage_id: lineage,
    revision_id: revision,
    payload: forecastPayload as unknown as Record<string, unknown>,
    limitations: [...forecastPayload.limitations, ...limitations],
    authority_ref: identity.authority_ref,
  });
  ...
  const scenarioMath = executeCap04PureThreeScenarioMathV1({
    ...
    runtime_config: { ref: input.runtime_config.object_id, hash: input.runtime_config.determinism_hash, payload: config },
    forcing_window: structuredClone(input.forcing_window),
  });
  const scenario = buildCap04ScenarioSetRecordV1({
  ...
  const checkpoint = buildEnvelopeV1({
    ...
    payload: {
      ...
      residual_commit_status: "PENDING_S5_C_PROVIDER",
      next_tick_logical_time: nextTime,
      tick_sequence: Number(sequence),
      latest_pointer_regression_authorized: false,
    },
  });

  const correctedSet: Cap08S4CorrectedCanonicalSetV1 = { state, forecast, scenario, tick, checkpoint };
  const authorityWithoutHash: Omit<Cap08S4AppendForwardAuthorityV1, "determinism_hash"> = {
    ...
    t17_predecessor: {
      ...
      previous_tick_sequence: Number(sequence),
