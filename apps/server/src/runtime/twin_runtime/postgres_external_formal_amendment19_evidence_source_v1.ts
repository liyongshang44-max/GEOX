// Purpose: Amendment-19 External Formal database Evidence source for hourly Runtime progression under daily-batch KBS publication.
// Boundary: read-only facts access only; no provider/R2 access, scheduler claim, persistence write, Runtime Config selection, or timestamp relabel.
// Delayed exact rainfall/historical ET0 are optional at boundary T; soil and causal future-forcing families remain required.
...
export const MCFT_CAP09_AMENDMENT19_DATABASE_EVIDENCE_SOURCE_ID_V1 =
  "MCFT_CAP09_AMENDMENT19_DATABASE_EVIDENCE_SOURCE_V1" as const;
export const MCFT_CAP09_AMENDMENT19_DATABASE_LOOKBACK_HOURS_V1 = 36 as const;
...
type FamilyV1 = "soil" | "rainfall" | "historical_et0" | "future_weather" | "future_et0";
...
function parseRowV1(row: EvidenceFactRowV1): CanonicalReplayEvidenceRecordV1 {
  const envelope = typeof row.record_json === "string"
    ? objectV1(JSON.parse(row.record_json), "AM19_EXTERNAL_DB_ENVELOPE_INVALID")
    : objectV1(row.record_json, "AM19_EXTERNAL_DB_ENVELOPE_INVALID");
  const wrapperType = requiredTextV1(envelope.type, "AM19_EXTERNAL_DB_WRAPPER_TYPE_REQUIRED");
  const payload = objectV1(envelope.payload, "AM19_EXTERNAL_DB_PAYLOAD_REQUIRED") as unknown as CanonicalReplayEvidenceRecordV1;
  if (payload.record_type !== wrapperType) throw new Error(`AM19_EXTERNAL_DB_WRAPPER_TYPE_MISMATCH:${row.fact_id}`);
  if (!(ALLOWED_RECORD_TYPES_V1 as readonly string[]).includes(payload.record_type)) {
    throw new Error(`AM19_EXTERNAL_DB_RECORD_TYPE_NOT_ALLOWED:${payload.record_type}`);
  }
  return structuredClone(payload);
}

function authorityForV1(record: CanonicalReplayEvidenceRecordV1): EvidenceAuthorityV1 {
  const authority = AUTHORITY_BY_RECORD_TYPE_V1[record.record_type as AllowedRecordTypeV1];
  if (!authority) throw new Error(`AM19_EXTERNAL_DB_RECORD_TYPE_NOT_ALLOWED:${record.record_type}`);
  return authority;
}

function eventTimeV1(record: CanonicalReplayEvidenceRecordV1, authority: EvidenceAuthorityV1): string {
  return canonicalIsoV1(record.role_time?.[authority.event_time_field], `AM19_EXTERNAL_DB_EVENT_TIME_INVALID:${record.record_type}`);
}

function validateStaticAuthorityV1(record: CanonicalReplayEvidenceRecordV1, scope: TwinScopeKeyV1): EvidenceAuthorityV1 {
  exactScopeV1(scopeFromRecordV1(record), scope, `AM19_EXTERNAL_DB_SCOPE_MISMATCH:${record.record_type}`);
  const authority = authorityForV1(record);
  if (record.binding_id !== authority.binding_id) throw new Error(`AM19_EXTERNAL_DB_BINDING_MISMATCH:${record.record_type}`);
  if (record.epistemic_class !== authority.epistemic_class) throw new Error(`AM19_EXTERNAL_DB_EPISTEMIC_MISMATCH:${record.record_type}`);
  if (record.quality?.status !== "PASS" && record.quality?.status !== "LIMITED") {
    throw new Error(`AM19_EXTERNAL_DB_QUALITY_NOT_ELIGIBLE:${record.record_type}`);
  }
  requiredTextV1(record.source_record_id, `AM19_EXTERNAL_DB_SOURCE_RECORD_ID_REQUIRED:${record.record_type}`);
  requiredTextV1(record.source_record_hash, `AM19_EXTERNAL_DB_SOURCE_RECORD_HASH_REQUIRED:${record.record_type}`);
  return authority;
}

function targetExactIntervalV1(record: CanonicalReplayEvidenceRecordV1, logicalTime: string): boolean {
  const start = canonicalIsoV1(record.role_time?.interval_start, `AM19_EXTERNAL_DB_INTERVAL_START_INVALID:${record.record_type}`);
  const end = canonicalIsoV1(record.role_time?.interval_end, `AM19_EXTERNAL_DB_INTERVAL_END_INVALID:${record.record_type}`);
...
export class PostgresExternalFormalAmendment19EvidenceSourceV1 {
  constructor(
    private readonly pool: ReadOnlyPoolV1,
    private readonly maxCandidateRecords = 1000,
  ) {
    if (!Number.isSafeInteger(maxCandidateRecords) || maxCandidateRecords < 3) {
      throw new Error("AM19_EXTERNAL_DB_MAX_CANDIDATES_INVALID");
    }
  }
  ...
  async loadCandidateRecords(input: {
    ...
    const logicalTime = canonicalHourV1(input.logical_time, "AM19_EXTERNAL_DB_LOGICAL_TIME_INVALID");
    exactScopeV1(input.scope, { ...MCFT_CAP09_EXTERNAL_FORMAL_SCOPE_V1 }, "AM19_EXTERNAL_DB_FORMAL_SCOPE_REQUIRED");
    const evidenceSnapshotTime = canonicalIsoV1(input.evidence_snapshot_time, "AM19_EXTERNAL_DB_EVIDENCE_SNAPSHOT_TIME_REQUIRED");
    if (evidenceSnapshotTime !== logicalTime) throw new Error("AM19_EXTERNAL_DB_BOUNDARY_SNAPSHOT_MUST_EQUAL_LOGICAL_TIME");

    const queryStart = addMinutesV1(logicalTime, -MCFT_CAP09_AMENDMENT19_DATABASE_LOOKBACK_HOURS_V1 * 60);
    ...
    for (const row of rows) {
      ...
      if (Date.parse(eventTime) > Date.parse(logicalTime)) {
        throw new Error(`AM19_EXTERNAL_DB_FUTURE_EVENT_FORBIDDEN:${record.record_type}`);
      }
      const ingestedAt = canonicalIsoV1(record.role_time?.ingested_at, `AM19_EXTERNAL_DB_INGESTED_AT_INVALID:${record.record_type}`);
      const availableAt = canonicalIsoV1(record.available_to_runtime_at, `AM19_EXTERNAL_DB_AVAILABLE_AT_INVALID:${record.record_type}`);
      if (Date.parse(eventTime) > Date.parse(availableAt) || Date.parse(availableAt) > Date.parse(ingestedAt)) {
        throw new Error(`AM19_EXTERNAL_DB_CAUSAL_ORDER_INVALID:${record.record_type}`);
      }
    ...
    for (const key of ["soil", "future_weather", "future_et0"] as const) {
      if (family[key] < 1) throw new Error(`AM19_EXTERNAL_DB_REQUIRED_CAUSAL_FAMILY_MISSING:${key}`);
    }
