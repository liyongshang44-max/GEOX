...
import type {
  ExecuteAssimilatedContinuationTickInputV1,
  ExecuteAssimilatedContinuationTickResultV1,
} from "./assimilated_continuation_tick_service_v1.js";
import type {
  PreparedNextTickInputV1,
  TwinScopeKeyV1,
} from "./ports.js";

export const MAX_ASSIMILATED_CONTIGUOUS_TICKS_V1 = 24;

export const ASSIMILATED_CONTINUATION_TICK_INTERVAL_MS_V1 =
  60 * 60 * 1000;

export type PrepareAssimilatedNextTickInputPortV1 = {
  prepareNextTickInput(
    scope: TwinScopeKeyV1,
  ): Promise<PreparedNextTickInputV1>;
};

export type ExecuteOneAssimilatedContinuationTickPortV1 = {
  executeOneTick(
    input: ExecuteAssimilatedContinuationTickInputV1,
  ): Promise<ExecuteAssimilatedContinuationTickResultV1>;
};

export type RunAssimilatedContiguousRangeInputV1 = {
  scope: TwinScopeKeyV1;
  ...
  crop_stage_context:
    ExecuteAssimilatedContinuationTickInputV1["crop_stage_context"];
  lease_owner: string;
  lease_duration_seconds: number;
};

export type RunAssimilatedContiguousRangeResultV1 = {
  ...
  tick_results: ExecuteAssimilatedContinuationTickResultV1[];
...
function addHoursV1(
  ...
  return new Date(
    Date.parse(value)
      + hours * ASSIMILATED_CONTINUATION_TICK_INTERVAL_MS_V1,
  ).toISOString();
}

export class AssimilatedContiguousRangeServiceV1 {
  constructor(
    private readonly handoffService:
      PrepareAssimilatedNextTickInputPortV1,
    private readonly tickService:
      ExecuteOneAssimilatedContinuationTickPortV1,
  ) {}
  ...
  async runAssimilatedContiguousRangeV1(
    ...
    const intervalCount =
      (
        targetMs - startMs
      ) / ASSIMILATED_CONTINUATION_TICK_INTERVAL_MS_V1;
    ...
    if (
      requestedTickCount
      > MAX_ASSIMILATED_CONTIGUOUS_TICKS_V1
    ) {
      throw new Error(
