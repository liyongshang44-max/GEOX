...
export type RuntimeLeaseClaimV1 = TwinScopeKeyV1 & {
  lease_owner: string;
...
export type PreparedNextTickInputV1 = TwinScopeKeyV1 & {
  ...
  previous_state_runtime_config_ref: string;
...
export interface RuntimeAuthoritySnapshotRepositoryPortV1 {
  commitRealityBindingSnapshot(snapshot: RealityBindingRuntimeSnapshotV1): Promise<{
    status: "INSERTED" | "EXISTING_IDEMPOTENT_SUCCESS";
    binding_id: string;
  }>;
