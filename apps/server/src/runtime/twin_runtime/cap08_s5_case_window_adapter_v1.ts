// Purpose: adapt the CAP-08 formal 24-Tick case profile to the mature CAP-06 case-window contract while preserving exact zero-excess physical inputs.
...
export const CAP08_S5_ZERO_EXCESS_COMPATIBILITY_POLICY_V1 =
  "ZERO_EXCESS_PRESERVED_AND_CLASSIFIED_LOW_EXCESS_FOR_SHARED_ENGINE_V1" as const;
...
export function buildCap08S5CaseWindowV1(input: {
  ...
  const compatibilityCases = input.loadedCases.map((source) => {
    ...
    if (units < 0n) {
      throw new Error(`CAP08_S5_NEGATIVE_EXCESS_FORBIDDEN:${exact.residual_ref}`);
    }
  ...
  const cases = shared.cases.map((caseItem) => {
    ...
    const zero = excessUnits(exact.excess_above_field_capacity_mm) === 0n;
    return {
      ...caseItem,
      excess_above_field_capacity_mm: exact.excess_above_field_capacity_mm,
      wetness_regime: zero ? "LOW_EXCESS" as const : caseItem.wetness_regime,
    };
  });
