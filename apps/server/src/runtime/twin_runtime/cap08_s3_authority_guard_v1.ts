...
export type Cap08S3AuthorityFingerprintV1 = {
  ...
  active_model_activation_count: number;
  digest: string;
};
...
export class Cap08S3AuthorityGuardV1 {
  ...
  async capture(scope: TwinScopeKeyV1): Promise<Cap08S3AuthorityFingerprintV1> {
    const [activeLineage, latestState, latestCheckpoint, latestForecast, latestScenario, activation] = await Promise.all([
    ...
    const basis = {
      ...
      active_model_activation_count: Number(activation.rows[0].n),
    };
    return {
      schema_version: "geox_mcft_cap08_s3_authority_fingerprint_v1",
      ...basis,
