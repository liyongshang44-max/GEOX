// Purpose: establish the MCFT-CAP-08.S3 B00 bootstrap root, execute the exact T00-T23 Decision/Action Replay, and use persisted completion plus episode authority for zero-write replay.
// Boundary: one explicit bounded Replay invocation only; no late correction, Residual, Calibration, Shadow, route, scheduler, live ingestion, production Runtime source, or MCFT-CAP-09 authority.
...
import {
  Cap08S3FormalRangeServiceV1,
  type RunCap08S3FormalRangeInputV1,
  type RunCap08S3FormalRangeResultV1,
} from "./cap08_s3_formal_range_service_v1.js";
import type { TwinScopeKeyV1 } from "./ports.js";

export type ExecuteCap08S3FormalRuntimeInputV1 = Omit<RunCap08S3FormalRangeInputV1, "scope"> & {
...
export type ExecuteCap08S3FormalRuntimeResultV1 = {
  ...
  range: RunCap08S3FormalRangeResultV1;
...
export class Cap08S3FormalRuntimeServiceV1 {
  constructor(
    private readonly bootstrapService: A0BootstrapRuntimeServiceV1,
    private readonly rangeService: Cap08S3FormalRangeServiceV1,
  ) {}

  private bootstrapInputV1(
    input: ExecuteCap08S3FormalRuntimeInputV1,
  ...
  async execute(input: ExecuteCap08S3FormalRuntimeInputV1): Promise<ExecuteCap08S3FormalRuntimeResultV1> {
    ...
    if (input.bootstrap_runtime_config.logical_time !== bootstrapLogicalTime) {
      throw new Error("CAP08_S3_B00_RUNTIME_CONFIG_LOGICAL_TIME_MISMATCH");
    }
    const rangeInput: RunCap08S3FormalRangeInputV1 = {
    ...
    if (bootstrap.next_tick_logical_time !== CAP08_S1_RUNTIME_START_V1) {
      throw new Error("CAP08_S3_B00_HANDOFF_TO_T00_REQUIRED");
    }
