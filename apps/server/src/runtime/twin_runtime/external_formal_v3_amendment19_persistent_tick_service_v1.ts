// Purpose: production-facing Amendment-19 persistent tick service.
// Boundary: caller supplies an already-claimed scheduler slot, exact manifest pin, boundary-frozen DB Evidence, crop context and observer time.
// The same Amendment-19 canonical core used by persistence-free qualification is the only State/Forecast semantic execution path.
...
export const EXTERNAL_FORMAL_V3_AMENDMENT19_PERSISTENT_TICK_SERVICE_ID_V1 =
  "MCFT_CAP09_EXTERNAL_FORMAL_V3_AMENDMENT19_PERSISTENT_TICK_SERVICE_V1" as const;
export const EXTERNAL_FORMAL_V3_AMENDMENT19_WATERMARK_ID_V1 = "PROVIDER_AVAILABILITY_WATERMARK_V1" as const;

export type ExternalFormalV3Amendment19ManifestSlotPinV1 = {
  manifest_ref: string;
...
export interface ExternalFormalV3Amendment19DatabaseEvidenceSourcePortV1 {
...
export type ExecuteExternalFormalV3Amendment19PersistentTickInputV1 = {
  claim: ShadowOnlineSlotClaimV1;
  manifest_slot: ExternalFormalV3Amendment19ManifestSlotPinV1;
...
export type ExecuteExternalFormalV3Amendment19PersistentTickResultV1 = {
  service_id: typeof EXTERNAL_FORMAL_V3_AMENDMENT19_PERSISTENT_TICK_SERVICE_ID_V1;
  provider_availability_watermark: typeof EXTERNAL_FORMAL_V3_AMENDMENT19_WATERMARK_ID_V1;
  status:
  ...
  current_interval_forcing_mode: "EXACT_PROVIDER_INTERVAL_PAIR" | "PRIOR_STEP_CAUSAL_ASSUMPTION_PAIR" | null;
...
function memberV1(recordSet: Cap04ARecordSetV1, objectType: string): CanonicalObjectEnvelopeV1 {
  ...
  if (matches.length !== 1) throw new Error(`EXTERNAL_FORMAL_V3_AM19_MEMBER_CARDINALITY:${objectType}`);
  return matches[0]!;
}

function validateManifestAndClaimV1(input: ExecuteExternalFormalV3Amendment19PersistentTickInputV1): {
  logical_time: string;
  scheduler_wall_clock_observed_at: string;
  evidence_snapshot_time: string;
  observer_started_at: string;
  ...
  exactScopeV1(input.claim.boundary.scope, scope, "EXTERNAL_FORMAL_V3_AM19_CLAIM_SCOPE_MISMATCH");
  if (input.claim.state !== "CLAIMED") throw new Error("EXTERNAL_FORMAL_V3_AM19_CLAIM_STATE_REQUIRED");
  requiredStringV1(input.claim.lease_owner, "EXTERNAL_FORMAL_V3_AM19_LEASE_OWNER_REQUIRED");
  if (typeof input.claim.fencing_token !== "bigint" || input.claim.fencing_token <= 0n) throw new Error("EXTERNAL_FORMAL_V3_AM19_FENCING_TOKEN_REQUIRED");
  if (!Number.isInteger(input.lease_duration_seconds) || input.lease_duration_seconds <= 0) throw new Error("EXTERNAL_FORMAL_V3_AM19_LEASE_DURATION_INVALID");

  const logicalTime = canonicalHourV1(input.manifest_slot.logical_time, "EXTERNAL_FORMAL_V3_AM19_MANIFEST_LOGICAL_TIME_INVALID");
  if (input.claim.boundary.logical_time !== logicalTime) throw new Error("EXTERNAL_FORMAL_V3_AM19_CLAIM_LOGICAL_TIME_MISMATCH");
  if (input.claim.boundary.slot_id !== input.manifest_slot.slot_id) throw new Error("EXTERNAL_FORMAL_V3_AM19_CLAIM_SLOT_ID_MISMATCH");
  if (input.claim.boundary.interval_seconds !== 3600) throw new Error("EXTERNAL_FORMAL_V3_AM19_PT1H_BOUNDARY_REQUIRED");
  for (const [value, code] of [
    [input.manifest_slot.manifest_ref, "EXTERNAL_FORMAL_V3_AM19_MANIFEST_REF_REQUIRED"],
    [input.manifest_slot.manifest_hash, "EXTERNAL_FORMAL_V3_AM19_MANIFEST_HASH_REQUIRED"],
    [input.manifest_slot.epoch_id, "EXTERNAL_FORMAL_V3_AM19_EPOCH_ID_REQUIRED"],
    [input.manifest_slot.runtime_config_ref, "EXTERNAL_FORMAL_V3_AM19_RUNTIME_CONFIG_REF_REQUIRED"],
    [input.manifest_slot.runtime_config_hash, "EXTERNAL_FORMAL_V3_AM19_RUNTIME_CONFIG_HASH_REQUIRED"],
    [input.manifest_slot.crop_stage_context_ref, "EXTERNAL_FORMAL_V3_AM19_CROP_CONTEXT_REF_REQUIRED"],
    [input.manifest_slot.crop_stage_context_hash, "EXTERNAL_FORMAL_V3_AM19_CROP_CONTEXT_HASH_REQUIRED"],
  ] as const) requiredStringV1(value, code);

  const schedulerObservedAt = canonicalIsoV1(input.claim.boundary.scheduler_wall_clock_observed_at, "EXTERNAL_FORMAL_V3_AM19_SCHEDULER_OBSERVED_AT_INVALID");
  const evidenceSnapshotTime = canonicalIsoV1(input.evidence_snapshot_time, "EXTERNAL_FORMAL_V3_AM19_EVIDENCE_SNAPSHOT_TIME_REQUIRED");
  const observerStartedAt = canonicalIsoV1(input.observer_started_at, "EXTERNAL_FORMAL_V3_AM19_OBSERVER_STARTED_AT_INVALID");
  if (evidenceSnapshotTime !== logicalTime) throw new Error("EXTERNAL_FORMAL_V3_AM19_BOUNDARY_SNAPSHOT_MUST_EQUAL_LOGICAL_TIME");
  if (Date.parse(schedulerObservedAt) < Date.parse(logicalTime)) throw new Error("EXTERNAL_FORMAL_V3_AM19_SCHEDULER_BEFORE_LOGICAL_TIME");
  if (Date.parse(observerStartedAt) < Date.parse(schedulerObservedAt)) throw new Error("EXTERNAL_FORMAL_V3_AM19_OBSERVER_BEFORE_SCHEDULER_OBSERVATION");
  if (input.crop_stage_context.determinism_hash !== input.manifest_slot.crop_stage_context_hash) {
    throw new Error("EXTERNAL_FORMAL_V3_AM19_CROP_CONTEXT_HASH_MISMATCH");
  }
  return {
...
function runtimeLeaseFromClaimV1(input: ExecuteExternalFormalV3Amendment19PersistentTickInputV1): RuntimeLeaseClaimV1 {
  return {
    ...externalScopeV1(),
...
function canonicalForecastMathV1(forecast: CanonicalObjectEnvelopeV1): {
  ...
  if (payload.status !== "COMPLETED") throw new Error("EXTERNAL_FORMAL_V3_AM19_SCENARIO_REQUIRES_COMPLETED_FORECAST");
...
function existingARecordSetConfigV1(recordSet: Cap04ARecordSetV1, input: ExecuteExternalFormalV3Amendment19PersistentTickInputV1): void {
  validateCap04ARecordSetV1(recordSet);
  if (recordSet.operation_key.logical_time !== input.manifest_slot.logical_time) throw new Error("EXTERNAL_FORMAL_V3_AM19_EXISTING_A_TIME_MISMATCH");
  if (recordSet.aggregate_identity_input.runtime_config_ref !== input.manifest_slot.runtime_config_ref) throw new Error("EXTERNAL_FORMAL_V3_AM19_EXISTING_A_CONFIG_REF_MISMATCH");
...
export class ExternalFormalV3Amendment19PersistentTickServiceV1 {
  ...
  async executeClaimedTick(input: ExecuteExternalFormalV3Amendment19PersistentTickInputV1): Promise<ExecuteExternalFormalV3Amendment19PersistentTickResultV1> {
    ...
    else if (handoff.next_logical_tick_time !== timing.logical_time) throw new Error("EXTERNAL_FORMAL_V3_AM19_REQUESTED_TICK_NOT_NEXT_PERSISTED_TICK");

    const runtimeConfig = await this.runtimeConfigRepository.readRuntimeConfig(input.manifest_slot.runtime_config_ref);
    if (!runtimeConfig) throw new Error("EXTERNAL_FORMAL_V3_AM19_RUNTIME_CONFIG_NOT_FOUND");
    if (runtimeConfig.object_id !== input.manifest_slot.runtime_config_ref) throw new Error("EXTERNAL_FORMAL_V3_AM19_RUNTIME_CONFIG_REF_MISMATCH");
    if (runtimeConfig.determinism_hash !== input.manifest_slot.runtime_config_hash) throw new Error("EXTERNAL_FORMAL_V3_AM19_RUNTIME_CONFIG_HASH_MISMATCH");
    if (runtimeConfig.logical_time !== timing.logical_time || runtimeConfig.as_of !== timing.logical_time) throw new Error("EXTERNAL_FORMAL_V3_AM19_RUNTIME_CONFIG_TIME_MISMATCH");
    ...
    if (!aRecordSet) {
      if (externalRuntime.parent_runtime_config_ref !== handoff.previous_state_runtime_config_ref
        || externalRuntime.parent_runtime_config_hash !== handoff.previous_state_runtime_config_hash) throw new Error("EXTERNAL_FORMAL_V3_AM19_PARENT_RUNTIME_CONFIG_MISMATCH");
      if (externalRuntime.crop_stage_context_authority.context_ref !== input.manifest_slot.crop_stage_context_ref
        || externalRuntime.crop_stage_context_authority.context_hash !== input.manifest_slot.crop_stage_context_hash) throw new Error("EXTERNAL_FORMAL_V3_AM19_MANIFEST_CROP_CONTEXT_MISMATCH");
      ...
      if (evidence.database_write_count !== 0) throw new Error("EXTERNAL_FORMAL_V3_AM19_EVIDENCE_SOURCE_WRITE_FORBIDDEN");
      if (evidence.provider_request_count !== 0) throw new Error("EXTERNAL_FORMAL_V3_AM19_RUNTIME_PROVIDER_FETCH_FORBIDDEN");
      const candidate = executeExternalFormalAmendment19CanonicalTickV1({
        scope,
        logical_time: timing.logical_time,
        created_at: timing.logical_time,
      ...
      if (candidate.provider_request_count !== 0 || candidate.database_write_count !== 0 || candidate.scheduler_write_count !== 0) {
        throw new Error("EXTERNAL_FORMAL_V3_AM19_CANONICAL_CORE_SIDE_EFFECT_FORBIDDEN");
      }
      ...
      if (!readback || readback.aggregate_determinism_hash !== aRecordSet.aggregate_determinism_hash) throw new Error("EXTERNAL_FORMAL_V3_AM19_A_READBACK_MISMATCH");
      aRecordSet = readback;
    }

    validateCap04ARecordSetV1(aRecordSet);
    const runtimeHealth = healthFromPersistedARecordSetV1(aRecordSet);
    const forcingMode = forcingModeFromPersistedARecordSetV1(aRecordSet);
    ...
    const common = {
      service_id: EXTERNAL_FORMAL_V3_AMENDMENT19_PERSISTENT_TICK_SERVICE_ID_V1,
      provider_availability_watermark: EXTERNAL_FORMAL_V3_AMENDMENT19_WATERMARK_ID_V1,
      ...
      current_interval_forcing_mode: forcingMode,
      provider_wait_required: false as const,
    ...
    if (aRecordSet.operation_key.operation_variant === CAP04_A2_OPERATION_VARIANT_V1) {
      if (forecastPayload.status !== "BLOCKED") throw new Error("EXTERNAL_FORMAL_V3_AM19_A2_FORECAST_STATUS_MISMATCH");
      ...
      if (illegalScenario !== null) throw new Error("EXTERNAL_FORMAL_V3_AM19_A2_SCENARIO_FORBIDDEN");
      const next = await this.handoffService.prepareNextTickInput(scope);
      if (next.next_logical_tick_time !== addOneHourV1(timing.logical_time)) throw new Error("EXTERNAL_FORMAL_V3_AM19_NEXT_HANDOFF_TIME_MISMATCH");
    ...
    if (aRecordSet.operation_key.operation_variant !== CAP04_A1_OPERATION_VARIANT_V1 || forecastPayload.status !== "COMPLETED") throw new Error("EXTERNAL_FORMAL_V3_AM19_A1_FORECAST_STATUS_MISMATCH");
    ...
    const scenarioCandidate = buildCap04ScenarioSetRecordV1({ source_forecast: forecast, scenario_math_result: scenarioMath, created_at: timing.logical_time });
    ...
    if (bRecord) {
      ...
      if (bRecord.aggregate_determinism_hash !== scenarioCandidate.aggregate_determinism_hash) throw new Error("EXTERNAL_FORMAL_V3_AM19_SCENARIO_IDEMPOTENCY_CONFLICT");
    } else {
    ...
    if (!bReadback) throw new Error("EXTERNAL_FORMAL_V3_AM19_SCENARIO_READBACK_NOT_FOUND");
    validateCap04ScenarioSetRecordV1(bReadback, forecast);
    if (bReadback.aggregate_determinism_hash !== bRecord.aggregate_determinism_hash) throw new Error("EXTERNAL_FORMAL_V3_AM19_SCENARIO_READBACK_HASH_MISMATCH");
    bRecord = bReadback;
