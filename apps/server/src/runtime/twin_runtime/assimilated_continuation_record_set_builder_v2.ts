...
import {
  buildAssimilatedContinuationRecordSetIdentityV2,
  type AssimilatedContinuationRecordSetV2,
} from "../../domain/twin_runtime/assimilated_continuation_record_set_identity_v2.js";
import {
  ASSIMILATED_CONTINUATION_METHOD_ID_V1,
  ASSIMILATED_CONTINUATION_OBSERVATION_SELECTOR_ID_V2,
...
export type BuildAssimilatedContinuationRecordSetInputV2 = {
...
function buildAssimilationUpdatePayloadV2(input: {
  assimilation: AssimilatedContinuationPosteriorV1;
  evidence_window: AssimilatedContinuationEvidenceWindowV2;
  runtime_config: CanonicalObjectEnvelopeV1;
  transition_ref: string;
  state_ref: string;
}): AssimilatedContinuationUpdatePayloadV2 {
  const result = input.assimilation;
  return {
    status: result.status,
    disposition: result.disposition,
    policy_id: "MCFT_CAP_03_OBSERVATION_ASSIMILATION_POLICY_V2",
...
export function buildAssimilatedContinuationRecordSetV2(
  input: BuildAssimilatedContinuationRecordSetInputV2,
  ...
  validateAssimilatedContinuationRuntimeConfigPayloadV2(input.runtime_config.payload);
  const config = input.runtime_config.payload as unknown as AssimilatedContinuationRuntimeConfigPayloadV2;
  if (config.record_set_contract_id !== ASSIMILATED_CONTINUATION_RECORD_SET_CONTRACT_ID_V2) {
  ...
  const assimilationPayload = buildAssimilationUpdatePayloadV2({
  ...
  const identity = buildAssimilatedContinuationRecordSetIdentityV2({
    ...
    aggregate_identity_input: {
      ...
      observation_policy_version: `${ASSIMILATED_CONTINUATION_OBSERVATION_SELECTOR_ID_V2}:1`,
  ...
  const recordSet: AssimilatedContinuationRecordSetV2 = { ...identity, members };
  validateAssimilatedContinuationCrossReferencesV2(recordSet);
  return recordSet;
}
