// Purpose: orchestrate one Amendment-18 Formal V3 slot with read-only readiness
// checks before the durable scheduler claim, then execute with the exact same scheduler
// claim/fence through ExternalFormalV3Amendment11PersistentTickServiceV1.
// Boundary: no provider/R2 access, no timer loop, no implicit-latest config selection,
...
import type {
  ExecuteExternalFormalV3Amendment11PersistentTickResultV1,
  ExternalFormalV3Amendment11DatabaseEvidenceSourcePortV1,
  ExternalFormalV3Amendment11ManifestSlotPinV1,
  ExternalFormalV3Amendment11PersistentTickServiceV1,
} from "./external_formal_v3_amendment11_persistent_tick_service_v1.js";
import type {
...
export const EXTERNAL_FORMAL_V3_A18_RUNNER_ID_V1 = "MCFT_CAP09_EXTERNAL_FORMAL_V3_A18_RUNNER_V1" as const;
export const EXTERNAL_FORMAL_V3_A18_RUNNER_WATERMARK_ID_V1 = "PROVIDER_AVAILABILITY_WATERMARK_V1" as const;

export type ExternalFormalV3A18ManifestSlotPinV1 = ExternalFormalV3Amendment11ManifestSlotPinV1 & {
  parent_runtime_config_ref: string;
  parent_runtime_config_hash: string;
  crop_stage_context_materialization_hash: string;
};

export type ExternalFormalV3A18WindowManifestV1 = {
  manifest_ref: string;
  ...
  slots: readonly ExternalFormalV3A18ManifestSlotPinV1[];
};

export interface ExternalFormalV3A18CropContextMaterializerPortV1 {
  materialize(input: {
    logical_time: string;
...
type TickServicePortV1 = Pick<ExternalFormalV3Amendment11PersistentTickServiceV1, "executeClaimedTick">;

export type ExecuteExternalFormalV3A18RunnerInputV1 = {
...
export type ExecuteExternalFormalV3A18RunnerResultV1 =
  | {
      runner_id: typeof EXTERNAL_FORMAL_V3_A18_RUNNER_ID_V1;
      status: "NO_DUE_SLOT";
  ...
  | {
      ...
      tick_result: ExecuteExternalFormalV3Amendment11PersistentTickResultV1;
      provider_request_count: 0;
      r2_request_count: 0;
    }
  | {
      runner_id: typeof EXTERNAL_FORMAL_V3_A18_RUNNER_ID_V1;
      status: "FAILED_TERMINAL_RECORDED";
      slot_id: ShadowOnlineSlotIdV1;
      logical_time: string;
      detail: string;
      claim_attempted: true;
...
): ExternalFormalV3A18ManifestSlotPinV1 {
  if (!sameScopeV1(boundary.scope, manifest.scope)) throw new Error("EXTERNAL_FORMAL_A18_RUNNER_BOUNDARY_SCOPE_MISMATCH");
  const matches = manifest.slots.filter((slot) => slot.slot_id === boundary.slot_id && slot.logical_time === boundary.logical_time);
  if (matches.length !== 1) throw new Error("EXTERNAL_FORMAL_A18_RUNNER_EXACT_MANIFEST_SLOT_REQUIRED");
  const slot = matches[0];
  if (slot.manifest_ref !== manifest.manifest_ref || slot.manifest_hash !== manifest.manifest_hash || slot.epoch_id !== manifest.epoch_id) {
    throw new Error("EXTERNAL_FORMAL_A18_RUNNER_MANIFEST_IDENTITY_MISMATCH");
  }
  return slot;
}

function exactRuntimeConfigPreclaimV1(
  config: CanonicalObjectEnvelopeV1,
  slot: ExternalFormalV3A18ManifestSlotPinV1,
): void {
  if (config.object_id !== slot.runtime_config_ref || config.determinism_hash !== slot.runtime_config_hash) {
    throw new Error("EXTERNAL_FORMAL_A18_RUNNER_RUNTIME_CONFIG_PIN_MISMATCH");
  }
  const payload = config.payload as Record<string, unknown>;
  if (payload.effective_logical_time !== slot.logical_time) throw new Error("EXTERNAL_FORMAL_A18_RUNNER_RUNTIME_CONFIG_TIME_MISMATCH");
  if (payload.parent_runtime_config_ref !== slot.parent_runtime_config_ref
    || payload.parent_runtime_config_hash !== slot.parent_runtime_config_hash) {
    throw new Error("EXTERNAL_FORMAL_A18_RUNNER_RUNTIME_CONFIG_PARENT_MISMATCH");
  }
  ...
  if (!crop || crop.context_ref !== slot.crop_stage_context_ref || crop.context_hash !== slot.crop_stage_context_hash) {
    throw new Error("EXTERNAL_FORMAL_A18_RUNNER_RUNTIME_CONFIG_CROP_BINDING_MISMATCH");
  }
  if (payload.config_selection_mode !== "EXPLICIT_REF_HASH_PIN_ONLY") {
    throw new Error("EXTERNAL_FORMAL_A18_RUNNER_EXPLICIT_CONFIG_PIN_REQUIRED");
  }
}

export class ExternalFormalV3Amendment11RunnerV1 {
  constructor(
    private readonly manifest: ExternalFormalV3A18WindowManifestV1,
    private readonly scheduler: SchedulerPortSubsetV1,
    private readonly runtimeConfigRepository: RuntimeConfigReadPortV1,
    private readonly cropContextMaterializer: ExternalFormalV3A18CropContextMaterializerPortV1,
    ...
    if (!sameScopeV1(manifest.scope, { ...MCFT_CAP09_EXTERNAL_FORMAL_SCOPE_V1 })) {
      throw new Error("EXTERNAL_FORMAL_A18_RUNNER_EXACT_T3R1_SCOPE_REQUIRED");
    }
    if (manifest.slots.length !== 24) throw new Error("EXTERNAL_FORMAL_A18_RUNNER_EXACT_24_MANIFEST_SLOTS_REQUIRED");
  }

  async executeOneDueSlot(input: ExecuteExternalFormalV3A18RunnerInputV1): Promise<ExecuteExternalFormalV3A18RunnerResultV1> {
    const throughLogicalTime = canonicalIsoV1(input.through_logical_time, "EXTERNAL_FORMAL_A18_RUNNER_THROUGH_TIME_INVALID");
    const snapshotTime = canonicalIsoV1(input.evidence_snapshot_time, "EXTERNAL_FORMAL_A18_RUNNER_SNAPSHOT_TIME_INVALID");
    const observerStartedAt = canonicalIsoV1(input.observer_started_at, "EXTERNAL_FORMAL_A18_RUNNER_OBSERVER_TIME_INVALID");
    requiredTextV1(input.lease_owner, "EXTERNAL_FORMAL_A18_RUNNER_LEASE_OWNER_REQUIRED");
    if (!Number.isInteger(input.lease_duration_seconds) || input.lease_duration_seconds <= 0 || input.lease_duration_seconds > 3600) {
      throw new Error("EXTERNAL_FORMAL_A18_RUNNER_LEASE_DURATION_INVALID");
    }
    if (Date.parse(snapshotTime) > Date.parse(observerStartedAt)) throw new Error("EXTERNAL_FORMAL_A18_RUNNER_OBSERVER_BEFORE_SNAPSHOT");

    const due = await this.scheduler.listMissedSlots({ scope: this.manifest.scope, through_logical_time: throughLogicalTime });
    if (due.length === 0) {
      return { runner_id: EXTERNAL_FORMAL_V3_A18_RUNNER_ID_V1, status: "NO_DUE_SLOT", claim_attempted: false, provider_request_count: 0, r2_request_count: 0 };
    }
    ...
    if (!runtimeConfig) {
      return {
        ...
        detail: slot.runtime_config_ref,
        claim_attempted: false,
        provider_request_count: 0,
        r2_request_count: 0,
      };
    ...
    try {
      ...
      return {
        ...
        slot_id: slot.slot_id,
        logical_time: slot.logical_time,
        reason: "RUNTIME_CONFIG_PIN_MISMATCH",
    ...
    try {
      ...
      return {
        ...
        slot_id: slot.slot_id,
        logical_time: slot.logical_time,
        reason: "CROP_CONTEXT_BINDING_FAILED",
        detail: errorDetailV1(error),
        claim_attempted: false,
        provider_request_count: 0,
        r2_request_count: 0,
    ...
    try {
      ...
      await this.scheduler.recordTerminalResult({
        ...
        result: {
          ...
          state: "COMPLETED",
          tick_ref: null,
          health_ref: `${EXTERNAL_FORMAL_V3_A18_RUNNER_ID_V1}:${slot.slot_id}:COMPLETED`,
      ...
      await this.scheduler.recordTerminalResult({
        ...
        result: {
          ...
          state: "FAILED",
          tick_ref: null,
          health_ref: `${EXTERNAL_FORMAL_V3_A18_RUNNER_ID_V1}:${slot.slot_id}:FAILED`,
          terminal_at: observerStartedAt,
        },
      });
      return {
