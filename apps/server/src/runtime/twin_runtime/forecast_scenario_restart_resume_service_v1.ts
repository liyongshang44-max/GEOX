...
// Purpose: validate explicit CAP-04 Forecast/Scenario restart-resume and bounded forward-backfill intent against persisted checkpoint authority, then delegate to the verified contiguous range service.
...
import type {
  RunCap04ForecastScenarioRangeInputV1,
  RunCap04ForecastScenarioRangeResultV1,
} from "./forecast_scenario_range_service_v1.js";
import type {
  PreparedRestartInputV1,
  TwinScopeKeyV1,
} from "./ports.js";

export type ResumeCap04ForecastScenarioFromCheckpointPortV1 = {
  resumeFromCheckpointV1(scope: TwinScopeKeyV1): Promise<PreparedRestartInputV1>;
};

export type Cap04ForecastScenarioRangePortV1 = {
  runContiguousRange(
    input: RunCap04ForecastScenarioRangeInputV1,
  ): Promise<RunCap04ForecastScenarioRangeResultV1>;
};

export type RunCap04ForecastScenarioRestartResumeInputV1 =
  RunCap04ForecastScenarioRangeInputV1;

export type RunCap04ForecastScenarioBoundedBackfillInputV1 =
  RunCap04ForecastScenarioRangeInputV1 & {
    evidence_intent: "MISSED_SCHEDULE_CATCH_UP" | "LATE_EVIDENCE_REVISION";
    requested_start_logical_time?: string;
  };

export type Cap04ForecastScenarioRestartResumeResultV1 = {
  ...
  range_result: RunCap04ForecastScenarioRangeResultV1;
};
...
const CHECKPOINT_PROJECTION_DIVERGENCE_SOURCE_CODES_V1 = new Set([
  ...
  "PREVIOUS_FORECAST_RESULT_SCOPE_MISMATCH",
  "RUNTIME_CONFIG_SCOPE_MISMATCH",
  "LATEST_CHECKPOINT_OBJECT_TYPE_MISMATCH",
  "LATEST_STATE_OBJECT_TYPE_MISMATCH",
  "PREVIOUS_FORECAST_RESULT_OBJECT_TYPE_MISMATCH",
  "RUNTIME_CONFIG_OBJECT_TYPE_REQUIRED",
  ...
  "CHECKPOINT_FORECAST_RESULT_REF_MISMATCH",
  "CHECKPOINT_FORECAST_RESULT_LINEAGE_MISMATCH",
  "CHECKPOINT_STATE_RUNTIME_CONFIG_MISMATCH",
  "FORECAST_STATE_RUNTIME_CONFIG_MISMATCH",
  "PERSISTED_RUNTIME_CONFIG_MISMATCH",
  "SUCCESSFUL_FORECAST_POINTER_INVALID",
...
function validateRequestedStartV1(
  ...
  if (!Number.isFinite(parsed) || new Date(parsed).toISOString() !== requestedStartLogicalTime) {
    throw new Error("CAP04_BACKFILL_START_NOT_CANONICAL_HOUR");
  }
  ...
  if (requestedStartLogicalTime !== persistedStartLogicalTime) {
    throw new Error("CAP04_BACKFILL_START_NOT_PERSISTED_NEXT_TICK");
  }
}

export class Cap04ForecastScenarioRestartResumeServiceV1 {
  constructor(
    private readonly checkpointService: ResumeCap04ForecastScenarioFromCheckpointPortV1,
    private readonly rangeService: Cap04ForecastScenarioRangePortV1,
  ) {}

  async resumeFromCheckpoint(
    input: RunCap04ForecastScenarioRestartResumeInputV1,
  ): Promise<Cap04ForecastScenarioRestartResumeResultV1> {
    ...
    const rangeResult = await this.rangeService.runContiguousRange(input);
  ...
  async runBoundedBackfill(
    input: RunCap04ForecastScenarioBoundedBackfillInputV1,
  ): Promise<Cap04ForecastScenarioRestartResumeResultV1> {
    ...
    try {
      ...
      if (errorCodeV1(error) === "PERSISTED_NEXT_TICK_STATE_NOT_FOUND") {
        throw new Error("CAP04_BACKFILL_BEFORE_BOOTSTRAP");
      }
      rethrowRestartReadErrorV1(error);
    }
