...
// Purpose: provide the production persistence-free External Formal CAP04 orchestration path from explicit caller-supplied authority/evidence through frozen compatibility math to honest External A1/A2 canonical candidates.
...
import {
  ...
  type ExternalFormalCap04InputAuthorityV1,
} from "./external_formal_cap04_input_authority_v1.js";
import {
...
function exactExternalScopeV1(scope: TwinScopeKeyV1): void {
  exactScopeV1(scope, { ...MCFT_CAP09_EXTERNAL_FORMAL_SCOPE_V1 }, "EXTERNAL_CAP04_SERVICE_SCOPE_MISMATCH");
}
...
function compatibilityDynamicsConfigV1(
...
function stateDecimalBasisV1(state: CanonicalObjectEnvelopeV1): {
  ...
  if (typeof mean?.value !== "string" || typeof variance?.value !== "string") {
    throw new Error("EXTERNAL_CAP04_SERVICE_STATE_DECIMAL_BASIS_REQUIRED");
  }
  return {
    storage_mean_mm_decimal: mean.value,
    storage_variance_mm2_decimal: variance.value,
  };
}

function externalRuntimePayloadV1(
  ...
  if (payload.config_role !== "HOURLY_CAP04" || payload.effective_logical_time !== logicalTime) {
    throw new Error("EXTERNAL_CAP04_SERVICE_HOURLY_RUNTIME_CONFIG_REQUIRED");
  }
  return payload;
}
...
function commonRecordSetInputV1(
  ...
  return {
    ...
    previous_forecast_result_hash: input.handoff.previous_forecast_result_hash
      ?? `sha256:${input.handoff.previous_forecast_result_ref}`,
...
export function executeExternalFormalCap04CandidateV1(
  ...
  const externalRuntime = externalRuntimePayloadV1(input.runtime_config, logicalTime);
  ...
  const dynamics = executeHourlyWaterBalanceV1({
    ...
    gross_rainfall_mm_decimal: normalizeFixedDecimalV1(
      String(finiteNumberV1(base.rainfall_record.canonical_payload.value, "EXTERNAL_CAP04_SERVICE_RAINFALL_REQUIRED")),
      WATER_AMOUNT_SCALE_V1,
    ),
    historical_et0_mm_decimal: normalizeFixedDecimalV1(
      String(finiteNumberV1(base.historical_et0_record.canonical_payload.value, "EXTERNAL_CAP04_SERVICE_HISTORICAL_ET0_REQUIRED")),
      WATER_AMOUNT_SCALE_V1,
    ),
    ...
    config: compatibilityDynamicsConfigV1(compatibility),
  });
  ...
  const evidence: AssimilatedContinuationEvidenceWindowV2 = finalizeAssimilatedContinuationEvidenceWindowV2({
    window: preliminary,
    assimilation,
  });
  ...
  const decimalBasis = stateDecimalBasisV1(sourceState);
  ...
  if (forcingOutcome.status === "FAILED") {
    throw new Error(`EXTERNAL_CAP04_SERVICE_FUTURE_FORCING_FAILED:${forcingOutcome.reason_codes.join("|")}`);
  }
  ...
  const compatibilityForecastMath = executeCap04Pure72hForecastMathV1({
    source_posterior: {
      ...
      logical_time: logicalTime,
      computation_basis: decimalBasis,
    },
    runtime_config: {
      ref: input.runtime_config.object_id,
      hash: input.runtime_config.determinism_hash,
