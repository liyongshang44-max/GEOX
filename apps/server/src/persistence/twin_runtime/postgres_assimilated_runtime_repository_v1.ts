...
import { validateAssimilatedContinuationCrossReferencesV2 } from "../../domain/twin_runtime/assimilated_continuation_cross_ref_validator_v2.js";
import {
...
import type {
  ...
  AssimilatedContinuationPersistencePortV2,
...
export type VersionedAssimilatedContinuationRecordSetV1 =
  | AssimilatedContinuationRecordSetV1
  | AssimilatedContinuationRecordSetV2;

export function assertNoSameOperationKeyVersionConflictV1(input: {
  existing_contract_id: unknown;
  requested_contract_id: unknown;
}): void {
  const knownContracts = new Set<unknown>([
    ASSIMILATED_CONTINUATION_RECORD_SET_CONTRACT_ID_V1,
    ASSIMILATED_CONTINUATION_RECORD_SET_CONTRACT_ID_V2,
  ]);
  if (
    knownContracts.has(input.existing_contract_id)
    && knownContracts.has(input.requested_contract_id)
    && input.existing_contract_id !== input.requested_contract_id
  ) {
    throw new Error(
...
function requireMemberV1(
  recordSet: VersionedAssimilatedContinuationRecordSetV1,
...
export class PostgresAssimilatedRuntimeRepositoryV2
  extends PostgresRuntimeRepositoryV1
  implements AssimilatedContinuationPersistencePortV2 {
  constructor(private readonly poolV1: Pool) {
    super(poolV1);
  }
  ...
  async lookupAssimilatedContinuationRecordSet(
    ...
    const guard = await this.poolV1.query(
      "SELECT record_set_id,identity_basis FROM twin_object_idempotency_index_v1 WHERE identity_kind='A2_RECORD_SET' AND idempotency_key=$1",
      [idempotencyKey],
    );
    ...
    assertNoSameOperationKeyVersionConflictV1({
      existing_contract_id: identityBasis?.record_set_contract_id,
      requested_contract_id:
        ASSIMILATED_CONTINUATION_RECORD_SET_CONTRACT_ID_V2,
    });
    return this.readAssimilatedContinuationRecordSet(
      guard.rows[0].record_set_id,
    );
  }
  ...
  private async readAssimilatedContinuationRecordSetWithClientV1(
    ...
    if (facts.rows.length !== 8) return null;
    const members = facts.rows.map((row) => parseFactObjectV1(row.record_json));
    const aggregate = identityBasis.aggregate_identity_input as AssimilatedContinuationRecordSetV2["aggregate_identity_input"];
    const runtimeConfig = await this.verifyRuntimeConfigReferenceV1(
      client,
      aggregate.runtime_config_ref,
      aggregate.runtime_config_hash,
    );
    const recordSet: AssimilatedContinuationRecordSetV2 = {
      record_set_contract_id: ASSIMILATED_CONTINUATION_RECORD_SET_CONTRACT_ID_V2,
      continuation_operation_key: identityBasis.continuation_operation_key as AssimilatedContinuationRecordSetV2["continuation_operation_key"],
      continuation_operation_key_hash: String(
        identityBasis.continuation_operation_key_hash,
      ),
      continuation_record_set_id: recordSetId,
      ...
      member_object_ids: Object.fromEntries(
        members.map((member) => [member.object_type, member.object_id]),
      ) as AssimilatedContinuationRecordSetV2["member_object_ids"],
      aggregate_identity_input: aggregate,
      continuation_record_set_determinism_hash: guard.rows[0].determinism_hash,
      members,
    };
  ...
  async commitAssimilatedContinuationState(
    input: Parameters<
      AssimilatedContinuationPersistencePortV2["commitAssimilatedContinuationState"]
    >[0],
  ) {
    validateAssimilatedContinuationCrossReferencesV2(input.record_set);
    assertScopeMatchesV1(input.scope, input.record_set);
    const client = await this.poolV1.connect();
    const inject = (stage: string) => input.fault_injection?.(stage);
    let committed = false;
    try {
      await client.query("BEGIN");
      const existing = await client.query(
        "SELECT record_set_id,determinism_hash,identity_basis FROM twin_object_idempotency_index_v1 WHERE identity_kind='A2_RECORD_SET' AND idempotency_key=$1 FOR UPDATE",
        [input.record_set.continuation_idempotency_key],
      );
      if (existing.rows.length) {
        const identityBasis =
          existing.rows[0].identity_basis as
