...
export class PostgresRuntimeRepositoryV1 implements RuntimeConfigRepositoryPortV1, BootstrapPersistencePortV1 {
  ...
  async commitRuntimeConfig(config: CanonicalObjectEnvelopeV1) {
    ...
    try {
      ...
      await client.query("INSERT INTO facts (fact_id, occurred_at, source, record_json) VALUES ($1,$2::timestamptz,'system',$3)", [factId(config.object_id), config.logical_time, recordJson(config)]);
  ...
  async readRuntimeConfig(objectId: string): Promise<CanonicalObjectEnvelopeV1 | null> {
    const result = await this.pool.query("SELECT record_json FROM facts WHERE record_json::jsonb->>'type'='twin_runtime_config_v1' AND record_json::jsonb->'payload'->>'object_id'=$1 LIMIT 1", [objectId]);
    return result.rows.length ? (typeof result.rows[0].record_json === "string" ? JSON.parse(result.rows[0].record_json) : result.rows[0].record_json).payload : null;
  }

  async acquireLease(claim: Omit<RuntimeLeaseClaimV1, "fencing_token">): Promise<RuntimeLeaseClaimV1> {
    const values = scopeValues(claim);
    const result = await this.pool.query(`INSERT INTO twin_runtime_lease_v1 (tenant_id,project_id,group_id,field_id,season_id,zone_id,lease_owner,fencing_token,acquired_at,expires_at,heartbeat_at)
      VALUES ($1,$2,$3,$4,$5,$6,$7,1,transaction_timestamp(),transaction_timestamp()+($8||' seconds')::interval,transaction_timestamp())
      ON CONFLICT (tenant_id,project_id,group_id,field_id,season_id,zone_id) DO UPDATE SET lease_owner=EXCLUDED.lease_owner,fencing_token=twin_runtime_lease_v1.fencing_token+1,acquired_at=transaction_timestamp(),expires_at=transaction_timestamp()+($8||' seconds')::interval,heartbeat_at=transaction_timestamp()
      WHERE twin_runtime_lease_v1.expires_at <= transaction_timestamp() OR twin_runtime_lease_v1.lease_owner=EXCLUDED.lease_owner
      RETURNING fencing_token`, [...values, claim.lease_owner, claim.lease_duration_seconds]);
  ...
  async lookupA0RecordSet(idempotencyKey: string): Promise<A0RecordSetV1 | null> {
    const guard = await this.pool.query("SELECT record_set_id,determinism_hash,member_object_ids FROM twin_object_idempotency_index_v1 WHERE identity_kind='A0_RECORD_SET' AND idempotency_key=$1", [idempotencyKey]);
    if (!guard.rows.length) return null;
    return this.readBootstrapRecordSet(guard.rows[0].record_set_id);
  }
  ...
  async commitBootstrapState(input: Parameters<BootstrapPersistencePortV1["commitBootstrapState"]>[0]) {
    ...
    try {
      ...
      const initial = await client.query("SELECT 1 FROM facts WHERE record_json::jsonb->>'type'='twin_runtime_lineage_v1' AND record_json::jsonb->'payload'->'payload'->>'lineage_kind'='INITIAL' AND record_json::jsonb->'payload'->>'tenant_id'=$1 AND record_json::jsonb->'payload'->>'project_id'=$2 AND record_json::jsonb->'payload'->>'group_id'=$3 AND record_json::jsonb->'payload'->>'field_id'=$4 AND record_json::jsonb->'payload'->>'season_id'=$5 AND record_json::jsonb->'payload'->>'zone_id'=$6 LIMIT 1", scopeValues(input.scope));
      ...
      for (let index = 0; index < input.record_set.members.length; index += 1) {
        const object = input.record_set.members[index]; inject(`before_fact_${index + 1}_${object.object_type}`);
        const id = factId(object.object_id); factIds[object.object_id] = id;
        await client.query("INSERT INTO facts (fact_id,occurred_at,source,record_json) VALUES ($1,$2::timestamptz,'system',$3)", [id, object.logical_time, recordJson(object)]);
      ...
      await client.query("INSERT INTO twin_object_idempotency_index_v1 (identity_kind,idempotency_key,record_set_id,determinism_hash,member_object_ids,member_determinism_hashes) VALUES ('A0_RECORD_SET',$1,$2,$3,$4::jsonb,$5::jsonb)", [input.record_set.a0_idempotency_key,input.record_set.a0_record_set_id,input.record_set.a0_record_set_determinism_hash,JSON.stringify(input.record_set.members.map((member) => member.object_id)),JSON.stringify(Object.fromEntries(input.record_set.members.map((member) => [member.object_id,member.determinism_hash])))]);
      inject("before_commit"); await client.query("COMMIT");
  ...
  private async readBootstrapRecordSetWithClient(client: PoolClient, recordSetId: string): Promise<A0RecordSetV1 | null> {
    const guard = await client.query("SELECT idempotency_key,determinism_hash,member_object_ids FROM twin_object_idempotency_index_v1 WHERE identity_kind='A0_RECORD_SET' AND record_set_id=$1", [recordSetId]);
    if (!guard.rows.length) return null;
    const ids: string[] = guard.rows[0].member_object_ids;
    const facts = await client.query("SELECT record_json FROM facts WHERE record_json::jsonb->'payload'->>'object_id'=ANY($1::text[])", [ids]);
    if (facts.rows.length !== 9) return null;
    const members = facts.rows.map((row) => (typeof row.record_json === "string" ? JSON.parse(row.record_json) : row.record_json).payload) as CanonicalObjectEnvelopeV1[];
    return { a0_record_set_id: recordSetId, a0_idempotency_key: guard.rows[0].idempotency_key, a0_record_set_determinism_hash: guard.rows[0].determinism_hash, members };
  }

  async readBootstrapRecordSet(recordSetId: string): Promise<A0RecordSetV1 | null> {
    const client = await this.pool.connect();
    try { return await this.readBootstrapRecordSetWithClient(client, recordSetId); }
