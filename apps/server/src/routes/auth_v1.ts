...
function buildSessionPermissions(scopes: string[]): SessionPermissions {
  ...
  return {
    customer_read: hasAny(["fields.read", "ao_act.index.read", "action.read"]),
...
export function registerAuthV1Routes(app: FastifyInstance): void {
  app.get("/api/v1/auth/me", async (req, reply) => {
    ...
    if (!auth) return;
    return reply.send({ ok: true, actor_id: auth.actor_id, token_id: auth.token_id, tenant_id: auth.tenant_id, project_id: auth.project_id, group_id: auth.group_id, role: auth.role, scopes: auth.scopes, allowed_field_ids: auth.allowed_field_ids });
  });
  ...
  app.post("/api/v1/auth/login", async (req, reply) => {
    ...
    if (isNonEmptyString(idpLoginUrl)) {
      const resp = await fetch(String(idpLoginUrl), { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    ...
    return reply.send({ ok: true, token, provider: "local_allowlist", actor_id: rec.actor_id, token_id: rec.token_id, tenant_id: rec.tenant_id, project_id: rec.project_id, group_id: rec.group_id, role: rec.role ?? "admin", scopes: Array.isArray(rec.scopes) ? rec.scopes : [] });
  });
  ...
  app.post("/api/v1/auth/logout", async (req, reply) => {
    ...
    if (isNonEmptyString(idpLogoutUrl)) {
      ...
      const resp = await fetch(String(idpLogoutUrl), { method: "POST", headers: { "content-type": "application/json", authorization: authz }, body: JSON.stringify({ ok: true }) });
  ...
  app.get("/api/v1/auth/providers", async (req, reply) => {
    return reply.send({ ok: true, mode: isNonEmptyString(process.env.AUTH_IDP_LOGIN_URL) ? "external_idp" : "local_allowlist", idp_login_url: process.env.AUTH_IDP_LOGIN_URL ?? null, idp_logout_url: process.env.AUTH_IDP_LOGOUT_URL ?? null, login_path: `${hostBase(req)}/api/v1/auth/login`, logout_path: `${hostBase(req)}/api/v1/auth/logout` });
  });
}
