...
export function registerAcceptanceV1Routes(app: FastifyInstance, pool: Pool): void {
  ...
  app.post("/api/v1/acceptance/from-evidence-artifacts", async (req, reply) => {
    ...
    const asExec = await pool.query(`SELECT * FROM as_executed_record_v1 WHERE tenant_id=$1 AND project_id=$2 AND group_id=$3 AND as_executed_id=$4 LIMIT 1`, [tenant.tenant_id, tenant.project_id, tenant.group_id, body.as_executed_id]);
