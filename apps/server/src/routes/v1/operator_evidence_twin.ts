...
// Purpose: expose the Operator Evidence Twin read-only endpoint with H53.1 sensing, H53.2 WaterStressState, H53.3 Forecast/Scenario, and H53.4 Recommendation Candidate readback.
// Boundary: this route reads scoped facts/rows only; it never writes approvals, operation plans, AO-ACT tasks, receipts, acceptance, verification, ROI, or Field Memory.
...
const H53_2_SOURCE = "H53_2_WATER_STRESS_STATE_DERIVATION_V1";
const H53_2_VERSION = "h53.2.v1";
const H53_3_SOURCE = "H53_3_FORECAST_SCENARIO_DERIVATION_V1";
const H53_3_VERSION = "h53.3.v1";
...
function buildTwin(scope: Scope, field: Row | null, telemetry: Row | null, observation: Row | null, window: Row | null, weather: Row | null, waterStateFact: Row | null, forecastFact: Row | null, scenarioFact: Row | null, recommendationFact: Row | null) {
  ...
  const state = waterStateNode(waterStateFact);
  const forecast = forecastNode(forecastFact);
  const scenario = scenarioNode(scenarioFact);
  const recommendation = recommendationNode(recommendationFact);
  const approval = statusNode("APPROVAL", "人工审批", "approval", "approval_request_v1", "APPROVAL_NOT_CREATED_AFTER_H53_4");
  const operation = statusNode("OPERATION_PLAN", "作业计划", "operation_plan", "operation_plan_v1", "OPERATION_PLAN_NOT_CREATED_AFTER_H53_4");
  const aoAct = statusNode("AO_ACT", "AO-ACT", "task", "ao_act_task_v0", "AO_ACT_NOT_CREATED_AFTER_H53_4");
  const asExecuted = statusNode("AS_EXECUTED", "实执记录", "as_executed", "as_executed_record_v1", "AS_EXECUTED_NOT_CREATED_AFTER_H53_4");
  const evidence = statusNode("EVIDENCE", "执行证据", "evidence", "evidence_artifact_v1", "EVIDENCE_NOT_CREATED_AFTER_H53_4");
  const acceptance = statusNode("ACCEPTANCE", "执行验收", "acceptance", "acceptance_result_v1", "ACCEPTANCE_NOT_CREATED_AFTER_H53_4");
  const verification = statusNode("VERIFICATION", "灌后水分响应验证", "verification", "water_response_verification_v1", "VERIFICATION_NOT_CREATED_AFTER_H53_4");
  ...
  const gaps = [
    ...(!waterStateFact ? [gap("WATER_STRESS_STATE_DERIVED_PENDING", "H53.2 尚未生成水分压力状态。", "INFO", [state.id])] : []),
    ...(!forecastFact ? [gap("FORECAST_DERIVED_PENDING", "H53.3 尚未生成水分预测。", "INFO", [forecast.id])] : []),
    ...(!scenarioFact ? [gap("SCENARIO_DERIVED_PENDING", "H53.3 尚未生成情景。", "INFO", [scenario.id])] : []),
    ...(!recommendationFact ? [gap("RECOMMENDATION_DERIVED_PENDING", "H53.4 尚未生成建议候选。", "INFO", [recommendation.id])] : []),
    gap("APPROVAL_NOT_CREATED_AFTER_H53_4", "H53.4 只生成建议候选，不生成审批。", "INFO", [approval.id]),
    gap("ACCEPTANCE_NOT_CREATED_AFTER_H53_4", "H53.4 不生成灌后验收。", "INFO", [acceptance.id]),
  ];
  const summaryText = recommendationFact ? "H53.4 已生成建议候选；仍不生成审批、作业计划或执行。" : forecastFact && scenarioFact ? "H53.3 已生成预测与情景；建议候选等待 H53.4。" : "H53.2 已生成状态；预测与情景等待 H53.3。";
  return { version: "v1", surface: "OPERATOR", report_kind: "OPERATOR_EVIDENCE_TWIN", request_scope: { tenant_id: scope.tenantId, project_id: scope.projectId, group_id: scope.groupId, fieldId: scope.fieldId, field_id: scope.fieldId }, scope_policy: { required: true, accepted_scope_keys: ["tenant_id", "project_id", "group_id", "field_id"], scope_applied: scoped(scope), missing_reason: scoped(scope) ? null : "TENANT_PROJECT_GROUP_FIELD_SCOPE_REQUIRED" }, field: { field_id: scope.fieldId || "", field_name: text(field?.display_name ?? field?.field_name ?? scope.fieldId), crop_text: text(field?.crop ?? field?.crop_text) || null, tenant_id: scope.tenantId, project_id: scope.projectId, group_id: scope.groupId, canonical_route: `/app/operator/fields/${encodeURIComponent(scope.fieldId || "")}/evidence-twin`, legacy_routes: [`/operator/twin/fields/${encodeURIComponent(scope.fieldId || "")}`] }, current_state: { label: waterStateFact ? "水分压力状态已生成" : "感知数据已回读，状态待推导", code: rawPayload(waterStateFact)?.water_state ?? null, status: waterStateFact ? "AVAILABLE" : "LIMITED", confidence: { label: rawPayload(waterStateFact)?.confidence_level ?? null, score: n(rawPayload(waterStateFact)?.confidence_score), level: rawPayload(waterStateFact)?.confidence_level ?? null }, quality: { status: waterStateFact ? "AVAILABLE" : "LIMITED", quality_flags: [], blocking_reasons: waterStateFact ? [] : ["STATE_DERIVED_PENDING"], confidence_penalty: null }, latest_update_time: null, state_refs: waterStateFact ? refs({ kind: "fact", id: waterStateFact.fact_id, schema: "water_state_estimate_v1" }) : [], evidence_refs: waterStateFact ? arr(rawPayload(waterStateFact)?.evidence_refs) : [], summary_text: summaryText }, lineage: { raw_signals: [raw], observations: [obs, sensingWindow], state_estimates: [state], evidence: [evidence], verifications: [verification] }, water_stress_loop: { loop_id: "water_stress_loop_v1", label: "水分压力闭环", subtitle: "猎鹰 1 号", inputs: { soil_moisture: [raw, obs, sensingWindow], canopy_temperature: [], weather_forecast: [weatherInput], irrigation_event: [] }, water_stress_state: state, forecast, scenario, recommendation, approval, operation, ao_act: aoAct, as_executed: asExecuted, evidence, acceptance, verification, steps: STEP_DEFINITIONS.map(([code, order, label, kind]) => ({ ...stepByCode[code], step_code: code, order, label, kind, required_for_p0: true })) }, source_inventory: sourceInventory(telemetry, observation, window, weather, waterStateFact, forecastFact, scenarioFact, recommendationFact), quality: { status: forecastFact && scenarioFact && recommendationFact ? "AVAILABLE" : "LIMITED", blocking_reason: forecastFact && scenarioFact && recommendationFact ? null : "WATER_STRESS_LOOP_DERIVED_PENDING", low_quality_reasons: [], simulation_data_present: false, official_data_qualified: Boolean(telemetry && observation && window) }, gaps, boundary_rules: [boundary("NO_APPROVAL_CREATION", "H53.4 read model 不创建审批。"), boundary("NO_OPERATION_PLAN_CREATION", "H53.4 read model 不创建作业计划。"), boundary("NO_AO_ACT_TASK_CREATION", "H53.4 read model 不创建 AO-ACT task。"), boundary("NO_DISPATCH", "H53.4 read model 不派单。"), boundary("NO_ACCEPTANCE_SEED", "H53.4 不写验收。"), boundary("NO_FIELD_MEMORY_WRITE", "H53.4 不写 Field Memory。"), boundary("NO_ROI_WRITE", "H53.4 不写 ROI。")], legacy_conflict_policy: { canonical_routes: [`/app/operator/fields/${encodeURIComponent(scope.fieldId || "")}/evidence-twin`, `/app/operator/fields/${encodeURIComponent(scope.fieldId || "")}/evidence-twin/water-stress`], legacy_routes: [`/operator/twin/fields/${encodeURIComponent(scope.fieldId || "")}`], legacy_visible_by_url_only: true, delete_old_pages_first: false, route_governance_required: true } };
}
...
export function registerOperatorEvidenceTwinReadRoutes(app: FastifyInstance, pool: Pool): void {
  app.get("/api/v1/operator/fields/:fieldId/evidence-twin", async (req: any, reply) => {
    ...
    const [field, telemetry, observation, window, weather, waterState, forecast, scenario, recommendation] = await Promise.all([latestField(pool, scope), latestTelemetry(pool, scope, devices), latestObservation(pool, scope), latestWindow(pool, scope), latestWeather(pool, scope), latestH532WaterState(pool, scope), latestH533Forecast(pool, scope), latestH533Scenario(pool, scope), latestH534Recommendation(pool, scope)]);
    const twin = buildTwin(scope, field, telemetry, observation, window, weather, waterState, forecast, scenario, recommendation);
