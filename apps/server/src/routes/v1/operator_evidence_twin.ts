...
// Purpose: expose the Operator Evidence Twin read-only endpoint with H53.1 sensing readback and H53.2 WaterStressState readback.
// Boundary: this route reads scoped rows/facts and never writes facts, recommendations, approvals, AO-ACT tasks, receipts, acceptance, ROI, or Field Memory.
...
function buildTwin(scope: Scope, field: Row | null, telemetry: Row | null, observation: Row | null, window: Row | null, weather: Row | null, waterStateFact: Row | null) {
  ...
  const approval = statusNode("APPROVAL", "人工审批", "approval", "approval_request_v1", "APPROVAL_NOT_CREATED_IN_H53_2");
  const operation = statusNode("OPERATION_PLAN", "作业计划", "operation_plan", "operation_plan_v1", "OPERATION_PLAN_NOT_CREATED_IN_H53_2");
  const aoAct = statusNode("AO_ACT", "AO-ACT", "task", "ao_act_task_v0", "AO_ACT_NOT_CREATED_IN_H53_2");
  const asExecuted = statusNode("AS_EXECUTED", "实执记录", "as_executed", "as_executed_record_v1", "AS_EXECUTED_NOT_CREATED_IN_H53_2");
  const evidence = statusNode("EVIDENCE", "执行证据", "evidence", "evidence_artifact_v1", "EVIDENCE_NOT_CREATED_IN_H53_2");
  const acceptance = statusNode("ACCEPTANCE", "执行验收", "acceptance", "acceptance_result_v1", "ACCEPTANCE_NOT_CREATED_IN_H53_2");
  const verification = statusNode("VERIFICATION", "灌后水分响应验证", "verification", "water_response_verification_v1", "VERIFICATION_NOT_CREATED_IN_H53_2");
  ...
  const gaps = [
    ...
    gap("FORECAST_DERIVED_PENDING", "H53.2 只生成水分状态，不生成预测。", "INFO", [forecast.id]),
    gap("SCENARIO_DERIVED_PENDING", "H53.2 不生成情景、建议、审批或执行。", "INFO", [scenario.id]),
    gap("ACCEPTANCE_NOT_CREATED_IN_H53_2", "H53.2 不生成灌后验收。", "INFO", [acceptance.id]),
  ];
  return {
    version: "v1",
    surface: "OPERATOR",
    ...
    current_state: { label: waterStateFact ? "水分压力状态已生成" : telemetry || observation || window ? "感知数据已回读，状态待推导" : "感知数据缺失", code: rawPayload(waterStateFact)?.water_state ?? null, status: waterStateFact ? "AVAILABLE" : telemetry || observation || window ? "LIMITED" : "MISSING", confidence: { label: rawPayload(waterStateFact)?.confidence_level ?? null, score: n(rawPayload(waterStateFact)?.confidence_score), level: rawPayload(waterStateFact)?.confidence_level ?? null }, quality: { status: waterStateFact ? "AVAILABLE" : telemetry || observation || window ? "LIMITED" : "BLOCKING", quality_flags: [], blocking_reasons: waterStateFact ? [] : telemetry || observation || window ? ["STATE_DERIVED_PENDING"] : ["SENSING_SOURCE_MISSING"], confidence_penalty: null }, latest_update_time: null, state_refs: waterStateFact ? refs({ kind: "fact", id: waterStateFact.fact_id, schema: "water_state_estimate_v1" }) : [], evidence_refs: waterStateFact ? arr(rawPayload(waterStateFact)?.evidence_refs) : [], summary_text: waterStateFact ? "H53.2 已从 H53.1 sensing-only 输入生成水分压力状态；仍不生成预测、情景、建议、审批或执行。" : "H53.1 只展示 RawSignal / Observation；不从 seed 写入水分状态、建议、执行或验收。" },
    ...
    source_inventory: sourceInventory(telemetry, observation, window, weather, waterStateFact),
    quality: { status: telemetry && observation && window ? waterStateFact ? "AVAILABLE" : "LIMITED" : "BLOCKING", blocking_reason: telemetry && observation && window ? waterStateFact ? null : "STATE_DERIVED_PENDING" : "SENSING_SOURCE_MISSING", low_quality_reasons: [], simulation_data_present: false, official_data_qualified: Boolean(telemetry && observation && window) },
    gaps,
    boundary_rules: [boundary("NO_AO_ACT_TASK_CREATION", "H53.2 read model 不创建 AO-ACT task。"), boundary("NO_DISPATCH", "H53.2 read model 不派单。"), boundary("NO_APPROVAL_BYPASS", "H53.2 read model 不绕过人工审批。"), boundary("NO_RECOMMENDATION_SEED", "H53.2 不写建议候选。"), boundary("NO_FORECAST_OR_SCENARIO", "H53.2 不生成预测或情景。"), boundary("NO_ACCEPTANCE_SEED", "H53.2 不写验收。"), boundary("NO_FIELD_MEMORY_WRITE", "H53.2 不写 Field Memory。"), boundary("NO_ROI_WRITE", "H53.2 不写 ROI。")],
...
export function registerOperatorEvidenceTwinReadRoutes(app: FastifyInstance, pool: Pool): void {
  app.get("/api/v1/operator/fields/:fieldId/evidence-twin", async (req: any, reply) => {
    ...
    const [field, telemetry, observation, window, weather, waterState] = await Promise.all([latestField(pool, scope), latestTelemetry(pool, scope, devices), latestObservation(pool, scope), latestWindow(pool, scope), latestWeather(pool, scope), latestH532WaterState(pool, scope)]);
    const twin = buildTwin(scope, field, telemetry, observation, window, weather, waterState);
