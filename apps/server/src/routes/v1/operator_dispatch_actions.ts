...
function roleAllowsDispatch(role: unknown): boolean {
...
function isTerminalStatus(value: unknown): boolean {
  ...
  return status === "COMPLETED" || status === "SUCCESS" || status === "RECEIPT_RECEIVED" || status === "PENDING_ACCEPTANCE";
}

function isRetryableStatus(value: unknown): boolean {
  const status = normalizeStatus(value);
  return status === "DISPATCH_FAILED" || status === "EXECUTION_FAILED" || status === "INVALID_EXECUTION" || status === "STATE_WRITE_FAILED";
}
...
async function findTaskFact(pool: Pool, auth: AoActAuthContextV0, taskId: string): Promise<TaskFact | null> {
  const res = await pool.query(
    `SELECT fact_id, occurred_at, record_json
      ...
      LIMIT 1`,
    [taskId, auth.tenant_id, auth.project_id, auth.group_id],
  );
...
async function readLatestDispatchFacts(pool: Pool, auth: AoActAuthContextV0, taskIds: string[]): Promise<Map<string, DispatchFact>> {
  ...
  const res = await pool.query(
    `SELECT DISTINCT ON (record_json::jsonb#>>'{payload,act_task_id}') fact_id, occurred_at, record_json
       FROM facts
      WHERE (record_json::jsonb->>'type') = 'ao_act_dispatch_v1'
        AND (record_json::jsonb#>>'{payload,tenant_id}') = $1
        AND (record_json::jsonb#>>'{payload,project_id}') = $2
        AND (record_json::jsonb#>>'{payload,group_id}') = $3
    ...
    [auth.tenant_id, auth.project_id, auth.group_id, taskIds],
  );
  const out = new Map<string, DispatchFact>();
  for (const row of res.rows ?? []) {
    const record = parseRecordJson(row.record_json);
    const taskId = safeText(record?.payload?.act_task_id);
    if (!taskId) continue;
    out.set(taskId, {
      fact_id: String(row.fact_id ?? ""),
      occurred_at: String(row.occurred_at ?? ""),
      status: safeText(record?.payload?.status) || "DISPATCHED",
      reason: nullableText(record?.payload?.reason ?? record?.payload?.failure_reason),
    });
  }
  return out;
}

async function readStatesByTask(pool: Pool, auth: AoActAuthContextV0): Promise<Map<string, OperationStateV1>> {
  const states = await projectOperationStateV1(pool, {
    tenant_id: auth.tenant_id,
    project_id: auth.project_id,
    group_id: auth.group_id,
  });
  const out = new Map<string, OperationStateV1>();
  for (const state of states) {
    if (state.act_task_id) out.set(state.act_task_id, state);
  }
  return out;
}
...
function taskPayload(task: TaskFact | null): any {
  return task?.record_json?.payload ?? {};
}
...
function buildWorklistItem(auth: AoActAuthContextV0, task: TaskFact, state: OperationStateV1 | null, dispatch: DispatchFact | null): Record<string, unknown> {
  ...
  return {
    ...
    operation_id: state?.operation_id ?? payload.operation_id ?? payload.operation_plan_id ?? null,
    operation_plan_id: state?.operation_plan_id ?? payload.operation_plan_id ?? payload.operation_id ?? null,
    field_id: state?.field_id ?? payload.field_id ?? null,
    field_name: null,
    operation_name: state?.action_type ?? payload.action_type ?? null,
    status: normalizeStatus(current),
    execution_mode: payload.executor_id?.kind ?? payload.executor_kind ?? payload.execution_mode ?? null,
    task_created_at: task.occurred_at,
    dispatched_at: dispatch?.occurred_at ?? null,
    acked_at: null,
    receipt_received_at: state?.receipt_id ? new Date(state.last_event_ts || Date.now()).toISOString() : null,
    executor_text: payload.executor_id?.id ?? payload.executor_id ?? payload.device_id ?? null,
    failure_reason: dispatch?.reason ?? state?.invalid_reason ?? null,
    can_dispatch: dispatchPermission.allowed,
    can_retry: retryPermission.allowed,
    permission_reason: dispatchPermission.reason ?? retryPermission.reason,
    permissions: {
      can_dispatch: dispatchPermission.allowed,
      can_retry: retryPermission.allowed,
      reason: dispatchPermission.reason ?? retryPermission.reason,
...
async function handleDispatchAction(req: any, reply: any, pool: Pool, action: OperatorDispatchActionType): Promise<void> {
  ...
  if (!taskId || !task) {
    return sendFailure(reply, pool, auth, buildResponse({
      ...
      role,
      allowed: false,
      reason: "AO-ACT task 未生成，不能派发。",
      message: "AO-ACT task 未生成，不能派发。",
  ...
  if (!permission.allowed) {
    return sendFailure(reply, pool, auth, buildResponse({
      ...
      message: permission.reason ?? "当前任务状态不可派发。",
      error_code: task ? "INVALID_STATE" : "TARGET_NOT_FOUND",
    }));
  }
  ...
  const success = buildResponse({
    ...
    allowed: true,
    reason: null,
    message: action === "TASK_DISPATCH" ? "任务已派发。" : "任务已重新派发。",
  });

  try {
    await writeAuditFact(pool, auth, success);
    await writeDispatchFact(pool, auth, task, action, nextStatus, nullableText((req.body as any)?.note ?? (req.body as any)?.reason));
  } catch {
...
export function registerOperatorDispatchActionRoutes(app: FastifyInstance, pool: Pool): void {
  app.get("/api/v1/operator/dispatch/worklist", async (req, reply) => {
    ...
    const [statesByTask, dispatchByTask] = await Promise.all([
      readStatesByTask(pool, auth),
      readLatestDispatchFacts(pool, auth, taskIds),
    ]);
    const items = tasks.map((task) => {
      const taskId = safeText(task.record_json?.payload?.act_task_id);
      return buildWorklistItem(auth, task, statesByTask.get(taskId) ?? null, dispatchByTask.get(taskId) ?? null);
    });
    return reply.send({
      ...
      items,
      writeReady: true,
      message: "Operator dispatch actions are controlled by backend AO-ACT task state and receipt readiness.",
    });
  });
