...
function sourceQualityFlags(inventoryRow: Row, sourceRow: Row | null | undefined): string[] {
  const flags: string[] = [];
  ...
  const coverageRatio = numberOrNull(firstValue(sourceRow.coverage_ratio, sourceRow.coverageRatio));
  const minCoverageRatio = numberOrNull(firstValue(sourceRow.min_coverage_ratio, sourceRow.minCoverageRatio));
  if (coverageRatio !== null && minCoverageRatio !== null && coverageRatio < minCoverageRatio) {
    flags.push("COVERAGE_RATIO_BELOW_MINIMUM");
  }

  const maxGapMs = numberOrNull(firstValue(sourceRow.max_gap_ms, sourceRow.maxGapMs));
  const maxAllowedGapMs = numberOrNull(firstValue(sourceRow.max_allowed_gap_ms, sourceRow.maxAllowedGapMs));
  if (maxGapMs !== null && maxAllowedGapMs !== null && maxGapMs > maxAllowedGapMs) {
    flags.push("MAX_GAP_EXCEEDS_ALLOWED");
  }

  const qualityStatus = firstText(sourceRow.quality_status, sourceRow.qualityStatus).toUpperCase();
  if (qualityStatus && !["OK", "PASS", "PASSED", "GOOD", "AVAILABLE", "QUALIFIED"].includes(qualityStatus)) {
...
async function buildFieldEvidenceQuality(pool: Pool, scope: RequestScope, fieldId: string): Promise<Row> {
  ...
  const coverageRows = rows.map((row) => {
    ...
    return {
      ...
      coverage_ratio: numberOrNull(firstValue(sourceRow?.coverage_ratio, sourceRow?.coverageRatio)),
      max_gap_ms: numberOrNull(firstValue(sourceRow?.max_gap_ms, sourceRow?.maxGapMs)),
      actual_points: numberOrNull(firstValue(sourceRow?.actual_points, sourceRow?.actualPoints, sourceRow?.observed_points)),
      expected_points: numberOrNull(firstValue(sourceRow?.expected_points, sourceRow?.expectedPoints, sourceRow?.required_points)),
      quality_status: nullableText(firstValue(sourceRow?.quality_status, sourceRow?.qualityStatus)),
      confidence: nullableText(firstValue(sourceRow?.confidence, sourceRow?.confidence_level, sourceRow?.confidence_text)),
      coverage_details: sourceRow
        ...
        : null,
      missing_windows: available ? [] : [row.missing_reason || "SOURCE_INDEX_WINDOW_MISSING"],
      quality_flags: qualityFlags,
      confidence_penalty: qualityFlags.length > 0 ? "LIMITED_BY_SOURCE_ROW_QUALITY" : available ? null : "LIMITED_BY_MISSING_SOURCE_INDEX",
      evidence_refs: [
        ...asArray(row.latest_evidence_refs).map((item) => safeText(item)).filter(Boolean),
