...
export function registerFieldMemoryV1Routes(app: FastifyInstance, pool: Pool): void {
  ...
  app.post("/api/v1/field-memory/from-acceptance", async (req: any, reply: any) => {
    ...
    try {
      ...
      if (["ACCEPTANCE_VERDICT_NOT_PASS", "ACCEPTANCE_NOT_FORMAL", "FORMAL_EVIDENCE_NOT_PASSED", "CHAIN_VALIDATION_NOT_PASSED", "ACCEPTANCE_FIELD_ID_MISSING", "ACCEPTANCE_AMBIGUOUS", "OBSERVATION_PAIR_NOT_FOUND", "OBSERVATION_PAIR_AMBIGUOUS", "RECEIPT_AMBIGUOUS", "FIELD_MEMORY_PROJECT_ID_REQUIRED", "FIELD_MEMORY_GROUP_ID_REQUIRED"].includes(code)) {
