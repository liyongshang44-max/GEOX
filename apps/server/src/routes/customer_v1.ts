...
function safeJsonParse(raw: unknown): any | null { if (raw == null) return null; if (typeof raw === "string") { try { return JSON.parse(raw); } catch { return null; } } return raw as any; }
...
function operationToCustomerItem(op: OperationFactProjection, fieldNameById: Map<string, string | null>): CustomerOperationListItem { const formal = op.formal_acceptance && op.customer_visible_eligible; const operationType = operationDisplayType(op.operation_type); const fallbackState = { final_status: limitedFinalStatusFromState(), acceptance_status: limitedAcceptanceStatusFromState(), pending: formalPendingAcceptanceFromState() }; return { projection_source: formal ? "GUARDED_REPORT" : STATE_FALLBACK_TRUST.projection_source, fallback_limited: !formal, customer_visible_eligible: formal, blocking_reasons: formal ? [] : [...STATE_FALLBACK_TRUST.blocking_reasons], data_trust_status: formal ? "FORMAL" : "LIMITED", data_trust_text: formal ? FORMAL_TRUST_TEXT : LIMITED_TRUST_TEXT, operation_id: op.operation_id, operation_plan_id: op.operation_plan_id, field_id: op.field_id, field_name: op.field_id ? (fieldNameById.get(op.field_id) ?? null) : null, title: `${operationType}作业`, customer_title: `${operationType}作业`, operation_type: op.operation_type, final_status: formal ? "作业已通过验收" : fallbackState.final_status, acceptance_status: formal ? "PASS" : fallbackState.acceptance_status, evidence_status: formal ? "FORMAL_EVIDENCE_PASSED" : "NEEDS_REVIEW", evidence_summary_status: formal ? "PASS" : "PENDING", updated_at: op.updated_at, executed_at: null }; }
...
export function registerCustomerV1Routes(app: FastifyInstance, pool: Pool): void {
  app.get("/api/v1/customer/reports", async (req, reply) => { const ctx = scopeFromRequest(req, reply); if (!ctx) return; const generatedAt = new Date().toISOString(); const fields = await queryFieldRows(pool, ctx.tenant, ctx.scope); const operations = await queryOperationsFromFacts(pool, ctx.tenant, ctx.scope); const operationFieldIds = operations.map((op) => op.field_id).filter((x): x is string => Boolean(x)); const fieldIds = Array.from(new Set([...fields.map((field) => field.field_id), ...operationFieldIds])); const fieldNameById = new Map(fields.map((field) => [field.field_id, field.field_name] as const)); for (const [fieldId, name] of await queryFieldNameMap(pool, ctx.tenant, fieldIds)) if (!fieldNameById.has(fieldId)) fieldNameById.set(fieldId, name); const reports: CustomerReportListItem[] = [{ report_type: "OVERVIEW", title: "经营总览报告", subtitle: limitedSubtitle("基于当前客户驾驶舱可见数据生成"), href: "/customer/export", updated_at: generatedAt, status_text: CUSTOMER_CAPABILITY_REVIEW_TEXT, capability_status: "AVAILABLE", data_trust_status: "LIMITED", data_trust_text: LIMITED_TRUST_TEXT }]; for (const fieldId of fieldIds.slice(0, 20)) reports.push({ report_type: "FIELD", title: `${fieldNameById.get(fieldId) ?? "地块"} · 地块报告`, subtitle: limitedSubtitle("基于当前可见地块数据生成"), href: `/customer/fields/${encodeURIComponent(fieldId)}`, field_id: fieldId, field_name: fieldNameById.get(fieldId) ?? null, updated_at: generatedAt, status_text: CUSTOMER_CAPABILITY_REVIEW_TEXT, capability_status: "AVAILABLE", data_trust_status: "LIMITED", data_trust_text: LIMITED_TRUST_TEXT }); for (const op of operations.slice(0, 30)) { const operationTitle = operationDisplayType(op.operation_type); reports.push({ report_type: "OPERATION", title: `${operationTitle} · 作业报告`, subtitle: limitedSubtitle("基于当前近期作业生成，状态需以受保护作业报告为准"), href: `/customer/operations/${encodeURIComponent(op.operation_id)}`, operation_id: op.operation_id, operation_title: operationTitle, field_id: op.field_id, field_name: op.field_id ? (fieldNameById.get(op.field_id) ?? null) : null, updated_at: op.updated_at ?? generatedAt, status_text: op.formal_acceptance ? "能力可用，已有正式验收" : CUSTOMER_CAPABILITY_REVIEW_TEXT, capability_status: op.formal_acceptance ? "AVAILABLE" : "PENDING", data_trust_status: op.formal_acceptance ? "FORMAL" : "LIMITED", data_trust_text: op.formal_acceptance ? FORMAL_TRUST_TEXT : LIMITED_TRUST_TEXT }); } reports.push({ report_type: "EVIDENCE_VALUE", title: "证据与价值报告", subtitle: "证据包生成能力待接入；当前数据可信级别为有限记录，不代表正式经营结论", href: null, updated_at: generatedAt, status_text: CUSTOMER_CAPABILITY_REVIEW_TEXT, capability_status: "PENDING", data_trust_status: "LIMITED", data_trust_text: LIMITED_TRUST_TEXT }); return reply.send({ ok: true, source: "customer_reports_api", dataScope: "customer_report_center_v1", ...STATE_FALLBACK_TRUST, data_trust_status: "LIMITED", data_trust_text: LIMITED_TRUST_TEXT, generated_at: generatedAt, scope: ctx.scope, report_count: reports.length, reports }); });
  app.get("/api/v1/customer/operations", async (req, reply) => { const ctx = scopeFromRequest(req, reply); if (!ctx) return; const operations = await queryOperationsFromFacts(pool, ctx.tenant, ctx.scope); const fieldIds = Array.from(new Set(operations.map((op) => op.field_id).filter((x): x is string => Boolean(x)))); const fieldNameById = await queryFieldNameMap(pool, ctx.tenant, fieldIds); const items = operations.map((op) => operationToCustomerItem(op, fieldNameById)); const hasFormalOperation = items.some((item) => item.customer_visible_eligible === true); return reply.send({ ok: true, source: "customer_operations_api", dataScope: "customer_operations_v1", projection_source: hasFormalOperation ? "GUARDED_REPORT" : "STATE_FALLBACK_LIMITED", fallback_limited: !hasFormalOperation, customer_visible_eligible: hasFormalOperation, data_trust_status: hasFormalOperation ? "FORMAL" : "LIMITED", data_trust_text: hasFormalOperation ? FORMAL_TRUST_TEXT : LIMITED_TRUST_TEXT, blocking_reasons: hasFormalOperation ? [] : LIMITED_BLOCKING_REASONS, generated_at: new Date().toISOString(), scope: ctx.scope, operation_count: items.length, operations: items }); });
  app.get("/api/v1/customer/fields", async (req, reply) => { const ctx = scopeFromRequest(req, reply); if (!ctx) return; const fields = await queryFieldRows(pool, ctx.tenant, ctx.scope); const operations = await queryOperationsFromFacts(pool, ctx.tenant, ctx.scope); const openAlertsByField = await queryOpenAlertCountByField(pool, ctx.tenant, fields.map((field) => field.field_id)); const latestByField = new Map<string, OperationFactProjection>(); for (const op of operations) if (op.field_id && !latestByField.has(op.field_id)) latestByField.set(op.field_id, op); const items: CustomerFieldListItem[] = fields.map((field) => { const latest = latestByField.get(field.field_id); const agg = { pendingAcceptanceCount: operations.filter((op) => op.field_id === field.field_id && !op.formal_acceptance).length }; return { ...STATE_FALLBACK_TRUST, data_trust_status: "LIMITED", data_trust_text: LIMITED_TRUST_TEXT, field_id: field.field_id, field_name: field.field_name, risk_level: "UNKNOWN", risk_reasons: [], updated_at: latest?.updated_at ?? field.updated_at, crop_name: null, stage_name: null, recent_operation_id: latest?.operation_id ?? null, recent_operation_title: latest ? `${operationDisplayType(latest.operation_type)}作业` : null, open_alerts_count: Number(openAlertsByField.get(field.field_id) ?? 0), pending_acceptance_count: Number(agg?.pendingAcceptanceCount ?? 0) }; }); return reply.send({ ok: true, source: "customer_fields_api", dataScope: "customer_fields_v1", ...STATE_FALLBACK_TRUST, data_trust_status: "LIMITED", data_trust_text: LIMITED_TRUST_TEXT, generated_at: new Date().toISOString(), scope: ctx.scope, field_count: items.length, fields: items }); });

  app.get("/api/v1/customer/fields/:field_id/confirmed-twin-summary", async (req, reply) => { const ctx = scopeFromRequest(req, reply); if (!ctx) return; const fieldId = String((req.params as any)?.field_id ?? "").trim(); if (!fieldId || !isFieldAllowedByCustomerScope(ctx.scope, fieldId)) return reply.code(404).send({ ok: false, error: "NOT_FOUND" }); const fieldQ = await pool.query(`SELECT field_id FROM field_index_v1 WHERE tenant_id = $1 AND field_id = $2 LIMIT 1`, [ctx.tenant.tenant_id, fieldId]).catch(() => ({ rowCount: 0 } as any)); if (fieldQ.rowCount === 0) return reply.code(404).send({ ok: false, error: "NOT_FOUND" }); return reply.send({ ok: true, source: "customer_confirmed_twin_summary_api", dataScope: "OFFICIAL_CUSTOMER_DELIVERY_PORTAL", surface: "CUSTOMER", generated_at: new Date().toISOString(), writeReady: false, operatorTwinReady: false, adminControlPlaneReady: false, forecastRunReady: false, scenarioEditReady: false, recommendationSubmitReady: false, approvalReady: false, taskCreationReady: false, dispatchReady: false, customer_confirmed_twin_summary_v1: await customerConfirmedTwinSummary(pool, ctx.tenant, fieldId) }); });
  app.get("/api/v1/customer/fields/:fieldId/memory", async (req, reply) => { const ctx = scopeFromRequest(req, reply); if (!ctx) return; const fieldId = String((req.params as any)?.fieldId ?? "").trim(); if (!fieldId || !isFieldAllowedByCustomerScope(ctx.scope, fieldId)) return reply.code(404).send({ ok: false, error: "NOT_FOUND" }); const rows = await pool.query(`SELECT memory_id, field_id, operation_id, summary_text, summary, metric_key, metric_value, after_value, confidence, created_at, occurred_at, formal_acceptance_id FROM field_memory_v1 WHERE tenant_id = $1 AND project_id = $2 AND group_id = $3 AND field_id = $4 AND memory_lane = 'FORMAL_FIELD_MEMORY' AND trust_level = 'FORMAL_ACCEPTED' AND customer_visible_memory = true AND learning_eligible = true AND formal_acceptance_id IS NOT NULL ORDER BY COALESCE(created_at, occurred_at) DESC NULLS LAST LIMIT 20`, [ctx.tenant.tenant_id, ctx.tenant.project_id, ctx.tenant.group_id, fieldId]).catch(() => ({ rows: [] as any[] })); const items = (rows.rows ?? []).map((row: any) => ({ memory_id: String(row.memory_id ?? "").trim(), field_id: fieldId, operation_id: textOrNull(row.operation_id), formal_acceptance_id: textOrNull(row.formal_acceptance_id), title: "田块记忆", summary_text: textOrNull(row.summary_text ?? row.summary) ?? "暂无田块记忆摘要", metric_text: memoryMetricText(row), confidence_text: Number(row.confidence ?? 0) >= 0.8 ? "可信度较高" : "需继续跟踪", updated_at: row.created_at ?? row.occurred_at ?? null, data_trust_status: "FORMAL", data_trust_text: FORMAL_TRUST_TEXT })); return reply.send({ ok: true, source: "customer_field_memory_api", dataScope: "customer_field_memory_v1", field_id: fieldId, projection_source: "STATE_FALLBACK_LIMITED", fallback_limited: items.length === 0, customer_visible_eligible: items.length > 0, data_trust_status: items.length ? "FORMAL" : "LIMITED", data_trust_text: items.length ? FORMAL_TRUST_TEXT : LIMITED_TRUST_TEXT, blocking_reasons: items.length ? [] : LIMITED_BLOCKING_REASONS, generated_at: new Date().toISOString(), scope: ctx.scope, memory_count: items.length, items, memories: items }); });
  app.get("/api/v1/customer/fields/:fieldId/geometry", async (req, reply) => { const ctx = scopeFromRequest(req, reply); if (!ctx) return; const fieldId = String((req.params as any)?.fieldId ?? "").trim(); if (!fieldId || !isFieldAllowedByCustomerScope(ctx.scope, fieldId)) return reply.code(404).send({ ok: false, error: "NOT_FOUND" }); const fieldQ = await pool.query(`SELECT field_id, area_ha FROM field_index_v1 WHERE tenant_id = $1 AND field_id = $2 LIMIT 1`, [ctx.tenant.tenant_id, fieldId]); if (fieldQ.rowCount === 0) return reply.code(404).send({ ok: false, error: "NOT_FOUND" }); const geoQ = await pool.query(`SELECT polygon_geojson_json AS geojson, updated_ts_ms FROM field_polygon_v1 WHERE tenant_id = $1 AND field_id = $2 LIMIT 1`, [ctx.tenant.tenant_id, fieldId]); const geometry = normalizeGeometry(geoQ.rows?.[0]?.geojson ?? null); const areaHa = Number(fieldQ.rows?.[0]?.area_ha ?? NaN); const payload: CustomerFieldGeometryResponse = { field_id: fieldId, geometry_status: geometry ? "AVAILABLE" : (geoQ.rows?.[0]?.geojson == null ? "MISSING" : "INVALID"), geometry_format: "GEOJSON", geometry, area_mu: Number.isFinite(areaHa) ? Number((areaHa * 15).toFixed(4)) : null, centroid: geometryCentroid(geometry), updated_at: toIsoFromMs(geoQ.rows?.[0]?.updated_ts_ms ?? 0) }; return reply.send({ ok: true, source: "customer_fields_geometry_api", dataScope: "customer_fields_geometry_v1", scope: ctx.scope, ...payload }); });
}
