...
export function registerCustomerV1Routes(app: FastifyInstance, pool: Pool): void {
  ...
  app.get("/api/v1/customer/fields/:fieldId/geometry", async (req, reply) => {
    ...
    if (!fieldId || !isFieldAllowedByCustomerScope(ctx.scope, fieldId)) return reply.code(404).send({ ok: false, error: "NOT_FOUND", scope: ctx.scope });
    ...
    if (fieldQ.rowCount === 0) return reply.code(404).send({ ok: false, error: "NOT_FOUND", scope: ctx.scope });
    const geoQ = await pool.query(`SELECT polygon_geojson_json AS geojson, updated_ts_ms FROM field_polygon_v1 WHERE tenant_id = $1 AND field_id = $2 LIMIT 1`, [ctx.tenant.tenant_id, fieldId]).catch(() => ({ rows: [] as any[] }));
