...
type TrustFields = { projection_source: ProjectionSource; fallback_limited: boolean; customer_visible_eligible: boolean; blocking_reasons: string[]; data_trust_status: DataTrustStatusV1 };
type CustomerReportListItem = { report_id?: string | null; report_type: "OVERVIEW" | "FIELD" | "OPERATION" | "EVIDENCE_VALUE"; title: string; subtitle?: string | null; href?: string | null; field_id?: string | null; field_name?: string | null; operation_id?: string | null; operation_title?: string | null; updated_at?: string | null; status_text?: string | null; capability_status?: "AVAILABLE" | "PENDING" | "UNAVAILABLE"; data_trust_status: DataTrustStatusV1; };
...
const STATE_FALLBACK_TRUST: TrustFields = { projection_source: "STATE_FALLBACK_LIMITED", fallback_limited: true, customer_visible_eligible: false, blocking_reasons: ["state_fallback_limited_not_customer_official"], data_trust_status: "LIMITED" };
...
function limitedSubtitle(text: string): string { return `${text}；当前数据可信级别为 LIMITED，不代表正式经营结论`; }
...
export function registerCustomerV1Routes(app: FastifyInstance, pool: Pool): void {
  app.get("/api/v1/customer/reports", async (req, reply) => {
    ...
    const states = filterByCustomerScope(await projectOperationStateV1(pool, ctx.tenant), ctx.scope, (state) => (state as any).field_id);
    const fieldRows = await queryFieldRows(pool, ctx.tenant, ctx.scope);
    ...
    const reports: CustomerReportListItem[] = [{ report_type: "OVERVIEW", title: "经营总览报告", subtitle: limitedSubtitle("基于当前客户驾驶舱可见数据生成"), href: "/customer/export", updated_at: generatedAt, status_text: "能力可用，结论需复核", capability_status: "AVAILABLE", data_trust_status: "LIMITED" }];
    for (const fieldId of fieldIds.slice(0, 20)) reports.push({ report_type: "FIELD", title: `${fieldNameById.get(fieldId) ?? "地块"} · 地块报告`, subtitle: limitedSubtitle("基于当前可见地块数据生成"), href: `/customer/fields/${encodeURIComponent(fieldId)}`, field_id: fieldId, field_name: fieldNameById.get(fieldId) ?? null, updated_at: generatedAt, status_text: "能力可用，结论需复核", capability_status: "AVAILABLE", data_trust_status: "LIMITED" });
    const operationSeen = new Set<string>();
    for (const state of states) { const operationId = operationIdOf(state); if (!operationId || operationSeen.has(operationId)) continue; operationSeen.add(operationId); const fieldId = String((state as any).field_id ?? "").trim() || null; const operationTitle = String((state as any).action_type ?? "").trim() || "作业"; reports.push({ report_type: "OPERATION", title: `${operationTitle} · 作业报告`, subtitle: limitedSubtitle("基于当前近期作业生成，状态需以受保护作业报告为准"), href: `/customer/operations/${encodeURIComponent(operationId)}`, operation_id: operationId, operation_title: operationTitle, field_id: fieldId, field_name: fieldId ? (fieldNameById.get(fieldId) ?? null) : null, updated_at: stateUpdatedAt(state) ?? generatedAt, status_text: "需复核", capability_status: "PENDING", data_trust_status: "LIMITED" }); if (operationSeen.size >= 30) break; }
    reports.push({ report_type: "EVIDENCE_VALUE", title: "证据与价值报告", subtitle: "证据包生成能力待接入；当前不构成正式价值结论", href: null, updated_at: generatedAt, status_text: "待接入", capability_status: "PENDING", data_trust_status: "LIMITED" });
    return reply.send({ ok: true, source: "customer_reports_api", dataScope: "OFFICIAL_CUSTOMER_API", projection_source: "STATE_FALLBACK_LIMITED", fallback_limited: true, customer_visible_eligible: false, data_trust_status: "LIMITED", blocking_reasons: STATE_FALLBACK_TRUST.blocking_reasons, generated_at: generatedAt, scope: ctx.scope, report_count: reports.length, reports });
  });

  app.get("/api/v1/customer/operations", async (req, reply) => {
    const ctx = scopeFromRequest(req, reply); if (!ctx) return;
    const states = filterByCustomerScope(await projectOperationStateV1(pool, ctx.tenant), ctx.scope, (state) => (state as any).field_id);
    const fieldIds = Array.from(new Set(states.map((state: any) => String(state.field_id ?? "").trim()).filter(Boolean)));
    const fieldNameById = await queryFieldNameMap(pool, ctx.tenant, fieldIds);
    const operations = states.map((state: any): CustomerOperationListItem | null => {
      const operationId = operationIdOf(state); if (!operationId) return null;
    ...
    return reply.send({ ok: true, source: "customer_operations_api", dataScope: "OFFICIAL_CUSTOMER_API", projection_source: "STATE_FALLBACK_LIMITED", fallback_limited: true, customer_visible_eligible: false, data_trust_status: "LIMITED", blocking_reasons: STATE_FALLBACK_TRUST.blocking_reasons, generated_at: new Date().toISOString(), scope: ctx.scope, operation_count: operations.length, operations });
  });
  ...
  app.get("/api/v1/customer/fields", async (req, reply) => {
    ...
    const states = filterByCustomerScope(await projectOperationStateV1(pool, ctx.tenant), ctx.scope, (state) => (state as any).field_id);
    const fieldRows = await queryFieldRows(pool, ctx.tenant, ctx.scope);
    const stateFieldIds = Array.from(new Set(states.map((state: any) => String(state.field_id ?? "").trim()).filter(Boolean)));
    const fieldIds = fieldRows.length ? fieldRows.map((row) => row.field_id) : stateFieldIds;
    const fieldNameById = new Map(fieldRows.map((row) => [row.field_id, row.name] as const));
    for (const [fieldId, name] of await queryFieldNameMap(pool, ctx.tenant, fieldIds)) if (!fieldNameById.has(fieldId)) fieldNameById.set(fieldId, name);
    const openAlertsByField = await queryOpenAlertCountByField(pool, ctx.tenant, fieldIds);
    ...
    return reply.send({ ok: true, source: "customer_fields_api", dataScope: "OFFICIAL_CUSTOMER_API", projection_source: "STATE_FALLBACK_LIMITED", fallback_limited: true, customer_visible_eligible: false, data_trust_status: "LIMITED", blocking_reasons: STATE_FALLBACK_TRUST.blocking_reasons, generated_at: new Date().toISOString(), scope: ctx.scope, field_count: fields.length, fields });
  });
}
