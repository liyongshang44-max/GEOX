...
export function registerCustomerV1Routes(app: FastifyInstance, pool: Pool): void {
  ...
  app.get("/api/v1/customer/fields/:fieldId/memory", async (req, reply) => {
    ...
    const rows = await pool.query(`SELECT memory_id, field_id, operation_id, summary_text, summary, metric_key, metric_value, after_value, confidence, created_at, updated_at, occurred_at FROM field_memory_v1 WHERE tenant_id = $1 AND field_id = $2 ORDER BY COALESCE(updated_at, created_at, occurred_at) DESC NULLS LAST LIMIT 20`, [ctx.tenant.tenant_id, fieldId]).catch(() => ({ rows: [] as any[] }));
