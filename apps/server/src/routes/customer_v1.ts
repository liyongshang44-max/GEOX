...
type CustomerReportListItem = {
  ...
  data_trust_status: DataTrustStatusV1;
  data_trust_text: string;
};
...
function toMsLoose(value: unknown): number {
...
function limitedSubtitle(text: string): string {
  return `${text}；当前数据可信级别为有限记录，不代表正式经营结论`;
}

function operationDisplayType(operationType: string | null): string {
  const value = String(operationType ?? "").trim().toUpperCase();
  if (value === "IRRIGATION") return "灌溉";
  if (value === "PEST_INSPECTION") return "巡检";
  return value || "作业";
}
...
function normalizeGeometry(raw: unknown): Record<string, unknown> | null {
  ...
  if (obj.type === "Feature") {
    const geometry = normalizeGeometry(obj.geometry);
    return geometry ? { type: "Feature", properties: typeof obj.properties === "object" && obj.properties != null ? obj.properties : {}, geometry } : null;
  }
  if (obj.type === "FeatureCollection") {
    const features = Array.isArray(obj.features) ? obj.features.map((feature) => normalizeGeometry(feature)).filter(Boolean) : [];
...
function scopeFromRequest(req: FastifyRequest, reply: FastifyReply): AuthRouteContext | null {
...
async function queryFieldNameMap(pool: Pool, tenant: TenantTriple, fieldIds: string[]): Promise<Map<string, string | null>> {
  ...
  for (const row of q.rows ?? []) {
    ...
    if (fieldId) out.set(fieldId, textOrNull(row.field_name));
  }
  return out;
}
...
async function queryOpenAlertCountByField(pool: Pool, tenant: TenantTriple, fieldIds: string[]): Promise<Map<string, number>> {
  ...
  for (const row of q.rows ?? []) {
    const fieldId = String(row.field_id ?? "").trim();
    if (fieldId) out.set(fieldId, Number(row.count ?? 0));
  }
  return out;
}
...
async function queryOperationsFromFacts(pool: Pool, tenant: TenantTriple, scope: CustomerScopeV1): Promise<OperationFactProjection[]> {
  ...
  for (const row of acceptanceQ.rows ?? []) {
    ...
    if (operationId) acceptanceByOperation.set(operationId, payload);
  }
...
export function registerCustomerV1Routes(app: FastifyInstance, pool: Pool): void {
  app.get("/api/v1/customer/reports", async (req, reply) => {
    const ctx = scopeFromRequest(req, reply);
    if (!ctx) return;
    ...
    const reports: CustomerReportListItem[] = [{ report_type: "OVERVIEW", title: "经营总览报告", subtitle: limitedSubtitle("基于当前客户驾驶舱可见数据生成"), href: "/customer/export", updated_at: generatedAt, status_text: "可导出", capability_status: "AVAILABLE", data_trust_status: "LIMITED", data_trust_text: LIMITED_TRUST_TEXT }];
