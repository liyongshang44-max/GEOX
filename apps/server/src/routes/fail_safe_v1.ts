...
function requireManualInterventionHumanRoleV1(reply: any, auth: any): boolean {
  const role=String(auth?.role??'').trim();
  if(role==='admin'||role==='operator') return true;
  reply.status(403).send({ok:false,error:'MANUAL_INTERVENTION_ROLE_DENIED'});
  return false;
}
...
export function registerFailSafeV1Routes(app: FastifyInstance, pool: Pool): void {
  ...
  app.post('/api/v1/manual-takeovers/:takeover_id/ack', async (req, reply)=>{const auth=requireAoActAnyScopeV0(req,reply,['action.task.dispatch','security.admin']); if(!auth) return reply; if(!requireManualInterventionHumanRoleV1(reply,auth)) return reply; const body:any=req.body??{}; const p:any=req.params??{}; const tenant=tenantFromBodyOrAuthV1(body, auth); if(!requireTenantMatchOr404V1(reply,auth,tenant)) return; const takeover_id=String(p.takeover_id??''); await acknowledgeManualTakeoverV1(pool,{...tenant,takeover_id});
