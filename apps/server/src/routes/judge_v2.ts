    ...
    try {
      ...
      if (field_id) {
        await recordMemoryV1(pool, body.tenant_id, {
          ...
          trust_reasons: ["JUDGE_PASS_IS_TECHNICAL_SIGNAL_NOT_FORMAL_FIELD_MEMORY"],
          ...
          skill_refs: (() => {
            ...
            return ref ? [{ skill_id: ref.skill_id, skill_version: ref.skill_version, trace_id: ref.trace_id, run_id: ref.run_id }] : [];
          })(),
          evidence_refs: Array.isArray(inserted.evidence_refs) ? inserted.evidence_refs.map((v) => String(v)).filter((v) => v.length > 0) : [],
