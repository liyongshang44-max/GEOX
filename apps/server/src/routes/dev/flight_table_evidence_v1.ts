...
import { runFlightTableEvidenceAcceptanceExportV1 } from "../../services/flight_table/flight_table_evidence_v1.js";
...
function routeError(reply: FastifyReply, err: unknown) {
  ...
  if (message === "FLIGHT_TABLE_INVALID_EVIDENCE_LANE") return badRequest(reply, message);
  if (
    message === "FLIGHT_TABLE_OPERATION_ID_MISSING" ||
    message === "FLIGHT_TABLE_ACT_TASK_ID_MISSING" ||
    message === "FLIGHT_TABLE_RECEIPT_ID_MISSING" ||
    message === "FLIGHT_TABLE_FIELD_NOT_FOUND"
  ) return badRequest(reply, message);
  if (message.startsWith("FLIGHT_TABLE_EVIDENCE_EXPORT_CREATE_FAILED")) return reply.status(502).send({ ok: false, error: "FLIGHT_TABLE_EVIDENCE_EXPORT_CREATE_FAILED", message });
  if (message === "FLIGHT_TABLE_EVIDENCE_EXPORT_JOB_ID_MISSING") return reply.status(502).send({ ok: false, error: message });
  return reply.status(500).send({ ok: false, error: "FLIGHT_TABLE_EVIDENCE_INTERNAL_ERROR", message });
}
...
export function registerFlightTableEvidenceRoutesV1(app: FastifyInstance, pool: Pool): void {
  app.post("/api/v1/dev/flight-table/runs/:runId/evidence/run", async (req, reply) => {
    ...
    try {
      ...
      const result = await runFlightTableEvidenceAcceptanceExportV1({
        pool,
        run,
