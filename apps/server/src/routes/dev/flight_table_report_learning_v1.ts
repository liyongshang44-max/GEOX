...
import { runFlightTableReportLearningClosureV1 } from "../../services/flight_table/flight_table_report_learning_v1.js";
...
function routeError(reply: FastifyReply, err: unknown) {
  ...
  if (message === "FLIGHT_TABLE_OPERATION_ID_MISSING" || message === "FLIGHT_TABLE_FIELD_NOT_FOUND") return badRequest(reply, message);
  return reply.status(500).send({ ok: false, error: "FLIGHT_TABLE_REPORT_LEARNING_INTERNAL_ERROR", message });
}
...
export function registerFlightTableReportLearningRoutesV1(app: FastifyInstance, pool: Pool): void {
  app.post("/api/v1/dev/flight-table/runs/:runId/report-learning/run", async (req, reply) => {
    ...
    try {
      ...
      const result = await runFlightTableReportLearningClosureV1({
        pool,
        run,
