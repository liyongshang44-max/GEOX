...
export type SampleReceiptV1 = { receipt_id: string; sample_id: string; plan_id: string; sampling_plan_fact_id: string; tenant_id: string; project_id: string; group_id: string; field_id: string; zone_id?: string | null; collected_at_ts: number; collector_actor_id: string; sample_type: SamplingSampleTypeV1; depth_cm?: number | null; location_ref?: string | null; barcode?: string | null; evidence_refs: Array<SamplingReceiptEvidenceRefV1>; chain_of_custody_status: "RECORDED" | "MISSING" | "BROKEN" };
export type LabResultImportV1 = { import_id: string; sample_id: string; sample_receipt_fact_id: string; sampling_plan_fact_id: string; plan_id: string; tenant_id: string; project_id: string; group_id: string; field_id: string; imported_at_ts: number; lab_name?: string | null; metrics: Record<string, number | string | null>; units: Record<string, string>; evidence_refs: Array<SamplingEvidenceRefV1>; quality_status: "PASS" | "NEEDS_REVIEW" | "INVALID" };
export type SamplingAcceptanceV1 = { acceptance_id: string; plan_id: string; sample_id: string; import_id?: string | null; tenant_id: string; project_id: string; group_id: string; field_id: string; sampling_plan_fact_id: string; sample_receipt_fact_id?: string | null; lab_result_fact_id?: string | null; verdict: "PASS" | "FAIL" | "INSUFFICIENT_EVIDENCE"; reasons: string[]; evaluated_at_ts: number; evidence_refs: Array<SamplingEvidenceRefV1> };
