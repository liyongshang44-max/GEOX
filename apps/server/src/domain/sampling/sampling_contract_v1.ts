...
export type SamplingPlanV1 = {
  ...
  field_id: string;
  zone_id?: string | null;
  reason: SamplingReasonV1;
  sample_type: SamplingSampleTypeV1;
  required_depth_cm?: number | null;
  required_points: number;
  created_at_ts: number;
  evidence_refs: Array<SamplingEvidenceRefV1>;
};

export type SamplingReceiptEvidenceRefV1 = {
  kind: "raw_sample_v1" | "marker_v1" | "import_run_v1" | "fact_id";
  ref_id: string;
};

export type SampleReceiptV1 = {
  receipt_id: string;
  ...
  barcode?: string | null;
  evidence_refs: Array<SamplingReceiptEvidenceRefV1>;
...
export type SamplingAcceptanceV1 = {
...
export type SamplingAcceptanceEvaluateRequestV1 = {
  plan_id: string;
  plan_fact_id: string;
  sample_id: string;
  receipt_fact_id: string;
...
export type SamplingAcceptanceEvaluateResponseV1 = {
  ok: true;
  acceptance_id: string;
  fact_id: string;
  verdict: "PASS" | "FAIL" | "INSUFFICIENT_EVIDENCE";
...
export const SAMPLING_DOMAIN_HARD_RULES_V1 = {
  ...
  sampling_formal_sources_require_exact_fact_identity: "formal Sampling source selection requires immutable fact identity; latest-wins is forbidden",
} as const;
