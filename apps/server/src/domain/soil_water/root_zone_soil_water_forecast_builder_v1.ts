...
export type RootZoneSoilWaterForecastStatusV1 = "ESTIMATED" | "INSUFFICIENT_STATE" | "INVALID_INPUT" | "UNKNOWN";
export type RootZoneSoilWaterForecastWaterStatusV1 =
  ...
  | "STRESS"
  | "UNKNOWN";
export type RootZoneSoilWaterForecastBoundAppliedV1 = "NONE" | "LOWER_BOUND" | "UPPER_BOUND";
...
export type RootZoneSoilWaterForecastBuildInputV1 = {
  ...
  dailyWeather: Array<{ date: string; precipitation_mm: number; et0_mm: number; crop_coefficient: number }>;
  computed_at: string;
};
...
function sortedWeather(input: RootZoneSoilWaterForecastBuildInputV1["dailyWeather"]) {
...
export function buildRootZoneSoilWaterForecastV1(
  ...
  const dailyWeather = sortedWeather(input.dailyWeather);
  ...
  if (!isFiniteNumber(input.root_zone_available_water_capacity_mm) || input.root_zone_available_water_capacity_mm <= 0) invalidReasons.push("invalid_root_zone_available_water_capacity_mm");
  if (!isFiniteNumber(input.effective_rainfall_factor) || input.effective_rainfall_factor < 0 || input.effective_rainfall_factor > 1) invalidReasons.push("invalid_effective_rainfall_factor");
  if (dailyWeather.length === 0) invalidReasons.push("empty_daily_weather");
  ...
  if (sourceState?.input_status !== "ESTIMATED" && sourceState?.input_status !== "PARTIAL_ESTIMATE") stateReasons.push("source_state_not_estimated");
  if (awf == null || !isFiniteNumber(awf) || awf < 0 || awf > 1) stateReasons.push("invalid_source_available_water_fraction");

  const terminal = (status: RootZoneSoilWaterForecastStatusV1, reasons: string[]): RootZoneSoilWaterForecastPayloadV1 => ({
    ...base,
    daily_forecast: [],
    ...
    derivation: { model_version: ROOT_ZONE_SOIL_WATER_FORECAST_MODEL_VERSION_V1, thresholds },
  ...
  const daily_forecast = dailyWeather.map((day, index) => {
    ...
    if (projected < 0) { projected = 0; bound = "LOWER_BOUND"; }
    if (projected > input.root_zone_available_water_capacity_mm) { projected = round6(input.root_zone_available_water_capacity_mm); bound = "UPPER_BOUND"; }
    previous = projected;
    const fraction = round6(projected / input.root_zone_available_water_capacity_mm);
    return { day_index: index + 1, date: day.date, precipitation_mm: precipitation, effective_precipitation_mm: effectivePrecipitation, et0_mm: et0, crop_coefficient: cropCoefficient, estimated_crop_et_mm: cropEt, net_water_change_mm: net, projected_available_water_mm: projected, projected_available_water_fraction: fraction, forecast_water_status: classify(fraction), bound_applied: bound };
  });
  ...
  return {
    ...
    blocking_reasons: [],
    calculation_inputs: hashBase,
    derivation: { model_version: ROOT_ZONE_SOIL_WATER_FORECAST_MODEL_VERSION_V1, thresholds, bucket_model: "bounded_daily" },
    confidence: { level: sourceState.input_status === "ESTIMATED" ? "HIGH" : "MEDIUM", score: sourceState.input_status === "ESTIMATED" ? 0.9 : 0.7, basis: "deterministic_no_new_action_bucket_model" },
  };
}
