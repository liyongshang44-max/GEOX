...
export function buildCap06CaseWindowV1(input: {
  ...
  if (windowRefMembershipHash !== expectedWindowHash) {
    throw new Error(`CAP06_${input.role}_WINDOW_REF_MEMBERSHIP_HASH_MISMATCH`);
  }
...
export function buildCap06CaseWindowsV1(input: {
  ...
  const combinedCases = [...input.calibration.cases, ...input.holdout.cases];
  const fullResidualSetHash = semanticHashV1(combinedCases.map((item) => ({
    ref: item.residual_ref,
    hash: item.residual_hash,
  })));
  if (fullResidualSetHash !== input.calibration.source_s1_residual_set_hash) {
    throw new Error("CAP06_SOURCE_S1_RESIDUAL_SET_HASH_MISMATCH");
  }
  const fullCaseInputSetHash = semanticHashV1(combinedCases.map((item) => ({
    residual_ref: item.residual_ref,
    residual_hash: item.residual_hash,
    forecast_point_ref: item.source_forecast_point_ref,
    forecast_point_hash: item.source_forecast_point_hash,
  ...
  if (fullCaseInputSetHash !== input.calibration.source_s1_case_input_set_hash) {
  ...
  const semantic = {
    ...
    source_s1_case_input_set_hash: fullCaseInputSetHash,
