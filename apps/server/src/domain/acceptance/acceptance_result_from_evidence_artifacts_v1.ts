...
export function buildAcceptanceResultFromEvidenceArtifactsV1(input: AcceptanceResultFromEvidenceArtifactsInputV1): { submission: OperatorAcceptanceResultSubmissionPayloadV1; acceptanceResult: { type: "acceptance_result_v1"; payload: AcceptanceResultFromEvidenceArtifactsPayloadV1 } | null } {
  ...
  const badDev = input.evidenceArtifacts.some((f) => { const p = payload(f) as any; return text((f as any).type) !== "evidence_artifact_v1" || p.source_lane !== "FORMAL_OPERATION" || p.formal_eligible !== true || p.is_simulated !== false || p.evidence_level !== "FORMAL" || /dev|debug|simulated/i.test(`${p.source_lane} ${p.evidence_level} ${p.level} ${p.artifact_ref}`); });
  ...
  const mismatch = input.evidenceArtifacts.some((f) => { const p = payload(f) as any; const s = p.summary ?? {}; return p.tenant_id !== input.tenant_id || p.project_id !== input.project_id || p.group_id !== input.group_id || p.field_id !== input.field_id || p.act_task_id !== input.task_id || p.receipt_id !== input.receipt_id || s.as_executed_id !== input.as_executed_id || s.task_id !== input.task_id || s.receipt_id !== input.receipt_id || s.source !== "AS_EXECUTED_RECORD_V1" || (input.operation_plan_id !== null && s.operation_plan_id !== input.operation_plan_id); });
