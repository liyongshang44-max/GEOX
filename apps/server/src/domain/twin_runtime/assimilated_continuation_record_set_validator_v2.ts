...
import {
  ...
  validateAssimilatedContinuationTickDiscriminatorV2,
  validateAssimilatedContinuationUpdatePayloadV2,
...
function validateDispositionCandidateContractV1(
  payload: AssimilatedContinuationUpdatePayloadV2,
): void {
  ...
  const selected = selectedCandidates[0] as AssimilatedObservationCandidateV2;
...
export function validateAssimilatedContinuationRecordSetV2(
  recordSet: AssimilatedContinuationRecordSetV2,
): void {
  ...
  validateAssimilatedContinuationTickDiscriminatorV2(tick.payload);
  const assimilation = memberByTypeV1(recordSet, "twin_assimilation_update_v1");
  validateAssimilatedContinuationUpdatePayloadV2(assimilation.payload);
  const assimilationPayload = assimilation.payload as unknown as AssimilatedContinuationUpdatePayloadV2;
  ...
  const computedAggregateHash = computeAssimilatedContinuationRecordSetDeterminismHashV2(aggregate);
