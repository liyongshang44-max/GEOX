...
async function upsertRoiCandidate(pool: Pool, input: {
  ...
  const values = [
    ...
    input.candidate.source_skill_id ?? (input.skill_refs?.[0]?.skill_id ?? null),
    input.candidate.skill_trace_ref ?? (input.skill_trace_id ?? null),
