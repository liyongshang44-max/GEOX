...
export function projectRoiTrustV1(row: any, options?: { default_source_lane?: RoiSourceLaneV1 }): RoiTrustProjectionV1 {
  ...
  const formalAcceptanceId = text(row?.formal_acceptance_id ?? row?.acceptance_id);
  ...
  if (sourceLane === "FLIGHT_TABLE_DEV") trustLevel = "SIMULATED_DEV_ONLY";
  else if (formalAcceptanceId && formalEvidencePassed && chainValidationPassed) trustLevel = "FORMAL_ACCEPTED";
  else if (valueKind === "ASSUMPTION_BASED" || baselineType === "DEFAULT_ASSUMPTION") trustLevel = "HYPOTHESIS_ONLY";
  else if (!formalEvidencePassed || !chainValidationPassed) trustLevel = "INTERIM_SUPPORTED";

  const customerVisibleValue = customerVisibleExplicit === true
    ? trustLevel === "FORMAL_ACCEPTED"
    : trustLevel === "FORMAL_ACCEPTED";
