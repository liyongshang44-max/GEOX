...
export async function enrichOperationReportChainV1(params: { pool: Pool; report: any }): Promise<any> {
  ...
  const cropCode = text(rec.crop_code ?? plan.crop_code);
  const cropName = text(rec.crop_name ?? plan.crop_name) ?? (String(cropCode ?? "").toUpperCase() === "CORN" ? "玉米" : null);
  const seasonId = text(rec.season_id ?? plan.season_id);
  const cropStage = text(rec.crop_stage ?? plan.crop_stage);
  ...
  const enrichedIdentifiers = { ...(report?.identifiers ?? {}), prescription_id: text(report?.identifiers?.prescription_id ?? prescriptionId), approval_id: text(report?.identifiers?.approval_id ?? approvalRequestId), as_executed_id: text(report?.identifiers?.as_executed_id ?? asExecuted?.as_executed_id) };
