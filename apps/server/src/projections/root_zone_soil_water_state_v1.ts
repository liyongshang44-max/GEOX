...
import type {
  RootZoneSoilWaterPotentialClassV1,
  RootZoneSoilWaterStateInputStatusV1,
  RootZoneSoilWaterStatePayloadV1,
} from "../domain/soil_water/root_zone_soil_water_state_builder_v1.js";

type DbConn = Pool | PoolClient;

export const ROOT_ZONE_SOIL_WATER_STATE_INDEX_V1_TABLE =
  "root_zone_soil_water_state_index_v1";

export type RootZoneSoilWaterStateFactV1 = {
  type: "root_zone_soil_water_state_v1";
  payload: RootZoneSoilWaterStatePayloadV1;
};

export type RootZoneSoilWaterStateIndexV1 = RootZoneSoilWaterStatePayloadV1 & {
...
export function mapRootZoneSoilWaterStateRowV1(
  row: Record<string, unknown>,
): RootZoneSoilWaterStateIndexV1 {
  return {
    state_id: textOrEmpty(row.state_id),
    ...
    layer_estimate_refs: stringArray(row.layer_estimate_refs_json),
    layer_count: numberOrZero(row.layer_count),
    estimated_layer_count: numberOrZero(row.estimated_layer_count),
    blocked_layer_count: numberOrZero(row.blocked_layer_count),
    weighted_matric_potential_kpa: numberOrNull(row.weighted_matric_potential_kpa),
    root_zone_available_water_fraction: numberOrNull(row.root_zone_available_water_fraction),
    root_zone_water_potential_class: textOrEmpty(
      row.root_zone_water_potential_class,
    ) as RootZoneSoilWaterPotentialClassV1,
    worst_layer_class: textOrEmpty(
      row.worst_layer_class,
    ) as RootZoneSoilWaterStatePayloadV1["worst_layer_class"],
    stress_layer_count: numberOrZero(row.stress_layer_count),
    limited_layer_count: numberOrZero(row.limited_layer_count),
    input_status: textOrEmpty(row.input_status) as RootZoneSoilWaterStateInputStatusV1,
    ...
    confidence: parseJsonObject(row.confidence_json) as RootZoneSoilWaterStatePayloadV1["confidence"],
...
export async function upsertRootZoneSoilWaterStateIndexV1(
  pool: DbConn,
  payload: RootZoneSoilWaterStatePayloadV1,
  sourceFactId: string | null,
): Promise<RootZoneSoilWaterStateIndexV1> {
  await ensureRootZoneSoilWaterStateIndexV1(pool);

  const result = await pool.query(
    `
      INSERT INTO public.root_zone_soil_water_state_index_v1 (
        state_id,
        tenant_id,
        project_id,
        group_id,
      ...
      VALUES (
        ...
        $20::jsonb, $21::jsonb, $22::jsonb, $23, $24, $25, now()
      )
      ...
      DO UPDATE SET
        ...
        layer_estimate_refs_json = EXCLUDED.layer_estimate_refs_json,
        layer_count = EXCLUDED.layer_count,
        estimated_layer_count = EXCLUDED.estimated_layer_count,
        blocked_layer_count = EXCLUDED.blocked_layer_count,
        weighted_matric_potential_kpa = EXCLUDED.weighted_matric_potential_kpa,
        root_zone_available_water_fraction = EXCLUDED.root_zone_available_water_fraction,
        root_zone_water_potential_class = EXCLUDED.root_zone_water_potential_class,
        worst_layer_class = EXCLUDED.worst_layer_class,
        stress_layer_count = EXCLUDED.stress_layer_count,
        limited_layer_count = EXCLUDED.limited_layer_count,
    ...
    [
      ...
      toJsonbArray(payload.layer_estimate_refs),
      payload.layer_count,
      payload.estimated_layer_count,
      payload.blocked_layer_count,
      payload.weighted_matric_potential_kpa,
      payload.root_zone_available_water_fraction,
      payload.root_zone_water_potential_class,
      payload.worst_layer_class,
      payload.stress_layer_count,
      payload.limited_layer_count,
