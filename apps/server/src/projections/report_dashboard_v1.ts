...
export type CustomerDashboardAggregateV1 = DashboardProjectionTrustV1 & {
  ...
  recent_operations: Array<DashboardProjectionTrustV1 & { operation_id: string; operation_plan_id: string; field_id: string; field_name: string | null; title: string | null; customer_title: string | null; executed_at: string | null; final_status: string; acceptance_status: string | null; risk_level: OperationReportRiskLevel; risk_reasons: string[]; estimated_total_cost: number; execution_duration_ms: number | null; scenario_type?: string; formal_chain_status?: string; evidence_status?: string; fail_safe_status?: string; manual_takeover_status?: string; zone_rollup_status?: string; customer_visible_eligible?: boolean; needs_review?: boolean; blocking_reasons?: string[]; sampling_lab_result_status?: string; sampling_acceptance_status?: string }>;
...
export type FieldPortfolioSummaryV1 = Omit<CustomerDashboardAggregateV1, "top_risk_fields"> & { top_risk_fields: Array<{ field_id: string; risk_level: OperationReportRiskLevel; risk_reasons: string[]; operation_count: number; total_estimated_cost: number; last_executed_at: string | null }> };
...
function toMs(v: unknown): number | null { if (!v) return null; const ms = Date.parse(String(v)); return Number.isFinite(ms) ? ms : null; }
...
function isSimulatedOrTechnicalRoiItem(item: any): boolean { const valueKind = upper(item?.value_kind); const sourceLane = upper(item?.source_lane ?? item?.trust_level ?? item?.memory_lane); return item?.is_simulated === true || sourceLane.includes("SIMULATED") || sourceLane.includes("TECHNICAL") || sourceLane === "AS_EXECUTED_SIGNAL" || sourceLane === "FLIGHT_TABLE_DEV" || valueKind === "SIMULATED" || valueKind === "TECHNICAL_SIGNAL"; }
...
function buildFieldValueSummary(reports: OperationReportV1[]): RoiSummaryTrustV1 {
  ...
  return { total_roi_items: items.length, trusted_value_items: trustedItems.length, hypothesis_items: assumptionBased, measured_items: items.filter((x: any) => upper(x.value_kind) === "MEASURED").length, estimated_items: estimated, assumption_based_items: assumptionBased, insufficient_items: insufficient, insufficient_evidence_items: insufficient, simulated_or_technical_items: simulatedOrTechnical, low_confidence_items: lowConfidence, water_saved_items: waterSaved, labor_saved_items: laborSaved, early_warning_items: earlyWarning, first_pass_acceptance_items: firstPassAcceptance, has_customer_visible_value: hasValue, customer_value_text: hasValue ? `本地块已有 ${trustedItems.length} 条可信价值记录` : "暂无可作为客户正式价值结论的记录" };
}

function buildDashboardRoiSummary(reports: OperationReportV1[]): RoiSummaryTrustV1 { const summary = buildFieldValueSummary(reports); return { ...summary, customer_value_text: summary.has_customer_visible_value ? `当前共有 ${summary.trusted_value_items} 条可信价值记录` : "暂无可作为客户正式价值结论的记录" }; }
...
export function projectCustomerDashboardAggregateV1(params: { reports: OperationReportV1[]; field_name_by_id?: Map<string, string | null>; open_alerts_by_field?: Map<string, number>; pending_acceptance_by_field?: Map<string, number>; pending_actions_summary?: { total_open_alerts: number; unassigned_alerts: number; in_progress_alerts: number; sla_breached_alerts: number; closed_today_alerts; pending_acceptance?: number }; device_summary?: { offline_fields: number; total_devices: number; offline_devices: number } }): CustomerDashboardAggregateV1 {
  ...
  const recentOperations = sortedReports.slice(0, 5).map((report) => { const trust = reportTrust(report); const fieldId = resolveReportFieldId(report); return { ...trust, operation_id: (report as any).identifiers?.operation_id, operation_plan_id: (report as any).identifiers?.operation_plan_id, field_id: fieldId, field_name: params.field_name_by_id?.get(fieldId) ?? null, title: (report as any).operation_title ?? null, customer_title: (report as any).customer_title ?? null, executed_at: toMs((report as any).execution?.execution_finished_at) == null ? null : (report as any).execution?.execution_finished_at, final_status: safeReportFinalStatus(report), acceptance_status: safeReportAcceptanceStatus(report), risk_level: report.risk.level, risk_reasons: [...report.risk.reasons], estimated_total_cost: Number((report as any).cost?.estimated_total ?? 0), execution_duration_ms: (report as any).sla?.execution_duration_ms ?? null, scenario_type: report.formal_scenario?.scenario_type ?? undefined, formal_chain_status: report.formal_scenario?.formal_chain_status ?? undefined, evidence_status: report.formal_scenario?.evidence_status ?? undefined, fail_safe_status: (report as any).fail_safe?.status ?? undefined, manual_takeover_status: (report as any).manual_takeover?.status ?? undefined, zone_rollup_status: (report as any).zone_matrix?.length ? ((report as any).zone_matrix.every((zone: any) => zone.zone_acceptance_result === "PASS") ? "PASS" : "NEEDS_REVIEW") : undefined, customer_visible_eligible: trust.customer_visible_eligible, needs_review: dashboardNeedsReview(trust, report), blocking_reasons: [...trust.blocking_reasons], sampling_lab_result_status: (report as any).sampling?.lab_result_status ?? undefined, sampling_acceptance_status: (report as any).sampling?.acceptance_status ?? undefined }; });
  const reasonCount = new Map<string, number>(); let globalRisk: OperationReportRiskLevel = "LOW"; let estimatedTotalCost = 0, actualTotalCost = 0, slaSum = 0, slaCount = 0, healthy = 0, atRisk = 0, pendingAcceptanceCount = 0; const fieldAgg = new Map<string, { field_id: string; field_name: string | null; risk_level: OperationReportRiskLevel; risk_reasons: Set<string>; open_alerts_count: number; pending_acceptance_count: number; last_operation_at: string | null; last_operation_ms: number }>();
