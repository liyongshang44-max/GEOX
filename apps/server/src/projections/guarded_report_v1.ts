...
function formalAcceptanceId(item: any): string | null { const value = String(item?.formal_acceptance_id ?? "").trim(); return value ? value : null; }
export function isFormalCustomerValueItem(item: any): boolean { return item?.customer_visible_value === true && item?.trust_level === "FORMAL_ACCEPTED" && item?.source_lane === "FORMAL_ACCEPTANCE" && formalAcceptanceId(item) != null && item?.formal_evidence_passed === true && item?.chain_validation_passed === true; }

function isFormalFieldResponseMemory(item: any): boolean {
  return item?.memory_type === "FIELD_RESPONSE_MEMORY"
    && item?.memory_lane === "FORMAL_FIELD_MEMORY"
    && item?.trust_level === "FORMAL_ACCEPTED"
    && item?.customer_visible_memory === true
...
function guardRoiLedger(roi: any, trusted: boolean, trust: GuardTrustLevelV1): any {
  ...
  const guardItem = (item: any) => { const sourceFormalAcceptanceId = formalAcceptanceId(item); const formalValue = trusted && isFormalCustomerValueItem(item); return { ...item, formal_acceptance_id: formalValue ? sourceFormalAcceptanceId : null, source_lane: formalValue ? "FORMAL_ACCEPTANCE" : item?.source_lane, formal_evidence_passed: formalValue, chain_validation_passed: formalValue, customer_visible_value: formalValue, trust_level: formalValue ? "FORMAL_ACCEPTED" : (item?.trust_level ?? trust), customer_text: formalValue ? item?.customer_text : "该价值记录未通过正式链路校验，仅作为内部线索。" }; };
...
function guardFieldMemory(memory: any, trusted: boolean, trust: GuardTrustLevelV1): any {
  const source = memory ?? {};
  const fieldResponseSource = Array.isArray(source.field_response_memory) ? source.field_response_memory : [];
  const deviceSource = Array.isArray(source.device_reliability_memory) ? source.device_reliability_memory : [];
  const skillSource = Array.isArray(source.skill_performance_memory) ? source.skill_performance_memory : [];
  const formalFieldResponse = trusted ? fieldResponseSource.filter(isFormalFieldResponseMemory) : [];
  const hiddenCounts = {
    field_response_memory: Math.max(0, fieldResponseSource.length - formalFieldResponse.length),
    device_reliability_memory: deviceSource.length,
    skill_performance_memory: skillSource.length,
  };
  return {
    field_response_memory: formalFieldResponse,
    device_reliability_memory: [],
    skill_performance_memory: [],
    hidden_by_guard: !trusted || hiddenCounts.field_response_memory > 0 || hiddenCounts.device_reliability_memory > 0 || hiddenCounts.skill_performance_memory > 0,
    trust_level: trusted ? "FORMAL_ACCEPTED" : trust,
    hidden_counts: hiddenCounts,
    formal_memory_filter: {
      memory_lane: "FORMAL_FIELD_MEMORY",
      trust_level: "FORMAL_ACCEPTED",
      customer_visible_memory: true,
      learning_eligible: true,
      formal_acceptance_id_required: true,
    },
  };
}
...
function collectFormalScenarioSourceRefs(report: any): string[] { const ids = report?.identifiers ?? {}; return unique([ids.recommendation_id ? `recommendation:${ids.recommendation_id}` : null, ids.prescription_id ? `prescription:${ids.prescription_id}` : null, ids.approval_id ? `approval:${ids.approval_id}` : null, ids.operation_plan_id ? `operation_plan:${ids.operation_plan_id}` : null, ids.act_task_id ? `ao_act_task:${ids.act_task_id}` : null, ids.receipt_id ? `receipt:${ids.receipt_id}` : null, report?.acceptance?.acceptance_id ? `acceptance:${report.acceptance.acceptance_id}` : null, report?.evidence?.formal_evidence_passed === true ? "evidence:formal_passed" : null, report?.roi_ledger?.summary?.has_customer_visible_value === true ? "roi:customer_visible" : null, Array.isArray(report?.field_memory?.field_response_memory) && report.field_memory.field_response_memory.length > 0 ? "field_memory:visible" : null]); }
...
export function applyGuardedFieldReportV1(fieldReport: any): any { if (!fieldReport || typeof fieldReport !== "object") return fieldReport; const next = clone(fieldReport); next.recent_operations = Array.isArray(next.recent_operations) ? next.recent_operations.map(guardOperationListItem) : []; const hasFormalValue = summaryHasFormalCustomerValue(next.value_summary) && hasFormalOperationInAggregate(next) && hasFormalFieldMemoryInAggregate(next); next.value_summary = { ...(next.value_summary ?? {}), has_customer_visible_value: hasFormalValue, customer_value_text: hasFormalValue ? next.value_summary?.customer_value_text : "该地块价值记录需通过正式链路校验后才可作为可信收益。", trust_level: hasFormalValue ? "FORMAL_ACCEPTED" : "LIMITED_FALLBACK" }; next.guarded_projection = { enabled: true, source: "field_report_guard" }; return next; }
export function applyGuardedCustomerOperationsResponseV1(payload: any): any { if (!payload || typeof payload !== "object") return payload; const next = clone(payload); next.operations = Array.isArray(next.operations) ? next.operations.map(guardOperationListItem) : []; next.guarded_projection = { enabled: true, source: "customer_operations_guard", note: "Customer operations from state fallback are limited unless backed by guarded reports." }; return next; }
export function applyGuardedCustomerReportsResponseV1(payload: any): any { if (!payload || typeof payload !== "object") return payload; const next = clone(payload); next.reports = Array.isArray(next.reports) ? next.reports.map((report: any) => report?.report_type === "OPERATION" ? { ...report, status_text: "需正式链路校验", capability_status: "PENDING", trust_level: "LIMITED_FALLBACK", customer_visible_eligible: false } : report) : []; next.guarded_projection = { enabled: true, source: "customer_reports_guard" }; return next; }
