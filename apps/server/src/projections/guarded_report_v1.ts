...
function blockingReasons(report: any): string[] { return Array.from(new Set([...(Array.isArray(report?.chain_validation?.blocking_reasons) ? report.chain_validation.blocking_reasons : []), ...(Array.isArray(report?.missing_links) ? report.missing_links.map((x: unknown) => `missing:${x}`) : [])].map((x) => String(x ?? "").trim()).filter(Boolean))); }
...
function supportedMeasuredValue(item: any): boolean {
  const evidenceCount = Array.isArray(item?.evidence_refs) ? item.evidence_refs.length : 0;
  const memoryCount = Array.isArray(item?.field_memory_refs) ? item.field_memory_refs.length : 0;
  return upper(item?.value_kind) === "MEASURED"
    && (evidenceCount > 0 || memoryCount > 0)
    && String(item?.customer_text ?? "").trim().length > 0;
}
...
function guardRoiLedger(roi: any, trusted: boolean, trust: GuardTrustLevelV1): any {
  ...
  const guardItem = (item: any) => {
    ...
    return {
      ...
      source_lane: formalValue ? "FORMAL_ACCEPTANCE" : (item?.source_lane ?? null),
      formal_evidence_passed: formalValue ? true : item?.formal_evidence_passed,
      chain_validation_passed: formalValue ? true : item?.chain_validation_passed,
      customer_text: formalValue ? item?.customer_text : "Internal value signal only.",
    };
  };
...
function formalScenarioTypeFor(report: any): string { const existing = upper(report?.formal_scenario?.scenario_type); if (["FORMAL_IRRIGATION", "DEVICE_ANOMALY", "FORMAL_VARIABLE_OPERATION", "FORMAL_SAMPLING", "FORMAL_FERTILIZATION", "FORMAL_PEST_DISEASE_INSPECTION"].includes(existing)) return existing; if (report?.pest_disease_inspection) return "FORMAL_PEST_DISEASE_INSPECTION"; if (report?.fertilization) return "FORMAL_FERTILIZATION"; if (report?.sampling?.sample_id || report?.sampling?.plan_id) return "FORMAL_SAMPLING"; if (Array.isArray(report?.zone_matrix) && report.zone_matrix.length > 0) return "FORMAL_VARIABLE_OPERATION"; const failSafe = upper(report?.fail_safe?.status); const manualTakeover = upper(report?.manual_takeover?.status); if ((failSafe && failSafe !== "NONE") || (manualTakeover && manualTakeover !== "NONE")) return "DEVICE_ANOMALY"; const actionText = [report?.operation_title, report?.customer_title, report?.action_type, report?.as_executed?.actual_params?.action_type, report?.suggested_action?.action_type].map((x) => String(x ?? "").trim()).join("|").toUpperCase(); if (actionText.includes("IRRIG") || actionText.includes("灌溉")) return "FORMAL_IRRIGATION"; if (report?.identifiers?.recommendation_id || report?.identifiers?.prescription_id || report?.identifiers?.act_task_id) return "FORMAL_IRRIGATION"; return "UNKNOWN"; }
function formalChainStatusFor(trust: GuardTrustLevelV1, guardStatus: GuardStatusV1): CustomerFormalChainStatusV1 { if (trust === "FORMAL_CHAIN_PASSED" || guardStatus === "PASSED") return "PASSED"; if (trust === "NEEDS_REVIEW" || guardStatus === "NEEDS_REVIEW") return "NEEDS_REVIEW"; return "BLOCKED"; }
function formalEvidenceStatusFor(trust: GuardTrustLevelV1, guardStatus: GuardStatusV1): CustomerFormalEvidenceStatusV1 { if (trust === "FORMAL_CHAIN_PASSED" || guardStatus === "PASSED") return "FORMAL_EVIDENCE_PASSED"; if (trust === "SIMULATED_DEV_ONLY" || guardStatus === "SIMULATED") return "SIMULATED"; if (trust === "INSUFFICIENT_FORMAL_EVIDENCE" || guardStatus === "INSUFFICIENT_EVIDENCE" || trust === "NEEDS_REVIEW") return "INSUFFICIENT_EVIDENCE"; return "BLOCKED"; }
function collectFormalScenarioSourceRefs(report: any): string[] { const ids = report?.identifiers ?? {}; return unique([ids.recommendation_id ? `recommendation:${ids.recommendation_id}` : null, ids.prescription_id ? `prescription:${ids.prescription_id}` : null, ids.approval_id ? `approval:${ids.approval_id}` : null, ids.operation_plan_id ? `operation_plan:${ids.operation_plan_id}` : null, ids.act_task_id ? `ao_act_task:${ids.act_task_id}` : null, ids.receipt_id ? `receipt:${ids.receipt_id}` : null, report?.acceptance?.acceptance_id ? `acceptance:${report.acceptance.acceptance_id}` : null, report?.acceptance?.formal_acceptance === true ? "acceptance:formal" : null, report?.evidence?.formal_evidence_passed === true ? "evidence:formal_passed" : null, report?.roi_ledger?.summary?.has_customer_visible_value === true ? "roi:customer_visible" : null, report?.field_memory && report.field_memory.hidden_by_guard !== true ? "field_memory:visible" : null, ...(Array.isArray(report?.status_chain) ? report.status_chain.map((x: any) => String(x?.key ?? "").trim() ? `status_chain:${String(x.key).trim()}:${String(x?.status ?? "").trim()}` : null) : []), ...(Array.isArray(report?.formal_scenario?.source_refs) ? report.formal_scenario.source_refs : [])]); }
function reconcileFormalScenarioProjectionV1(report: any, trust: GuardTrustLevelV1, guardStatus: GuardStatusV1, trusted: boolean, reasons: string[]): any { const existing = report?.formal_scenario && typeof report.formal_scenario === "object" ? report.formal_scenario : {}; return { ...existing, scenario_type: formalScenarioTypeFor(report), formal_chain_status: formalChainStatusFor(trust, guardStatus), evidence_status: formalEvidenceStatusFor(trust, guardStatus), customer_visible_eligible: trusted, needs_review: trusted ? Boolean(existing.needs_review ?? false) : true, blocking_reasons: unique([...(Array.isArray(existing.blocking_reasons) ? existing.blocking_reasons : []), ...reasons]), source_refs: collectFormalScenarioSourceRefs(report) }; }

export function applyGuardedOperationReportV1(report: any): any { if (!report || typeof report !== "object") return report; const next = clone(report); const trust = trustLevelFor(next); const guardStatus = guardStatusFor(trust); const trusted = trust === "FORMAL_CHAIN_PASSED"; const reasons = blockingReasons(next); next.customer_status = trusted ? upper(next.execution?.final_status ?? "COMPLETE") : guardStatus; next.trust_level = trust; next.chain_status = guardStatus; next.needs_review = !trusted; next.is_simulated = trust === "SIMULATED_DEV_ONLY"; next.customer_visible_eligible = trusted; next.guarded_projection = { enabled: true, passed: trusted, trust_level: trust, chain_status: guardStatus, customer_visible_eligible: trusted, blocking_reasons: reasons }; if (!trusted) { next.execution = { ...(next.execution ?? {}), final_status: trust === "SIMULATED_DEV_ONLY" ? "SIMULATED_DEV_ONLY" : "NEEDS_REVIEW", customer_status: guardStatus, guarded: true }; next.acceptance = { ...(next.acceptance ?? {}), status: trust === "SIMULATED_DEV_ONLY" ? "SIMULATED" : trust === "INSUFFICIENT_FORMAL_EVIDENCE" ? "INSUFFICIENT_EVIDENCE" : "NEEDS_REVIEW", verdict: null, formal_acceptance: false, missing_evidence: true, missing_items: Array.from(new Set([...(Array.isArray(next.acceptance?.missing_items) ? next.acceptance.missing_items : []), ...reasons])).slice(0, 20) }; next.evidence = { ...(next.evidence ?? {}), evidence_status: trust === "SIMULATED_DEV_ONLY" ? "SIMULATED" : "INSUFFICIENT_EVIDENCE", trusted: false, formal_evidence_passed: false }; } else { next.acceptance = { ...(next.acceptance ?? {}), formal_acceptance: true }; next.evidence = { ...(next.evidence ?? {}), trusted: true, formal_evidence_passed: true }; } if (next.prescription && statusOf(next, "prescription") && statusOf(next, "prescription") !== "DONE") next.prescription = { ...next.prescription, status: statusOf(next, "prescription"), formal_prescription: false }; else if (next.prescription) next.prescription = { ...next.prescription, formal_prescription: trusted }; next.roi_ledger = guardRoiLedger(next.roi_ledger, trusted, trust); next.roi = guardRoiLedger(next.roi, trusted, trust); next.field_memory = guardFieldMemory(next.field_memory, trusted, trust); if (next.evidence_pack_summary && !trusted) next.evidence_pack_summary = { ...next.evidence_pack_summary, status: trust === "SIMULATED_DEV_ONLY" ? "PENDING" : "FAILED", evidence_count: 0, insufficient_reason: next.evidence_pack_summary.insufficient_reason ?? "not formally trusted" }; next.formal_scenario = reconcileFormalScenarioProjectionV1(next, trust, guardStatus, trusted, reasons); return next; }
...
export function applyGuardedFieldReportV1(fieldReport: any): any { if (!fieldReport || typeof fieldReport !== "object") return fieldReport; const next = clone(fieldReport); next.recent_operations = Array.isArray(next.recent_operations) ? next.recent_operations.map(guardOperationListItem) : []; const hasFormalValue = summaryHasFormalCustomerValue(next.value_summary) && hasFormalOperationInAggregate(next); next.value_summary = { ...(next.value_summary ?? {}), has_customer_visible_value: hasFormalValue, customer_value_text: hasFormalValue ? next.value_summary?.customer_value_text : "Value requires formal chain validation.", trust_level: hasFormalValue ? "FORMAL_ACCEPTED" : "LIMITED_FALLBACK" }; next.guarded_projection = { enabled: true, source: "field_report_guard" }; return next; }
