...
function countBundleEvidence(bundle: OperationEvidenceBundleSummaryInputV1 | null | undefined): EvidenceKindCounts {
  return { artifacts: arrayOf(bundle?.artifacts).length, logs: arrayOf(bundle?.logs).length, media: arrayOf(bundle?.media).length, metrics: arrayOf(bundle?.metrics).length };
}
...
function normalizeManifestLabel(value: unknown): string | null {
  ...
  if (!text || text.includes("/") || text.includes("\\") || text.length > 80) return null;
  return text;
}
...
function getEvidencePackFiles(exportJob: unknown): Array<Record<string, unknown>> {
  const files = getEvidencePackRecord(exportJob).files;
...
function findEvidencePackFile(exportJob: unknown, part: "bundle" | "manifest" | "checksums"): Record<string, unknown> | null {
  return getEvidencePackFiles(exportJob).find((file) => String(file.download_part ?? "").trim().toLowerCase() === part) ?? null;
}
function deriveSafeManifest(input: OperationEvidenceSummaryBuilderInputV1): string | null {
  ...
  return manifestFile ? normalizeManifestLabel(manifestFile.name) ?? "manifest.json" : null;
}
function deriveSafeSha256(input: OperationEvidenceSummaryBuilderInputV1): string | null {
  ...
  return isSafeSha256(summary.sha256) ?? isSafeSha256(job.artifact_sha256) ?? isSafeSha256(bundleFile?.sha256) ?? null;
}
function deriveSafeDownloadUrl(input: OperationEvidenceSummaryBuilderInputV1): string | null {
  const summary = getEvidenceSummaryRecord(input.evidence_summary);
  const bundleFile = findEvidencePackFile(input.evidence_export_job, "bundle");
  const delivery = asRecord(getEvidencePackRecord(input.evidence_export_job).delivery) ?? {};
  return isSafeRelativeDownloadUrl(summary.download_url) ?? isSafeRelativeDownloadUrl(bundleFile?.download_path) ?? isSafeRelativeDownloadUrl(delivery.object_store_download_url) ?? null;
}
...
function buildTraceSummary(counts: EvidenceKindCounts, formalCount: number, simulatedCount: number): string {
  return [`现场照片 ${counts.media} 项`, `执行日志 ${counts.logs} 项`, `监测指标 ${counts.metrics} 项`, `附件记录 ${counts.artifacts} 项`, `正式证据 ${formalCount} 项`, `模拟/调试证据 ${simulatedCount} 项`].join("；");
}

function buildDefaultSummary(status: OperationEvidencePackSummaryStatusV1): string {
  if (status === "READY") return "本次作业已形成正式证据包摘要，可用于后续验收复核。";
  if (status === "PENDING") return "本次作业已有部分证据记录，但正式证据仍待补齐或复核。";
  if (status === "FAILED") return "本次作业证据包未通过完整性复核，需要补充证据或由技术支持复核。";
  return "暂无有效正式证据。";
}

function buildInsufficientReason(params: { status: OperationEvidencePackSummaryStatusV1; exportJobStatus: ReturnType<typeof normalizeExportJobStatus>; acceptanceMissingEvidence: boolean; invalidEvidenceState: boolean; formalBlockingReasons: string[] }): string | null {
  if (params.exportJobStatus === "ERROR") return "证据包生成失败，需技术支持复核。";
  if (params.invalidEvidenceState) return "执行记录存在，但正式证据不足或无效，需补充后复核。";
  if (params.acceptanceMissingEvidence) return "验收记录提示证据不足，需补齐现场记录或监测数据。";
  if (params.status === "MISSING") return "当前未查询到可用于验收的正式证据记录。";
  if (params.status === "PENDING" && params.formalBlockingReasons.length > 0) return `正式证据未就绪：${params.formalBlockingReasons.slice(0, 3).join("、")}`;
  if (params.status === "PENDING") return "证据包仍在生成或等待正式证据复核。";
  return null;
}
...
export function buildOperationEvidencePackSummaryV1(input: OperationEvidenceSummaryBuilderInputV1): OperationEvidencePackSummaryV1 {
  ...
  return {
    ...
    generated_at: firstIso(evidenceSummary.generated_at, exportJob.updated_at, exportJob.updated_ts_ms, exportJob.created_ts_ms, payloadOf(input.acceptance)?.generated_at, input.now ?? new Date()),
  };
}
