...
`,
  ...
  {
    name: path.join("hosting", "mcft_cap09_phase5_identity_bootstrap.js"),
    content: `import { runMcftCap09Phase5ServiceIdentityBootstrapFromEnvironmentV1 } from "../apps/server/src/infra/mcft_cap09_phase5_service_identity_bootstrap_v1.js";

runMcftCap09Phase5ServiceIdentityBootstrapFromEnvironmentV1().catch((error) => {
  console.error(\`FATAL: MCFT-CAP-09 Phase5 service identity bootstrap failed: \${error instanceof Error ? error.stack ?? error.message : String(error)}\`);
  process.exit(1);
});
`,
  },
