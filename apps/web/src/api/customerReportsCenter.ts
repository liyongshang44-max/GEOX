...
type CustomerReportsApiEnvelope =
  | { ok?: boolean; reports?: CustomerReportCenterItem[]; generated_at?: string | null; data?: unknown }
  | { ok?: boolean; customer_reports?: CustomerReportCenterItem[]; generated_at?: string | null }
  | CustomerReportCenterItem[];

function normalizeReportsPayload(payload: unknown): { reports: CustomerReportCenterItem[]; generatedAt?: string | null } {
  if (Array.isArray(payload)) return { reports: payload };
  if (!payload || typeof payload !== "object") return { reports: [] };
  const obj = payload as Record<string, unknown>;
  if (Array.isArray(obj.reports)) return { reports: obj.reports as CustomerReportCenterItem[], generatedAt: obj.generated_at as string | null | undefined };
  if (Array.isArray(obj.customer_reports)) return { reports: obj.customer_reports as CustomerReportCenterItem[], generatedAt: obj.generated_at as string | null | undefined };
...
function toFallbackReports(aggregate: CustomerDashboardAggregateV1): CustomerReportCenterItem[] {
  ...
  const reports: CustomerReportCenterItem[] = [
    {
  ...
  for (const item of aggregate.top_risk_fields ?? []) {
    ...
    reports.push({
      report_type: "FIELD",
      title: `${String((item as any).field_name ?? "地块")} · 地块报告`,
      subtitle: "基于当前可见风险地块生成",
  ...
  for (const item of aggregate.recent_operations ?? []) {
    ...
    reports.push({
      ...
      field_name: (item as any).field_name ?? null,
      updated_at: (item as any).updated_at ?? (item as any).executed_at ?? generatedAt,
      status_text: "可查看",
      capability_status: "AVAILABLE",
    });
  }
