...
function toFallbackReports(aggregate: CustomerDashboardAggregateV1): CustomerReportCenterItem[] {
  ...
  for (const item of aggregate.top_risk_fields ?? []) {
    ...
    reports.push({ report_type: "FIELD", title: `${String((item as any).field_name ?? "地块")} · 地块报告`, subtitle: "地块范围、最近作业、风险诊断、价值与田块记忆", href: `/customer/fields/${encodeURIComponent(fieldId)}`, field_id: fieldId, field_name: (item as any).field_name ?? null, updated_at: generatedAt, status_text: "可查看", capability_status: "AVAILABLE" });
  }
  ...
  reports.push({ report_type: "EVIDENCE_VALUE", title: "证据与价值报告", subtitle: "证据包、价值记录与可信状态", href: null, updated_at: generatedAt, status_text: "数据不足", capability_status: "PENDING" });
  return reports;
}
...
export async function fetchCustomerReportsCenter(): Promise<CustomerReportsCenterResponse> {
  try {
    ...
    return { source: "dashboard_aggregate_fallback", dataScope: "FALLBACK_RECENT_ONLY", is_fallback: true, generated_at: (aggregate as any).generated_at ?? new Date().toISOString(), report_count: reports.length, reports, data_scope_note: "当前仅展示驾驶舱与近期可见对象对应报告入口，完整列表待同步" };
  } catch {
