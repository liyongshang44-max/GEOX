  ...
  return (
    <article className="operatorRoiRow">
      ...
      <div className="operatorRoiMeta">
        <div><span>价值类型</span><strong>{row.valueKindText}</strong></div>
        <div><span>置信度</span><strong>{row.confidenceText}</strong></div>
        <div><span>假设条件</span><strong>{row.assumptionText}</strong></div>
        <div><span>记录时间</span><strong>{row.createdAtText}</strong></div>
      ...
      <details className="operationTechDetailsMuted">
        ...
        <div className="operatorRoiMeta customerSpacingTopSm">
          <div><span>ROI 记录</span><strong>{row.technicalRefs.roiIdText}</strong></div>
          <div><span>作业编号</span><strong>{row.technicalRefs.operationIdText}</strong></div>
          <div><span>处方编号</span><strong>{row.technicalRefs.prescriptionIdText}</strong></div>
          <div><span>证据引用</span><strong>{row.technicalRefs.evidenceRefText}</strong></div>
          <div><span>计算方法</span><strong>{row.technicalRefs.calculationMethodText}</strong></div>
      ...
      <div className="operatorRoiActions">
        ...
        {operatorQuery ? <Link to={`/operator/field-memory?${operatorQuery}`}>查看田块记忆</Link> : null}
        {operatorQuery ? <Link to={`/operator/evidence?${operatorQuery}`}>查看作业证据摘要入口</Link> : null}
      </div>
    </article>
  );
}
...
function RoiSection({ title, description, rows }: { title: string; description: string; rows: OperatorRoiLedgerRowVm[] }): React.ReactElement {
  return (
    <section className="operatorRoiSection">
      ...
      ) : <div className="operatorQueueEmpty">暂无该类价值明细。</div>}
    </section>
  );
}
    ...
    <OperatorLayout title={meta.title} lead={meta.lead}>
      <div className="operatorRoiLedgerPage">
        <form className="operatorRoiFilters" onSubmit={applyFilters}>
          <label>
            <span>地块编号</span>
            <input value={fieldInput} onChange={(event) => setFieldInput(event.target.value)} placeholder="按地块编号筛选" />
          </label>
          <label>
            <span>作业编号</span>
            <input value={operationInput} onChange={(event) => setOperationInput(event.target.value)} placeholder="按作业编号筛选" />
          </label>
        ...
        {vm?.needsOperationSelection ? (
          <section className="operatorEmptyState">
            <h2>{vm.operationSelectionTitle}</h2>
            <p>{vm.operationSelectionDescription}</p>
            <ul className="customerList customerSpacingTopXs">
              {vm.operationSelectionItems.map((item) => <li key={item} className="customerListItem">{item}</li>)}
            </ul>
          </section>
        ) : null}
        ...
        {!loading && vm ? (
          <>
            <section className="operatorWorkbenchSummary">
              ...
              <div><span>价值明细总数</span><strong>{vm.totalCount}</strong></div>
              <div><span>当前筛选</span><strong>{vm.filterText}</strong></div>
            ...
            {vm.totalCount === 0 ? <OperatorEmptyState title={vm.emptyTitle} description={vm.emptyDescription} reason="没有价值明细时不伪造记录、证据引用或实测收益。" /> : null}

            <section className="operatorRoiGrid" aria-label="价值明细分组">
              <RoiSection title="实测记录" description="只有基线与实际结果同时存在时，才按实测口径展示。" rows={vm.measuredRows} />
              <RoiSection title="估算记录" description="估算值必须按估算口径解读，不等同于实测收益。" rows={vm.estimatedRows} />
              <RoiSection title="假设记录" description="基于假设或缺少完整证据链的价值记录。" rows={vm.assumptionRows} />
              <RoiSection title="证据不足 / 实测条件不足" description="无基线、缺实际结果或证据不足的价值项。" rows={vm.insufficientRows} />
            </section>
          </>
        ) : null}
      </div>
