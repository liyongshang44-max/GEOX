  ...
  return (
    <article className="operatorDispatchRow">
      ...
      <div className="operatorDispatchFlow" aria-label="任务派发状态链路">
        <div><span>{labelOperatorTerm("AO-ACT Task")}</span><strong>{row.taskText}</strong></div>
        <div><span>{labelOperatorTerm("Dispatch")}</span><strong>{row.dispatchText}</strong></div>
        <div><span>{labelOperatorTerm("ACK")}</span><strong>{row.ackText}</strong></div>
        <div><span>{labelOperatorTerm("Receipt")}</span><strong>{row.receiptText}</strong></div>
      </div>
      ...
      <div className="operatorDispatchActions">
        {row.taskHref ? <Link to={row.taskHref}>查看执行任务</Link> : null}
        {row.receiptHref ? <Link to={row.receiptHref}>查看执行回执</Link> : null}
        <PermissionGate
