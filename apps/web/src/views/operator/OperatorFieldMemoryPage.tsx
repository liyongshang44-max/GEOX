  ...
  return (
    <article className="operatorFieldMemoryRow">
      ...
      <div className="operatorFieldMemoryMeta">
        <div><span>记忆类型</span><strong>{row.memoryTypeText}</strong></div>
        <div><span>变化前</span><strong>{row.beforeText}</strong></div>
        <div><span>变化后</span><strong>{row.afterText}</strong></div>
        <div><span>变化量</span><strong>{row.deltaText}</strong></div>
        <div><span>置信度</span><strong>{row.confidenceText}</strong></div>
        <div><span>学习结果</span><strong>{row.learnedText}</strong></div>
      ...
      <details className="operationTechDetailsMuted">
        ...
        <div className="operatorFieldMemoryMeta customerSpacingTopSm">
          <div><span>记忆记录</span><strong>{row.technicalRefs.memoryIdText}</strong></div>
          <div><span>建议记录</span><strong>{row.technicalRefs.recommendationIdText}</strong></div>
          <div><span>执行任务</span><strong>{row.technicalRefs.taskIdText}</strong></div>
          <div><span>验收记录</span><strong>{row.technicalRefs.acceptanceIdText}</strong></div>
          <div><span>价值记录</span><strong>{row.technicalRefs.roiIdText}</strong></div>
          <div><span>证据引用</span><strong>{row.technicalRefs.evidenceRefsText}</strong></div>
          <div><span>技能引用</span><strong>{row.technicalRefs.skillRefsText}</strong></div>
      ...
      <div className="operatorFieldMemoryNotice">田块记忆用于复盘作业前后变化和学习结果；客户层只展示摘要，运营层保留默认折叠的技术引用。</div>
      <div className="operatorFieldMemoryActions">
        ...
        {operatorQuery ? <Link to={`/operator/roi-ledger?${operatorQuery}`}>查看关联价值记录</Link> : null}
        {operatorQuery ? <Link to={`/operator/evidence?${operatorQuery}`}>查看证据摘要入口</Link> : null}
      </div>
    </article>
  );
}
    ...
    <OperatorLayout title={meta.title} lead={meta.lead}>
      <div className="operatorFieldMemoryPage">
        <form className="operatorFieldMemoryFilters" onSubmit={applyFilters}>
          ...
          <label>
            <span>记忆类型</span>
            <input value={memoryTypeInput} onChange={(event) => setMemoryTypeInput(event.target.value)} placeholder="按记忆类型筛选" />
          </label>
        ...
        {!loading && vm ? (
          <>
            ...
            {vm.totalCount === 0 ? <OperatorEmptyState title={vm.emptyTitle} description={vm.emptyDescription} reason={vm.permissionDenied ? "权限不足时不回退到客户摘要。" : "无田块记忆时不伪造变化前、变化后或变化量。"} /> : null}
