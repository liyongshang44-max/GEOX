...
export default function CommercialDashboardPage(): React.ReactElement {
  ...
  const [ruleLeaderboard, setRuleLeaderboard] = React.useState<{
    top: Array<{ ruleId: string; scorePct: number }>;
    watch: Array<{ ruleId: string; scorePct: number }>;
  }>({ top: [], watch: [] });
  ...
  React.useEffect(() => {
    ...
    void fetchDashboardRecommendations(100).then((items) => {
      ...
      const statsByRule = new Map<string, { total: number; count: number }>();
      for (const item of items) {
        const ruleId = String((item as any)?.rule_id ?? (item as any)?.rule_hit?.[0]?.rule_id ?? "").trim();
        if (!ruleId) continue;
        const rawScore = Number((item as any)?.rule_score ?? NaN);
        if (!Number.isFinite(rawScore)) continue;
        const normalizedScore = Math.max(0, Math.min(1, rawScore));
        const hit = statsByRule.get(ruleId) ?? { total: 0, count: 0 };
        hit.total += normalizedScore;
        hit.count += 1;
        statsByRule.set(ruleId, hit);
      }
      const ranked = Array.from(statsByRule.entries())
        .map(([ruleId, stats]) => ({
          ruleId,
          scorePct: stats.count > 0 ? Math.round((stats.total / stats.count) * 100) : 0,
        }))
      ...
      setRuleLeaderboard({
        ...
        watch: [...ranked].reverse().slice(0, 3),
      });
    }).catch(() => {
      if (mounted) {
        setSmartRecommendations({ todayCount: 0, latest: null });
        setRuleLeaderboard({ top: [], watch: [] });
      }
    });
    return () => {
      mounted = false;
  ...
  return (
    <div className="productPage demoDashboardPage">
      ...
      <section className="card" style={{ marginTop: 16 }}>
        <div className="sectionTitle">规则表现榜</div>
        <div className="decisionList" style={{ marginTop: 12 }}>
          <div className="decisionItemStatic">
            <div className="decisionItemTitle">表现最佳规则 TOP3</div>
            <div className="decisionItemMeta">
              {ruleLeaderboard.top.length
                ? ruleLeaderboard.top.map((item) => `${item.ruleId} → ${item.scorePct}%`).join("；")
                : "暂无规则表现数据"}
            </div>
          </div>
          <div className="decisionItemStatic">
            <div className="decisionItemTitle">表现较差规则（需要关注）</div>
            <div className="decisionItemMeta">
              {ruleLeaderboard.watch.length
                ? ruleLeaderboard.watch.map((item) => `${item.ruleId} → ${item.scorePct}%`).join("；")
                : "暂无需要关注规则"}
            </div>
          </div>
        </div>
