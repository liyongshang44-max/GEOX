...
export default function FieldReportPage(): React.ReactElement {
  ...
  const hasFieldMemory = "lines" in vm.fieldMemory;
    ...
    <div className="customerReportCanvas">
      <div className="customerReportSheet customerPageGapMd fieldReportLayout">
        <section className="customerCard fieldHeaderCard">
          <div>
            <div className="customerEyebrow">GEOX / FieldReport</div>
        ...
        <section className="fieldGrid fieldGrid3">
          <article className="customerCard">
            ...
            {riskOperationHref ? <Link to={riskOperationHref}>查看相关作业</Link> : <span className="muted">查看相关作业（不可用）</span>}
          </article>
          <article className="customerCard">
            ...
            {evidenceSummaryExists ? <Link to="#">查看证据摘要</Link> : <span className="muted">查看证据摘要（不可用）</span>}
          </article>
          <article className="customerCard">
            <h3 className="customerCardTitle">当前建议</h3>
            <div>{vm.nextAction?.title || "暂无建议"}</div>
        ...
        <section className="fieldGrid fieldGrid3">
          <article className="customerCard">
            <h3 className="customerCardTitle">ROI 与价值</h3>
            <div>{vm.roiSummary.displayText}</div>
          </article>
          <article className="customerCard">
            <h3 className="customerCardTitle">田块记忆</h3>
            {hasFieldMemory ? <div>{vm.fieldMemory.displayText}</div> : <div className="muted">暂无 Field Memory</div>}
          </article>
          <article className="customerCard mapPlaceholderCard" aria-disabled="true">
            <h3 className="customerCardTitle">地图位</h3>
            {hasGeometry ? <div className="muted">P1（地图能力预留）</div> : <div className="muted">P1（无 geometry，不显示地图）</div>}
          </article>
        </section>
      </div>
    </div>
