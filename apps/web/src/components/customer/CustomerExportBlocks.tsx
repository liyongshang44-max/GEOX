...
export function DashboardExportBlocks({ vm }: { vm: CustomerDashboardPageVm }): React.ReactElement {
  ...
  const topRisks = Array.isArray(dashboard.topRiskFields) ? dashboard.topRiskFields.slice(0, 5) : [];
  const actionItems = Array.isArray(dashboard.actionItems) ? dashboard.actionItems : [];
  const pendingRows = actionItems.slice(0, 6).map((item: any) => [item?.title, item?.currentStatus, item?.nextStep, item?.formality]);
  const reviewNotes = ["风险提示用于处理优先级，不等同于正式验收或收益结论。", "设备恢复和人工复核前，不展示执行成功或价值结论。", "证据不足、待验收或需要复核的作业，不能生成正式客户价值结论。", dashboard.usagePath?.formalityNote];
  return <div className="customerCompactReport"><section className="customerCard"><h2 className="customerCardTitle">1. 本期摘要</h2><p className="customerSpacingTopSm">{safeExportText(dashboard.usagePath?.statusText, "当前经营状态待确认")}</p><p className="customerMetricLabel customerSpacingTopXs">{safeExportText(dashboard.summaryScopeText, "统计范围待确认")}</p></section><section className="customerCard"><h2 className="customerCardTitle">2. 主要风险</h2><PrintTable headers={["地块", "风险", "原因", "下一步"]} rows={topRisks.map((item: any) => [item?.fieldName, item?.riskLabel, Array.isArray(item?.reasons) ? item.reasons.map((reason: unknown) => safeExportText(reason, "")).filter(Boolean).join("；") || "暂无风险原因" : "暂无风险原因", "查看地块风险原因和最近作业证据"])} emptyText="暂无主要风险" /></section><section className="customerCard"><h2 className="customerCardTitle">3. 待处理事项</h2><PrintTable headers={["事项", "当前状态", "下一步", "正式性提示"]} rows={pendingRows} emptyText="暂无待处理事项" /></section><section className="customerCard"><h2 className="customerCardTitle">4. 作业进展</h2><PrintTable headers={["作业", "地块", "更新时间", "验收", "正式链路", "复核状态"]} rows={recentOperations.map((item: any) => [item?.operationName, item?.fieldName, item?.updatedAtText, item?.acceptanceText, item?.formalChainStatusText, item?.needsReviewText])} emptyText="暂无近期作业" /></section><section className="customerCard"><h2 className="customerCardTitle">5. 设备状态</h2><PrintTable headers={["项目", "状态"]} rows={[["在线设备", dashboard.deviceSummary?.onlineText ?? "待确认"], ["离线设备", dashboard.deviceSummary?.offlineText ?? "待确认"], ["处理提示", dashboard.deviceSummary?.actionText ?? "待人工核查"]]} emptyText="暂无设备状态" /></section><section className="customerCard"><h2 className="customerCardTitle">6. 证据与验收</h2><p className="customerSpacingTopSm">{safeExportText(dashboard.evidenceSummary?.displayText, "证据状态待确认")}</p></section><section className="customerCard"><h2 className="customerCardTitle">7. 价值记录</h2><PrintTable headers={["项目", "内容"]} rows={[["当前状态", dashboard.roiSummary?.currentStatus], ["为什么", dashboard.roiSummary?.whyText], ["下一步", dashboard.roiSummary?.nextStepText], ["正式性提示", dashboard.roiSummary?.formalityText], ["价值摘要", dashboard.roiSummary?.customerValueText]]} emptyText="暂无价值记录" /></section><section className="customerCard"><h2 className="customerCardTitle">8. 附注：哪些结论尚待复核</h2><ul className="customerList customerSpacingTopSm">{reviewNotes.map((item, index) => <li key={`${index}-${safeExportText(item, "")}`} className="customerListItem">{safeExportText(item, "复核说明待确认")}</li>)}</ul></section><footer className="customerCard"><p className="customerMetricLabel">报告由 GEOX 自动生成，可用于客户或管理层复盘；其中待复核结论已在附注中标明，不能当作正式收益或验收结论。</p></footer></div>;
}
