...
import { buildC8FieldMainVisualVm, buildC8OperationMainVisualVm } from "../../viewmodels/customerC8FormalReportVm";
...
function MainVisualExportBlocks({ title, rows }: { title: string; rows: Array<{ label: string; value: string }> }): React.ReactElement {
  return <div className="customerCompactReport"><section className="customerCard"><h2 className="customerCardTitle">{title}</h2><PrintTable headers={["项目", "内容"]} rows={rows.map((row) => [row.label, row.value])} emptyText="暂无正式摘要" /></section><footer className="customerCard"><p className="customerMetricLabel">报告由 GEOX 正式 report API 提供，供客户复盘使用。</p></footer></div>;
}

export function FieldExportBlocks({ vm, report }: { vm: FieldReportPageVm; report?: FieldReportDetailV1 | null }): React.ReactElement {
  const genericMainVisual = report ? buildCustomerFieldReportMainVisualVm(report) : null;
  const c8MainVisual = report ? buildC8FieldMainVisualVm(report) : null;
  const mainVisual = c8MainVisual ?? genericMainVisual;
  if (mainVisual) return <MainVisualExportBlocks title={mainVisual.title} rows={mainVisual.rows} />;
...
export function OperationExportBlocks({ vm, report }: { vm: OperationReportPageVm; report?: OperationReportV1 | null }): React.ReactElement {
  const genericMainVisual = report ? buildCustomerOperationReportMainVisualVm(report) : null;
