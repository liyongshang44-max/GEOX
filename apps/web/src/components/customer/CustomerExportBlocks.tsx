...
function C8ExportBlocks({ title, rows }: { title: string; rows: Array<{ label: string; value: string }> }): React.ReactElement {
  return <div className="customerCompactReport"><section className="customerCard"><h2 className="customerCardTitle">{title}</h2><PrintTable headers={["项目", "内容"]} rows={rows.map((row) => [row.label, row.value])} emptyText="暂无 C8 正式摘要" /></section><footer className="customerCard"><p className="customerMetricLabel">报告由 GEOX 正式 report API 提供，供客户复盘使用。</p></footer></div>;
}
