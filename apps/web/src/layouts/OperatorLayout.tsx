...
// Purpose: provide the H59 Operator Runtime Console shell and H63 Pilot Readiness navigation.
...
const OPERATOR_NAV_ITEMS: OperatorNavItem[] = [
  ...
  },
  {
    ...
    label: "Settings",
    to: "/operator/twin",
    hint: "Operator settings are planned for a later route acceptance; this shell only freezes the product navigation slot.",
    status: "coming-soon",
  },
];
...
function renderNavItem(item: OperatorNavItem, pathname: string): React.ReactElement {
  ...
  if (item.status !== "enabled") {
    return (
      <span key={item.key} className={"customerShellNavItem customerShellNavItemDisabled" + activeClass} title={item.hint} aria-disabled="true" data-nav-status={item.status}>
  ...
  return (
    <NavLink key={item.key} to={item.to} title={item.hint} data-nav-status={item.status} className={() => "customerShellNavItem" + activeClass}>
      <span>{item.label}</span>
      <small>{statusLabel}</small>
    </NavLink>
  );
      ...
      <div className="customerShellMainWrap">
        <header className="customerShellTopbar">
          ...
          <div className="customerShellTopActions">
            <span className="customerShellUserMuted">Read-only v1<br /><small>Runtime shell</small></span>
          </div>
        </header>
        <section className="customerShellMeta operatorRuntimeModeBanner" aria-label="Runtime mode and live-device nonclaims">
          {RUNTIME_NONCLAIMS.map((claim) => <strong key={claim}>{claim}</strong>)}
        </section>
