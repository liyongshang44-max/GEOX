  ...
  return (
    <section className="operatorReplayDemo__panel" aria-label={localizedText(replayCopy.panels.deviceEvidence, locale)}>
      ...
      <div className="operatorReplayDemo__table">
        ...
        {vm.deviceEvidence.rows.map((row) => <div className="operatorReplayDemo__tableRow" key={row.label}><span data-locale-neutral="true">{row.label}</span><span data-locale-neutral="true">{row.value}</span><span>{replayMetadataStatus(locale, row.metadataStatus)}</span></div>)}
      </div>
    </section>
  );
}
