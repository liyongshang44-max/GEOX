...
type ReplayDemoHashesPanelProps = { vm: ReplayDemoViewModel };
...
export default function ReplayDemoHashesPanel({ vm }: ReplayDemoHashesPanelProps): React.ReactElement {
  ...
  const regionLabel = t(REPLAY_DEMO_COPY.panels.hashesAria);
  return (
    <section className="operatorReplayDemo__panel operatorReplayDemo__hashes" aria-label={regionLabel}>
      ...
      <ProductHorizontalScrollRegion ariaLabel={regionLabel} overflowOwner="operator-replay-hashes">
        <div className="operatorReplayDemo__table">
          ...
          {vm.hashes.rows.map((row) => <div className="operatorReplayDemo__tableRow" key={row.label}><span data-locale-neutral="true" data-long-token="true">{row.label}</span><span data-locale-neutral="true" data-long-token="true">{row.value}</span><span>{replayMetadataStatus(locale, row.metadataStatus)}</span></div>)}
        </div>
