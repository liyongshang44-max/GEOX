...
export default function ReplayDemoLocalizedPage(): React.ReactElement {
  ...
  return <ProductPageShell surface="operator" width="wide" ariaLabel={localizedText(copy.title, locale)} className="operatorReplayDemo operatorProductSurface"
    top={<ProductPageHeader eyebrow={localizedText(copy.eyebrow, locale)} title={localizedText(copy.title, locale)} lead={localizedText(copy.heroLead, locale)} metadata={`${text("Route", "路由")}: ${vm.route} / ${text("Mode", "模式")}: ${vm.mode}`} nonclaim={localizedText(copy.nonclaimLead, locale)} />}
  ...
  >
    ...
    <ProductScopeBar surface="operator" items={[{ label: text("Route", "路由"), value: vm.route }, { label: text("Mode", "模式"), value: vm.mode }, { label: text("Boundary", "边界"), value: text("Review Only", "仅审查") }]} />
    <div className="operatorProductMetricGrid"><ProductMetricTile label={text("Demo Mode", "演示模式")} value={vm.mode} source="P51 gateway viewer snapshot" status={<ProductStatusBadge status="replayBacked" />} /><ProductMetricTile label={text("Trace Refs", "追踪引用")} value={vm.evidenceRefs.length} source="Replay demo view model" /><ProductMetricTile label={text("Gateway Status", "网关状态")} value={text("Not Online", "未上线")} description={text("Replay demo does not imply production connectivity.", "回放演示不表示生产连接。") } source="Replay demo boundary" status={<ProductStatusBadge status="notOnline" />} /></div>
