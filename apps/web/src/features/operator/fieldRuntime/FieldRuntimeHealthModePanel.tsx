...
export default function FieldRuntimeHealthModePanel({ health }: { health: FieldRuntimeHealthViewModel }): React.ReactElement {
  ...
  const rows = [
    ...
    [t("readOnly"), health.boundary.readOnly ? "true" : "false"],
  ...
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__healthMode" data-h62-panel="runtime-mode">
    <div className="operatorFieldRuntime__panelHeader"><div><p className="operatorFieldRuntime__eyebrow">{t("health")}</p><h2 className="operatorFieldRuntime__panelTitle">{t("healthMode")}</h2></div><span className="operatorFieldRuntime__panelMeta">{health.mode}</span></div>
