...
export default function FieldRuntimeZoneResponsePanel({ residual }: { residual: FieldRuntimeResidualViewModel }): React.ReactElement {
  ...
  const regionLabel = t("zoneResponse");
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__zoneResponse" data-h60h-panel="zone-response">
    ...
    <ProductHorizontalScrollRegion ariaLabel={regionLabel} overflowOwner="operator-residual-zone-response">
      <div className="operatorFieldRuntime__residualTable" role="table" aria-label={regionLabel}>
        ...
        {residual.zoneResponse.rows.map((row) => <div className="operatorFieldRuntime__residualTableRow" role="row" key={row.zoneId}><span data-long-token="true">{row.zoneId}</span><span>{row.statusText}</span><span>{row.deltaValue}</span></div>)}
      </div>
