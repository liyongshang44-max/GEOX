...
export default function FieldRuntimeHealthEvidencePipelinePanel({ health }: { health: FieldRuntimeHealthViewModel }): React.ReactElement {
  ...
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__healthPipeline" data-h62-panel="evidence-pipeline">
    ...
    <div className="operatorFieldRuntime__healthTable" role="table" aria-label={text("Evidence Pipeline Matrix", "证据管线矩阵")}><div className="operatorFieldRuntime__healthTableHeader" role="row"><span>{text("Stage", "阶段")}</span><span>{t("source")}</span><span>{t("status")}</span><span>{text("Write Surface", "写入界面")}</span></div>{health.evidencePipeline.map((row) => <div className="operatorFieldRuntime__healthTableRow" role="row" key={row.stage}><span>{row.stage}</span><span>{row.source}</span><span>{row.status}</span><span>{row.writeSurface ? "true" : "false"}</span></div>)}</div>
  </article>;
}
