...
export default function FieldRuntimeExecutionTailPanel({ residual }: { residual: FieldRuntimeResidualViewModel }): React.ReactElement {
  ...
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__executionTail" data-h60h-panel="execution-tail">
    ...
    <div className="operatorFieldRuntime__residualTable" role="table" aria-label={t("executionTail")}>
      ...
      {residual.executionTail.map((stage) => <div className="operatorFieldRuntime__residualTableRow" role="row" key={stage.stageCode}><span>{stage.stageCode}</span><span>{stage.label}</span><span>{stage.statusText}</span><span>{stage.ref}</span></div>)}
    </div>
  </article>;
}
