...
export default function FieldRuntimeHealthTraceabilityPanel({ health }: { health: FieldRuntimeHealthViewModel }): React.ReactElement {
  ...
  const regionLabel = text("Traceability Availability Matrix", "可追溯性可用性矩阵");
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__healthTraceability" data-h62-panel="traceability">
    <div className="operatorFieldRuntime__panelHeader"><div><p className="operatorFieldRuntime__eyebrow">{t("healthTraceability")}</p><h2 className="operatorFieldRuntime__panelTitle">{text("Traceability Availability", "可追溯性可用性")}</h2></div><span className="operatorFieldRuntime__panelMeta" data-long-token="true">createsTrace=false</span></div>
    ...
    <ProductHorizontalScrollRegion ariaLabel={regionLabel} overflowOwner="operator-health-traceability">
      <div className="operatorFieldRuntime__healthTable" role="table" aria-label={regionLabel}><div className="operatorFieldRuntime__healthTableHeader" role="row"><span>{text("Audit Route", "审计路由")}</span><span>{text("Replay Demo Route", "回放演示路由")}</span><span>{text("Bridge Available", "桥接可用")}</span><span>createsTrace</span></div><div className="operatorFieldRuntime__healthTableRow" role="row"><span data-long-token="true">{traceability.fieldRuntimeAuditRoute}</span><span><Link to={traceability.replayDemoRoute}>{text("Replay Demo Only", "仅回放演示")}</Link></span><span>{traceability.traceReadbackBridgeAvailable ? "true" : "false"}</span><span>{traceability.createsTrace ? "true" : "false"}</span></div></div>
