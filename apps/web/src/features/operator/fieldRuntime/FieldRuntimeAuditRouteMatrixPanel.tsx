...
export default function FieldRuntimeAuditRouteMatrixPanel({ audit }: { audit: FieldRuntimeAuditViewModel }): React.ReactElement {
  ...
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__auditRouteMatrix" data-h60k-panel="audit-route-matrix">
    <div className="operatorFieldRuntime__panelHeader"><div><p className="operatorFieldRuntime__eyebrow">{t("audit")}</p><h2 className="operatorFieldRuntime__panelTitle">{t("auditRouteMatrix")}</h2></div><span className="operatorFieldRuntime__panelMeta">{audit.canonicalRouteFamily}</span></div>
    <p className="operatorFieldRuntime__stubLead">{text("Route ownership metadata only.", "仅展示路由所有权元数据。")}</p>
    <div className="operatorFieldRuntime__auditTable" role="table" aria-label={t("auditRouteMatrix")}><div className="operatorFieldRuntime__auditTableHeader" role="row"><span>{text("Route", "路由")}</span><span>{text("Owner", "所有者")}</span><span>{t("status")}</span></div>{routeRows.map((route) => <div className="operatorFieldRuntime__auditTableRow" role="row" key={route}><span>{route}</span><span>FieldRuntimeRoutePage</span><span>preserved</span></div>)}</div>
  </article>;
}
