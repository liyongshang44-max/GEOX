...
export default function FieldRuntimeRoutePage({ tab }: FieldRuntimeRoutePageProps): React.ReactElement {
  ...
  return (
    <FieldRuntimeLayout
      viewModel={viewModel}
      workspaceLoadState={useReadModelLoadState((tab === "overview" || tab === "state") && scoped, "Workspace read model is not loaded for this route.", workspaceLoader) as never}
      evidenceLoadState={useReadModelLoadState(tab === "evidence" && scoped, "Evidence read model is not loaded for this route.", evidenceLoader) as never}
      forecastLoadState={useReadModelLoadState(tab === "forecast" && scoped, "Forecast read model is not loaded for this route.", forecastLoader) as never}
      scenarioLoadState={useReadModelLoadState(tab === "scenario" && scoped, "Scenario read model is not loaded for this route.", scenarioLoader) as never}
      residualLoadState={useReadModelLoadState(tab === "residual" && scoped, "Residual read model is not loaded for this route.", residualLoader) as never}
      calibrationLoadState={useReadModelLoadState(tab === "calibration" && scoped, "Calibration read model is not loaded for this route.", calibrationLoader) as never}
