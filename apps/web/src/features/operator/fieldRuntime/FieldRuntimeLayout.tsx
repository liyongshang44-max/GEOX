...
// Purpose: render the Field Runtime product shell, boundary banner, tabs, migrated read-only content through H60-K, and static future-tab body.
// Boundary: this layout owns presentation only; Health migrates later in H62.
...
function renderTabContent(
  ...
  calibrationLoadState: FieldRuntimeCalibrationLoadState | undefined,
  auditLoadState: FieldRuntimeAuditLoadState | undefined,
): React.ReactElement {
    ...
    >
      <header className="operatorFieldRuntime__header" aria-label="Field Runtime header">
        ...
        <dl className="operatorFieldRuntime__meta" aria-label="Field Runtime route identity">
          <div><dt>Field ID</dt><dd>{viewModel.fieldId}</dd></div>
          <div><dt>Current route</dt><dd>{viewModel.currentRoute}</dd></div>
          <div><dt>Runtime Mode</dt><dd>{viewModel.runtimeMode}</dd></div>
          <div><dt>Read-only boundary</dt><dd>{viewModel.readOnly ? "true" : "false"}</dd></div>
        </dl>
      </header>
