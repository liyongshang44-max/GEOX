...
const COPY = {
  rule: { zh: "只读核验边界规则", en: "Read-only Verification Boundary Rule" },
...
export default function FieldRuntimeResidualBoundaryPanel({ residual }: { residual: FieldRuntimeResidualViewModel }): React.ReactElement {
  ...
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__residualBoundary" data-h60h-panel="residual-boundary">
    ...
    {residual.boundaryRules.length > 0 ? <div><p className="operatorFieldRuntime__panelMeta">{label("Verification Read-model Boundary Rules", "核验读模型边界规则")}</p><ul className="operatorFieldRuntime__boundaryList">{residual.boundaryRules.map((rule, index) => <li key={`${index}:${rule}`}>{operatorSafeDisplay(rule, locale, COPY.rule)}</li>)}</ul></div> : null}
  </article>;
}
