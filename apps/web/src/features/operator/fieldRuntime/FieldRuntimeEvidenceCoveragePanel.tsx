...
export default function FieldRuntimeEvidenceCoveragePanel({ evidence }: Props): React.ReactElement {
  ...
  const regionLabel = t("coverageAria");
  return <article className="operatorFieldRuntime__panel" data-h60e-panel="evidence-coverage">
    ...
    <ProductHorizontalScrollRegion ariaLabel={regionLabel} overflowOwner="operator-evidence-coverage">
      <div className="operatorFieldRuntime__coverageTable" role="table" aria-label={regionLabel}>
        ...
        {evidence.coverageRows.map((row) => <div className="operatorFieldRuntime__tableRow" role="row" key={row.metric + row.sourceTable}>
          <span><strong>{row.metric}</strong><small data-long-token="true">{row.sourceTable}</small></span>
          <span>{row.available ? t("available") : t("limited")}</span>
          <span>{row.rowCount}</span><span data-long-token="true">{row.latestTsText}</span><span>{row.evidenceRefCount}</span>
