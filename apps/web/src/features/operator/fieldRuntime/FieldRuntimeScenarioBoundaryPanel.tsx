...
const LEGACY_SCENARIO_ROUTE = "/operator/twin/fields/:fieldId/scenarios";

const COPY = {
  legacyPrefix: { zh: "旧情景提交继续隔离在", en: "Legacy scenario submission remains isolated under" },
...
export default function FieldRuntimeScenarioBoundaryPanel({ scenario }: { scenario: FieldRuntimeScenarioViewModel }): React.ReactElement {
  ...
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__scenarioBoundary" data-h60g-panel="scenario-boundary">
    ...
    <p className="operatorFieldRuntime__scenarioNotice">{c(COPY.legacyPrefix)} <span data-locale-neutral="true">{LEGACY_SCENARIO_ROUTE}</span>.</p>
