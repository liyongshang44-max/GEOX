...
export default function FieldRuntimeHealthGatewayBoundaryPanel({ health }: { health: FieldRuntimeHealthViewModel }): React.ReactElement {
  ...
  return <article className="operatorFieldRuntime__panel operatorFieldRuntime__healthGatewayBoundary" data-h62-panel="gateway-boundary">
    <div className="operatorFieldRuntime__panelHeader"><div><p className="operatorFieldRuntime__eyebrow">{text("Gateway Snapshot Boundary", "网关快照边界")}</p><h2 className="operatorFieldRuntime__panelTitle">{text("Gateway Snapshot Boundary", "网关快照边界")}</h2></div><span className="operatorFieldRuntime__panelMeta">{gateway.gatewaySource}</span></div>
    ...
    <div className="operatorFieldRuntime__healthTable" role="table" aria-label={text("Gateway Boundary Matrix", "网关边界矩阵")}><div className="operatorFieldRuntime__healthTableHeader" role="row"><span>{text("Route", "路由")}</span><span>{t("source")}</span><span>{text("Live Gateway Claimed", "声明实时网关")}</span><span>{text("Production Gateway Claimed", "声明生产网关在线")}</span></div><div className="operatorFieldRuntime__healthTableRow" role="row"><span><Link to={gateway.replayDemoRoute}>{text("Replay Demo Only", "仅回放演示")}</Link></span><span>{gateway.gatewaySource}</span><span>{gateway.liveGatewayClaimed ? "true" : "false"}</span><span>{gateway.productionGatewayOnlineClaimed ? "true" : "false"}</span></div></div>
  </article>;
}
