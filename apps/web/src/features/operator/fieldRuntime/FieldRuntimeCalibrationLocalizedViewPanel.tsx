...
export default function FieldRuntimeCalibrationLocalizedViewPanel({ loadState }: { loadState?: FieldRuntimeCalibrationLoadState }): React.ReactElement {
  ...
  if (loadState.status === "error") return <article className="operatorFieldRuntime__panel"><h2 className="operatorFieldRuntime__panelTitle">{t("calibration")}</h2><p>{t("calibrationFailed")}: <span data-locale-neutral="true">{loadState.message}</span></p></article>;
  ...
  return <div className="operatorFieldRuntime__calibrationGrid" data-h60i="calibration-tab-ready" data-calibration-source={calibration.source}>
    <article className="operatorFieldRuntime__panel" data-h60i-panel="calibration-intro">
      <div className="operatorFieldRuntime__panelHeader"><div><p className="operatorFieldRuntime__eyebrow">{t("calibration")}</p><h2 className="operatorFieldRuntime__panelTitle">{t("calibrationReview")}</h2></div><span className="operatorFieldRuntime__panelMeta" data-locale-neutral="true">operator_field_twin_calibration_replay_v1</span></div>
    ...
    <article className="operatorFieldRuntime__panel operatorFieldRuntime__calibrationTimeline" data-h60i-panel="calibration-replay-timeline">
      ...
      <div className="operatorFieldRuntime__calibrationTimelineList">{calibration.replayTimeline.map((item) => <section className="operatorFieldRuntime__calibrationTimelineItem" key={item.stage + item.refId}><div data-locale-neutral="true"><p className="operatorFieldRuntime__panelMeta">{item.stage}</p><strong>{item.label}</strong></div><p>{t("status")}: <span data-locale-neutral="true">{item.statusText}</span></p><p>{t("sourceTable")}: <span data-locale-neutral="true">{item.sourceTable}</span></p><details className="operatorFieldRuntime__calibrationRefs"><summary>{item.evidenceRefs.length} {t("evidenceRefs")}</summary><ul data-locale-neutral="true">{item.evidenceRefs.map((ref) => <li key={ref}>{ref}</li>)}</ul></details></section>)}</div>
    </article>
    ...
    <article className="operatorFieldRuntime__panel operatorFieldRuntime__calibrationSummary" data-h60i-panel="calibration-summary">
      ...
      <div className="operatorFieldRuntime__metricGrid operatorFieldRuntime__calibrationMetadata"><section className="operatorFieldRuntime__metricCard"><p className="operatorFieldRuntime__panelMeta">{t("status")}</p><strong data-locale-neutral="true">{summary.statusText}</strong></section><section className="operatorFieldRuntime__metricCard"><p className="operatorFieldRuntime__panelMeta">{text("Reason", "原因")}</p><strong data-locale-neutral="true">{summary.reasonText}</strong></section><section className="operatorFieldRuntime__metricCard"><p className="operatorFieldRuntime__panelMeta">{text("Review Availability Metadata", "审查可用性元数据")}</p><strong>{summary.reviewAvailabilityMetadata ? copy(COPY.yes) : copy(COPY.no)}</strong></section><section className="operatorFieldRuntime__metricCard"><p className="operatorFieldRuntime__panelMeta">{text("Write-readiness Metadata Only", "仅写入准备度元数据")}</p><strong>{summary.writeReadinessMetadata ? copy(COPY.yes) : copy(COPY.no)}</strong></section></div>
    </article>

    <article className="operatorFieldRuntime__panel operatorFieldRuntime__replayGaps" data-h60i-panel="replay-gaps"><div className="operatorFieldRuntime__panelHeader"><div><p className="operatorFieldRuntime__eyebrow">{t("replayGaps")}</p><h2 className="operatorFieldRuntime__panelTitle">{t("replayGaps")}</h2></div><span className="operatorFieldRuntime__panelMeta">{text("Replay Gap Status", "回放缺口状态")}</span></div><div className="operatorFieldRuntime__calibrationGapList">{calibration.replayGaps.length === 0 ? <p>{text("No replay gaps returned.", "未返回回放缺口。")}</p> : null}{calibration.replayGaps.map((gap) => <section className="operatorFieldRuntime__metricCard" key={gap.gapCode}><p className="operatorFieldRuntime__panelMeta">{text("Replay Gap Status", "回放缺口状态")}: <span data-locale-neutral="true">{gap.gapStatus}</span></p><strong data-locale-neutral="true">{gap.label}</strong><span data-locale-neutral="true">{gap.gapCode}</span></section>)}</div></article>
