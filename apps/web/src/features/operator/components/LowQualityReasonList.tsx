  ...
  return (
    <article className="operatorPanel" data-card="LowQualityReasonList">
      ...
      <ul className="operatorList">
        {reasons.map((reason) => <li key={reason.source_table}>{reason.source_table}：{reason.reason} · 证据引用： {reason.evidence_refs.join(", ") || "none"}</li>)}
        {gaps.map((gap) => <li key={gap.gap_code}>{gap.severity} · {gap.label} · 缺失原因： {gap.gap_code}</li>)}
      </ul>
    </article>
  );
}
