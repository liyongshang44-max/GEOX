...
import { buildSimulatorDisplay, buildSkillCarrierVm, type CarrierSourceType, type SkillCarrierVm } from "../../../viewmodels/skillCarrierVm";

function formatDebugTime(ts: unknown): string {
  const value = Number(ts ?? NaN);
...
export default function DeviceOnboardingPage(): React.ReactElement {
  ...
  const simulatorDisplay = buildSimulatorDisplay({
    status: effectiveSimulatorStatus,
    skillCategories: skill?.categories ?? [],
    fallbackState: vm?.display.simulatorStateText,
    simulatorError,
  });
  const copy = vm?.copy;
      ...
      <SectionCard title="载体接入状态总览">
        <div className="contentGridTwo alignStart" style={{ marginBottom: 12 }}>
          <div className="decisionItemStatic">
            ...
            <div className="decisionItemMeta">{copy?.carrierIdLabel ?? "载体编号"}：{overview?.deviceId || "-"}</div>
            <div className="decisionItemMeta">{copy?.carrierNameLabel ?? "载体名称"}：{overview?.displayName || "未命名载体"}</div>
            <div className="decisionItemMeta">{copy?.carrierModeLabel ?? "承载模式"}：{display?.carrierModeText ?? "-"}</div>
          </div>
          <div className="decisionItemStatic">
            <div className="decisionItemTitle">技能承载信息</div>
            <div className="decisionItemMeta">{copy?.skillCategoryLabel ?? "当前技能类别"}：{display?.skillCategoryText ?? "-"}</div>
            <div className="decisionItemMeta">{copy?.bindTargetLabel ?? "当前绑定目标"}：{display?.bindTargetText ?? "-"}</div>
            <div className="decisionItemMeta">{copy?.deviceTypeLabel ?? "当前设备类型"}：{overview?.deviceType || "未识别"}</div>
          </div>
          <div className="decisionItemStatic">
            <div className="decisionItemTitle">现场绑定信息</div>
            <div className="decisionItemMeta">{copy?.fieldLabel ?? "当前绑定地块"}：{overview?.fieldId || "未绑定"}</div>
            <div className="decisionItemMeta">{copy?.inputStatusLabel ?? "当前输入状态"}：{display?.inputStatusText ?? "-"}</div>
          </div>
          <div className="decisionItemStatic">
            <div className="decisionItemTitle">最近感知信息</div>
            <div className="decisionItemMeta">{copy?.latestInputLabel ?? "最近感知时间"}：{display?.latestInputText ?? "-"}</div>
            <div className="decisionItemMeta">{copy?.latestHeartbeatLabel ?? "最近心跳时间"}：{display?.latestHeartbeatText ?? "-"}</div>
          </div>
        </div>
        <div className="metaText" style={{ marginTop: 8 }}>
          {loading ? "正在加载承载状态…" : copy?.carrierSummary ?? "-"}
        </div>
        ...
        {sourceType === "physical" ? (
          ...
          <div style={{ display: "grid", gap: 10 }}>
            <div className="decisionItemStatic">
              ...
              <div className="decisionItemMeta">{copy?.simulatorControlSummary ?? "-"}</div>
              <div className="decisionItemMeta">{copy?.inputStatusLabel ?? "当前输入状态"}：{simulatorDisplay.simulatorStateText}</div>
            </div>
            <div className="decisionItemStatic">
              ...
              <div className="contentGridTwo alignStart" style={{ marginBottom: 10 }}>
                <div className="field">
                  <span className="metaLabel">{copy?.simulatorStatusLabel ?? "当前状态"}</span>
                  <div className="metaText">{simulatorDisplay.simulatorStateText}</div>
                </div>
                <div className="field">
                  <span className="metaLabel">{copy?.simulatorLastTickLabel ?? "最近一次模拟输入时间"}</span>
                  <div className="metaText">{simulatorDisplay.simulatorLastTickText}</div>
                </div>
                <div className="field">
                  <span className="metaLabel">{copy?.simulatorIntervalLabel ?? "当前模拟输入周期"}</span>
                  <div className="metaText">{simulatorDisplay.simulatorIntervalText}</div>
                </div>
                <div className="field">
                  <span className="metaLabel">当前承载说明</span>
                  <div className="metaText">{simulatorDisplay.simulatorControlSummary}</div>
                </div>
              </div>
              ...
              <details style={{ marginTop: 10 }}>
                ...
                <div className="contentGridTwo alignStart" style={{ marginTop: 8 }}>
                  <div className="field"><span className="metaLabel">原始运行状态字段</span><div className="metaText">{String(effectiveSimulatorStatus?.running ?? "-")}</div></div>
                  ...
                  <div className="field"><span className="metaLabel">启动时间戳</span><div className="metaText">{formatDebugTime(effectiveSimulatorStatus?.started_ts_ms)}</div></div>
                  <div className="field"><span className="metaLabel">停止时间戳</span><div className="metaText">{formatDebugTime(effectiveSimulatorStatus?.stopped_ts_ms)}</div></div>
                  <div className="field"><span className="metaLabel">{copy?.simulatorErrorLabel ?? "最近错误"}</span><div className="metaText">{simulatorDisplay.simulatorErrorText}</div></div>
                  ...
                  <div className="field"><span className="metaLabel">最近 tick 时间戳</span><div className="metaText">{simulatorDisplay.simulatorLastTickText}</div></div>
                </div>
              </details>
            </div>
          </div>
      ...
      <SectionCard title="技术详情 / 展开调试信息">
        <details>
          ...
          <div className="contentGridTwo alignStart" style={{ marginTop: 10 }}>
            ...
            <div className="field"><span className="metaLabel">设备标识</span><div className="metaText">{overview?.deviceId || "-"}</div></div>
            ...
            <div className="field"><span className="metaLabel">模拟感知是否已启动</span><div className="metaText">{String(overview?.simulatorStarted ?? "-")}</div></div>
            ...
            <div className="field"><span className="metaLabel">最近遥测时间</span><div className="metaText">{display?.latestInputText ?? "-"}</div></div>
            <div className="field"><span className="metaLabel">最近心跳时间</span><div className="metaText">{display?.latestHeartbeatText ?? "-"}</div></div>
          </div>
        </details>
      </SectionCard>
