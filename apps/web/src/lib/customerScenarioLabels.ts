...
const CUSTOMER_REASON_MAP: Record<string, string> = {
  ...
  "stage_1_sensing_summary": "缺少正式传感器触发摘要",
  "soil_moisture/threshold/deficit": "缺少土壤水分诊断依据",
  "soil_moisture_threshold_deficit": "缺少土壤水分诊断依据",
  ...
  "pest_disease_acceptance_pass_not_treatment": "巡检证据已有记录，可作为后续处理建议依据；仍以正式链路校验为准，且不代表防治闭环。",
};
...
export function pestDiseaseInspectionCustomerSummaryText(input: any): string {
  ...
  if (acceptance === "PASS") return "巡检证据已有记录，可作为后续处理建议依据；仍以正式链路校验为准，且不代表防治闭环。";
