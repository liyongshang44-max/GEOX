...
export function customerTrustContextFromReportLike(value: any): CustomerTrustContext {
  ...
  return {
    chain_passed: value?.chain_validation?.passed === true || guarded?.passed === true || upper(value?.chain_status ?? value?.chainStatus ?? value?.formal_chain_status ?? formal.formal_chain_status ?? guarded?.chain_status) === "PASSED",
    customer_visible_eligible: value?.customer_visible_eligible !== false && value?.customerVisibleEligible !== false && formal.customer_visible_eligible !== false,
    ...
    chain_status: text(value?.chain_status ?? value?.chainStatus ?? value?.formal_chain_status ?? formal.formal_chain_status ?? guarded?.chain_status) || null,
    ...
    formal_acceptance: value?.formal_acceptance === true || value?.acceptance?.formal_acceptance === true || formal.formal_acceptance === true,
    fallback_limited: value?.fallback_limited === true || guarded?.fallback_limited === true,
  };
}
...
export function isCustomerFormalChainPassed(value: any): boolean {
  ...
  const chainPassed = ctx.chain_passed === true || chainStatus === "PASSED";
  ...
  const fallbackLimited = ctx.fallback_limited === true || trustLevel === "LIMITED_FALLBACK";
