...
function buildRow(item: CustomerOperationListItem): CustomerOperationsIndexRowVm {
  ...
  return { operationId, title: sanitizeCustomerText(item.customer_title || item.title || labelOperationType(item.operation_type), "未命名作业"), fieldName: sanitizeCustomerText(item.field_name, "地块名称待补充"), operationTypeText: labelOperationType(item.operation_type), statusFilter, statusText: labelFinalStatus(item.final_status), acceptanceText: labelAcceptanceStatus(item.acceptance_status), evidenceText: mapEvidenceText(item), updatedAtText: toDateTimeText(item.updated_at ?? item.executed_at ?? item.generated_at), href: operationId ? `/customer/operations/${encodeURIComponent(operationId)}` : "/customer/operations" };
}
...
function scopeCopy(response: CustomerOperationsListResponse): { subtitle: string; badge: string; note?: string; isPreview: boolean } {
  ...
  if (mode === "INTERNAL_PREVIEW") return { subtitle: "内部预览：当前按全域客户视图展示作业。", badge: "内部预览 / 全域预览", note: response.scope?.reason, isPreview: true };
  if (mode === "DENIED") return { subtitle: "暂无授权地块，因此暂无可见作业。", badge: "暂无授权地块", note: response.scope?.reason || "当前账户未授权任何地块", isPreview: false };
  ...
  if (response.dataScope === "FALLBACK_RECENT_ONLY") return { subtitle: "P1-A Preview：当前仅展示近期作业，非全部作业列表。", badge: "P1-A Preview", note: response.data_scope_note || "当前仅展示近期作业，非全部作业列表", isPreview: true };
  if (response.dataScope === "ERROR_EMPTY") return { subtitle: "作业列表暂不可用，请稍后刷新。", badge: "暂不可用", note: response.data_scope_note || "作业列表暂不可用，请稍后刷新", isPreview: true };
