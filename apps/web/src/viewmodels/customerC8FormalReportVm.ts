...
type MainVisualStatus = "FORMAL_READY" | "INSUFFICIENT_REPORT";

export type C8CustomerMainVisualVm = {
  status: MainVisualStatus;
  title: string;
  subtitle: string;
...
function deviceText(devices: unknown): string | null {
  ...
  const haystacks = rows.map((device) => [device?.device_id, device?.display_name, device?.display_kind_text, device?.sensing_role_text, device?.capability, device?.metric, ...(Array.isArray(device?.capabilities) ? device.capabilities : [])].map(text).join(" ").toLowerCase());
...
function fieldFormalValidation(report: FieldReportDetailV1): ValidationResult {
  ...
  if (fieldReportId(report) !== C8_FIELD_REPORT_ID) reasons.push("报告地块不匹配");
  ...
  if (!deviceText(sensing.devices)) reasons.push("缺少 C8 感知/执行设备摘要");
...
function operationFormalValidation(report: OperationReportV1): ValidationResult {
  ...
  const reasons: string[] = [];
  if (!isC8OperationReport(report)) reasons.push("报告作业不匹配");
  ...
  if (root.formal_scenario?.customer_visible_eligible !== true) reasons.push("正式场景未通过客户可见门禁");
  ...
  if (!(["PASS", "APPROVED", "SUCCESS", "SUCCEEDED"].includes(acceptance))) reasons.push("验收未通过");
...
function insufficientVm(title: string, reasons: string[], technicalRows: Array<{ label: string; value: string }>): C8CustomerMainVisualVm {
  return {
...
function fieldTechnicalRows(report: FieldReportDetailV1): Array<{ label: string; value: string }> {
  ...
  return [
    ...
    { label: "field_id", value: fieldReportId(report) || "--" },
...
export function buildC8FieldMainVisualVm(report: FieldReportDetailV1): C8CustomerMainVisualVm | null {
  ...
  const title = text(fieldContext.field_name ?? root.field?.field_name) || "C8 地块报告";
...
export function buildC8OperationMainVisualVm(report: OperationReportV1): C8CustomerMainVisualVm | null {
  if (!operationReportIds(report).includes(C8_OPERATION_REPORT_ID)) return null;
  const root = report as any;
  const validation = operationFormalValidation(report);
  const technicalRows = operationTechnicalRows(report);
  const title = text(root.customer_title ?? root.operation_title) || "C8 作业报告";
  if (!validation.ok) return insufficientVm(title, validation.reasons, technicalRows);
