...
function objectText(item: OperatorFieldMemoryItem): string {
  const parts = [item.fieldId ? `地块=${item.fieldId}` : "", item.operationId ? `作业=${item.operationId}` : ""].filter(Boolean);
...
export function buildOperatorFieldMemoryVm(response: OperatorFieldMemoryResponse): OperatorFieldMemoryVm {
  ...
  return {
    ...
    emptyDescription: permissionDenied ? "当前身份无权查看运营田块记忆明细。" : "当前过滤条件下没有记忆类型、执行前、执行后、变化量或证据引用可展示。",
  };
}
