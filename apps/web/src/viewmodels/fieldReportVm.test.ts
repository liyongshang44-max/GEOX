...
const vmUnnamedField = buildFieldReportVm({
  generated_at: "2026-05-01T00:00:00Z",
  field: { field_id: "f-2", field_name: "" },
  overview: {
    current_risk_level: "LOW",
    open_alerts_count: 0,
    pending_acceptance_count: 0,
...
assert.equal(vmUnnamedField.hero.title.includes("地块"), true);

const visibleText = JSON.stringify({ vm, vmUnnamedField }).toLowerCase();
