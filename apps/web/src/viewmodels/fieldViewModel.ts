...
function resolveStatus(args: { hasAlerts: boolean; hasFailedTask: boolean }): { code: FieldConsoleStatus; label: string; dot: string } {
  if (args.hasFailedTask) return { code: "error", label: "异常", dot: "🔴" };
  if (args.hasAlerts) return { code: "risk", label: "风险", dot: "⚠" };
...
export function buildFieldViewModel(params: {
  ...
  const hasAlerts = Number(detail?.recent_alerts?.length ?? 0) > 0 || recentRecommendations.some((x: any) => {
  ...
  const status = resolveStatus({ hasAlerts, hasFailedTask });
  ...
  const currentTask = currentOperation
    ? {
      ...
      deviceId: String(currentOperation.device_id || "-"),
  ...
  const timeline = [
    ...allOperations.slice(0, 8).map((x: any) => {
      ...
      return { ts, timeLabel: formatTimelineTime(ts), text: `${action}执行` };
    }),
  ...
  const defaultEvidence = detail?.latest_evidence
    ? [{ id: String(detail.latest_evidence?.evidence_id || "latest"), text: "无农药检测通过" }]
    : [];
