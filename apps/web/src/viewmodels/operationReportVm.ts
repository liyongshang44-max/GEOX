...
export function buildOperationReportVm(report: OperationReportV1): OperationReportPageVm {
  ...
  const asAppliedSummary = !isVariableOperation
    ? "该作业不适用覆盖记录"
    : (asAppliedCoverageStatus === "AVAILABLE"
  ...
  const rawSections: CustomerReportSectionVm[] = [
    ...
    { key: "EXECUTION", status: hasAsExecuted ? "AVAILABLE" : "MISSING", title: "执行", summary: hasAsExecuted ? mapOperationStatusToCustomerLabel(report.execution.final_status) : "暂无实际执行记录", items: hasAsExecuted ? [{ label: "执行对象", value: executionTarget }, { label: "人/设备", value: executorText }, { label: "开始时间", value: kv(report.execution.execution_started_at, "--") }, { label: "结束时间", value: kv(report.execution.execution_finished_at, "--") }, { label: "执行参数", value: executionParamsText }, { label: "执行摘要", value: asExecutedSummary }, { label: "覆盖状态", value: kv(asAppliedData?.coverage_status, !isVariableOperation ? "NOT_APPLICABLE" : "MISSING") }, { label: "覆盖记录", value: asAppliedSummary }, { label: "计划-实际偏差", value: kv(asAppliedData?.planned_vs_actual_deviation, "--") }] : [], emptyState: hasAsExecuted ? undefined : { title: "暂无实际执行记录", description: "当前尚无实际执行记录。" } },
