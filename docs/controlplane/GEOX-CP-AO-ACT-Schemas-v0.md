Sprint 10 · Step 4 — AO-ACT v0 Contracts / Schemas（Freeze Candidate v2）
...
冻结裁决依据：你对 6 个选择点的裁决（全部定死）+ 两处验收口径收敛（A/B）
...
允许 string 值 仅当该 key 在 parameter_schema.keys[] 中定义：
...
必须 ∈ Sprint 10 Step 1 已冻结 allowlist（v0）
...
time_window (required, immutable)

object: start_ts number(ms), end_ts number(ms)

规则：start_ts <= end_ts

parameter_schema (required, immutable) ✅（你裁决：必须随 task 携带）
...
bounds 校验：number 的 min/max；enum 的 in-list

parameters (required, immutable)
...
constraints (required, immutable) ✅（你裁决：字段必须存在，允许 {}）
...
meta (optional, immutable)

object（审计元数据，不得含决策/农技）
...
parameter_schema.required，且 parameters keys 必须 1:1 覆盖 schema.keys
...
executor_id (required, immutable)

object: kind enum "human"|"script"|"device", id string, namespace string

execution_time (required, immutable)

object: start_ts number, end_ts number; start_ts <= end_ts

execution_coverage (required, immutable)

object: kind "area"|"path"|"field", ref string

指针式引用：同 Task 的“非自然语言”约束（可复用相同校验）

resource_usage (required, immutable) ✅（你裁决：固定 4 keys 必须存在，值可 null）

object 必须包含且仅至少包含以下 keys（值可 null）：
...
logs_refs (required, immutable) ✅（你裁决：minItems=1）

array(minItems=1) of object:
...
constraint_check (required, immutable)
...
observed_parameters (required, immutable)
