...
GEOX Judge 需要在不破坏系统闭合性（唯一规范真值、语义不可变、可审计可复现）的前提下，引入“前端可配置参数”的能力。该能力必须满足：
...
* `config/judge/default.json` 为唯一 canonical reference；
...
* **拒绝策略**：未知字段/路径 → 400，config hash 不匹配 → 409，无 silent ignore。
...
* GET `/api/judge/config` 输出与 canonical reference 对齐；
...
* canonical reference hash 不匹配 → 409 Conflict；
* 行为可复现、无静默忽略。
...
* 任何演进不得破坏 v1 的 canonical reference 与闭合性原则。
