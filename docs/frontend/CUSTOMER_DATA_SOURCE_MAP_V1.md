...
| `FieldReportPage` | `buildFieldReportVm` | `fetchFieldReport` (`/api/v1/reports/field/:fieldId`) | 同上全部禁止项 | `FieldReportExportPage` 必须与 `FieldReportPage` 同源（同 `fieldId`、同数据源） | P0 详情页。 |
| `OperationReportPage` | `buildOperationReportVm` | `fetchOperationReport` (`/api/v1/reports/operation/:operationId`) | 同上全部禁止项 | `CustomerReportExportPage` 必须与 `OperationReportPage` 同源（同 `operationId`、同数据源） | P0 详情页。 |
