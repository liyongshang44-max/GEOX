...
ck('baseline',c.baseline_commit==='8326cf87fb01f7377a8b55d784ffa9027fbd725b');
ck('stage',(r.completion_status==='implementation_ready_for_review'&&r.final_closure_status==='not_started')||(r.completion_status==='complete'&&r.final_closure_status==='final_tag_main_verified'&&r.final_commit==='658cdba17ccd2a37327b52a0ff8becb694ffdf47'));
ck('stable',dry.determinism_hash===dry2.determinism_hash);
...
ck('zero',dry.forbidden_downstream_fact_count===0&&wr.forbidden_downstream_fact_count===0&&ch.forbidden_downstream_fact_count===0);
for(let i=0;i<20;i++){const x=R(['--fixture','b'+i]);ck('blocked_'+i,x.ok&&x.active_twin_forecast_run_v1_created===false&&x.active_twin_prediction_v1_created===false&&x.forecast_horizon_v1_created===false&&x.scenario_projection_v1_created===false);}
