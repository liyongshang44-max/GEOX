...
const BASE='8b92dce9b3240add23b78c558117da57d9521d5c';
...
const RULE={status_file:STATUS,field_path:'s1_candidate_implemented',allowed_candidate_values:[true],focused_workflow:'mcft-cap-09-s1-adapter-contracts',standard_workflow:'ci',predecessor_effective_evidence_required:true};
...
function artifact(name){const root=path.resolve(process.env.MCFT_CAP09_S0_EFFECTIVE_ARTIFACT_DIR||'acceptance-input/cap09-s0-effective'),q=[root];while(q.length){const d=q.pop();if(!fs.existsSync(d))continue;for(const e of fs.readdirSync(d,{withFileTypes:true})){const f=path.join(d,e.name);if(e.isDirectory())q.push(f);else if(e.name===name)return f;}}throw new Error(`ARTIFACT_MISSING:${name}`);}
function authorityFalse(o,p){for(const k of ['implementation_authorized','runtime_source_authorized','canonical_write_authorized','live_ingestion_authorized','background_scheduler_authorized','public_http_writer_authorized','model_activation_authorized','controlled_action_authorized'])must(o[k]===false,`${p}:${k}`);}
try{
 const base=process.env.MCFT_BASE_SHA,head=git('rev-parse','HEAD');must(base===BASE,`BASE:${base}`);const changed=git('diff','--name-only',`${base}...HEAD`).split(/\r?\n/).filter(Boolean).sort();eq(changed,FILES,'CHANGED_FILES');must(Number(git('rev-list','--count',`${base}..HEAD`))===1,'COMMIT_COUNT');
 const marker=['MCFT','CANDIDATE','DECLARATION','V2'].join('_');for(const f of FILES){must(!read(f).includes(marker),`DECLARATION:${f}`);must(!/^(apps|packages|migrations)\//.test(f),`RUNTIME_PATH:${f}`);}must(git('rev-parse',`${base}:${REG}`)==='e066ad7e6ec57f8dae9d0c2a41a492434deec4e0','BASE_REGISTRY_BLOB');must(git('diff','--quiet',`${base}...HEAD`,'--',TASK,SCOPE,SIGNAL,CURRENT,S0)==='','FROZEN_AUTHORITY_DRIFT');
 const att=JSON.parse(fs.readFileSync(artifact('MCFT_CAP_09_S0_EXACT_SHA_ATTESTATION.json'),'utf8'));const loc=JSON.parse(fs.readFileSync(artifact('MCFT_CAP_09_S0_ATTESTATION_RETENTION_LOCATOR.json'),'utf8'));must(att.status==='PASS'&&att.subject_sha==='7381d0f8ac56fe9f75fd78ce189920cb9ed99bf4','S0_ATTESTATION');must(att.effective_authority?.s0_authorization_effective===true&&att.effective_authority?.effective_next_slice==='S1','S0_EFFECTIVE');must(att.effective_authority?.s1_candidate_declaration_authorized===true&&att.effective_authority?.s1_authorized_scope==='ADAPTER_CONTRACTS_AND_CONFIGURATION_FREEZE_ONLY','S1_SCOPE');must(att.effective_authority?.runtime_source_authorized===false&&att.effective_authority?.background_scheduler_authorized===false&&att.effective_authority?.canonical_write_authorized===false,'S0_RUNTIME_BOUNDARY');must(loc.retention_level==='R2'&&loc.readback_verified===true&&loc.locked_version_delete_denied===true,'S0_R2');
