...
const BASE='7e8563c13305a551e819d1b8e7ac5f01f9d3764f',S0_MERGE='abadd19b2bd7460b397acbac6181253732b49fae',S0_HEAD='8a01b0a8a9d5ceeb9082200cf32712e1480160c0',S0_TREE='04c46d73ce0fcb2b833bf2c8abb22c935e29e9c4',S0_RUN=30920841465,S0_ART=8896997027;
...
(async()=>{const base=process.env.MCFT_BASE_SHA,head=sh('rev-parse','HEAD');try{
 ...
 const pr=await api('/pulls/2828');ok(pr.merged===true&&pr.merge_commit_sha===S0_MERGE&&pr.head.sha===S0_HEAD,'S0_PR');
 ...
 ok(rec.base_main_sha===BASE&&rec.s0_candidate_head_sha===S0_HEAD&&rec.s0_candidate_tree_sha===S0_TREE&&rec.s0_merge_sha===S0_MERGE,'BOOTSTRAP_BINDING');
