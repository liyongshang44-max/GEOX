...
try{
 ...
 assert.equal(c.target_station.canonical_label,'Hickory Corners');assert.equal(c.target_station.enviroweather_slug,'kbs');assert.equal(c.target_station.identity_predecessor,'EA1F_EXPECTED_STATION_LABEL_AND_SLUG');
 assert.equal(c.discovery_request.mode,'unfiltered_list_diagnostic');assert.equal(c.discovery_request.network_parameter_policy,'OMIT_ONLY_FOR_DIAGNOSTIC_AFTER_DOCUMENTED_ENVIRONET_FILTER_RETURNED_HTTP_400');
 assert(c.discovery_request.qualification_rule.includes('HICKORY_CORNERS_OR_KBS_IDENTITY'));assert(c.discovery_request.network_equivalence_rule.includes('DOES_NOT_RECLASSIFY_ANY_OTHER_NETWORK_AS_ENVIRONET'));
 assert.equal(c.observation_request.network_filter_required_after_identity_resolution,true);
 ...
 for(const m of ['list:CONFIG.discovery_request.product_id','network:CONFIG.network','EA1G_HICKORY_CORNERS_NOT_FOUND_IN_UNFILTERED_SCQC60_LIST','parseFlaggedNumber','companionFlag','recent_unestimated_distinct_hours','EA1G_SCQC60_SOLAR_FIELD_REQUIRED','EA1G_SCQC60_RAIN_FIELD_REQUIRED','raw_numeric_observation_values_emitted:false','database_write_count:0','formal_evidence_write_count:0']) assert(p.includes(m),`EA1G_PROBE_MARKER_REQUIRED:${m}`);
 ...
 write({schema_version:'geox_mcft_cap09_ea1g_awdn_scqc60_static_acceptance_v1',status:'PASS',base_sha:BASE,ea1f_blob_sha:blob,changed_files:changed,exact_file_count:changed.length,discovery_mode:'unfiltered_list_diagnostic',target_identity_predecessor:'EA1F',other_network_substitution_forbidden:true,official_host_failover_only:true,runtime_product_source_delta:0,migration_delta:0,database_write_delta:0,formal_evidence_write_delta:0,formal_window_started:false});
