...
try{
 ...
 for(const m of ['active:CONFIG.discovery_request.product_id','productid:CONFIG.product_id','parseFlaggedNumber','companionFlag','recent_unestimated_distinct_hours','EA1G_SCQC60_SOLAR_FIELD_REQUIRED','EA1G_SCQC60_RAIN_FIELD_REQUIRED','raw_numeric_observation_values_emitted:false','database_write_count:0','formal_evidence_write_count:0']) assert(p.includes(m),`EA1G_PROBE_MARKER_REQUIRED:${m}`);
