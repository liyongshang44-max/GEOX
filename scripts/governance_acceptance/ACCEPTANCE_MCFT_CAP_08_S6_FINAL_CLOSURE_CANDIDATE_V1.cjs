...
try{
 ...
 const s=load(S),e=load(E),l=load(L),b=load(B);
 assert.equal(s.s6_candidate_implemented,true); assert.equal(s.delivery_state,'CANDIDATE_IMPLEMENTED_NOT_EFFECTIVE');
 ...
 const out={schema_version:'geox_mcft_cap08_s6_final_closure_candidate_qualification_v1',status:'PASS',base_main_sha:BASE,candidate_head_sha:EXACT,candidate_tree_sha:git('rev-parse','HEAD^{tree}'),changed_file_count:8,hard_acceptance_item_count:24,hard_acceptance_pass_count:24,formal_run_a_workflow_run_id:30845476698,formal_run_b_workflow_run_id:30877450717,formal_comparator_workflow_run_id:30900706086,semantic_digest:c.semantic_digest_a,semantic_equivalence:true,difference_count:0,candidate_merge_tree_equality_verified:false,r2_attestation_verified:false,capability_complete:false,next_legal_action:'PROTECTED_MERGE_THEN_EXACT_SHA_R2_ATTESTATION'};
