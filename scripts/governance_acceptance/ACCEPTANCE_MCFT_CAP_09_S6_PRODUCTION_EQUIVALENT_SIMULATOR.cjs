...
try {
  ...
  assert.deepEqual(changed, [...boundary.files].sort(), "S6_SIM_EXACT_CHANGED_FILE_BOUNDARY_REQUIRED");
  ...
  const sources = changed.filter((file) => /\.(ts|yml)$/.test(file)).map((file) => fs.readFileSync(path.join(ROOT, file), "utf8")).join("\n");
  for (const marker of [
