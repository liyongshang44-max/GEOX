...
const RESULT = path.join(ROOT, 'acceptance-output/MCFT_CAP_06_S11D_REPAIRED_FINAL_CANDIDATE_RESULT.json');
...
const TEMPORARY_PATHS = [
  '.github/workflows/mcft-cap-06-hard-ledger-generator.yml',
  ...
  'scripts/governance_acceptance/MATERIALIZE_MCFT_CAP_06_HARD_ACCEPTANCE_ITEM_RECORDS.cjs',
  'scripts/governance_acceptance/MATERIALIZE_MCFT_CAP_06_REQUIRED_DELIVERABLES.cjs',
  'scripts/governance_acceptance/MATERIALIZE_MCFT_CAP_06_S11D_CLOSURE_REPAIR_STATE.cjs',
  'scripts/governance_acceptance/PATCH_MCFT_CAP_06_S11D_REPAIR_RUNTIME_DELTA.cjs',
  'scripts/governance_acceptance/MATERIALIZE_MCFT_CAP_06_RESOLVED_TASKBOOK_V2.cjs',
  'scripts/governance_acceptance/MATERIALIZE_MCFT_CAP_06_HISTORICAL_WORKFLOW_FREEZE.cjs',
  'scripts/governance_acceptance/MATERIALIZE_MCFT_CAP_06_S11D_REPAIRED_FINAL_CANDIDATE.cjs',
];
...
function assertAttestationContract(value, label) {
  ...
  assert.equal(value.repair_merge_sha_attestation_workflow_ref, WORKFLOW, `${label}_ATTESTATION_WORKFLOW_INVALID`);
...
function assertHistoricalWorkflow(relativePath) {
  ...
  assert.equal(workflow.includes('# Historical MCFT-CAP-06 workflow: automatic triggers frozen after final repair.'), true, `HISTORICAL_WORKFLOW_MARKER_MISSING:${relativePath}`);
...
function main() {
  const baseline = String(process.env.MCFT_CAP_06_S11D_FINAL_CANDIDATE_BASE_REF || DEFAULT_BASE).trim();
  ...
  assert.equal(changed.some((file) => FORBIDDEN_PREFIXES.some((prefix) => file.startsWith(prefix))), false, 'FINAL_CANDIDATE_PRODUCT_RUNTIME_OR_MIGRATION_FILE_CHANGED');
  assert.equal(changed.some((file) => /route|controller|openapi|scheduler/i.test(file)), false, 'FINAL_CANDIDATE_FORBIDDEN_SURFACE_CHANGED');
  ...
  assert.equal(taskbookManifest.execution_control.active_delivery_slice_id, null);
  assert.equal(taskbookManifest.execution_control.next_action, null);
  assert.equal(taskbookManifest.terminal_state.closure_effective, true);
  assert.equal(taskbookManifest.terminal_state.capability_complete, true);
  assert.equal(taskbookManifest.terminal_state.effectiveness_condition, CONDITION);
  assert.equal(taskbookManifest.successor_capability_line_authorized, false);
  ...
  assert.equal(ledger.attestation_contract.attestation_workflow_ref, WORKFLOW);
  ...
  for (const projectionPath of PROJECTIONS) {
  ...
  assert.equal(closure.hard_acceptance_effective_projection.effective_condition, CONDITION);
  ...
  assert.equal(repair.final_candidate_materialized, true);
  assert.equal(repair.repair_candidate_validated, true);
  assert.equal(repair.merge_sha_attestation_passed, false);
  assert.equal(repair.hard_acceptance.effective_condition, CONDITION);
  ...
  const attestationWorkflow = text(WORKFLOW);
  ...
  assert.equal(exists(GATE), true, 'ATTESTATION_GATE_MISSING');

  for (const temporaryPath of TEMPORARY_PATHS) assert.equal(exists(temporaryPath), false, `TEMPORARY_REPAIR_PATH_REMAINS:${temporaryPath}`);

  const result = {
    schema_version: 'geox_mcft_cap_06_s11d_repaired_final_candidate_result_v1',
    status: 'PASS',
    baseline,
