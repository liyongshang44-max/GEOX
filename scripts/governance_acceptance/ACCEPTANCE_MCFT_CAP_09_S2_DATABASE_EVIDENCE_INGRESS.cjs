...
const BASE = '3c55344f0463b3603ee92da150273b19b45137fa';
...
const FROZEN_BLOBS = {
  ...
  'scripts/governance_acceptance/mcft_cap_09_registry_lifecycle_classifier_v1.cjs': '5a9815de592eae8f394c9a760d7ff42b574b616b',
  'scripts/governance_acceptance/ACCEPTANCE_MCFT_CAP_09_S2_REGISTRY_CROSS_LIFECYCLE_REPAIR.cjs': '564c533e306d871e60357bdafb7cbb2bb2e68cdb',
};
...
async function pullRequest(number) {
  ...
  const response = await fetch(`https://api.github.com/repos/${process.env.GITHUB_REPOSITORY}/pulls/${number}`, {
    headers: {
      ...
      'User-Agent': 'geox-cap09-s2-postmerge-correction',
    },
  });
...
function falseAuthorities(status) {
  for (const key of [
...
(async () => {
  try {
    ...
    for (const [file, blob] of Object.entries(FROZEN_BLOBS)) {
      ...
      ok(git('diff', '--quiet', `${base}...HEAD`, '--', file) === '', 'FROZEN_FILE_CHANGED:' + file);
    }
    ...
    ok(config.future_forcing_policy ===
      'ISSUED_INGESTED_AVAILABLE_BY_BOUNDARY_IS_ELIGIBLE_CONTEXT', 'CONFIG_FUTURE_FORCING');
    ...
    ok(candidate.focused_workflow_blob_sha === 'b9eee566fdab32fe50377d477e1c47e068021c5a', 'CANDIDATE_WORKFLOW_BLOB');
    ok(candidate.duplicate_identity_policy ===
      'TYPE_ORIGIN_SOURCE_ROLE_EVENT_TIME_PLUS_CANONICAL_PAYLOAD_V1', 'CANDIDATE_DUPLICATE_POLICY');
    ...
    ok(predecessor.subject_sha === S1_SUBJECT
      ...
      && predecessor.artifact_id === S1_ARTIFACT, 'PREDECESSOR_IDENTITY');
    ok(predecessor.s2_postmerge_semantic_correction_routing_merge_sha === BASE,
      'PREDECESSOR_ROUTING_MERGE');
