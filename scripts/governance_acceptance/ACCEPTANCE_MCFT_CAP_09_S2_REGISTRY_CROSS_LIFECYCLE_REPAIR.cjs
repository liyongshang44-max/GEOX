...
try{
 if(process.argv.includes('--s5-candidate-lifecycle-repair')){
  must(base===FROZEN_SUBJECT,'EXACT_S5_CANDIDATE_ROUTING_REPAIR_BASE_REQUIRED');oneCommit(base);must(sameFiles(files,REPAIR_FILES),'EXACT_S5_CANDIDATE_ROUTING_REPAIR_BOUNDARY_REQUIRED');noDeclaration(files);
  ...
  for(const token of ['s5-candidate-lifecycle-repair','s5-candidate-signal','S5_CANDIDATE_FILES',"apps/server/src/runtime/twin_runtime/postgres_read_only_execution_evidence_adapter_v1.ts"])must(classifier.includes(token),`S5_CLASSIFIER_TOKEN_REQUIRED:${token}`);
  for(const token of ['s5-candidate-lifecycle-repair','s5-candidate-signal','--s5-candidate-route-only']){must(router.includes(token),`S5_ROUTER_TOKEN_REQUIRED:${token}`);must(workflow.includes(token),`S5_S2_WORKFLOW_TOKEN_REQUIRED:${token}`);}
  const result={status:'PASS',lifecycle:'S5_CANDIDATE_LIFECYCLE_ROUTING_REPAIR',base_sha:base,head_sha:git('rev-parse','HEAD'),changed_files:files,candidate_transition:false,registry_transition:false,runtime_source_delta:0,migration_delta:0,external_effectiveness:false,first_legal_next_action:'MCFT_CAP_09_S5_SHADOW_ONLINE_CANONICAL_INTEGRATION_CANDIDATE'};write('MCFT_CAP_09_S2_REGISTRY_CROSS_LIFECYCLE_REPAIR_RESULT.json',result);write('MCFT_CAP_09_S5_REGISTRY_LIFECYCLE_ROUTE_RESULT.json',result);console.log(JSON.stringify(result,null,2));process.exit(0);
 }
 ...
 delegate();
}catch(error){const failure={status:'FAIL',error:String(error?.message??error)};write('MCFT_CAP_09_S2_REGISTRY_CROSS_LIFECYCLE_REPAIR_RESULT.json',failure);if(process.argv.some(v=>v.startsWith('--s5-candidate-')))write('MCFT_CAP_09_S5_REGISTRY_LIFECYCLE_ROUTE_RESULT.json',failure);console.error(error);process.exitCode=1;}
