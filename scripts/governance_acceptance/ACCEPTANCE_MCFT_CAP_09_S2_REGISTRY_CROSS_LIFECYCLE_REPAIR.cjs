...
const REPAIR_BASE='508da08b2c5855e6391bc87e0d56042fc9232a97';
const LEGACY_FILES=[
...
const current=base===REPAIR_BASE&&JSON.stringify(changed)===JSON.stringify(REPAIR_FILES);
assert(legacy||current,'EXACT_S2_LIFECYCLE_REPAIR_BASE_AND_BOUNDARY_REQUIRED');
...
for(const token of ['s2-registry-registration','s2-candidate-signal','s2-cross-lifecycle-repair','MCFT_REGISTRY_LANE_INVALID']) assert(classifier.includes(token),`CLASSIFIER_TOKEN_REQUIRED:${token}`);
if(current){
 const workflow=fs.readFileSync('.github/workflows/mcft-cap-09-s2-registry-registration.yml','utf8');
 for(const token of ['MCFT_REGISTRY_LANE: s2-registry-registration','MCFT_S2_REGISTRATION_MODE','s2-cross-lifecycle-repair','s2-registry-registration','s2-candidate-signal','31007579256','8930987741','ACCEPTANCE_MCFT_CAP_09_S2_DATABASE_EVIDENCE_INGRESS.cjs --registration-lifecycle']) assert(workflow.includes(token),`WORKFLOW_TOKEN_REQUIRED:${token}`);
}
const output={
 schema_version:'geox_mcft_cap_09_s2_registry_cross_lifecycle_repair_result_v1',
 status:'PASS',
 repair_generation:current?'S2_REGISTRATION_WORKFLOW_CANDIDATE_ROUTING_REPAIR':'INITIAL_S2_CROSS_LIFECYCLE_REPAIR',
 base_sha:base,
 ...
 next_legal_action:current?'REBUILD_MCFT_CAP_09_S2_DATABASE_EVIDENCE_INGRESS_CANDIDATE':'MCFT_CAP_09_S2_REGISTRY_REGISTRATION'
};
