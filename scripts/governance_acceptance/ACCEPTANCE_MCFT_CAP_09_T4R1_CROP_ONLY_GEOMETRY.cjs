...
const CONFIG = 'docs/digital_twin/mcft/cap_09/GEOX-MCFT-CAP-09-T4R1-CROP-ONLY-GEOMETRY-AUTHORITY-V1.json';
const PROBE = 'scripts/runtime_acceptance/PROBE_MCFT_CAP_09_T4R1_CROP_ONLY_GEOMETRY.mjs';
const GATE = 'scripts/governance_acceptance/ACCEPTANCE_MCFT_CAP_09_T4R1_CROP_ONLY_GEOMETRY.cjs';
const WORKFLOW = '.github/workflows/mcft-cap-09-t4r1-crop-only-geometry.yml';
const OUT = 'acceptance-output/MCFT_CAP_09_T4R1_CROP_ONLY_GEOMETRY_GOVERNANCE_RESULT.json';
const ALLOWED = [
...
try {
  ...
  assert.equal(base, EXPECTED_BASE, 'T4R1_GEOMETRY_EXACT_BASE_REQUIRED');
  assert.equal(subject, head, 'T4R1_GEOMETRY_EXACT_HEAD_REQUIRED');
  assert.equal(git('merge-base', base, head), base, 'T4R1_GEOMETRY_BASE_NOT_ANCESTOR');
  const changed = git('diff', '--name-only', `${base}...HEAD`).split('\n').filter(Boolean).sort();
  assert.deepEqual(changed, ALLOWED, 'T4R1_GEOMETRY_EXACT_EIGHT_FILE_BOUNDARY_REQUIRED');

  for (const [file, expectedBlob] of Object.entries(PRESERVED)) {
    assert.equal(git('rev-parse', `HEAD:${file}`), expectedBlob, `T4R1_GEOMETRY_PREDECESSOR_MUTATED:${file}`);
  }

  const config = JSON.parse(read(CONFIG));
  assert.equal(config.schema_version, 'geox_mcft_cap09_t4r1_crop_only_geometry_authority_v1');
  assert.equal(config.frontier, 'T4R1_CROP_ONLY_GEOMETRY_AUTHORITY');
  assert.equal(config.exact_base_protected_main, EXPECTED_BASE);
  assert.equal(config.candidate_scope.treatment, 'T4');
  ...
  assert(config.conservative_subzone_policy.minimum_center_strip_clearance_m >= 10, 'T4R1_GEOMETRY_MINIMUM_STRIP_CLEARANCE_TOO_WEAK');
  assert(config.conservative_subzone_policy.minimum_outer_boundary_margin_m >= 10, 'T4R1_GEOMETRY_MINIMUM_OUTER_MARGIN_TOO_WEAK');
  assert(config.conservative_subzone_policy.minimum_end_boundary_margin_m >= 20, 'T4R1_GEOMETRY_MINIMUM_END_MARGIN_TOO_WEAK');
  ...
  for (const [key, predecessor] of Object.entries(config.authority_predecessors)) {
    assert.equal(PRESERVED[predecessor.path], predecessor.blob_sha, `T4R1_GEOMETRY_PREDECESSOR_PIN_INVALID:${key}`);
  }
  for (const claim of [
    'NO_WHOLE_T4R1_PLOT_AS_CROP_ONLY',
  ...
  ]) assert(config.hard_nonclaims.includes(claim), `T4R1_GEOMETRY_NONCLAIM_MISSING:${claim}`);
  ...
  ]) has(probe, marker, 'T4R1_GEOMETRY_PROBE_RULE_MISSING');
  assert(!probe.includes('field_kbs_mcse_t1r1'), 'T4R1_GEOMETRY_T1R1_FIELD_RELABEL_FORBIDDEN');
  assert(!probe.includes('zone_kbs_mcse_t1r1_formal_v1'), 'T4R1_GEOMETRY_T1R1_ZONE_RELABEL_FORBIDDEN');

  const workflow = read(WORKFLOW);
  has(workflow, 'pull_request:', 'T4R1_GEOMETRY_PR_TRIGGER_REQUIRED');
  has(workflow, 'merge_group:', 'T4R1_GEOMETRY_MERGE_GROUP_TRIGGER_REQUIRED');
  has(workflow, 'persist-credentials: false', 'T4R1_GEOMETRY_READ_ONLY_CHECKOUT_REQUIRED');
  has(workflow, 'PROBE_MCFT_CAP_09_T4R1_CROP_ONLY_GEOMETRY.mjs', 'T4R1_GEOMETRY_LIVE_PROBE_REQUIRED');
  assert(!/^\s{2}push:/m.test(workflow), 'T4R1_GEOMETRY_PUSH_TRIGGER_FORBIDDEN');
  assert(!workflow.includes('workflow_dispatch:'), 'T4R1_GEOMETRY_MANUAL_DISPATCH_FORBIDDEN');

  write({
    schema_version: 'geox_mcft_cap09_t4r1_crop_only_geometry_governance_result_v1',
    status: 'PASS',
    subject_sha: head,
    exact_base_sha: base,
