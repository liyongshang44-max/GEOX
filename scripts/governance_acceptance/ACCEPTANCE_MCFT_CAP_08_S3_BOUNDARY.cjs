...
const FORMAL_BASE = '8e88fdd91af25ef82f765f7d4a21b1471e2ab008';
...
const S4_STATUS = 'docs/digital_twin/mcft/cap_08/GEOX-MCFT-CAP-08-S4-DELIVERY-STATUS-V1.json';
const S4_CONTRACT = 'docs/digital_twin/mcft/cap_08/GEOX-MCFT-CAP-08-S4-CONTRACT-V1.json';
...
try {
  ...
  const waiver = json(WAIVER);
  const predecessor = json(PREDECESSOR);
  const s4Status = json(S4_STATUS);
  const s4Contract = json(S4_CONTRACT);
  ...
  assert.equal(git('rev-parse', `HEAD:${S4_STATUS}`), git('rev-parse', `${base}:${S4_STATUS}`), 'S3_S4_STATUS_SEED_DRIFT');
  assert.equal(git('rev-parse', `HEAD:${S4_CONTRACT}`), git('rev-parse', `${base}:${S4_CONTRACT}`), 'S3_S4_CONTRACT_DRIFT');
  ...
  const s4Rules = cap08.candidate_transition_fields.filter((entry) => entry.status_file === S4_STATUS && entry.field_path === 's4_candidate_implemented');
  ...
  assert.equal(git('rev-parse', `HEAD:${REGISTRY}`), git('rev-parse', `${base}:${REGISTRY}`), 'S3_REGISTRY_DRIFT');
  ...
  const result = {
    schema_version: 'geox_mcft_cap08_s3_boundary_result_v3',
    status: 'PASS',
