...
eq(blob("HEAD",amendmentPath),"1c9b3b14144cf78492a2083aa3247c0feaf4b9ef","AMENDMENT06_CANDIDATE_BLOB_MISMATCH");
...
for (const marker of [
  ...
  "→ actual UTC O00–O23"
]) if (!taskbook.includes(marker)) fail(`AMENDMENT06_TASKBOOK_MARKER_MISSING:${marker}`);
...
]) if (!a5.includes(marker)) fail(`AMENDMENT06_AMENDMENT05_MARKER_MISSING:${marker}`);
...
eq(rejection.effect_if_exact_head_rejection_proof_passes_and_candidate_merges_to_protected_main.formal_window_started,false,"AMENDMENT06_WINDOW_MUST_BE_UNSTARTED");
...
for (const forbidden of [
  "accelerated Formal clock is permitted",
  "retroactive O00 execution is permitted",
  "delete expired configs",
  "truncate the Formal database",
  "new A0 bootstrap is authorized"
]) if (amendment.includes(forbidden)) fail(`AMENDMENT06_FORBIDDEN_AUTHORITY_TEXT:${forbidden}`);
...
const out = {
  ...
  minimum_future_epoch_lead_hours:48,
