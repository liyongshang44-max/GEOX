...
const PFE14_CLASS_B_NARROW_PROVIDER_QUALIFICATION = 'docs/frontend-productization/PFE-14-CLASS-B-NARROW-PROVIDER-QUALIFICATION-V1.json';
...
const PFE14_PAGE_ONLY_EXCEPTION = ['apps/web/src/features/operator/fieldRuntime/McftCanonicalFieldRuntimeRoutePage.tsx'].sort();
const PFE14_API_ONLY_EXCEPTION = ['apps/web/src/api/mcftFieldTwinRuntime.ts'].sort();
...
const PFE14_CLASS_B_OPERATIONAL_FIELDS_PRODUCTIZATION_FILES = [
  '.github/workflows/pfe-14-class-b-operational-fields-productization-v1.yml',
  'apps/web/src/api/mcftFieldTwinRuntime.ts',
  'apps/web/src/features/operator/fieldRuntime/Pfe14OperationalReadbackPanel.tsx',
  'apps/web/src/styles/pfe14OperationalReadback.css',
  'docs/frontend-productization/PFE-14-CLASS-B-OPERATIONAL-FIELDS-PRODUCTIZATION-CANDIDATE-V1.json',
  'docs/frontend-productization/PFE-14-CLASS-B-OPERATIONAL-FIELDS-PRODUCTIZATION-CANDIDATE-V1.md',
  'scripts/frontend_acceptance/ACCEPTANCE_PFE_14_CLASS_B_OPERATIONAL_FIELDS_PRODUCTIZATION_V1.cjs',
].sort();

function git(args) { return cp.execFileSync('git', args, { cwd: ROOT, encoding: 'utf8' }).trim(); }
function baseSha() { const value = String(process.env.MCFT_BASE_SHA || '').trim(); assert.match(value, /^[0-9a-f]{40}$/, 'MCFT_BASE_SHA_INVALID'); git(['cat-file', '-e', `${value}^{commit}`]); return value; }
function changedFiles(base = baseSha()) { const value = git(['diff', '--name-only', `${base}...HEAD`]); return value ? value.split(/\r?\n/).filter(Boolean).sort() : []; }
...
function successorCapabilityIds(actual) { return [...new Set(actual.flatMap((file) => { const match = file.match(/^docs\/digital_twin\/mcft\/cap_(\d+)\//); return match ? [Number(match[1])] : []; }).filter((value) => Number.isInteger(value) && value > 7))].sort((a, b) => a - b); }
function successorAuthorityShape(actual) { if (!actual.includes(REGISTRY) || successorCapabilityIds(actual).length === 0) return false; if (actual.some((file) => file.startsWith('docs/digital_twin/mcft/cap_07/'))) return false; if (actual.some((file) => PROTECTED_CAP07_FILES.includes(file))) return false; if (actual.some((file) => file.startsWith('.github/workflows/mcft-cap-07-'))) return false; if (actual.some((file) => /scripts\/(?:frontend|governance|runtime)_acceptance\/.*MCFT_CAP_07/.test(file))) return false; return true; }
...
function pfe14ClassBNarrowProviderProofBound(a) { const p = a.class_b_narrow_provider_proof; return p?.subject_sha === '6b60ebd30192af212139a2fae0906f07cc5bbb5c' && p?.focused_run_id === 31610213981 && p?.historical_provider_run_id === 31610214284 && p?.standard_ci_run_id === 31610213882 && p?.artifact_id === 9146859396 && p?.artifact_digest === 'sha256:bac4b91dd6982f5548cf79633f70c2fe8d9ac61d17ca9b14de9788d06b1f1268' && p?.real_postgresql_projection_pass === true && p?.historical_provider_regression_pass === true && p?.frontend_runtime_page_audit_pass === true && p?.full_acceptance_pass === true && p?.commercial_mvp0_release_gate_pass === true && p?.all_pass === true && p?.merged_to_protected_main === false; }
...
function pfe14AuthorityAllowsClassBOperationalFieldsProductization() {
  if (!fs.existsSync(path.join(ROOT, PFE14_AUTHORITY)) || !fs.existsSync(path.join(ROOT, PFE14_CLASS_B_NARROW_PROVIDER_QUALIFICATION))) return false;
  const a = readJson(PFE14_AUTHORITY), q = readJson(PFE14_CLASS_B_NARROW_PROVIDER_QUALIFICATION);
  const fields = ['runtime_degradation_status','degradation_reason_codes','forecast_status','scenario_source_eligible','slot_window'];
  return a.phase_id === 'PFE-14' && a.slice_id === 'PFE-14.S4' && a.record_status === 'S4_CLASS_B_NARROW_PROVIDER_QUALIFIED_FRONTEND_PRODUCTIZATION_AUTHORIZED_NOT_EFFECTIVE' && pfe14HistoricalReadbackProofBound(a) && pfe14ClassBNarrowProviderProofBound(a) && a.class_b_narrow_provider_qualified === true && a.class_b_narrow_provider_frontend_consumption_authorized === true && a.class_b_narrow_frontend_productization_authorized === true && JSON.stringify(a.class_b_narrow_provider_fields) === JSON.stringify(fields) && a.class_b_operational_extension_implementation_authorized === false && a.class_c_field_implementation_authorized === false && a.s4_route_source_authorized === false && a.shadow_online_label_authorized === false && a.authoritative_runtime_context_authorized === false && a.s4_effective === false && a.first_legal_next_action === 'PFE_14_PRODUCTIZE_NARROW_CLASS_B_OPERATIONAL_FIELDS' && q.record_status === 'CLASS_B_NARROW_PROVIDER_QUALIFIED_FRONTEND_PRODUCTIZATION_AUTHORIZED_S4_NOT_EFFECTIVE' && q.qualified_subject_sha === '6b60ebd30192af212139a2fae0906f07cc5bbb5c' && q.focused_run_id === 31610213981 && q.historical_provider_run_id === 31610214284 && q.standard_ci_run_id === 31610213882 && q.artifact_id === 9146859396 && q.artifact_digest === 'sha256:bac4b91dd6982f5548cf79633f70c2fe8d9ac61d17ca9b14de9788d06b1f1268' && q.all_pass === true && q.protected_main_merge_claimed === false && JSON.stringify(q.qualified_provider_fields) === JSON.stringify(fields) && q.frontend_productization?.authorized === true && q.frontend_productization?.existing_api_client_type_change_authorized === true && q.frontend_productization?.existing_operational_panel_change_authorized === true && q.frontend_productization?.existing_css_change_authorized === true && q.frontend_productization?.new_frontend_route_authorized === false && q.frontend_productization?.new_api_method_authorized === false && q.frontend_productization?.backend_change_authorized === false && q.frontend_productization?.browser_status_derivation_authorized === false && q.frontend_productization?.canonical_payload_parsing_authorized === false && q.frontend_productization?.not_materialized_failure_inference_authorized === false && q.generic_class_b_implementation_authorized === false && q.class_c_implementation_authorized === false && q.pfe14_s4_effective === false;
}

function pfe14S4ProductConsumerShape(actual) { return sameFiles(actual, PFE14_S4_PRODUCT_CONSUMER_FILES) && pfe14AuthorityAllowsProductConsumer(); }
function pfe14StateForecastShape(actual) { return sameFiles(actual, PFE14_STATE_FORECAST_PRODUCTIZATION_FILES) && pfe14AuthorityAllowsStateForecastProductization(); }
function pfe14EvidenceHealthShape(actual) { return sameFiles(actual, PFE14_EVIDENCE_HEALTH_PRODUCTIZATION_FILES) && pfe14AuthorityAllowsEvidenceHealthProductization(); }
function pfe14ClassBOperationalFieldsShape(actual) { return sameFiles(actual, PFE14_CLASS_B_OPERATIONAL_FIELDS_PRODUCTIZATION_FILES) && pfe14AuthorityAllowsClassBOperationalFieldsProductization(); }
function resolveS5Mode(actual) { if (sameFiles(actual, LEGACY_REMEDIATION_FILES) || sameFiles(actual, PFE14_GATE_REMEDIATION_FILES)) return 'POST_CLOSURE_SUCCESSOR_GATE_REMEDIATION_MODE'; if (s6Committed() && pfe14ClassBOperationalFieldsShape(actual)) return 'PFE14_CLASS_B_OPERATIONAL_FIELDS_AUTHORIZED_PRODUCTIZATION_MODE'; if (s6Committed() && pfe14EvidenceHealthShape(actual)) return 'PFE14_EVIDENCE_HEALTH_AUTHORIZED_PRODUCTIZATION_MODE'; if (s6Committed() && pfe14StateForecastShape(actual)) return 'PFE14_STATE_FORECAST_AUTHORIZED_PRODUCTIZATION_MODE'; if (s6Committed() && pfe14S4ProductConsumerShape(actual)) return 'PFE14_S4_AUTHORIZED_PRODUCT_CONSUMER_MODE'; if (s6Committed() && actual.length === 0) return 'POST_CLOSURE_STEADY_STATE_REGRESSION_MODE'; if (s6Committed() && successorAuthorityShape(actual)) return 'POST_CLOSURE_SUCCESSOR_AUTHORITY_MODE'; return 'LEGACY_S5_ACCEPTANCE_MODE'; }
function resolveS6Mode(actual) { if (sameFiles(actual, LEGACY_REMEDIATION_FILES) || sameFiles(actual, PFE14_GATE_REMEDIATION_FILES)) return 'POST_CLOSURE_SUCCESSOR_GATE_REMEDIATION_MODE'; if (pfe14ClassBOperationalFieldsShape(actual)) return 'PFE14_CLASS_B_OPERATIONAL_FIELDS_AUTHORIZED_PRODUCTIZATION_MODE'; if (pfe14EvidenceHealthShape(actual)) return 'PFE14_EVIDENCE_HEALTH_AUTHORIZED_PRODUCTIZATION_MODE'; if (pfe14StateForecastShape(actual)) return 'PFE14_STATE_FORECAST_AUTHORIZED_PRODUCTIZATION_MODE'; if (pfe14S4ProductConsumerShape(actual)) return 'PFE14_S4_AUTHORIZED_PRODUCT_CONSUMER_MODE'; if (actual.length === 0) return 'POST_CLOSURE_STEADY_STATE_REGRESSION_MODE'; if (sameFiles(actual, HISTORICAL_BOOTSTRAP_FILES)) return 'CAP08_REGISTRY_BOOTSTRAP_MODE'; if (successorAuthorityShape(actual)) return 'POST_CLOSURE_SUCCESSOR_AUTHORITY_MODE'; throw new Error(`S6_LIFECYCLE_MODE_UNRESOLVED:${JSON.stringify(actual)}`); }
...
function assertProtectedCap07Unchanged(base, allowed = []) { const set = new Set(allowed); for (const file of PROTECTED_CAP07_FILES) { if (set.has(file)) continue; const diff = cp.spawnSync('git', ['diff', '--quiet', `${base}...HEAD`, '--', file], { cwd: ROOT }); assert.equal(diff.status, 0, `CAP07_PROTECTED_FILE_CHANGED:${file}`); } }
function checkCurrentCap07RegistryContract() { const cap07 = cap07RegistryEntry(readJson(REGISTRY)); for (const [statusFile, field, workflow] of [[S5_STATUS,'s5_candidate_implemented','mcft-cap-07-s5-operator-integration'],[S6_STATUS,'s6_candidate_implemented','mcft-cap-07-s6-closure']]) { const rule = cap07.candidate_transition_fields.find((item) => item.status_file === statusFile && item.field_path === field); assert.ok(rule, `CAP07_CANDIDATE_RULE_MISSING:${field}`); assert.deepEqual(rule.allowed_candidate_values, [true]); assert.equal(rule.focused_workflow, workflow); } assert.equal(cap07.implementation_authorized, false); assert.equal(cap07.runtime_source_authorized, false); assert.equal(cap07.successor_capability_authorized, false); }
function selfTestClassifier() { const successor = [REGISTRY,'docs/digital_twin/mcft/cap_08/A.json'].sort(); assert.equal(successorAuthorityShape(successor), true); assert.equal(successorAuthorityShape([...successor,S6_STATUS].sort()), false); assert.equal(resolveS6Mode(LEGACY_REMEDIATION_FILES), 'POST_CLOSURE_SUCCESSOR_GATE_REMEDIATION_MODE'); if (pfe14AuthorityAllowsProductConsumer()) assert.equal(pfe14S4ProductConsumerShape(PFE14_S4_PRODUCT_CONSUMER_FILES), true); if (pfe14AuthorityAllowsStateForecastProductization()) { assert.equal(pfe14StateForecastShape(PFE14_STATE_FORECAST_PRODUCTIZATION_FILES), true); assert.equal(pfe14StateForecastShape([...PFE14_STATE_FORECAST_PRODUCTIZATION_FILES,'apps/web/src/api/mcftFieldTwinRuntime.ts']), false); } if (pfe14AuthorityAllowsEvidenceHealthProductization()) { assert.equal(pfe14EvidenceHealthShape(PFE14_EVIDENCE_HEALTH_PRODUCTIZATION_FILES), true); assert.equal(pfe14EvidenceHealthShape([...PFE14_EVIDENCE_HEALTH_PRODUCTIZATION_FILES,'apps/web/src/api/mcftFieldTwinRuntime.ts']), false); } if (pfe14AuthorityAllowsClassBOperationalFieldsProductization()) { assert.equal(pfe14ClassBOperationalFieldsShape(PFE14_CLASS_B_OPERATIONAL_FIELDS_PRODUCTIZATION_FILES), true); assert.equal(resolveS5Mode(PFE14_CLASS_B_OPERATIONAL_FIELDS_PRODUCTIZATION_FILES), 'PFE14_CLASS_B_OPERATIONAL_FIELDS_AUTHORIZED_PRODUCTIZATION_MODE'); assert.equal(pfe14ClassBOperationalFieldsShape([...PFE14_CLASS_B_OPERATIONAL_FIELDS_PRODUCTIZATION_FILES,'apps/web/src/features/operator/fieldRuntime/McftCanonicalFieldRuntimeRoutePage.tsx']), false); } }
...
function accept(mode) {
  ...
  const apiMode = mode === 'PFE14_CLASS_B_OPERATIONAL_FIELDS_AUTHORIZED_PRODUCTIZATION_MODE';
  const exceptions = mode === 'PFE14_S4_AUTHORIZED_PRODUCT_CONSUMER_MODE' ? PFE14_ALLOWED_CAP07_PRODUCT_FILES : pageMode ? PFE14_PAGE_ONLY_EXCEPTION : apiMode ? PFE14_API_ONLY_EXCEPTION : [];
  ...
  if (mode === 'POST_CLOSURE_SUCCESSOR_GATE_REMEDIATION_MODE') {
    check('SUCCESSOR_GATE_REMEDIATION_BOUNDARY_IS_EXACT', () => assert.ok(sameFiles(actual,LEGACY_REMEDIATION_FILES) || sameFiles(actual,PFE14_GATE_REMEDIATION_FILES)));
    check('SUCCESSOR_GATE_REMEDIATION_DOES_NOT_CHANGE_REGISTRY', () => assert.equal(actual.includes(REGISTRY), false));
    check('SUCCESSOR_GATE_REMEDIATION_DOES_NOT_CHANGE_CAP07_STATUS', () => { assert.equal(actual.includes(S5_STATUS), false); assert.equal(actual.includes(S6_STATUS), false); });
    check('SUCCESSOR_GATE_CLASSIFIER_SELFTEST', selfTestClassifier);
    check('S5_ACCEPTANCE_WRAPPER_CALLS_SHARED_HELPER', () => { const wrapper = fs.readFileSync(path.join(ROOT,S5_ACCEPTANCE),'utf8'); assert.ok(wrapper.includes(HELPER)); assert.ok(wrapper.includes(LEGACY_ACCEPTANCE_SOURCE_SHA)); for (const token of ['PFE14_S4_AUTHORIZED_PRODUCT_CONSUMER_MODE','PFE14_STATE_FORECAST_AUTHORIZED_PRODUCTIZATION_MODE','PFE14_EVIDENCE_HEALTH_AUTHORIZED_PRODUCTIZATION_MODE','PFE14_CLASS_B_OPERATIONAL_FIELDS_AUTHORIZED_PRODUCTIZATION_MODE']) assert.ok(wrapper.includes(token), `WRAPPER_MODE_MISSING:${token}`); });
  } else if (mode === 'PFE14_S4_AUTHORIZED_PRODUCT_CONSUMER_MODE') {
    check('PFE14_S4_PRODUCT_CONSUMER_BOUNDARY_IS_EXACT', () => assert.deepEqual(actual,PFE14_S4_PRODUCT_CONSUMER_FILES)); check('PFE14_S4_PRODUCT_CONSUMER_AUTHORITY_IS_PROOF_BOUND', () => assert.equal(pfe14AuthorityAllowsProductConsumer(),true)); check('PFE14_S4_PRODUCT_CONSUMER_PRESERVES_REGISTRY', () => assertCap07RegistryPreserved(base)); check('PFE14_S4_PRODUCT_CONSUMER_DOES_NOT_CHANGE_ROUTE_OWNER', () => assert.equal(actual.includes('apps/web/src/app/routes/operatorFieldRuntimeRoutes.tsx'),false)); check('PFE14_S4_PRODUCT_CONSUMER_EXCEPTIONS_ARE_EXACT', () => assert.deepEqual(actual.filter((file)=>PROTECTED_CAP07_FILES.includes(file)).sort(),PFE14_ALLOWED_CAP07_PRODUCT_FILES));
  } else if (mode === 'PFE14_STATE_FORECAST_AUTHORIZED_PRODUCTIZATION_MODE') {
    check('PFE14_STATE_FORECAST_BOUNDARY_IS_EXACT', () => assert.deepEqual(actual,PFE14_STATE_FORECAST_PRODUCTIZATION_FILES)); check('PFE14_STATE_FORECAST_AUTHORITY_IS_CURRENT_OR_PROOF_BOUND', () => assert.equal(pfe14AuthorityAllowsStateForecastProductization(),true)); check('PFE14_STATE_FORECAST_PRESERVES_REGISTRY', () => assertCap07RegistryPreserved(base)); check('PFE14_STATE_FORECAST_DOES_NOT_CHANGE_ROUTE_OR_API_OWNER', () => { assert.equal(actual.includes('apps/web/src/app/routes/operatorFieldRuntimeRoutes.tsx'),false); assert.equal(actual.includes('apps/web/src/api/mcftFieldTwinRuntime.ts'),false); }); check('PFE14_STATE_FORECAST_EXCEPTION_IS_EXACT', () => assert.deepEqual(actual.filter((file)=>PROTECTED_CAP07_FILES.includes(file)).sort(),PFE14_PAGE_ONLY_EXCEPTION));
  } else if (mode === 'PFE14_EVIDENCE_HEALTH_AUTHORIZED_PRODUCTIZATION_MODE') {
    check('PFE14_EVIDENCE_HEALTH_BOUNDARY_IS_EXACT', () => assert.deepEqual(actual,PFE14_EVIDENCE_HEALTH_PRODUCTIZATION_FILES)); check('PFE14_EVIDENCE_HEALTH_AUTHORITY_IS_CURRENT', () => assert.equal(pfe14AuthorityAllowsEvidenceHealthProductization(),true)); check('PFE14_EVIDENCE_HEALTH_PRESERVES_REGISTRY', () => assertCap07RegistryPreserved(base)); check('PFE14_EVIDENCE_HEALTH_PRESERVES_CAP07_STATUS', () => { assert.equal(actual.includes(S5_STATUS),false); assert.equal(actual.includes(S6_STATUS),false); }); check('PFE14_EVIDENCE_HEALTH_DOES_NOT_CHANGE_ROUTE_OR_API_OWNER', () => { assert.equal(actual.includes('apps/web/src/app/routes/operatorFieldRuntimeRoutes.tsx'),false); assert.equal(actual.includes('apps/web/src/api/mcftFieldTwinRuntime.ts'),false); }); check('PFE14_EVIDENCE_HEALTH_EXCEPTION_IS_EXACT', () => assert.deepEqual(actual.filter((file)=>PROTECTED_CAP07_FILES.includes(file)).sort(),PFE14_PAGE_ONLY_EXCEPTION));
  } else if (mode === 'PFE14_CLASS_B_OPERATIONAL_FIELDS_AUTHORIZED_PRODUCTIZATION_MODE') {
    check('PFE14_CLASS_B_OPERATIONAL_FIELDS_BOUNDARY_IS_EXACT', () => assert.deepEqual(actual,PFE14_CLASS_B_OPERATIONAL_FIELDS_PRODUCTIZATION_FILES));
    check('PFE14_CLASS_B_OPERATIONAL_FIELDS_AUTHORITY_IS_PROOF_BOUND', () => assert.equal(pfe14AuthorityAllowsClassBOperationalFieldsProductization(),true));
    check('PFE14_CLASS_B_OPERATIONAL_FIELDS_PRESERVES_REGISTRY', () => assertCap07RegistryPreserved(base));
    check('PFE14_CLASS_B_OPERATIONAL_FIELDS_PRESERVES_CAP07_STATUS', () => { assert.equal(actual.includes(S5_STATUS),false); assert.equal(actual.includes(S6_STATUS),false); });
    check('PFE14_CLASS_B_OPERATIONAL_FIELDS_DOES_NOT_CHANGE_ROUTE_OR_PAGE_OWNER', () => { assert.equal(actual.includes('apps/web/src/app/routes/operatorFieldRuntimeRoutes.tsx'),false); assert.equal(actual.includes('apps/web/src/features/operator/fieldRuntime/McftCanonicalFieldRuntimeRoutePage.tsx'),false); });
    check('PFE14_CLASS_B_OPERATIONAL_FIELDS_API_EXCEPTION_IS_EXACT', () => assert.deepEqual(actual.filter((file)=>PROTECTED_CAP07_FILES.includes(file)).sort(),PFE14_API_ONLY_EXCEPTION));
  } else if (mode === 'POST_CLOSURE_SUCCESSOR_AUTHORITY_MODE') {
    check('SUCCESSOR_AUTHORITY_FILE_SHAPE_IS_FAIL_CLOSED', () => assert.equal(successorAuthorityShape(actual),true)); check('SUCCESSOR_AUTHORITY_PRESERVES_CAP07_REGISTRY_ENTRY', () => assertCap07RegistryPreserved(base));
  } else if (mode === 'POST_CLOSURE_STEADY_STATE_REGRESSION_MODE') {
    check('STEADY_STATE_HAS_ZERO_CHANGED_FILES', () => assert.deepEqual(actual,[])); check('STEADY_STATE_PRESERVES_CAP07_REGISTRY_ENTRY', () => assertCap07RegistryPreserved(base));
  } else if (mode === 'CAP08_REGISTRY_BOOTSTRAP_MODE') {
    check('HISTORICAL_CAP08_BOOTSTRAP_BOUNDARY_IS_EXACT', () => assert.deepEqual(actual,HISTORICAL_BOOTSTRAP_FILES)); check('HISTORICAL_CAP08_BOOTSTRAP_PRESERVES_CAP07_REGISTRY_ENTRY', () => assertCap07RegistryPreserved(base));
