...
try{
  ...
  const expectedZeroRuntimeProvisioningStatus=workspaceBoundPreIdentity?"ZERO_INSTANCE_UNAVAILABLE_INERT_BOOTSTRAP_AUTHORIZED":"NOT_PROVEN";
  ...
  req(hostAuth.next_stage?.stage==="BIND_REAL_NON_GITHUB_PLATFORM_SERVICE_IDENTITIES"&&hostAuth.next_stage?.status===(workspaceBoundPreIdentity?"RENDER_INERT_IDENTITY_BOOTSTRAP_AUTHORIZED_AWAITING_EXECUTION":platformAuthorizedPreIdentity?"RENDER_PLATFORM_AUTHORIZED_AWAITING_SAFE_SERVICE_IDENTITY_PROVISIONING":"BLOCKED_ON_EXTERNAL_PLATFORM_AND_SERVICE_IDENTITIES"),"OWNER_PROVISIONING_HOST_AUTHORITY_NEXT_STAGE_REQUIRED");
  if(platformAuthorizedPreIdentity){
    ...
    if(workspaceBoundPreIdentity){
      ...
      req(a.next_stage?.service_creation_execution_status==="AUTHORIZED_INERT_BOOTSTRAP_ONLY"&&
        a.next_stage?.new_runtime_start_authority_required_to_create_render_workers===false&&
        a.next_stage?.inert_identity_bootstrap_authority==="ALLOW_RENDER_INERT_IDENTITY_BOOTSTRAP"&&
        a.next_stage?.inert_identity_bootstrap_execution_status==="NOT_YET_EXECUTED",
        "OWNER_PROVISIONING_RENDER_INERT_BOOTSTRAP_AUTHORITY_REQUIRED");
      req(hostAuth.provider_semantic_evidence?.background_worker_create_contract?.num_instances_minimum===1&&
        hostAuth.provider_semantic_evidence?.background_worker_create_contract?.zero_instance_create_allowed===false&&
        hostAuth.provider_semantic_evidence?.suspend_contract?.service_id_required_before_suspend===true&&
        hostAuth.provider_semantic_evidence?.create_then_suspend_satisfies_zero_runtime_boundary===false&&
        hostAuth.provider_semantic_evidence?.inert_identity_bootstrap?.authority_code==="ALLOW_RENDER_INERT_IDENTITY_BOOTSTRAP"&&
        hostAuth.provider_semantic_evidence?.inert_identity_bootstrap?.image_path==="docker.io/library/alpine@sha256:28bd5fe8b56d1bd048e5babf5b10710ebe0bae67db86916198a6eec434943f8b"&&
        hostAuth.provider_semantic_evidence?.inert_identity_bootstrap?.later_real_worker_transition?.allowed_under_this_authority===false,
        "OWNER_PROVISIONING_RENDER_PROVIDER_SEMANTIC_EVIDENCE_REQUIRED");
    }
