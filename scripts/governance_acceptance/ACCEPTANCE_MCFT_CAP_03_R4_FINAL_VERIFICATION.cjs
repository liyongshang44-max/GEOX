...
assert.equal(finalVerification.verified_on_main, true);
assert.equal(finalVerification.verified_main_commit, 'cda1016542300bbc477a1c72023401aaaad954bc');
...
assert.equal(finalVerification.verification_effectiveness.verification_exact_head_ci_run, 'CI_4801');
assert.equal(finalVerification.verification_effectiveness.head_merge_file_delta_count, 0);
assert.equal(finalVerification.successor_authorized, false);
ok('R4 final verification is effective on main with zero remaining findings and no CAP-04 authorization');
...
assert.equal(mainVerification.post_completion_remediation_verification.verification_status, 'VERIFIED_ON_MAIN');
...
assert.equal(mainVerification.post_completion_remediation_verification.verification_exact_head_ci_run, 'CI_4801');
assert.equal(mainVerification.post_completion_remediation_verification.head_merge_file_delta_count, 0);
assert.equal(mainVerification.successor_authorized, false);
ok('main verification records the effective R4 result and preserves capability and successor boundaries');
...
ok('all three R4 remediation steps remain merged-main effective');
