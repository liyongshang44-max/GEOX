...
try {
  ...
  for (const line of ['MCFT-CAP-06','MCFT-CAP-07']) assert.deepEqual(
    registry.capabilities.find((entry) => entry.capability_line === line),
  ...
  assert.equal(source.includes(['MCFT','CANDIDATE','DECLARATION','V2'].join('_')), false);
  ...
  write({schema_version:'geox_mcft_cap08_s5_pre_candidate_governance_result_v1',status:'FAIL',error:error instanceof Error ? error.message : String(error)});
  throw error;
}
