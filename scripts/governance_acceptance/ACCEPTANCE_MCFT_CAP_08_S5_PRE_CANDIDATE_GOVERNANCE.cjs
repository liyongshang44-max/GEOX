...
const BASE = 'ba57380cc20d18d59d6ae9a35ab8e30320000476';
...
function exactBoolean(value, field, expected) {
  assert.equal(value[field], expected, `S5_${field.toUpperCase()}_MISMATCH`);
}
...
try {
  ...
  assert.equal(boundary.base_sha, base, 'S5_GOVERNANCE_BOUNDARY_BASE_MISMATCH');
  ...
  exactBoolean(boundary, 'candidate_declaration_present', false);
  exactBoolean(boundary, 's5_candidate_implemented', false);
  exactBoolean(boundary, 'implementation_authorized', false);
  exactBoolean(boundary, 'production_runtime_source_authorized', false);
  assert.equal(changed.some((file) => file.startsWith('apps/server/') || file.startsWith('apps/web/') || file.includes('migration') || file.includes('scheduler')), false, 'S5_GOVERNANCE_RUNTIME_OR_PRODUCT_SOURCE_FORBIDDEN');
  ...
  assert.equal(contract.semantic_digest, semanticDigest(contract), 'S5_CONTRACT_SEMANTIC_DIGEST_MISMATCH');
  ...
  assert.equal(contract.residual_contract.mapping, 'R-i = Forecast T(i-1) H=1 + FVO-i');
  ...
  assert.equal(contract.candidate_oracle.parameter_path, 'dynamics_parameters.drainage_coefficient_per_hour');
  ...
  exactBoolean(contract, 'implementation_authorized', false);
  exactBoolean(contract, 'candidate_declaration_authorized', false);
  exactBoolean(contract, 's5_effective', false);
  exactBoolean(contract, 's6_authorized', false);
  ...
  exactBoolean(status, 's5_candidate_implemented', false);
  exactBoolean(status, 'implementation_authorized', false);
  exactBoolean(status, 'runtime_source_authorized', false);
  exactBoolean(status, 'bounded_canonical_transaction_authorized', false);
  exactBoolean(status, 'independent_review_required', false);
  exactBoolean(status, 'independent_review_satisfied', false);
  exactBoolean(status, 'independent_review_performed', false);
  exactBoolean(status, 'independent_review_waived', true);
  exactBoolean(status, 'technical_gate_relaxation', false);
  ...
  exactBoolean(status, 'production_runtime_source_authorized', false);
  exactBoolean(status, 's5_effective', false);
  exactBoolean(status, 's6_authorized', false);
  exactBoolean(status, 'mcft_cap_09_authorized', false);
  ...
  exactBoolean(predecessor, 'required_artifact_readback', true);
  assert.equal(predecessor.source_merge_subject_sha, null);
  exactBoolean(predecessor, 'predecessor_effectiveness_satisfied', false);
  exactBoolean(predecessor, 'implementation_entry_authorized', false);
  ...
  exactBoolean(review, 'independent_review_required', false);
  exactBoolean(review, 'independent_review_satisfied', false);
  exactBoolean(review, 'independent_review_performed', false);
  exactBoolean(review, 'independent_review_waived', true);
  exactBoolean(review, 'technical_gate_relaxation', false);
  ...
  for (const value of [topCurrent, topBase]) {
    delete value.authority_set_revision;
    delete value.authority_set_change_id;
  ...
  for (const line of ['MCFT-CAP-06', 'MCFT-CAP-07']) {
    assert.deepEqual(registry.capabilities.find((entry) => entry.capability_line === line), baseRegistry.capabilities.find((entry) => entry.capability_line === line), `S5_REGISTRY_PREDECESSOR_CAPABILITY_DRIFT:${line}`);
  }
  ...
  currentCap08.authoritative_candidate_status_paths = currentCap08.authoritative_candidate_status_paths.filter((item) => item !== P.status);
  currentCap08.candidate_transition_fields = currentCap08.candidate_transition_fields.filter((entry) => !(entry.status_file === P.status && entry.field_path === 's5_candidate_implemented'));
  ...
  const rules = cap08.candidate_transition_fields.filter((entry) => entry.status_file === P.status && entry.field_path === 's5_candidate_implemented');
  ...
  assert.equal(source.includes(marker), false, 'S5_GOVERNANCE_CANDIDATE_DECLARATION_FORBIDDEN');
