...
const expected=beforeSource.slice(0,target)+newLine+beforeSource.slice(target+oldLine.length);
assert(source.trim()===expected.trim(),'BSEC-120 source changed beyond exact create capability repair');
