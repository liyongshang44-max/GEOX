...
const PREDECESSOR=path.join(ROOT,'acceptance-output/MCFT_CAP_09_PHASE1_TYPED_CAP08_PREDECESSOR_RESULT.json');
const SUCCESSOR=path.join(ROOT,'acceptance-output/MCFT_CAP_09_PHASE1_TYPED_CAP08_SUCCESSOR_RESULT.json');
const OUT=path.join(ROOT,'acceptance-output/MCFT_CAP_09_PHASE1_TYPED_CAP08_FROZEN_REPLAY_EQUIVALENCE_V1_RESULT.json');
...
function collectDifferences(left,right,currentPath='$',out=[]){
  ...
  if(left&&right&&typeof left==='object'&&typeof right==='object'){
    ...
    for(const key of keys){
      if(!Object.prototype.hasOwnProperty.call(left,key)||!Object.prototype.hasOwnProperty.call(right,key))out.push({path:`${currentPath}.${key}`,left:left?.[key],right:right?.[key]});
...
try{
  ...
  assert.equal(predecessor.successor_uses_typed_replay_host_composition,false,'PREDECESSOR_MUST_USE_FROZEN_COMPOSITION');
  assert.equal(successor.lane,'SUCCESSOR','SUCCESSOR_LANE_ID');
  assert.equal(successor.successor_uses_typed_replay_host_composition,true,'SUCCESSOR_TYPED_COMPOSITION_REQUIRED');
  ...
  const result={
    schema_version:'geox_mcft_cap09_phase1_typed_cap08_frozen_replay_equivalence_v1_result',
    status:'PASS',
    ...
    difference_count:0,
    successor_uses_typed_replay_host_composition:true,
