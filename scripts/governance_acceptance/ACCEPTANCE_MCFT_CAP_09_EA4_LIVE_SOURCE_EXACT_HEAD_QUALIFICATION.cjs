...
const result = {schema_version:'geox_mcft_cap09_ea4_live_source_exact_head_governance_result_v1',status:'FAIL',base_sha:BASE,exact_file_count:0,public_raw_value_emission_count:0,database_write_count:0,formal_evidence_write_count:0,formal_window_started:false};
...
try {
  ...
  for (const key of predecessorKeys) {
    const actual = blob(BASE,F[key]); result.predecessor_blobs[key] = actual;
  ...
  req(ea3.successor_boundary?.first_legal_successor_after_effective_ea3_merge === 'S6-EA4_LIVE_SOURCE_EXACT_HEAD_QUALIFICATION' && ea3.successor_boundary?.ea4_must_not_bypass_raw_retention_barrier === true, 'EA4_EA3_SUCCESSOR_OR_RETENTION_BOUNDARY_DRIFT');

  req(auth.record_status === 'EA4_LIVE_SOURCE_EXACT_HEAD_QUALIFICATION_CANDIDATE_NOT_EFFECTIVE' && auth.base_main_sha === BASE, 'EA4_AUTHORITY_STATUS_OR_BASE_DRIFT');
  const authorityKeyMap = {taskbook:'task',amendment_01:'a1',ea2_source_matrix:'source',ea2_package:'pkg'};
  for (const [key,entry] of Object.entries(auth.predecessor_authorities || {})) {
    const mapped = authorityKeyMap[key] || key;
  ...
  req(auth.kbs.soil_latest_max_age_minutes===30 && auth.kbs.raw_hourly_latest_max_age_hours===6 && auth.kbs.minimum_recent_complete_hourly_et0_intervals===24 && auth.kbs.minimum_recent_numeric_rain_hours===24 && auth.kbs.raw_values_may_be_emitted===false, 'EA4_KBS_QUALIFICATION_LIMIT_DRIFT');
  ...
  req(retention.class==='RUNNER_PRIVATE_TRANSIENT_RETENTION_QUALIFICATION_ONLY_NOT_FORMAL_DURABLE_RETENTION' && retention.raw_bytes_must_be_written_before_decode===true && retention.per_object_sha256_receipt_required===true, 'EA4_RETENTION_CLASS_OR_BARRIER_DRIFT');
  ...
  req(effect.live_source_qualified===false && effect.gfs_72h_full_value_pipeline_qualified===false && effect.future_et0_72h_value_execution_qualified===false && effect.ea2_package_formal_eligible===false && effect.ea5_candidate_development_authorized===false, 'EA4_PREMATURE_PASS_OR_SUCCESSOR_EFFECT');
  ...
  for (const token of ['PRIVATE_ROOT = Path(tempfile.mkdtemp','path.write_bytes(body)','reread = path.read_bytes()','EA4_RETENTION_DIGEST_MISMATCH','retain_raw("KBS_SOIL_ENDPOINT25"','retain_raw("KBS_RAW_HOURLY_13"','retain_raw("GFS_PGRB2_FILTER_RESPONSE"','retain_raw("GFS_SFLUX_IDX"','retain_raw("GFS_SFLUX_EXACT_GRIB_MESSAGE"','APCP_SECTION4_AMBIGUITY','future_et0.append','negative_clipping_performed":False','ea2_package_formal_eligible":False','ea5_candidate_development_authorized":True']) req(probe.includes(token), `EA4_PROBE_REQUIRED_TOKEN_MISSING:${token}`);
  ...
  Object.assign(result,{status:'PASS',authority_blob:blob('HEAD',F.auth),probe_blob:blob('HEAD',F.probe),live_qualification_required:true,discovery_unexecuted:true,ea2_package_formal_eligible:false,ea5_candidate_development_authorized:false,public_raw_value_emission_count:0,database_write_count:0,formal_evidence_write_count:0,formal_window_started:false,mcft_cap09_completed:false});
} catch (error) {
