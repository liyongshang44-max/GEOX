...
// Purpose: gate the exact MCFT-CAP-02 pure hourly Dynamics slice in draft, final-premerge, and explicit historical postmerge contexts.
...
const DYNAMICS_MERGE_COMMIT = 'bc97acbd589c454b7417d6ed4567c103b655ee78';
...
const EVIDENCE_SLICE = 'MCFT-CAP-02.MCFT-05.CONTINUATION-EVIDENCE-WINDOW-V1';
...
function checkBoundary() {
  const target = MODE === 'postmerge' ? DYNAMICS_MERGE_COMMIT : 'HEAD';
  try {
    cp.execFileSync(process.platform === 'win32' ? 'git.exe' : 'git', ['merge-base', '--is-ancestor', BASELINE, target], {
      cwd: ROOT,
      stdio: 'ignore',
    });
    check(true, `${MODE} Dynamics target descends from verified contracts/config merge commit`);
  } catch {
    check(false, `${MODE} Dynamics target descends from verified contracts/config merge commit`);
  }

  if (MODE === 'postmerge') {
    try {
      cp.execFileSync(process.platform === 'win32' ? 'git.exe' : 'git', ['merge-base', '--is-ancestor', DYNAMICS_MERGE_COMMIT, 'HEAD'], {
        cwd: ROOT,
        stdio: 'ignore',
      });
      check(true, 'Dynamics merge commit is an ancestor of current HEAD');
    } catch {
      check(false, 'Dynamics merge commit is an ancestor of current HEAD');
    }
  }

  try {
    const changed = git(['diff', '--name-only', `${BASELINE}...${target}`]).split(/\r?\n/).filter(Boolean).sort();
    check(JSON.stringify(changed) === JSON.stringify(EXACT_CHANGED_FILES), `historical Dynamics changed-file set has ${EXACT_CHANGED_FILES.length} files`);
    ...
    check(true, 'historical Dynamics git diff --check PASS');
  } catch (error) {
    check(false, `historical Dynamics changed-file boundary and diff check available: ${error.message}`);
  }
}
...
function checkStatusAndDebt() {
  ...
  const evidence = delivery.slices.find((slice) => slice.delivery_slice_id === EVIDENCE_SLICE);
  ...
  if (MODE === 'postmerge') {
    ...
    check(delivery.status === 'CONTINUATION_EVIDENCE_WINDOW_IN_PROGRESS' || delivery.status === 'CONTINUATION_EVIDENCE_WINDOW_READY_FOR_MERGE', 'postmerge capability advanced to Evidence Window');
    check(delivery.latest_verified_main_commit === DYNAMICS_MERGE_COMMIT, 'Dynamics merged-main commit exact');
    check(delivery.active_delivery_slice_id === EVIDENCE_SLICE, 'active slice advanced to continuation Evidence Window');
    check(current?.status === 'MERGED', 'postmerge Dynamics status exact');
    check(current?.merge_commit === DYNAMICS_MERGE_COMMIT, 'postmerge Dynamics merge commit exact');
    check(current?.merged_main_acceptance?.final_gate === '66_PASS_0_FAIL', 'Dynamics merged-main Gate evidence exact');
    check(
      ['IN_PROGRESS', 'READY_FOR_MERGE'].includes(evidence?.status)
        && activeDeliverySlices.length === 1
        && activeDeliverySlices[0]?.delivery_slice_id === EVIDENCE_SLICE,
      'continuation Evidence Window is the only active delivery slice in an allowed premerge state',
    );
    const persistence = delivery.slices.find((slice) => slice.delivery_slice_id === 'MCFT-CAP-02.MCFT-03.CONTINUATION-PERSISTENCE-V1');
    check(persistence?.status === 'BLOCKED', 'continuation persistence remains blocked');
    check(Array.isArray(delivery.next_authorized_slice_ids) && delivery.next_authorized_slice_ids.length === 0, 'no additional downstream slice authorized while Evidence Window is active');
  } else {
...
function checkSourceAnchors() {
  ...
  check(predecessorGate.includes('no additional downstream slice authorized while Dynamics is active'), 'contracts/config postmerge Gate distinguishes active Dynamics state');
}
