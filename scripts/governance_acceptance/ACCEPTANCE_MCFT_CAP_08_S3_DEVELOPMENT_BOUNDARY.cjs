...
const FROZEN_PREDECESSOR_FILES = [
...
try {
  ...
  const predecessorBlobs = FROZEN_PREDECESSOR_FILES.map((file) => ({
    file,
    base_blob: git('rev-parse', `${base}:${file}`),
    head_blob: git('rev-parse', `HEAD:${file}`),
  }));
  assert.deepEqual(
    predecessorBlobs.filter((entry) => entry.base_blob !== entry.head_blob),
    [],
    'S3_PREFLIGHT_PREDECESSOR_CORE_DRIFT',
  );
  assert.deepEqual(
    actual.filter((file) => FROZEN_PREDECESSOR_FILES.includes(file)),
    [],
    'S3_PREFLIGHT_PREDECESSOR_FILE_IN_CHANGESET',
  );
  ...
  for (const required of [
    ...
    'scripts/runtime_acceptance/mcft_cap08_s3_acceptance_support_v1.ts',
  ...
  const result = {
    schema_version: 'geox_mcft_cap08_s3_development_boundary_result_v4',
    status: 'PASS',
    ...
    frozen_predecessor_file_count: predecessorBlobs.length,
    frozen_predecessor_files: predecessorBlobs,
