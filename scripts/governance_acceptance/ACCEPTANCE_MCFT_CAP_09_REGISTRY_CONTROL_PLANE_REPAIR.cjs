...
const TRUSTED_CAP08_CORE_BLOB='b296609faf190de78a82febbbd042fd52f61b46a';
...
const RESOLVER_CORE='scripts/governance_acceptance/MCFT_REGISTRY_FOCUSED_WORKFLOW_APPLICABILITY_V1_CORE.cjs',CAP08='scripts/governance_acceptance/ACCEPTANCE_MCFT_CAP_08_S2_PRE_CANDIDATE_FOUNDATION.cjs',CAP08_CORE='scripts/governance_acceptance/ACCEPTANCE_MCFT_CAP_08_S2_PRE_CANDIDATE_FOUNDATION_CORE.cjs';
const EXPECT=['.github/workflows/mcft-cap-09-registry-control-plane-repair.yml',BOUNDARY,CAP08,CAP08_CORE,VALIDATOR,RESOLVER,RESOLVER_CORE].sort();
...
try{
 ...
 const b=js(BOUNDARY);ok(b.base_main_sha===base&&b.change_class==='MCFT_CAP_09_REGISTRY_CONTROL_PLANE_REPAIR','BOUNDARY_IDENTITY');ok(b.changed_file_count===7&&same(b.changed_files,EXPECT),'BOUNDARY_FILES');ok(b.registry_delta===0&&b.candidate_transition===false&&b.runtime_source_delta===0&&b.global_ci_bypass===false,'BOUNDARY_ZERO_DELTA');
 ok(sh('rev-parse',`HEAD:${RESOLVER_CORE}`)===TRUSTED_CORE_BLOB,'RESOLVER_CORE_BLOB');ok(sh('rev-parse',`HEAD:${CAP08_CORE}`)===TRUSTED_CAP08_CORE_BLOB,'CAP08_CORE_BLOB');
 const cap08=rd(CAP08);for(const token of ['REGISTRY_CONTROL_PLANE_REPAIR_MODE','historical_foundation_reexecution:false','successor_regression_freeze_bypass:false'])ok(cap08.includes(token),'CAP08_WRAPPER_TOKEN:'+token);
