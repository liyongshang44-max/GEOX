...
ok(cur.first_legal_next_action==='MCFT_CAP_09_TRUSTED_REGISTRY_BOOTSTRAP'&&ds.first_legal_next_action==='MCFT_CAP_09_TRUSTED_REGISTRY_BOOTSTRAP','NEXT_ACTION');
...
for(const token of ['STAGE_1B_SHADOW_ONLINE_CLOSURE','24 actual hourly UTC scheduler boundaries','PersistentSequentialSchedulerAdapter','NO_CAP09_IMPLEMENTATION_AUTHORITY'])ok(taskbookText.includes(token),'TASKBOOK:'+token);
