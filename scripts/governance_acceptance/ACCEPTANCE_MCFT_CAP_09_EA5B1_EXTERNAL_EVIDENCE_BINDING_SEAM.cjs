...
const F = {
  ...
  a0service: 'apps/server/src/runtime/twin_runtime/a0_bootstrap_runtime_service_v1.ts',
...
try {
  ...
  for (const key of ['profile','a0window','selector','contWindow','a0service','acceptance','authority']) {
  ...
  const a0service = read(F.a0service);
  ...
  for (const [key, expected] of Object.entries({
    ...
    a0_bootstrap_service: PINS.a0service,
  ...
  req(authority.implemented_boundary?.a0_bootstrap_service_threads_explicit_soil_binding_authority === true, 'EA5B1_A0_SERVICE_SEAM_REQUIRED');
  ...
  req(a0service.includes('authorized_soil_observation_binding_id?: string') && a0service.includes('authorized_soil_binding_id: input.authorized_soil_observation_binding_id'), 'EA5B1_A0_SERVICE_THREADING_MISSING');

  for (const fileText of [profile,a0window,selector,contWindow,a0service]) {
    req(!/DATABASE_URL|POSTGRES(?:QL)?|NEON_DATABASE_URL|\bfetch\s*\(|https?:\/\//.test(fileText), 'EA5B1_NETWORK_OR_DATABASE_SURFACE_FORBIDDEN');
  }
  req(!/formal_evidence_write|INSERT\s+INTO|public\.facts/i.test(profile + a0window + selector + contWindow + a0service), 'EA5B1_FORMAL_WRITE_SURFACE_FORBIDDEN');
