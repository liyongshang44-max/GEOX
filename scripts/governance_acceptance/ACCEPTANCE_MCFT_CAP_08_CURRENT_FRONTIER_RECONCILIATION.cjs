...
const LIFECYCLE_REPAIR_FILES = [
  '.github/workflows/mcft-cap-08-current-frontier-reconciliation.yml',
  '.github/workflows/mcft-cap-08-s5-effectiveness-settlement.yml',
...
(async () => {
  ...
  try {
    ...
    if (process.argv.includes('--lifecycle-repair')) {
      assert(baseSha === LIFECYCLE_BASE, `LIFECYCLE_BASE_MISMATCH:${baseSha}`);
      assert(JSON.stringify(changed)===JSON.stringify(LIFECYCLE_REPAIR_FILES), `LIFECYCLE_BOUNDARY_MISMATCH:${JSON.stringify(changed)}`);
      const current=read('.github/workflows/mcft-cap-08-current-frontier-reconciliation.yml');
      const settlement=read('.github/workflows/mcft-cap-08-s5-effectiveness-settlement.yml');
      const prequalification=read('.github/workflows/mcft-cap-08-s5-replay-dataset-v2-prequalification.yml');
      for (const token of ['lifecycle-repair','post-closure','run-id: 30908130962','artifact-ids: 8891897316','s5-settlement','s5-prequalification']) assert(current.includes(token), `CURRENT_FRONTIER_LIFECYCLE_TOKEN_MISSING:${token}`);
      for (const token of ['lifecycle-repair','exact-settlement','successor-inactive']) assert(settlement.includes(token), `S5_SETTLEMENT_LIFECYCLE_TOKEN_MISSING:${token}`);
      for (const token of ['lifecycle-repair','exact-prequalification','exact-settlement','successor-inactive']) assert(prequalification.includes(token), `S5_PREQUALIFICATION_LIFECYCLE_TOKEN_MISSING:${token}`);
      assert(!prequalification.includes("if: steps.lifecycle.outputs.settlement != 'true'"),'LEGACY_PREQUALIFICATION_BOOLEAN_ROUTE_PRESENT');
      assert((prequalification.match(/steps\.lifecycle\.outputs\.mode == 'exact-prequalification'/g)||[]).length >= 6,'DESTRUCTIVE_PREQUALIFICATION_NOT_FULLY_CONFINED');
      for (const file of changed) assert(!read(file).includes(candidateMarker), `CANDIDATE_DECLARATION_FORBIDDEN:${file}`);
      const result={status:'PASS',capability_line_id:'MCFT-CAP-08',change_class:'S5_SUCCESSOR_WORKFLOW_LIFECYCLE_REPAIR',base_sha:baseSha,head_sha:headSha,changed_files:changed,lifecycle_modes:['lifecycle-repair','post-closure','s5-settlement','s5-prequalification','s5-architecture','exact-settlement','exact-prequalification','successor-inactive'],s5_database_reexecution:false,s5_effectiveness_reexecution:false,candidate_declaration:false,runtime_source_delta:0,registry_delta:0,delivery_status_delta:0,taskbook_byte_delta:0,mcft_cap_09_authorized:false};
      write(result); console.log(JSON.stringify(result,null,2)); return;
    }
    assert(execFileSync('git',['merge-base','--is-ancestor',SUBJECT,baseSha],{encoding:'utf8'})!==undefined,'POST_CLOSURE_BASE_NOT_DESCENDED_FROM_COMPLETION');
    assert(JSON.stringify(changed)===JSON.stringify(POST_CLOSURE_FILES), `CHANGED_FILE_BOUNDARY_MISMATCH:${JSON.stringify(changed)}`);
