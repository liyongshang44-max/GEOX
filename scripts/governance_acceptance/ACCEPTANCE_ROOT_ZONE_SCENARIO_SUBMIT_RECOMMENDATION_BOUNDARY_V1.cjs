...
assert(route.includes("project_id = $2"), "root-zone lookup requires project_id exact scope");
assert(route.includes("group_id = $3"), "root-zone lookup requires group_id exact scope");
assert(route.includes("field_id = $4"), "root-zone lookup requires field_id exact scope");
assert(route.includes("zone_id = $5"), "root-zone lookup requires zone_id exact scope");
assert(route.includes("scenario_set_id = $6"), "root-zone lookup requires scenario_set_id exact binding");
assert(!route.includes("latestRootZoneScenarioSetCandidateById"), "degraded root-zone scope fallback helper is absent");
const exactLookupStart = route.indexOf("async function latestRootZoneScenarioSetById(");
const exactLookupEnd = route.indexOf("async function latestRootZoneSubmissionByIdempotencyKey(", exactLookupStart);
assert(exactLookupStart >= 0 && exactLookupEnd > exactLookupStart, "exact root-zone lookup function found");
const exactLookup = route.slice(exactLookupStart, exactLookupEnd);
assert(!exactLookup.includes(".catch("), "exact root-zone lookup must not suppress database errors");
