...
if(!successorOnly&&!integrated){
  fail(
    "BIO_STAGE_EFFECT_EXACT_SUCCESSOR_OR_INTEGRATED_BOUNDARY_REQUIRED",
...
if(integrated){
  ...
  const resolver=qcp.dependency_resolvers?.BIOLOGICAL_STAGE_EFFECTIVENESS_GRADUATION_V1;
  if(!resolver||resolver.kind!=="EXACT_PATH_SET"){
    fail("BIO_STAGE_EFFECT_INTEGRATED_QCP_RESOLVER_REQUIRED");
  }
  const expectedOwned=successorPaths.slice().sort();
  const actualOwned=(resolver.paths||[]).slice().sort();
  eq(JSON.stringify(actualOwned),JSON.stringify(expectedOwned),"BIO_STAGE_EFFECT_INTEGRATED_QCP_PATHS_REQUIRED");
  const check=(qcp.checks||[]).find(x=>x.check_id==="BIOLOGICAL_STAGE_EFFECTIVENESS_GRADUATION");
  if(!check)fail("BIO_STAGE_EFFECT_INTEGRATED_QCP_CHECK_REQUIRED");
}

const a=JSON.parse(fs.readFileSync(successorPaths[1],"utf8"));
...
const arch=fs.readFileSync(successorPaths[3],"utf8");
for(const marker of [
...
const crop=fs.readFileSync(successorPaths[4],"utf8");
for(const marker of [
...
const wf=fs.readFileSync(successorPaths[0],"utf8");
...
console.log(JSON.stringify({
  ...
  qualification_boundary:integrated?"SUCCESSOR_PLUS_QCP_REGISTRATION":"SUCCESSOR_ONLY",
  real_effectiveness_issued:false,
