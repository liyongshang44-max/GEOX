...
for (const table of ['field_index_v1', 'field_polygon_v1', 'device_index_v1', 'device_binding_index_v1', 'device_status_index_v1', 'device_capability', 'telemetry_index_v1', 'device_observation_index_v1', 'alert_event_index_v1', 'field_memory_v1', 'approval_requests_v1', 'operation_state_v1_optional']) assert(Array.isArray(exported.tables?.[table]), `export-json missing table ${table}`);
...
if (apiBase) {
  httpOk(`${apiBase.replace(/\/+$/, '')}/api/health`).then((ok) => {
    if (ok) {
      const r = spawnSync(process.execPath, [SEED, '--verify-api', '--tenant', 'tenantA', '--base-url', apiBase], { cwd: ROOT, encoding: 'utf8' });
