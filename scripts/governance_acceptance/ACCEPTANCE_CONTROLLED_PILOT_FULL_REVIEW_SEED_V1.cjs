...
function assertFormalChain(exported) {
  ...
  assert(c.irrigation_requirement?.derivation?.source_type === 'irrigation_requirement_skill_input_v1', 'irrigation requirement derivation source_type invalid', c.irrigation_requirement?.derivation);
...
function assertIrrigationRequirementExportContract(exported) {
  ...
  assert(requirement?.quality?.derivation_status === 'DERIVED_FROM_FORMAL_SKILL_INPUT', 'irrigation requirement derivation status mismatch', requirement?.quality);
  assert(requirement?.derivation?.source_input_id === 'iskill_input_c8_irrigation_001', 'irrigation requirement derivation source_input_id mismatch', requirement?.derivation);
...
function assertFieldMemoryExportContract(exported) {
  ...
  assert(firstPayload(exported, 'field_memory_record_v1')?.record_state === 'RECORD_COMMITTED', 'P30 field memory record proof missing', exported.facts_by_type?.field_memory_record_v1);
...
async function main() {
  ...
  need('seed P29/P30 formal-memory proof fixture', c8Dataset, ['field_memory_candidate_v1', 'field_memory_record_v1', 'CANDIDATE_RECORDED', 'RECORD_COMMITTED', 'FIELD_MEMORY_CANDIDATE_GATE_CONTRACT_V0', 'FIELD_MEMORY_RECORD_GATE_CONTRACT_V0', 'controlled_fixture_only: true']);
  ...
  need('seed irrigation requirement H9 derivation flow', c8Dataset, ['deriveC8IrrigationRequirementFromFormalSkillInputV1', 'irrigationRequirementDerivation', 'DERIVED_FROM_FORMAL_SKILL_INPUT', 'irrigation_requirement_from_skill_input_v1']);
