...
const OUT=path.join(ROOT,"acceptance-output/MCFT_CAP_09_PRODUCTION_KBS_BASELINE_POINTER_SCHEMA_REMEDIATION_PREFLIGHT_V1_RESULT.json");
...
try{
 const subject=String(process.env.SUBJECT_SHA||"");assert.match(subject,/^[0-9a-f]{40}$/,"KBS_POINTER_SCHEMA_PREFLIGHT_SUBJECT_REQUIRED");
 const seed=String(process.env.SEED_DATABASE_URL||"").trim();assert.ok(seed,"KBS_POINTER_SCHEMA_PREFLIGHT_SEED_URL_REQUIRED");
 ...
 assert.equal(armAuthorized,arm.armed===true,"KBS_POINTER_SCHEMA_ARM_AUTHORIZATION_COHERENCE_REQUIRED");
 assert.equal(authorityAuthorized,armAuthorized,"KBS_POINTER_SCHEMA_AUTHORITY_ARM_MISMATCH");
 ...
 assert.deepEqual(authority.target.expected_pointer_columns,COLUMNS);
 assert.deepEqual(authority.target.expected_pointer_constraints,CONSTRAINTS);
 ...
 for(const k of ["runtime_process_start_authorized","production_owner_activation_authorized","formal_v5_arm_authorized","a0_authorized","o00_authorized"])assert.equal(arm[k],false,"KBS_POINTER_SCHEMA_LATER_AUTHORITY_FORBIDDEN:"+k);

 const u=new URL(seed);u.pathname="/"+TARGET;const url=u.toString();
 assert.equal(q(url,"SELECT current_database()"),TARGET,"KBS_POINTER_SCHEMA_DATABASE_MISMATCH");
 ...
 assert.equal(owner,currentUser,"KBS_POINTER_SCHEMA_TARGET_TABLE_OWNER_MUST_BE_CURRENT_USER");
 ...
 assert.equal(tableCount,41,"KBS_POINTER_SCHEMA_EXACT_41_TABLES_REQUIRED");
 assert.equal(routineCount,3,"KBS_POINTER_SCHEMA_EXACT_3_ROUTINES_REQUIRED");
 ...
 if(!absent&&!complete)throw new Error("KBS_POINTER_SCHEMA_PARTIAL_STATE_FORBIDDEN:columns="+observedColumns.length+":constraints="+observedConstraints.length);
 if(arm.armed===true)assert.equal(absent,true,"KBS_POINTER_SCHEMA_ARMED_ALREADY_MATERIALIZED_FORBIDDEN");
 const leaseRows=Number(q(url,"SELECT count(*)::int FROM public.external_evidence_producer_lease_v1"));
 write({schema_version:"geox_mcft_cap09_production_kbs_baseline_pointer_schema_remediation_preflight_v1",status:absent?"PASS_REMEDIATION_REQUIRED":"PASS_ALREADY_MATERIALIZED",subject_sha:subject,database_name:TARGET,current_user:currentUser,target_table_owner:owner,table_count:tableCount,routine_count:routineCount,lease_row_count:leaseRows,observed_pointer_columns:observedColumns,observed_pointer_constraints:observedConstraints,pointer_schema_absent:absent,pointer_schema_complete:complete,arm_observed:arm.armed===true,remediation_authorized_observed:armAuthorized,authority_authorized_observed:authorityAuthorized,database_mutation:false,row_mutation:false,role_mutation:false,runtime_process_start:false,production_owner_activation:false,provider_request_count:0,formal_v5_arm:false,a0_bootstrap:false,o00_started:false});
