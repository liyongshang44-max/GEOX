...
assertIncludes(route, 'const formalGate = buildFormalAcceptanceGateV1(receiptFact.record_json ?? {}, executionJudge)', 'acceptance route');
...
// Formal execution must depend on formal execution/delivery/effect evidence, not execution_time alone.
...
assertIncludes(route, 'hasReceiptCompletenessSkillPass', 'formal execution evidence gate');
assertIncludes(route, 'hasFormalExecutionEvidenceV1', 'formal execution evidence gate');
assertIncludes(route, 'const formal_execution_passed = executionJudgePassed || receiptCompletenessSkillPassed || formalExecutionEvidencePassed', 'formal execution evidence gate');
assertIncludes(route, 'FORMAL_EXECUTION_EVIDENCE_REQUIRED', 'formal execution evidence gate');
