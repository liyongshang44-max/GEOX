...
try{
 ...
 const w=read(WORKFLOW);for(const token of ["mode='workflow-repair'","mode='registry-existing-paths-correction'","mode='s1-registry-registration'","mode='candidate-signal'","mode='bootstrap'","mode='unsupported'","ACCEPTANCE_MCFT_CAP_09_TRUSTED_REGISTRY_S1_LIFECYCLE_REPAIR.cjs","ACCEPTANCE_MCFT_CAP_09_S1_REGISTRY_REGISTRATION.cjs","GEOX-MCFT-CAP-09-S1-REGISTRY-REGISTRATION-V1.json","MCFT_CAP_09_S1_REGISTRY_REGISTRATION_RESULT.json"])must(w.includes(token),`WORKFLOW_TOKEN:${token}`);
 const result={status:'PASS',change_class:'MCFT_CAP_09_TRUSTED_REGISTRY_S1_REGISTRATION_LIFECYCLE_REPAIR',base_sha:base,head_sha:head,changed_files:FILES,historical_validator_unchanged:true,registry_delta:0,candidate_transition:false,implementation_authorized:false,runtime_source_delta:0,first_legal_next_action:'MERGE_REPAIR_THEN_REBUILD_MCFT_CAP_09_S1_REGISTRY_REGISTRATION'};write(result);console.log(JSON.stringify(result,null,2));
