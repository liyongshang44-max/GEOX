...
const predecessorPins = {
  ...
  "apps/server/src/runtime/twin_runtime/future_forcing_selector_v1.ts": "cc6d49cb02986ced8b18864c8e824ceaae0cf530"
};
...
]) has(taskbook, marker, "AMENDMENT07_TASKBOOK_AUTHORITY_MISSING");
...
for (const marker of [
  ...
  "from the same cycle"
]) has(amendment05, marker, "AMENDMENT07_AMENDMENT05_AUTHORITY_MISSING");

const ea4 = json("docs/digital_twin/mcft/cap_09/GEOX-MCFT-CAP-09-EA4-LIVE-SOURCE-EXACT-HEAD-QUALIFICATION-V1.json");
eq(ea4.kbs.raw_hourly_latest_max_age_hours, 6, "AMENDMENT07_KBS_MAX_AGE_MUST_REMAIN_SIX_HOURS");
eq(ea4.kbs.raw_hourly_csv, "https://lter.kbs.msu.edu/datatables/13.csv", "AMENDMENT07_KBS_SOURCE_IDENTITY_DRIFT");
eq(ea4.gfs.future_file_waiting_forbidden, true, "AMENDMENT07_FUTURE_FILE_WAITING_MUST_REMAIN_FORBIDDEN");
eq(ea4.gfs.cross_cycle_substitution_authorized, false, "AMENDMENT07_CROSS_CYCLE_SUBSTITUTION_MUST_REMAIN_FORBIDDEN");

const recovery = json("docs/digital_twin/mcft/cap_09/GEOX-MCFT-CAP-09-EA4-RECOVERY-AUTHORITY-V1.json");
eq(recovery.recovery_proof.same_source_identity, true, "AMENDMENT07_EA4_SAME_SOURCE_REQUIRED");
eq(recovery.recovery_proof.source_substitution_performed, false, "AMENDMENT07_EA4_SOURCE_SUBSTITUTION_FORBIDDEN");
eq(recovery.recovery_proof.freshness_threshold_changed, false, "AMENDMENT07_EA4_FRESHNESS_CHANGE_FORBIDDEN");
eq(recovery.recovery_proof.attempts[0].probe_started_at_utc, "2026-08-09T05:47:28.938254Z", "AMENDMENT07_EA4_PROBE_START_REQUIRED");
eq(recovery.recovered_kbs_facts.latest_record_utc, "2026-08-09T04:00:00Z", "AMENDMENT07_EA4_LATEST_KBS_HOUR_REQUIRED");
eq(recovery.recovered_future_facts.source_cycle_utc, "2026-08-09T00:00:00Z", "AMENDMENT07_EA4_GFS_CYCLE_REQUIRED");
eq(recovery.current_authority_effect_if_merged.live_source_qualified, true, "AMENDMENT07_EA4_LIVE_SOURCE_QUALIFIED_REQUIRED");
...
]) has(continuation, marker, "AMENDMENT07_EXISTING_EXACT_INTERVAL_CAUSALITY_MISSING");
...
]) has(externalInput, family, "AMENDMENT07_EXTERNAL_FIVE_FAMILY_CONTRACT_MISSING");
has(externalInput, "EXTERNAL_CAP04_REQUIRED_EVIDENCE_FAMILY_MISSING", "AMENDMENT07_EXTERNAL_FIVE_FAMILY_FAIL_CLOSED_MISSING");
...
]) has(externalCandidate, marker, "AMENDMENT07_EXTERNAL_CAP04_EXECUTION_SEAM_MISSING");
...
for (const marker of [
  ...
  "72"
]) has(futureSelector, marker, "AMENDMENT07_FUTURE_FORCING_CAUSALITY_MISSING");
...
const effect = ea5e1.effect_if_exact_head_proof_passes_and_candidate_merges_to_protected_main;
eq(effect.ea5e1_post_rebase_formal_db_preflight_effective, true, "AMENDMENT07_EA5E1_EFFECT_REQUIRED");
eq(effect.formal_window_input_manifest_frozen, true, "AMENDMENT07_EA5E1_MANIFEST_MUST_REMAIN_FROZEN");
eq(effect.formal_o00_start_authorized, false, "AMENDMENT07_O00_MUST_REMAIN_UNAUTHORIZED");
eq(effect.formal_execution_count, "0/24", "AMENDMENT07_FORMAL_EXECUTION_MUST_REMAIN_ZERO");
...
]) has(amendment, marker, "AMENDMENT07_REQUIRED_RULING_MISSING");
for (const marker of [
  "event_time <= T",
...
]) has(amendment, marker, "AMENDMENT07_REQUIRED_CAUSAL_RULE_MISSING");
for (const forbidden of [
  ...
  if (amendment.includes(forbidden)) fail(`AMENDMENT07_PREMATURE_CLAIM:${forbidden}`);
}

const lagHours = 7;
const delayedCollectorOffsetMinutes = 390;
const delayedCutoffOffsetMinutes = 432;
const runtimeObserverOffsetMinutes = 437;
eq(lagHours, ea4.kbs.raw_hourly_latest_max_age_hours + 1, "AMENDMENT07_LAG_DERIVATION_INVALID");
if (delayedCutoffOffsetMinutes <= delayedCollectorOffsetMinutes) fail("AMENDMENT07_DELAYED_COLLECTOR_MARGIN_INVALID");
if (runtimeObserverOffsetMinutes - delayedCutoffOffsetMinutes < 5) fail("AMENDMENT07_RUNTIME_INGESTION_MARGIN_INVALID");
...
]) has(workflow, marker, "AMENDMENT07_WORKFLOW_PROOF_STEP_MISSING");
for (const forbidden of [
  "pull_request_target",
  ...
  if (workflow.includes(forbidden)) fail(`AMENDMENT07_WORKFLOW_SIDE_EFFECT_CAPABILITY_FORBIDDEN:${forbidden}`);
}
...
const result = {
  ...
  future_forcing_post_logical_time_availability_authorized: false,
  scheduler_eligibility_lag_hours: lagHours,
  runtime_observer_minute_utc: 17,
  delayed_exact_hour_collector_offset_minutes: delayedCollectorOffsetMinutes,
  delayed_exact_hour_availability_cutoff_offset_minutes: delayedCutoffOffsetMinutes,
  minimum_ingestion_margin_minutes: 5,
