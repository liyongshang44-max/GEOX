...
function req(rel){const x=file(rel);if(!x)throw Error(`EA5E2_DEP_FILE_MISSING:${rel}`);return x;}
...
function liveSets(){const t=fs.readFileSync(LIVE,"utf8"),push=new Set(listBlock(t,"\n    paths:\n","\n  workflow_dispatch:"));const i=t.indexOf("critical=("),j=t.indexOf("\n          )",i),critical=new Set();for(const l of t.slice(i,j).split(/\r?\n/)){const m=/^\s+([^\s()]+)\s*$/.exec(l);if(m&&m[1]!=="critical=(")critical.add(m[1].replace(/^['"]|['"]$/g,""));}return{push,critical};}
...
function main(){const g=closure(),missingMust=MUST.filter(x=>!g.includes(x)),{push,critical}=liveSets(),patterns=listBlock(fs.readFileSync(STATIC_WF,"utf8"),"\n    paths:\n","\n  workflow_dispatch:").map(glob),uncovered=g.filter(x=>!patterns.some(r=>r.test(x))),expected=digest(g),actual=MARK_RE.exec(fs.readFileSync(CARRIER,"utf8"))?.[1]??null;const p={schema_version:"geox_mcft_cap09_ea5e2_runtime_dependency_graph_v2",status:missingMust.length||uncovered.length||!push.has(CARRIER)||!critical.has(CARRIER)||actual!==expected?"FAIL":"PASS",runtime_dependency_graph_count:g.length,runtime_dependency_graph_paths:g,expected_dependency_graph_sha256:expected,carrier_dependency_graph_sha256:actual,binding_carrier_path:CARRIER,binding_carrier_in_live_push:push.has(CARRIER),binding_carrier_in_exact_main_critical:critical.has(CARRIER),required_runtime_discovery_missing:missingMust,static_gate_uncovered_paths:uncovered,historical_runner_qualification_mutated:false,provider_request_count:0,database_read_count:0,database_write_count:0,formal_effect:false};fs.mkdirSync("acceptance-output",{recursive:true});fs.writeFileSync("acceptance-output/MCFT_CAP_09_EA5E2_RUNTIME_DEPENDENCY_GRAPH.json",JSON.stringify(p,null,2)+"\n");console.log(JSON.stringify(p));if(p.status!=="PASS")throw Error(`EA5E2_RUNTIME_DEPENDENCY_GRAPH_UNBOUND:${JSON.stringify({missingMust,uncovered,carrierPush:p.binding_carrier_in_live_push,carrierCritical:p.binding_carrier_in_exact_main_critical,digestMatch:actual===expected})}`);}
main();
