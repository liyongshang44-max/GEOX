...
const ZERO_FIELDS = ["database_write_count","runtime_write_count","scheduler_write_count","provider_request_count","r2_request_count"];
...
function classify(persistent, subject) {
  ...
  if (persistent.status === "PASS") return {schema_version:"geox_mcft_cap09_amendment19_persistent_graduation_trigger_classification_v1",status:"PASS",subject_sha:subject,crop_authority_id:CROP_AUTHORITY,crop_context_materialization_profile:CROP_MATERIALIZATION_PROFILE,trigger_mode:"FRESH_QUALIFICATION",fresh_pass:true,read_only:false,new_machine_gate_claim:true};
  ...
  for (const key of ZERO_FIELDS) need(Number(persistent[key]) === 0, `AM19_GRADUATION_TRIGGER_READ_ONLY_ZERO_REQUIRED:${key}`);
  ...
  return {schema_version:"geox_mcft_cap09_amendment19_persistent_graduation_trigger_classification_v1",status:"PASS",subject_sha:subject,crop_authority_id:CROP_AUTHORITY,crop_context_materialization_profile:CROP_MATERIALIZATION_PROFILE,trigger_mode:"IDEMPOTENT_READ_ONLY_NO_GATE",fresh_pass:false,read_only:true,source_persistent_status:persistent.status,existing_success_evidence_unchanged:true,new_machine_gate_claim:false,formal_database_write_count:0,formal_r2_write_count:0,scheduler_write_count:0,runtime_write_count:0,final_actual_24h_still_required:true,formal_o00_started:false,mcft_cap09_completed:false};
}

function selftest() {
  const subject = "1".repeat(40);
  const identity = {crop_authority_id:CROP_AUTHORITY,crop_context_materialization_profile:CROP_MATERIALIZATION_PROFILE};
  ...
  const readOnly={schema_version:SCHEMA,status:"ALREADY_QUALIFIED_READ_ONLY",subject_sha:subject,qualified_subject_sha:subject,main_database_name:MAIN_DATABASE,blocked_database_name:BLOCKED_DATABASE,...identity,database_write_count:0,runtime_write_count:0,scheduler_write_count:0,provider_request_count:0,r2_request_count:0,new_machine_gate_claim:false,existing_success_evidence_unchanged:true,final_actual_24h_still_required:true,formal_o00_started:false,mcft_cap09_completed:false};
  const replay=classify(readOnly,subject); need(replay.fresh_pass===false&&replay.read_only===true&&replay.new_machine_gate_claim===false,"AM19_GRADUATION_TRIGGER_SELFTEST_READ_ONLY_FAILED");
  const negatives=[
...
if(process.argv.includes("--selftest")){selftest();}else{const [persistentPath,subject,outPath]=process.argv.slice(2);if(![persistentPath,subject,outPath].every(Boolean))fail("AM19_GRADUATION_TRIGGER_USAGE:persistent-result subject output");const persistent=JSON.parse(fs.readFileSync(path.resolve(persistentPath),"utf8"));const result=classify(persistent,subject);fs.mkdirSync(path.dirname(path.resolve(outPath)),{recursive:true});fs.writeFileSync(path.resolve(outPath),`${JSON.stringify(result,null,2)}\n`);console.log(JSON.stringify(result));}
