...
// Purpose: verify that P3-03 defines a read-only operator gate read model planning contract.
...
const ACCEPTANCE = 'P3_03_OPERATOR_GATE_READ_MODEL_PLANNING';
const NEXT_STEP = 'P3_04_DRY_RUN_REPORT_READ_MODEL_PLANNING';
const DOC = 'docs/tasks/P3-03-Operator-Gate-Read-Model-Planning.md';

const FILES = {
  previousDoc: 'docs/tasks/P3-02-Operator-Preflight-Read-Model-Planning.md',
  previousAcceptance: 'scripts/governance_acceptance/P3_02_OPERATOR_PREFLIGHT_READ_MODEL_PLANNING.cjs',
  currentDoc: DOC,
};
...
function main() {
  ...
  assert('previous_step_verified', containsAll(previousDoc, ['P3_02_OPERATOR_PREFLIGHT_READ_MODEL_PLANNING', ACCEPTANCE]) && containsAll(previousAcceptance, ['P3_02_OPERATOR_PREFLIGHT_READ_MODEL_PLANNING', ACCEPTANCE]), { files: [FILES.previousDoc, FILES.previousAcceptance] });

  assert('document_identity_verified', containsAll(doc, [ACCEPTANCE, 'operator_gate_read_model_v0', 'Input references', 'Required output fields', 'Allowed state vocabulary', 'Boundary ledger', NEXT_STEP]), { file: DOC });

  assert('read_model_identity_verified', containsAll(doc, ['read_model_id: operator_gate_read_model_v0', 'read_model_kind: planning_contract', 'surface_ref: operator_gate_panel', 'mode: read_only', 'status: candidate']), { file: DOC });
  ...
  assert('required_output_field_count_verified', outputFields.length === 16, { outputFields });
  assert('allowed_state_count_verified', states.length === 11, { states });
