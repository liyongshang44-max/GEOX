...
// Scan candidate capability surfaces, excluding this gate's own literal deny-list declarations.
...
const gateText=fs.readFileSync(gateFile,"utf8");
for(const required of ["fs.readFileSync","git(\"diff\"","git(\"rev-parse\""]){if(!gateText.includes(required))throw new Error(`SUCCESSOR_SCANNER_GATE_READ_ONLY_PRIMITIVE_REQUIRED:${required}`)}
for(const forbidden of ["execSync(\"psql","spawn(\"psql","http.request","https.request"]){if(gateText.includes(forbidden))throw new Error(`SUCCESSOR_SCANNER_GATE_SIDE_EFFECT_PRIMITIVE_FORBIDDEN:${forbidden}`)}
