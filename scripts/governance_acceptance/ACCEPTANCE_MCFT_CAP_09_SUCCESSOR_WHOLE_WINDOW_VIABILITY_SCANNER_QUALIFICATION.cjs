...
const all=files.map(f=>fs.readFileSync(f,"utf8")).join("\n");for(const marker of ["GEOX_MCFT_CAP09_S6_DATABASE_URL","INSERT INTO facts","UPDATE facts","DELETE FROM facts","workflow_dispatch:","aws s3","curl ","fetch("]){if(all.includes(marker))throw new Error(`SUCCESSOR_SCANNER_SIDE_EFFECT_CAPABILITY_FORBIDDEN:${marker}`)}
