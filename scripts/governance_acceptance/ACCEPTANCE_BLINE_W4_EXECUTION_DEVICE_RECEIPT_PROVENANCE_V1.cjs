...
const p1Loop=read("apps/server/scripts/p1_skill_loop_minimal.mjs");
for(const marker of ["GEOX_EXECUTOR_TOKEN","GEOX_EXECUTOR_ACTOR_ID","executorHeaders","headers: executorHeaders"])assert(p1Loop.includes(marker),"P1 skill-loop executor caller migration drift",marker);
assert(!p1Loop.includes('id: "p1_smoke_executor"'),"P1 skill-loop retains caller-declared executor identity");
assert(p1Loop.includes('executor_id: { kind: "script", id: EXECUTOR_ACTOR_ID, namespace: "executor_runtime_v1" }'),"P1 receipt executor identity is not authenticated-actor bound");
