...
assert(!sensing.includes("ensureDeviceObservationProjectionV1"),"active HTTP sensing still executes runtime observation-projection DDL");
assert(!/CREATE\s+(TABLE|INDEX)|ALTER\s+TABLE/i.test(sensing),"active HTTP sensing route contains runtime DDL");
