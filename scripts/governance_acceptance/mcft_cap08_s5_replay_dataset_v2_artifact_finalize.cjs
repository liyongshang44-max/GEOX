...
const subject = process.env.MCFT_SUBJECT_SHA || cp.execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim();
const parents = cp.execFileSync('git',['rev-list','--parents','-n','1',subject],{encoding:'utf8'}).trim().split(/\s+/);
...
const tree = (sha) => cp.execFileSync('git',['rev-parse',`${sha}^{tree}`],{encoding:'utf8'}).trim();
