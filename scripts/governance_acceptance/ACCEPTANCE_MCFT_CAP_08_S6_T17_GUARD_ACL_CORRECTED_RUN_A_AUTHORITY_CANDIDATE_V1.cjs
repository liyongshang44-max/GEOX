...
const BASE='923a794e0fa865a4a8493680b1b8ac2e98e57fbc';
const OUT=path.join(ROOT,'acceptance-output/MCFT_CAP_08_S6_T17_GUARD_ACL_CORRECTED_RUN_A_AUTHORITY_CANDIDATE_RESULT.json');
const CANDIDATE='docs/digital_twin/mcft/cap_08/GEOX-MCFT-CAP-08-S6-T17-GUARD-ACL-CORRECTED-FORMAL-RUN-A-AUTHORITY-CANDIDATE-V1.json';
const OBJECTS='docs/digital_twin/mcft/cap_08/GEOX-MCFT-CAP-08-S6-T17-GUARD-ACL-CORRECTED-RUN-A-AUTHORITY-OBJECT-SET-V1.json';
const BOUNDARY='docs/digital_twin/mcft/cap_08/GEOX-MCFT-CAP-08-S6-T17-GUARD-ACL-CORRECTED-RUN-A-AUTHORITY-BOUNDARY-V1.json';
const ISSUANCE='docs/digital_twin/mcft/cap_08/GEOX-MCFT-CAP-08-S6-T17-GUARD-ACL-CORRECTED-RUN-A-AUTHORITY-ISSUANCE-V1.json';
const WORKFLOW='.github/workflows/mcft-cap-08-s6-t17-guard-acl-corrected-run-a-authority-candidate.yml';
const VALIDATOR='scripts/governance_acceptance/ACCEPTANCE_MCFT_CAP_08_S6_T17_GUARD_ACL_CORRECTED_RUN_A_AUTHORITY_CANDIDATE_V1.cjs';
const CONSUMED='docs/digital_twin/mcft/cap_08/GEOX-MCFT-CAP-08-S6-T17-INTERLEAVE-CORRECTED-FORMAL-RUN-A-EXECUTION-AUTHORITY-EFFECTIVE-V1.json';
...
const DBNAME='geox_mcft_cap08_s6_run_a_replacement_007_30770000007';
...
function effective(candidate){
  ...
  authority.expires_at='2099-08-05T17:45:00.000Z';
...
try{
  ...
  assert.equal(git('rev-parse',`HEAD:${CANDIDATE}`),'2cc7b11711d5afe0063bb74e84455dadc36f436d');
  assert.equal(git('rev-parse',`HEAD:${OBJECTS}`),'37ad609382dc1f40a8e77272ffab7ffd525e01c5');
  assert.equal(git('rev-parse',`HEAD:${CONSUMED}`),'a5bf87adbacd43cf2177a9f9bc433cf9e0e8bcb3');

  assert.equal(consumed.authority_consumed,true);
  assert.equal(consumed.operational_run_instance_id,'MCFT-CAP-08-S6-FORMAL-RUN-A-20260802-REPLACEMENT-006');
  ...
  assert.equal(candidate.logical_database_identity.physical_name_template,'geox_mcft_cap08_s6_run_a_replacement_007_<github_run_id>');
  ...
  assert.equal(candidate.replaces_consumed_authority.failed_workflow_run_id,30758716511);

  assert.equal(objects.object_count,54);
  assert.equal(objects.exact_subject_sha,BASE);
  assert.equal(objects.exact_database_bootstrap_object_set['apps/server/src/infra/mcft_cap08_database_platform_bootstrap_v1.ts'],'31ef663a85ea93fe0319db598515840b12111135');
  assert.equal(objects.exact_database_bootstrap_object_set['apps/server/src/infra/mcft_cap08_database_platform_bootstrap_base_v1.ts'],'720e14c5567c227b02c39579f17ec80c2f1fbf5b');
  assert.equal(objects.exact_database_bootstrap_object_set['scripts/runtime_acceptance/ACCEPTANCE_MCFT_CAP_08_PLATFORM_SECURITY_BOOTSTRAP_DB.ts'],'2fa649c8f176fbc4b4d3c73611b39131209d7b1c');
  ...
  assert.equal(identity.materializePhysicalDatabaseNameV1(effective(candidate),'30770000007'),DBNAME);

  const wrapper=text('apps/server/src/infra/mcft_cap08_database_platform_bootstrap_v1.ts');
  ...
  const result={
    schema_version:'geox_mcft_cap08_s6_t17_guard_acl_corrected_run_a_authority_candidate_result_v1',
    status:'PASS',
    base_main_sha:BASE,
  ...
  save({schema_version:'geox_mcft_cap08_s6_t17_guard_acl_corrected_run_a_authority_candidate_result_v1',status:'FAIL',error:error instanceof Error?error.message:String(error)});
  console.error(error);
  process.exitCode=1;
}
