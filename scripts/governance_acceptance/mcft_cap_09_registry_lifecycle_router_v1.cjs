...
if(mode==='s6-registry-registration')process.exitCode=runNode([process.env.MCFT_REGISTRY_LANE==='s6-registry-registration'?'scripts/governance_acceptance/ACCEPTANCE_MCFT_CAP_09_S6_REGISTRY_REGISTRATION.cjs':'scripts/governance_acceptance/ACCEPTANCE_MCFT_CAP_09_S2_REGISTRY_CROSS_LIFECYCLE_REPAIR.cjs',...(process.env.MCFT_REGISTRY_LANE==='s6-registry-registration'?[]:['--s6-registration-route-only'])]);
