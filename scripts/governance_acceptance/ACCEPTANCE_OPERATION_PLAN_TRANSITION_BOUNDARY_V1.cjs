...
function listFilesRecursive(relativeDir) {
  ...
  for (const entry of fs.readdirSync(absoluteDir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) out.push(...listFilesRecursive(relativePath));
...
for (const forbidden of ['ao_act_task_v0', '/api/v1/actions/task', 'ao_act_receipt_v1', 'roi_ledger_v1', 'field_memory_v1']) {
  assert(!route.includes(forbidden), `route forbidden token ${forbidden}`);
}

assert(!/dispatch/i.test(route.replace('dispatch_created', '')), 'route must not create dispatch');

for (const forbidden of ['from "pg"', "from 'pg'", 'Fastify', '../routes', 'process.env', 'Date.now', 'new Date', 'randomUUID']) {
  assert(!builder.includes(forbidden), `builder forbidden token ${forbidden}`);
}
...
const customerFiles = [
  ...listFilesRecursive('apps/web/src'),
  ...listFilesRecursive('apps/server/src/routes'),
].filter((p) => /customer|client|reports|dashboard/.test(p));
