...
const ACCEPTANCE_NAME = "ACCEPTANCE_ROOT_ZONE_SOIL_WATER_STATE_SCHEMA_MIGRATION_V1";
const MIGRATION_PATH = "apps/server/db/migrations/2026_06_21_root_zone_soil_water_state_v1.sql";
const TABLE_NAME = "root_zone_soil_water_state_index_v1";
const PROBE_ID = "h32_root_zone_soil_water_state_acceptance_schema_probe";
...
const REQUIRED_INDEXES = [
  "idx_root_zone_soil_water_state_index_v1_scope_latest",
  "idx_root_zone_soil_water_state_index_v1_field_latest",
];
...
async function insertProbeRow(client) {
  await client.query(
    `
      ...
      VALUES (
        $1, 'tenant_probe', 'project_probe', 'group_probe', 'field_probe', 'zone_probe',
        60, 0, 0, 0, 'UNKNOWN', 'UNKNOWN', 0, 0,
        'INSUFFICIENT_LAYER_ESTIMATES', 'schema_probe_hash', '2026-06-21T00:00:00Z'
      )
    `,
    [PROBE_ID],
  );
  ...
  const result = await client.query(
    `
      ...
      FROM public.root_zone_soil_water_state_index_v1
      WHERE state_id = $1
    `,
    [PROBE_ID],
  );
