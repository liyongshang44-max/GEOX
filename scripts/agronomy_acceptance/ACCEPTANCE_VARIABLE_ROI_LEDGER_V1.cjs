...
(async () => {
  ...
  const receiptPayload = {
    ...
    executor_id: {
      kind: 'script',
      id: 'acceptance_variable_roi_ledger_v1',
  ...
  const asExecutedResp = requireOk(
  ...
  const roiResp = requireOk(
    await fetchJson(`${base}/api/v1/roi-ledger/from-as-executed`, {
      method: 'POST',
      token,
      body: {
        as_executed_id,
  ...
  const byAs = requireOk(
    await fetchJson(`${base}/api/v1/roi-ledger/by-as-executed/${encodeURIComponent(as_executed_id)}?tenant_id=${encodeURIComponent(tenant_id)}&project_id=${encodeURIComponent(project_id)}&group_id=${encodeURIComponent(group_id)}`, {
      method: 'GET',
      token,
    }),
    'by as-executed',
  ...
  const checks = {
    ...
    variable_water_saved_amount_correct: Number(vws?.delta?.amount) === 1
      && String(vws?.delta?.unit ?? '').toLowerCase() === 'mm',
    zone_completion_rate_correct: Number(zcr?.actual?.zone_count) === 2
      && Number(zcr?.actual?.completed_zone_count) === 2
      && Number(zcr?.actual?.completion_rate) === 1
      && Number(zcr?.actual?.avg_coverage_percent) === 97,
    variable_execution_reliability_correct: Number(ver?.actual?.zone_count) === 2
      && Number(ver?.actual?.applied_count) === 2
      && Number(ver?.actual?.skipped_count) === 0
      && Math.abs(Number(ver?.actual?.max_abs_deviation_percent) - 3.3333333333333335) < 0.02,
    ...
    openapi_contains_variable_roi_types: Boolean(
      openapi.ok
