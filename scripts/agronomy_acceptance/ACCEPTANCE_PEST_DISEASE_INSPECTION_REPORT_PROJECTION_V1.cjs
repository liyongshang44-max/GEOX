...
async function insertOperationPlanFact(pool, scope, operation_plan_id, field_id, inspection_id) {
  ...
  const record = {
    ...
    payload: {
      ...
      operation_plan_id,
      field_id,
      action_type: 'PEST_DISEASE_INSPECTION',
      scenario_type: 'FORMAL_PEST_DISEASE_INSPECTION',
      pest_disease_inspection_id: inspection_id,
      inspection_id,
      status: 'CREATED',
      created_at_ts: now,
    },
...
async function run() {
  ...
  try {
    ...
    await requireOk(await post(base, '/api/v1/inspection/pest-disease/request', adminToken, {
      ...scope,
      inspection_id,
      field_id,
      ...
      priority: 'HIGH',
      evidence_refs: [],
      reasons: ['report_projection_gate'],
    }), 'inspection request');
    ...
    const assessment = await requireOk(await post(base, '/api/v1/inspection/pest-disease/assessment', adminToken, {
      ...scope,
      ...
      reasons: ['formal_observation_complete'],
    }), 'inspection assessment');

    const review = await requireOk(await post(base, '/api/v1/inspection/pest-disease/review', adminToken, {
      ...scope,
      ...
      reviewer_actor_id: 'report_projection_gate',
      reviewed_at_ts: Date.now(),
      review_note: 'Approved for report projection gate.',
      evidence_refs: [{ kind: 'fact_id', ref_id: assessment.fact_id }],
    }), 'inspection review');

    const acceptance = await requireOk(await post(base, '/api/v1/inspection/pest-disease/acceptance/evaluate', operatorToken, {
      assessment_id: assessment.assessment_id,
    ...
    assert.equal(report?.pest_disease_inspection?.inspection_id, inspection_id);
    assert.equal(report?.pest_disease_inspection?.assessment_id, assessment.assessment_id);
    assert.equal(report?.pest_disease_inspection?.assessment_status, 'CONFIRMED');
    assert.equal(report?.pest_disease_inspection?.media_count > 0, true);
    assert.equal(report?.pest_disease_inspection?.geo_evidence_present, true);
    assert.equal(report?.pest_disease_inspection?.reviewed_by_human, true);
    assert.equal(report?.pest_disease_inspection?.customer_visible_eligible, true);

    console.log('PASS acceptance pest disease inspection report projection v1', {
      operation_plan_id,
      inspection_id,
      assessment_id: assessment.assessment_id,
      inspection_acceptance_id: acceptance.inspection_acceptance_id,
    });
  } finally {
