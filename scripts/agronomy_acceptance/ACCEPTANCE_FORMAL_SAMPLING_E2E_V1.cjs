...
async function main() {
  ...
  try {
    ...
    const plan = requireOk(await fetchJson(`${base}/api/v1/sampling/plan`, { method: 'POST', token, body: {
      subject_ref: { project_id: scope.project_id, field_id: `field_${run}` },
      sampling_kind: 'FORMAL_SAMPLING',
      requested_by: 'acceptance_script',
      requested_at_ts: Date.now(),
      notes: `run=${run}`,
    } }), 'create sampling plan');
    checks.sampling_plan_created = true;

    const aoSenseTaskRef = rid('ao_sense_task'); // formal chain evidence for "create AO-SENSE task or sampling task"

    const receipt = requireOk(await fetchJson(`${base}/api/v1/sampling/receipt`, { method: 'POST', token, body: {
      plan_id: plan.plan_id,
      sample_ref: { sample_id },
      collected_at_ts: Date.now(),
      collector_id: 'collector_formal_sampling',
      evidence_refs: [
        { kind: 'ao_sense_task', ref_id: aoSenseTaskRef },
        { kind: 'photo', ref_id: rid('photo') },
      ],
    } }), 'create sample receipt with evidence refs');
    checks.sample_receipt_created = true;

    const lab = requireOk(await fetchJson(`${base}/api/v1/sampling/lab-result`, { method: 'POST', token, body: {
      sample_id,
      report_ref: rid('lab_report'),
      imported_at_ts: Date.now(),
      metrics: { ph: 6.5, ec: 1.2, quality_status: 'PASS' },
    } }), 'import lab result');
    ...
    const acceptance = evaluateSamplingAcceptance({
      receipt: sampleFact.type === 'sample_receipt_v1' ? sampleFact : null,
    ...
    assert.equal(checks.sampling_plan_created, true, 'sampling_plan_created');
    assert.equal(checks.sample_receipt_created, true, 'sample_receipt_created');
    assert.equal(checks.lab_result_imported, true, 'lab_result_imported');
    assert.equal(checks.sampling_acceptance_evaluated, true, 'sampling_acceptance_evaluated');
    assert.equal(checks.invalid_lab_result_not_pass, true, 'invalid_lab_result_not_pass');
    assert.equal(checks.customer_report_downgraded_when_evidence_missing, true, 'customer_report_downgraded_when_evidence_missing');

    const output = { ok: true, scenario: 'FORMAL_SAMPLING', checks, refs: { plan_id: plan.plan_id, receipt_id: receipt.receipt_id, lab_result_id: lab.lab_result_id } };
