...
function runWith(e){const out=execFileSync('node',['--import','tsx','--input-type=module','-e',`const mod = await import('./apps/server/src/runtime/runtime_security_v1.ts');\nconsole.log(JSON.stringify(mod.getRuntimeSecurityStatusV1()));`],{cwd:path.resolve(__dirname,'../..'),env:{...process.env,GEOX_TOKEN:'',GEOX_AO_ACT_TOKEN:'',AO_ACT_TOKEN:'',GEOX_TOKENS_FILE:'',GEOX_TOKEN_SSOT_PATH:'',GEOX_TOKENS_JSON:'',...e},encoding:'utf8'});return JSON.parse(out.trim().split('\n').pop());}
(async()=>{const checks={};
