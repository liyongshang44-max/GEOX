...
const deviceCredentialSecret = (fx: any) => `${fx.run_id}:${fx.device_id}:credential`;
...
async function upsertDevice(pool: Pool, fx: any) {
  ...
  await pool.query(`INSERT INTO device_credential_index_v1 (tenant_id, device_id, credential_id, credential_hash, status, issued_ts_ms, revoked_ts_ms, created_ts_ms, updated_ts_ms) VALUES ($1,$2,$3,$4,'ACTIVE',$5,NULL,$5,$5) ON CONFLICT (tenant_id, device_id, credential_id) DO UPDATE SET credential_hash=EXCLUDED.credential_hash, status='ACTIVE', revoked_ts_ms=NULL, updated_ts_ms=EXCLUDED.updated_ts_ms`, [fx.tenant_id, fx.device_id, fx.credential_id, sha(deviceCredentialSecret(fx)), ts]);
...
async function postRaw(base: string, token: string, fx: any, overrides: any = {}, expectOk = true, ctx: FormalScenarioKernelContextV1 | null = null) {
  ...
  const requestToken = ['device','gateway'].includes(String(body.source ?? '').toLowerCase()) ? deviceCredentialSecret(fx) : token;
  const resp = await fetchJson(`${base}/api/v1/sensing/raw-samples`, { method: 'POST', token: requestToken, body });
