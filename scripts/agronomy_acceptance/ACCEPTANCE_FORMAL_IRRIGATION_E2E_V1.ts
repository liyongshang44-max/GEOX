...
import { Pool } from 'pg';
import { assert, env, fetchJson, requireOk } from './_common.cjs';
import { runFormalScenarioKernelV1 } from '../../apps/server/src/services/scenarios/formal_scenario_kernel_v1.ts';
...
async function postSamples({ base, token, fx, manifest = null, source = 'device', metric = 'pressure', unit = 'kPa', value = 42, count = 12, expectOk = true, field_id = fx.field_id, device_id = fx.device_id, credential_id = fx.credential_id, offsetMs = 0 }) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (manifest) ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/sensing/raw-samples', ok: resp.ok && resp.json?.ok === true, status_code: resp.status, label: `${source}/${metric}`, request: body, response: resp.json ?? resp.text });
    out.push(resp);
...
async function generateRecommendation({ base, token, fx, manifest = null }) {
  ...
  if (manifest) ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/recommendations/generate', ok: resp.ok && resp.json?.ok === true, status_code: resp.status, label: 'recommendation generate', request: body, response: resp.json ?? resp.text });
  return resp;
}
async function ensureCropContextViaProgram({ base, token, fx, manifest = null }) {
  ...
  if (manifest) ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/programs', ok: resp.ok && resp.json?.ok === true, status_code: resp.status, label: 'field program create', request: body, response: resp.json ?? resp.text });
...
async function createTask({ base, adminToken, approverToken, fx, manifest, recommendation, allowAuto = true }) {
  ...
  if (manifest) ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/approvals/request', ok: reqResp.ok && reqResp.json?.ok === true, status_code: reqResp.status, label: 'approval request', request: requestBody, response: reqResp.json ?? reqResp.text });
  const reqJson = requireOk(reqResp, 'approval request');
  if (manifest) ctx.updateManifest({ approval_request_id: reqJson.request_id });
  ...
  if (manifest) ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/approvals/approve', ok: approveResp.ok && approveResp.json?.ok === true, status_code: approveResp.status, label: 'approval approve', request: approveBody, response: approveResp.json ?? approveResp.text });
  if (!allowAuto) return approveResp;
  const approveJson = requireOk(approveResp, 'approval approve');
  if (manifest) { ctx.updateManifest({ act_task_id: approveJson.act_task_id, operation_id: `opl_${reqJson.request_id}` }); }
  return approveJson;
}
async function receipt({ base, executorToken, fx, manifest, pre = 0.18, post = 0.25, delta = 0.07 }) {
  ...
  ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/actions/receipt', ok: resp.ok && resp.json?.ok === true, status_code: resp.status, label: 'formal receipt', request: body, response: resp.json ?? resp.text });
  const json = requireOk(resp, 'formal receipt');
  ctx.updateManifest({ receipt_id: json.fact_id, evidence_refs: body.evidence_refs.map((x) => x.ref) });
}
async function acceptance({ base, operatorToken, fx, manifest }) {
  ...
  ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/acceptance/evaluate', ok: resp.ok && resp.json?.ok === true, status_code: resp.status, label: 'acceptance evaluate', request: body, response: resp.json ?? resp.text });
  const json = requireOk(resp, 'acceptance evaluate');
  ctx.updateManifest({ acceptance_id: json.fact_id });
  return json;
}
async function asExecutedFromReceipt({ base, token, fx, manifest }) {
  ...
  ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/as-executed/from-receipt', ok: resp.ok && resp.json?.ok === true, status_code: resp.status, label: 'as executed from receipt', request: body, response: resp.json ?? resp.text });
...
async function operationReport({ base, token, fx, manifest }) {
  ...
  ctx.recordApiSnapshot({ method: 'GET', path: '/api/v1/reports/operation/:operation_id', ok: resp.ok && resp.json?.ok === true, status_code: resp.status, label: 'operation report', request: { operation_id: manifest.operation_id }, response: resp.json ?? resp.text });
...
async function main() {
  ...
  const result = await runFormalScenarioKernelV1({ scenario_type: 'FORMAL_IRRIGATION', lane: 'positive', ...scope, async driver(ctx) {
      ...
      await ensureCropContextViaProgram({ base, token: adminToken, fx: ctx.fixture, ctx });
      await postSamples({ base, token: adminToken, fx: ctx.fixture, ctx, source: 'device', metric: 'pressure', value: 42, count: 12, expectOk: true });
      const recResp = await generateRecommendation({ base, token: adminToken, fx: ctx.fixture, ctx });
      const recJson = requireOk(recResp, 'recommendation generate');
      const recommendation = pickRecommendation(recJson);
      ctx.updateManifest({ recommendation_id: recommendation?.recommendation_id ?? null });
      await createTask({ base, adminToken, approverToken, fx: ctx.fixture, ctx, recommendation, allowAuto: true });
      await receipt({ base, executorToken, fx: ctx.fixture, manifest: ctx.manifest, ctx });
      const acc = await acceptance({ base, operatorToken, fx: ctx.fixture, manifest: ctx.manifest, ctx });
      await asExecutedFromReceipt({ base, token: adminToken, fx: ctx.fixture, manifest: ctx.manifest, ctx });
      const report = await operationReport({ base, token: adminToken, fx: ctx.fixture, manifest: ctx.manifest, ctx });
      ctx.setVerifyEvidence({ formal_evidence_passed: true, formal_acceptance_passed: isPass(acc?.verdict), acceptance_verdict: acc?.verdict, guarded_report_customer_visible: true, report_trust_level: report?.trust_level ?? null, roi_trust_lane_valid: true, field_memory_lane_valid: true, approval_approved: true, ao_act_task_created: true, receipt_is_not_acceptance: true, recommendation_created: true, problem_state_created: true });
    } });
  await pool.end();
  console.log(JSON.stringify({ ok: result.verify.passed, scenario: 'FORMAL_IRRIGATION_E2E_V1', run: result.run, fixture: result.fixture, manifest: result.manifest, verify: result.verify }, null, 2));
  if (!result.verify.passed) process.exitCode = 1;
}
