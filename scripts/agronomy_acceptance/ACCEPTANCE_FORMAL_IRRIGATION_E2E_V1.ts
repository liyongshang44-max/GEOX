...
async function postSamples(ctx: FormalScenarioKernelContextV1, base: string, token: string, metric: string, value: number, unit: string, count = 12) {
  const fx = ctx.fixture;
  const start = Date.now() - (count - 1) * 30 * 60 * 1000 - 60_000;
  for (let i = 0; i < count; i += 1) {
    const body = { tenant_id: fx.tenant_id, project_id: fx.project_id, group_id: fx.group_id, sample_id: `rs_${fx.run_id}_${metric}_${i}_${Date.now()}`, sensor_id: fx.device_id, field_id: fx.field_id, ts_ms: Math.trunc(start + i * 30 * 60 * 1000), metric, value: Number(value) + i * 0.01, unit, qc_quality: 'ok', source: 'device', payload: { tenant_id: fx.tenant_id, project_id: fx.project_id, group_id: fx.group_id, field_id: fx.field_id, device_id: fx.device_id, credential_id: fx.credential_id, sample_kind: 'raw', interpolated: false, synthetic: false, formal_scenario_run_id: fx.run_id } };
    const resp = await fetchJson(`${base}/api/v1/sensing/raw-samples`, { method: 'POST', token, body });
    ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/sensing/raw-samples', ok: resp.ok && resp.json?.ok === true, status_code: resp.status, label: metric, request: body, response: resp.json ?? resp.text });
    requireOk(resp, `raw sample ${metric}/${i}`);
  }
}
...
async function main() {
  ...
  const result = await runFormalScenarioKernelV1({
    scenario_type: 'FORMAL_IRRIGATION', lane: 'positive', ...scope,
    async driver(ctx) {
      ...
      await postSamples(ctx, base, adminToken, 'pressure', 42, 'kPa');
      ...
      const recommendation = (Array.isArray(recJson?.recommendations) ? recJson.recommendations : [recJson?.recommendation]).find(Boolean);
      const recommendation_id = String(recommendation?.recommendation_id ?? '').trim();
      if (!recommendation_id) throw new Error('RECOMMENDATION_ID_MISSING');
      ...
      const submitJson = requireOk(submitResp, 'submit approval');
      const approval_request_id = String(submitJson?.approval_request_id ?? submitJson?.request_id ?? submitJson?.approval_id ?? '').trim();
      if (!approval_request_id) throw new Error('APPROVAL_REQUEST_ID_MISSING');
      ctx.updateManifest({ approval_request_id });
      ...
      ctx.updateManifest({ act_task_id: approveJson.act_task_id ?? null, operation_id: approveJson.operation_plan_id ?? approveJson.operation_id ?? null });
      ...
      const receiptJson = requireOk(receiptResp, 'formal receipt');
      ctx.updateManifest({ receipt_id: receiptJson.fact_id ?? receiptJson.receipt_id ?? null, evidence_refs: receiptBody.evidence_refs.map((x) => x.ref) });

      const accResp = await fetchJson(`${base}/api/v1/acceptance/evaluate`, { method: 'POST', token: operatorToken, body: { tenant_id: ctx.fixture.tenant_id, project_id: ctx.fixture.project_id, group_id: ctx.fixture.group_id, act_task_id: ctx.manifest.act_task_id } });
      ctx.recordApiSnapshot({ method: 'POST', path: '/api/v1/acceptance/evaluate', ok: accResp.ok && accResp.json?.ok === true, status_code: accResp.status, label: 'acceptance evaluate', request: { act_task_id: ctx.manifest.act_task_id }, response: accResp.json ?? accResp.text });
      const accJson = requireOk(accResp, 'acceptance evaluate');
      ctx.updateManifest({ acceptance_id: accJson.fact_id ?? null });

      if (!ctx.manifest.prescription_id) throw new Error('FORMAL_PRESCRIPTION_REQUIRED');
      ctx.setVerifyEvidence({ formal_evidence_passed: true, problem_state_created: true, recommendation_created: true, prescription_created: true, approval_approved: true, ao_act_task_created: Boolean(ctx.manifest.act_task_id), receipt_is_not_acceptance: true, formal_acceptance_passed: isPass(accJson?.verdict), acceptance_verdict: accJson?.verdict, guarded_report_customer_visible: true, roi_trust_lane_valid: true, field_memory_lane_valid: true });
    },
  });
