...
checks.normal_task_audit_exists=Array.isArray(normalAudit.json?.items)&&normalAudit.json.items.some((i)=>i.action==='action.task_created'&&i.target_id===normalId&&i.actor_id==='tok_admin_actor'&&i.token_id==='tok_admin');
...
checks.variable_task_audit_exists=Array.isArray(varAudit.json?.items)&&varAudit.json.items.some((i)=>i.action==='action.variable_task_created'&&i.target_id===varTask.json?.act_task_id&&i.actor_id==='tok_admin_actor'&&i.token_id==='tok_admin');
...
checks.receipt_submitted_audit_exists=Array.isArray(receiptAudit.json?.items)&&receiptAudit.json.items.some((i)=>i.action==='action.receipt_submitted'&&i.actor_id==='tok_admin_actor');
...
checks.acceptance_evaluated_audit_exists=Array.isArray(accAudit.json?.items)&&accAudit.json.items.some((i)=>i.action==='acceptance.evaluated'&&i.actor_id==='tok_admin_actor');
...
checks.skill_binding_switched_audit_exists=Array.isArray(skillAudit.json?.items)&&skillAudit.json.items.some((i)=>i.action==='skill.binding_switched'&&i.actor_id==='tok_admin_actor');
...
checks.deny_audit_exists=Array.isArray(denyAudit.json?.items)&&denyAudit.json.items.some((i)=>i.result==='DENY'&&i.actor_id==='tok_client_actor'&&i.token_id==='tok_client'&&i.error_code);
