...
const payloadObj = JSON.parse(payloadRaw);                         // Parse payload JSON.
payloadObj.ts_ms = Date.now();                                     // Stamp ts_ms at publish-time.
const payload = JSON.stringify(payloadObj);                        // Serialize payload JSON.
...
function AwaitCountAtLeastOne([string]$baseUrl, [string]$tok, [string]$device, [string]$metric, [int64]$startMs, [int64]$endMs, [int]$timeoutMs) { # Poll until count>=1.
  ...
  } while (($count -lt 1) -and ([int64][DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds() -lt $deadline)) # Loop until >=1 or timeout.

  if ($count -lt 1) { Fail("expected count>=1, got $count device=$device metric=$metric") } # Fail if not reached.
...
if (-not $env:DATABASE_URL) { Fail("MISSING env:DATABASE_URL in current session") } # Require DB URL for ingest side.

$metric = "soil_moisture"                                            # Metric under test.

# Generate a unique device id per run to avoid historical data accumulation.
...
# Register device (idempotent on unique id).
...
# Issue credential so telemetry-ingest will accept messages.
...
# Start telemetry-ingest in --once mode (same as A2), via pnpm, and let it subscribe before publish.
...
Start-Sleep -Milliseconds 900                                         # Give ingest time to connect+subscribe.

try {
  $payload = @{ metric=$metric; value=21.3; ts_ms=0; credential=$secret } | ConvertTo-Json -Compress -Depth 10 # Build payload JSON.
  $now = [int64][DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()     # Current time (ms).
  $startMs = $now - 8000                                              # Start of query window.
  $endMs   = $now + 12000                                             # End of query window.

  PublishTelemetryOnce -mqttUrl $MqttUrl -tenant $TenantId -deviceId $deviceId -payloadJson $payload -timeoutMs 8000 # Publish one message.
  AwaitCountAtLeastOne -baseUrl $BaseUrl -tok $tok -device $deviceId -metric $metric -startMs $startMs -endMs $endMs -timeoutMs 20000 # Wait for ingestion.
} finally {
