...
function changedFiles() {
  const files = new Set();
  const remoteDiff = runGit(['diff', '--name-only', 'origin/main...HEAD']);
  const localDiff = remoteDiff || runGit(['diff', '--name-only', 'main...HEAD']);
  if (localDiff) localDiff.split(/\r?\n/).map((line) => line.trim()).filter(Boolean).forEach((file) => files.add(file));
  statusFiles().forEach((file) => files.add(file));
  return [...files].sort();
}

function assert(name, passed, details = {}) {
  const record = { name, passed: passed === true, details };
...
function isForbidden(file) {
  return forbiddenExact.has(file) || forbiddenPrefixes.some((prefix) => file.startsWith(prefix));
}

function countBy(records, surface) {
  return records.filter((record) => record.surface === surface).length;
}

function parseExceptionRows(markdown) {
  return markdown
    .split(/\r?\n/)
    .filter((line) => /^\| PFA2-EX-\d+ \|/.test(line))
    .map((line) => line.split('|').slice(1, -1).map((value) => value.trim()))
    .map(([id, token, routeSurface, reason, owner, expiryPhase]) => ({ id, token, routeSurface, reason, owner, expiryPhase }));
}
...
function copyPairs(text) {
  ...
  while ((match = pattern.exec(text))) {
    pairs.push({ zh: match[2].trim(), en: match[4].trim() });
  }
  return pairs;
}
...
function nakedGovernedCopyViolations(files) {
  const violations = [];
  const propPattern = new RegExp(`(?:${governedProps.join('|')})\\s*=\\s*["']([^"']+)["']`);
  const neutralPattern = /^(?:GEOX|API|URL|JSON|SHA-256|AO-ACT|ID|Trace ID|\/[^\s]+|[A-Z0-9_.:-]+)$/;
  for (const file of files.filter((item) => item.endsWith('.tsx'))) {
    for (const line of addedDiffLines(file)) {
      ...
      if (neutralPattern.test(value)) continue;
      violations.push({ file, value, line: line.trim() });
    }
  }
  return violations;
}
...
try {
  ...
  assert('matrix_surface_counts', countBy(records, 'customer') === 9 && countBy(records, 'operator') === 13 && countBy(records, 'admin') === 7 && countBy(records, 'supporting') === 1, {
    customer: countBy(records, 'customer'),
    operator: countBy(records, 'operator'),
    admin: countBy(records, 'admin'),
    supporting: countBy(records, 'supporting'),
  });
  ...
  assert('matrix_all_routes_have_owner_and_sources', records.every((record) => typeof record.copyOwner === 'string' && record.copyOwner.length > 0 && Array.isArray(record.copySources) && record.copySources.length > 0));
  assert('matrix_all_routes_have_locale_markers', records.every((record) => Array.isArray(record.expectedZhMarkers) && record.expectedZhMarkers.length > 0 && Array.isArray(record.expectedEnMarkers) && record.expectedEnMarkers.length > 0));
  assert('matrix_forbidden_markers_present', records.every((record) => Array.isArray(record.zhForbiddenMarkers) && record.zhForbiddenMarkers.length > 0 && Array.isArray(record.enForbiddenMarkers) && record.enForbiddenMarkers.length > 0));
  ...
  assert('matrix_matches_pfe13_actual_routes', JSON.stringify(matrixRoutes) === JSON.stringify(inventoryRoutes), { matrixRoutes, inventoryRoutes });
  ...
  assert('exceptions_are_not_blanket', exceptions.every((item) => !/(allow all|allow technical terms|skip this route|forever)/i.test(`${item.reason} ${item.routeSurface} ${item.expiryPhase}`)));
  assert('matrix_exception_ids_resolve', records.every((record) => record.exceptionIds.every((id) => exceptionIds.has(id))));

  assert('owned_issues_assigned', ['PFA0-I18N-001', 'PFA0-CUS-001', 'PFA0-ADM-002'].every((token) => contract.includes(token) && issueClosure.includes(token)));
  ...
  const packageText = read('apps/web/package.json');
  ...
  assert('locale_toggle_uses_provider', toggleSource.includes('useLocale') && toggleSource.includes('setLocale'));
  assert('locale_toggle_does_not_navigate', !/(useNavigate|NavLink|<Link|window\.location|history\.pushState|history\.replaceState)/.test(toggleSource));
  assert('locale_toggle_accessibility_contract', toggleSource.includes('role="group"') && toggleSource.includes('aria-pressed') && toggleSource.includes('aria-label') && toggleSource.includes('data-locale-option'));
  assert('login_uses_locale_boundary', loginSource.includes('useLocale') && loginSource.includes('LocaleToggle') && loginSource.includes('LOGIN_COPY') && loginSource.includes('LOGIN_ERROR_COPY') && loginSource.includes('AUTH_REASON_COPY'));
  assert('login_does_not_render_raw_reason', !loginSource.includes('{reason}') && !loginSource.includes('params.get("reason") || ""}</'));

  const copyFiles = [repoPath('apps/web/src/lib/productSurfaceLabels.ts'), ...listFiles(repoPath('apps/web/src/lib/productCopy'), (file) => /\.(ts|tsx)$/.test(file))];
  ...
  assert('governed_copy_pairs_present', pairs.length >= 40, { pairs: pairs.length });
  assert('governed_copy_pairs_nonempty', pairs.every((pair) => pair.zh.length > 0 && pair.en.length > 0));
  const identicalPairs = pairs.filter((pair) => pair.zh === pair.en && !['GEOX', 'API', 'URL', 'JSON', 'SHA-256', 'AO-ACT', 'ID'].includes(pair.zh));
  assert('no_unregistered_identical_copy_pairs', identicalPairs.length === 0, { identicalPairs });

  assert('shared_state_catalog_complete', ['available', 'unavailable', 'blocked', 'readOnly', 'notConnected', 'notOnline', 'notStarted', 'disabled', 'degraded', 'stale', 'unknown', 'sourceMissing', 'evidenceUnavailable'].every((token) => read('apps/web/src/lib/productCopy/localeContract.ts').includes(`${token}:`)));
  assert('no_third_party_i18n_dependency', !/(i18next|react-intl|formatjs)/i.test(packageText));
  ...
  const jsxScannerFiles = changed.filter((file) => file.startsWith('apps/web/') && file.endsWith('.tsx'));
  const nakedCopy = nakedGovernedCopyViolations(jsxScannerFiles);
  assert('scoped_added_jsx_governed_copy_scanner', nakedCopy.length === 0, { violations: nakedCopy });
