...
ok(adapter.includes('function rawSignalNodes') && adapter.includes('function observationNodes') && adapter.includes('function stateEstimateNodes'), 'adapter provides first three Water Stress Loop node mappers');
ok(adapter.includes('RAW_SIGNAL') && adapter.includes('OBSERVATION') && adapter.includes('WATER_STRESS_STATE'), 'adapter preserves first three Water Stress Loop step codes');
