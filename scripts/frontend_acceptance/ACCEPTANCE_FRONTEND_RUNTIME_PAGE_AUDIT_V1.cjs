...
function mdList(items) {
  ...
  return items.map((item) => `- ${safeText(item, 350)}`).join('\n');
}
...
async function auditRouteUnsafe(browser, route) {
  ...
  await context.addInitScript(({ token }) => {
    ...
    const meta = {
      role: 'admin',
      actor_id: 'frontend-runtime-audit',
      token_id: 'frontend-runtime-audit',
      scopes: ['operator.read', 'operator.write', 'customer.read', 'security.admin', 'ao_act.task.write', 'ao_act.receipt.write'],
    };
  ...
  page.on('console', (msg) => {
    ...
    if (msg.type() === 'warning') {
      ...
      if (DUPLICATE_KEY_RE.test(text)) addFailure(result, `React duplicate key warning: ${text}`);
    }
  });
  ...
  page.on('requestfailed', (req) => {
    ...
    if (/favicon\.ico$/i.test(url)) {
      result.warnings.push(`favicon request failed: ${url}`);
      return;
    }
  ...
  page.on('response', (res) => {
    ...
    if (url.includes('/api/') && (status === 404 || status >= 500)) {
      ...
      result.warnings.push(`non-api ${status}: ${url}`);
    }
    if (DEVTOOLS_DISABLED && route === '/dev/flight-table' && url.includes('/api/v1/dev/flight-table/')) {
      addFailure(result, `devtools disabled but flight table dev API was requested: ${url}`);
    }
  });
  ...
  try {
    ...
    if (OPERATOR_ROUTES.has(route) && !EXPLICIT_OPERATOR_STATE.test(text)) {
    ...
    await page.screenshot({ path: path.join(ROOT, screenshotRel), fullPage: true }).catch((error) => {
      result.warnings.push(`screenshot failed: ${safeText(error.message || error)}`);
    });
...
function writeReport(results) {
  ...
  const lines = [];
  lines.push('# Frontend Runtime Page Audit Report');
  lines.push('');
  ...
  for (const result of results) {
    lines.push(`| \`${result.route}\` | ${result.pass ? 'PASS' : 'FAIL'} | \`${result.screenshotPath}\` | ${safeText(result.diagnosis.join('; '), 180).replace(/\|/g, '/')} |`);
  }
  lines.push('');
  for (const result of results) {
    ...
    lines.push(`- visible text sample: ${result.visibleTextSample ? `\`${result.visibleTextSample.replace(/`/g, "'")}\`` : '_empty_'}`);
    lines.push('- console errors:');
    lines.push(mdList(result.consoleErrors));
    lines.push('- console warnings:');
    lines.push(mdList(result.consoleWarnings));
    lines.push('- network 4xx/5xx:');
    lines.push(mdList(result.networkErrors));
    lines.push('- warnings:');
    lines.push(mdList(result.warnings));
    lines.push('- diagnosis:');
    lines.push(mdList(result.diagnosis));
    lines.push('');
  }
...
async function runAudit() {
  ...
  try {
    ...
    if (webProcess) {
      ...
      setTimeout(() => webProcess.kill('SIGKILL'), 3000).unref();
    }
  }
