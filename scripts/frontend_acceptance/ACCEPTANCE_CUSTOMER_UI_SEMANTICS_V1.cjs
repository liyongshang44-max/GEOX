...
const FORBIDDEN = [
  ...
  { token: join('IRRIG', 'ATION'), pattern: word(join('IRRIG', 'ATION')) },
...
const ALLOW_CONTEXT = [
  ...
  /sourceText/,
  /raw\s*enum/i,
  /IRRIG\w*.*(test|sanitize|unsafe|safe|filter|replace|map|label|normalize|includes|operationActionLabel|normalizeOperationNarrative)/i,
  /(test|sanitize|unsafe|safe|filter|replace|map|label|normalize|includes|operationActionLabel|normalizeOperationNarrative).*IRRIG\w*/i,
