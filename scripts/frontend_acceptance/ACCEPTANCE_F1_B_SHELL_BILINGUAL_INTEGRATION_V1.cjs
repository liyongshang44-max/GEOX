...
function countDirectTextCalls(text) {
  return (text.match(/\btext\s*\(/g) || []).length;
}
...
try {
  [files.customerLayout, files.operatorLayout, files.adminLayout, files.localeToggle, files.productSurfaceLabels, files.doc, files.acceptance]
  ...
  const scanForMojibake = [files.customerLayout, files.operatorLayout, files.adminLayout, files.productSurfaceLabels, files.doc]
    .map((file) => ({ file, hits: violations(read(file), mojibakePatterns) }))
  ...
  const result = {
    ok: true,
    acceptance: 'ACCEPTANCE_F1_B_SHELL_BILINGUAL_INTEGRATION_V1',
    phase: 'F1-B Shell / Navigation Bilingual Integration',
    shells: {
      customer: 'locale-toggle-and-bilingual-shell-copy',
      operator: 'locale-toggle-and-bilingual-shell-copy',
      admin: 'locale-toggle-and-bilingual-shell-copy',
    },
