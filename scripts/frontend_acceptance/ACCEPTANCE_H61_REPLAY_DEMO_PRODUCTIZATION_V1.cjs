...
function containsAll(content, tokens) { const n = norm(content); return tokens.every((token) => n.includes(norm(token))); }
function lacksAll(content, tokens) { const n = norm(content); return tokens.every((token) => !n.includes(norm(token))); }
function assert(name, condition, details = {}) { const passed = condition === true; assertions.push({ name, passed, details }); if (!passed) { const error = new Error('ASSERTION_FAILED:' + name); error.details = details; throw error; } console.log('[h61-replay-demo-productization] ok:', name); }
...
function main() {
  ...
  const routes = operatorRoutesBlock(app);
  const api = read(FILES.api);
  const page = read(FILES.page);
  const replayFiles = [FILES.viewModel, FILES.replayPage, FILES.hero, FILES.boundary, FILES.narrative, FILES.snapshot, FILES.gateway, FILES.standards, FILES.device, FILES.ingestion, FILES.traceability].map(read).join('\n');
  const css = read(FILES.css);
  const doc = read(FILES.doc);
  assert('route_topology_preserved', containsAll(routes, ['path="twin/gateway-demo" element={<OperatorGatewayDemoViewerPage />}']) && lacksAll(app, ['/operator/replay-demo', '/operator/runtime-demo', '/operator/live-demo']), { file: FILES.app });
  assert('static_get_only_preserved', containsAll(api, ['fetchP51GatewayViewerSnapshot', 'P51_GATEWAY_VIEWER_SNAPSHOT_URL', '/demo-runtime/p51-gateway-viewer-snapshot.json', 'method: "GET"']), { file: FILES.api });
  assert('operator_page_is_thin_wrapper', containsAll(page, ['ReplayDemoPage', 'return <ReplayDemoPage />']) && lacksAll(page, ['style={{', 'React.CSSProperties']), { file: FILES.page });
  assert('replay_product_copy_present', containsAll(replayFiles, ['Replay-backed Gateway Demo', 'Static checked-in snapshot', 'Gateway Path Replay', 'Snapshot Source', 'Standards Mapping', 'Device Evidence Package', 'Ingestion Window', 'Traceability', 'Hashes', 'Evidence refs']), { file: FILES.replayPage });
  assert('first_screen_boundary_present', containsAll(replayFiles, ['No live device connection', 'No production gateway claim', 'No Runtime Health claim', 'No field pilot claim', 'No AO-ACT dispatch', 'No facts write', 'No recommendation', 'No ROI write', 'No Field Memory write']), { file: FILES.boundary });
  assert('no_inline_styles_in_replay_demo', lacksAll(replayFiles + '\n' + page, ['style={{', 'React.CSSProperties']), { file: FILES.replayPage });
  assert('metadata_status_mapping_present', containsAll(read(FILES.viewModel), ['available', 'not_available', 'metadata_only', 'replay_backed_demo', 'p51_gateway_viewer_snapshot']), { file: FILES.viewModel });
  assert('css_entry_exists', containsAll(css, ['.operatorReplayDemo']), { file: FILES.css });
  assert('doc_records_scope_and_boundaries', containsAll(doc, ['H61 productizes replay-backed demo only', '/operator/twin/gateway-demo', 'source remains checked-in P51 gateway viewer snapshot', 'API remains static GET only', 'H61 does not create backend contract', 'H61 does not change route topology', 'H61 does not implement H62 Runtime Health']), { file: FILES.doc });
