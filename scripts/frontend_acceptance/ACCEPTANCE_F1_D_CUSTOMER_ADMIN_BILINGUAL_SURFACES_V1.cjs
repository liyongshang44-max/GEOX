...
try {
  ...
  ok('no_mojibake_in_f1d_files', diff.filter(exists).filter((file) => file !== files.acceptance && file !== 'scripts/frontend_acceptance/ACCEPTANCE_F1_C_OPERATOR_BILINGUAL_SURFACES_V1.cjs').map((file) => ({ file, hits: hits(read(file), mojibake) })).filter((entry) => entry.hits.length > 0).length === 0);
