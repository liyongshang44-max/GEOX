...
const allowedExactFiles = new Set([files.overview, files.gatewayWrapper, files.pilotCss, files.labels, files.doc, files.acceptance]);
...
const blockedExactFiles = new Set(['apps/web/src/app/App.tsx', 'apps/web/src/layouts/CustomerLayout.tsx', 'apps/web/src/layouts/AdminLayout.tsx', 'apps/web/src/layouts/OperatorLayout.tsx', 'package.json', 'pnpm-lock.yaml', 'pnpm-workspace.yaml']);
...
function hasAllInsensitive(text, tokens) {
...
function violationsInsensitive(text, tokens) {
  const lowered = text.toLowerCase();
...
function isAllowed(file) { return allowedExactFiles.has(file) || allowedPrefixes.some((prefix) => file.startsWith(prefix)); }
function isBlocked(file) { return blockedExactFiles.has(file) || blockedPrefixes.some((prefix) => file.startsWith(prefix)); }
...
try {
  ...
  assert('field_runtime_copy_present', hasAllInsensitive(productText, ['Field Runtime', 'Fields', 'Overview', 'Evidence', 'State', 'Forecast', 'Scenario', 'Residual', 'Calibration', 'Health', 'Audit', 'Read-only', 'No runtime mutation', 'No external command', 'No model state mutation', 'No value ledger mutation', 'No long-term field record mutation', '地块运行视图', '地块', '总览', '证据', '状态', '预测', '情景', '残差', '校准', '健康', '审计', '只读', '不修改运行状态', '不下发外部命令', '不修改模型状态', '不写入价值台账', '不写入长期地块记录']));
  assert('replay_demo_copy_present', hasAllInsensitive(productText, ['Replay-backed Gateway Demo', 'Replay-backed Gateway Snapshot', 'Replay-backed demo', 'not a live device connection', 'Production Gateway: Not online', 'Field Pilot: Not started', 'AO-ACT Dispatch: Disabled', 'Snapshot', 'Gateway path', 'Device evidence', 'Standards mapping', 'Traceability', '回放支撑网关演示', '回放支撑网关快照', '回放支撑演示', '不是实时设备连接', '生产网关：未上线', '田间试点：未开始', 'AO-ACT 派发：已禁用', '快照', '网关路径', '设备证据', '标准映射', '可追溯性']));
  assert('pilot_readiness_copy_present', hasAllInsensitive(productText, ['Pilot Readiness', 'readiness gate', 'planning gate', 'safety', 'stop rules', 'human role', 'rollback', 'not field execution', 'Field Pilot: Not started', 'AO-ACT Dispatch: Disabled', '试点准备度', '准备度门禁', '规划门禁', '安全', '停止规则', '人员角色', '回滚', '不是田间执行', '田间试点：未开始', 'AO-ACT 派发：已禁用']));
  assert('runtime_overview_copy_present', hasAllInsensitive(productText, ['Runtime Overview', 'Runtime status', 'Evidence coverage', 'Replay-backed boundary', 'Read-only runtime review', '运行总览', '运行状态', '证据覆盖', '回放支撑边界', '只读运行审查']));
  ...
  const positiveClaimHits = violationsInsensitive(productText, positiveClaimForbidden);
  assert('forbidden_positive_production_claims_absent', positiveClaimHits.length === 0, { positiveClaimHits });
  assert('negative_nonclaims_present', hasAllInsensitive(productText, ['not connected', 'not online', 'not started', 'disabled', '未连接', '未上线', '未开始', '已禁用']));
