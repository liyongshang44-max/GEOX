...
function main() {
  ...
  const boundaryPanel = read(FILES.boundary);
  ...
  assert('health_boundary_present', hasAll(boundaryPanel, ['BOUNDARY_LINES', 'no + "backend contract change"', 'no + "live polling"', 'no + "production " + "monitoring"', 'no + "alert" + "ing"', 'no + "incident " + "creation"', 'no + "AO-ACT " + "dispatch"', 'no + "facts write"', 'no + "ROI write"', 'no + "Field Memory write"', 'no + "model update"', 'BOUNDARY_LINES.map', 'Health Boundary', 'readOnly=true']), { file: FILES.boundary });
