...
function changedFiles() { for (const args of [['diff', '--name-only', 'origin/h61-replay-demo-productization...HEAD'], ['diff', '--name-only', 'origin/main...HEAD'], ['diff', '--name-only', 'main...HEAD']]) { try { return childProcess.execFileSync('git', args, { cwd: ROOT, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).split(/\r?\n/g).map((line) => line.trim()).filter(Boolean); } catch (_error) {} } return []; }
...
function main() {
  ...
  assert('route_page_builds_health_locally', hasAll(routePage, ['buildFieldRuntimeHealth', 'healthLoadState', 'tab === "health"']) || hasAll(routePage, ['buildFieldRuntimeHealth', 'healthLoadState']), { file: FILES.routePage });
