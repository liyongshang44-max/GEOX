...
try{
 ...
 if(arm.armed!==true){
  ...
  assert.equal(pref.database_name,TARGET,"KBS_POINTER_SCHEMA_PREFLIGHT_DATABASE_MISMATCH");
  assert.equal(pref.table_count,41,"KBS_POINTER_SCHEMA_PREFLIGHT_TABLE_COUNT_REQUIRED");
  if(authority.status==="READY_NOT_AUTHORIZED"){
    assert.equal(pref.status,"PASS_REMEDIATION_REQUIRED","KBS_POINTER_SCHEMA_UNARMED_PREFLIGHT_REQUIRED");
    assert.equal(pref.pointer_schema_absent,true,"KBS_POINTER_SCHEMA_UNARMED_ABSENCE_REQUIRED");
  }else{
    assert.equal(authority.status,"MATERIALIZED_SETTLED_UNARMED","KBS_POINTER_SCHEMA_UNARMED_AUTHORITY_STATUS_REQUIRED");
    assert.equal(pref.status,"PASS_ALREADY_MATERIALIZED","KBS_POINTER_SCHEMA_SETTLED_PREFLIGHT_REQUIRED");
    assert.equal(pref.pointer_schema_complete,true,"KBS_POINTER_SCHEMA_SETTLED_COMPLETENESS_REQUIRED");
  }
