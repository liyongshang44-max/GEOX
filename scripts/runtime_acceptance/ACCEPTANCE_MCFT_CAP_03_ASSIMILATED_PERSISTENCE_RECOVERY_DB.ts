...
if (process.env.MCFT_CAP_03_S3B_DESTRUCTIVE_ACCEPTANCE !== "1") {
  throw new Error("SET_MCFT_CAP_03_S3B_DESTRUCTIVE_ACCEPTANCE_1");
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    assert.ok(readback);
    assert.equal(readback.record_set_contract_id, "MCFT_CAP_03_ASSIMILATED_CONTINUATION_V1");
    assert.equal(
