...
function leaseClaimV1(owner:string,heartbeat:number):EvidenceProducerLeaseClaimV1{
  const second=String(Math.min(heartbeat,59)).padStart(2,"0");
  return {lease_contract_id:MCFT_CAP09_EVIDENCE_PRODUCER_LEASE_CONTRACT_ID_V1,scope:{...SCOPE},lease_owner:owner,fencing_token:3n,acquired_at:"2026-08-27T02:00:00.000Z",expires_at:"2026-08-27T02:10:00.000Z",heartbeat_at:`2026-08-27T02:00:${second}.000Z`,database_now:`2026-08-27T02:00:${second}.000Z`};
}
function leaseV1(trace:string[]=[]):EvidenceProducerLeasePortV1{
  let current:EvidenceProducerLeaseClaimV1|null=null,heartbeat=1;
  return {
    async acquireLease(input){trace.push("acquire");current=leaseClaimV1(input.lease_owner,heartbeat++);return current;},
    async renewLease(input){trace.push("renew");current={...input.claim,heartbeat_at:leaseClaimV1(input.claim.lease_owner,heartbeat).heartbeat_at,database_now:leaseClaimV1(input.claim.lease_owner,heartbeat++).database_now};return current;},
    async releaseLease(){trace.push("release");current=null;},
  };
}
...
async function waitForCancelledLeaseRenewalV1(input:{signal:AbortSignal}):Promise<"DUE"|"CANCELLED">{
  if(input.signal.aborted)return "CANCELLED";
  return await new Promise<"CANCELLED">((resolve)=>input.signal.addEventListener("abort",()=>resolve("CANCELLED"),{once:true}));
}
async function main():Promise<void>{
  ...
  const host=new EvidenceRuntimeHostV1({
    ...
    wait:{waitForLeaseRenewal:waitForCancelledLeaseRenewalV1,async waitAfterAttempt(input){waits.push(input.reason);}},
  ...
  assert.equal(run.stop_reason,"PLANNER_EXHAUSTED");assert.equal(run.cycle_attempt_count,3);assert.equal(run.successful_cycle_count,1);assert.equal(run.standby_cycle_count,1);assert.equal(run.retryable_failure_count,1);assert.equal(run.last_attempt_result?.lease_claim,null,"PHASE3_HOST_RELEASED_OWNER_CLAIM_MUST_NOT_ESCAPE_TO_PROCESS_CLEANUP");
  ...
  const heterogeneousHost=new EvidenceRuntimeHostV1({
    ...
    wait:{waitForLeaseRenewal:waitForCancelledLeaseRenewalV1,async waitAfterAttempt(){if(executed.length===3)stop=true;}},
  ...
  const fatalHost=new EvidenceRuntimeHostV1({lease:leaseV1(),planner:{async nextAttemptPlan(){return planV1("fatal","CANONICAL_WORK_ITEM_CYCLE",async()=>{throw new Error("FATAL:identity-corruption");});}},wait:{waitForLeaseRenewal:waitForCancelledLeaseRenewalV1,async waitAfterAttempt(){throw new Error("FATAL_HOST_WAIT_FORBIDDEN");}},health:{async recordHealth(event){fatalHealth.push(structuredClone(event));}},stop:{stopRequested:()=>false},failure_classifier:{classify:()=> "FATAL"}});
  ...
  let plannerCalls=0;
  const stoppedHost=new EvidenceRuntimeHostV1({lease:leaseV1(),planner:{async nextAttemptPlan(){plannerCalls+=1;throw new Error("STOPPED_PLANNER_FORBIDDEN");}},wait:{waitForLeaseRenewal:waitForCancelledLeaseRenewalV1,async waitAfterAttempt(){throw new Error("STOPPED_WAIT_FORBIDDEN");}},health:{async recordHealth(){}},stop:{stopRequested:()=>true},failure_classifier:{classify:()=> "FATAL"}});
  ...
  let notDueWaitCount=0;
  const notDueLeaseTrace:string[]=[];
  const notDueHost=new EvidenceRuntimeHostV1({lease:leaseV1(notDueLeaseTrace),planner:{async nextAttemptPlan(){return {status:"NOT_DUE" as const};}},wait:{waitForLeaseRenewal:waitForCancelledLeaseRenewalV1,async waitAfterAttempt(input){assert.equal(input.reason,"PLANNER_NOT_DUE");notDueWaitCount+=1;}},health:{async recordHealth(){}},stop:{stopRequested:()=>notDueWaitCount>=3},failure_classifier:{classify:()=> "FATAL"}});
  ...
  const longBaseLease=leaseV1(longLeaseTrace);
  let longRenewalCount=0;
  const longLease:EvidenceProducerLeasePortV1={
    acquireLease:(input)=>longBaseLease.acquireLease(input),
    async renewLease(input){
      const claim=await longBaseLease.renewLease(input);
      longRenewalCount+=1;
      if(longRenewalCount===3)longAttemptRelease();
      return claim;
    },
    releaseLease:(input)=>longBaseLease.releaseLease(input),
  };
  let longStopped=false,longWaitCount=0,longVirtualElapsedMs=0;
  const longHealth:EvidenceRuntimeHostHealthEventV1[]=[];
  const longHost=new EvidenceRuntimeHostV1({
    lease:longLease,
    planner:{async nextAttemptPlan(input){if(input.cycle_attempt>=1)return null;return planV1("long-inflight","CANONICAL_WORK_ITEM_CYCLE",async()=>{await longAttemptGate;return resultV1({id:"long-inflight",kind:"CANONICAL_WORK_ITEM_CYCLE",status:"COMPLETED",owner:"host-long"});});}},
    wait:{
      async waitAfterAttempt(input){assert.equal(input.reason,"SUCCESS_CADENCE");longStopped=true;},
      async waitForLeaseRenewal(input){
        if(input.signal.aborted)return "CANCELLED";
        longWaitCount+=1;
        longVirtualElapsedMs+=400;
        if(longWaitCount<=3)return "DUE";
        return await new Promise<"CANCELLED">((resolve)=>input.signal.addEventListener("abort",()=>resolve("CANCELLED"),{once:true}));
      },
    },
    health:{async recordHealth(event){longHealth.push(structuredClone(event));}},
    stop:{stopRequested:()=>longStopped},
    failure_classifier:{classify:()=> "FATAL"},
  });
  const longRun=await longHost.run({scope:SCOPE,lease_owner:"host-long",lease_duration_seconds:1});
  assert.equal(longRun.stop_reason,"STOP_REQUESTED");
  assert.equal(longRenewalCount,3,"PHASE3_HOST_INFLIGHT_OWNER_LEASE_RENEWAL_COUNT_REQUIRED");
  assert.equal(longVirtualElapsedMs>1000,true,"PHASE3_HOST_INFLIGHT_ATTEMPT_MUST_CROSS_ORIGINAL_LEASE_WINDOW");
  assert.deepEqual(longLeaseTrace,["acquire","renew","renew","renew","release"],"PHASE3_HOST_INFLIGHT_OWNER_LEASE_KEEPALIVE_TRACE_REQUIRED");
  assert.equal(longHealth.filter((event)=>event.detail==="ATTEMPT_IN_PROGRESS"&&event.status==="HEALTHY").length,3,"PHASE3_HOST_INFLIGHT_STRUCTURED_HEALTH_KEEPALIVE_REQUIRED");
  ...
  assert.equal(source.includes("waitForLeaseRenewal"),true,"PHASE3_HOST_INFLIGHT_LEASE_KEEPALIVE_WAIT_REQUIRED");
