...
function resultV1(input:{id:string;kind:EvidenceRuntimeHostAttemptKindV1;status:"COMPLETED"|"LEASE_HELD_BY_OTHER_OWNER";owner?:string;}):EvidenceRuntimeHostAttemptResultV1{
  return {attempt_contract_id:MCFT_CAP09_EVIDENCE_RUNTIME_HOST_ATTEMPT_CONTRACT_ID_V1,attempt_id:input.id,attempt_kind:input.kind,status:input.status,lease_claim:input.status==="COMPLETED"?{lease_contract_id:MCFT_CAP09_EVIDENCE_PRODUCER_LEASE_CONTRACT_ID_V1,scope:{...SCOPE},lease_owner:input.owner??"host-A",fencing_token:3n,acquired_at:"2026-08-27T02:00:00.000Z",expires_at:"2026-08-27T02:10:00.000Z",heartbeat_at:"2026-08-27T02:00:01.000Z",database_now:"2026-08-27T02:00:01.000Z"}:null,canonical_record_count:input.status==="COMPLETED"?2:0,visible_ingress_count:input.status==="COMPLETED"?2:0,evidence_supply_cursor_advance_count:input.status==="COMPLETED"?2:0,twin_state_mutation:false,runtime_tick_cursor_mutation:false};
}
