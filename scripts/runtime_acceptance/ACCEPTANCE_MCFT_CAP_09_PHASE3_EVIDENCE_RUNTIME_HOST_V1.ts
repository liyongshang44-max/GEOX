...
async function main():Promise<void>{
  ...
  const fatalHost=new EvidenceRuntimeHostV1({lease:leaseV1(),planner:{async nextAttemptPlan(){return planV1("fatal","CANONICAL_WORK_ITEM_CYCLE",async()=>{throw new Error("FATAL:identity-corruption");});}},wait:{async waitAfterAttempt(){throw new Error("FATAL_HOST_WAIT_FORBIDDEN");}},health:{async recordHealth(event){fatalHealth.push(structuredClone(event));}},stop:{stopRequested:()=>false},failure_classifier:{classify:()=> "FATAL"}});
  ...
  let plannerCalls=0;
  const stoppedHost=new EvidenceRuntimeHostV1({lease:leaseV1(),planner:{async nextAttemptPlan(){plannerCalls+=1;throw new Error("STOPPED_PLANNER_FORBIDDEN");}},wait:{async waitAfterAttempt(){throw new Error("STOPPED_WAIT_FORBIDDEN");}},health:{async recordHealth(){}},stop:{stopRequested:()=>true},failure_classifier:{classify:()=> "FATAL"}});
