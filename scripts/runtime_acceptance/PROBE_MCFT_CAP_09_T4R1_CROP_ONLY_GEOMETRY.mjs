...
const CONFIG_PATH = path.join(ROOT, 'docs/digital_twin/mcft/cap_09/GEOX-MCFT-CAP-09-T4R1-CROP-ONLY-GEOMETRY-AUTHORITY-V1.json');
const OUT = path.join(ROOT, 'acceptance-output/MCFT_CAP_09_T4R1_CROP_ONLY_GEOMETRY_RESULT.json');
...
function parseTable(text, requiredColumns) {
  ...
  assert(headers && delimiter, 'T4R1_GEOMETRY_REQUIRED_CSV_HEADER_NOT_FOUND');
  const rows = [];
  ...
  assert(rows.length > 0, 'T4R1_GEOMETRY_CSV_ROWS_REQUIRED');
  return rows;
}
function parseProviderQuadrilateral(raw, requiredSrid) {
  const value = String(raw || '').trim();
  const match = value.match(/^SRID=(\d+);POLYGON\(\(([^()]+)\)\)$/i);
  assert(match, 'T4R1_MAIN_PROVIDER_GEOMETRY_SIMPLE_POLYGON_REQUIRED');
  assert(Number(match[1]) === requiredSrid, 'T4R1_MAIN_PROVIDER_GEOMETRY_SRID_MISMATCH');
  const ring = match[2].split(',').map((token) => token.trim().split(/\s+/).map(Number));
  assert(ring.length >= 5, 'T4R1_MAIN_PROVIDER_GEOMETRY_RING_TOO_SHORT');
  assert(ring.every((point) => point.length === 2 && point.every(Number.isFinite)), 'T4R1_MAIN_PROVIDER_GEOMETRY_NONFINITE');
  assert(ring[0][0] === ring.at(-1)[0] && ring[0][1] === ring.at(-1)[1], 'T4R1_MAIN_PROVIDER_GEOMETRY_UNCLOSED');
  const points = ring.slice(0, -1);
  assert(points.length === CONFIG.provider_geometry_selector.required_distinct_vertex_count, 'T4R1_MAIN_PROVIDER_GEOMETRY_NOT_QUADRILATERAL');
  return points;
}
...
function isConvex(points) {
  ...
  for (let index = 0; index < local.length; index += 1) {
    ...
    assert(Math.abs(cross) > 1e-6, 'T4R1_MAIN_PROVIDER_GEOMETRY_COLLINEAR_VERTEX');
...
function deriveSubzone(points) {
  ...
  assert(validation.length === 0, `T4R1_DERIVED_SUBZONE_GEOMETRY_INVALID:${validation.join(',')}`);
...
async function digestPage(page, source) {
  ...
  assert(requested.protocol === 'https:' && requested.hostname === source.allowed_host, 'T4R1_GEOMETRY_UNAPPROVED_SOURCE_HOST');
  const response = await page.goto(requested.href, { waitUntil: 'domcontentloaded', timeout: 75_000 });
  assert(response?.ok(), `T4R1_GEOMETRY_HTTP_${response?.status() ?? 'NO_RESPONSE'}`);
  const finalUrl = new URL(response.url());
  assert(finalUrl.protocol === 'https:' && finalUrl.hostname === source.allowed_host, 'T4R1_GEOMETRY_REDIRECT_HOST_FORBIDDEN');
  const bytes = await response.body();
  const text = normalize(await page.locator('body').innerText());
  for (const marker of source.required_markers || []) {
    assert(text.toLowerCase().includes(normalize(marker).toLowerCase()), `T4R1_GEOMETRY_PROVIDER_MARKER_MISSING:${marker}`);
  }
  return {
...
async function main() {
  assert(/^[0-9a-f]{40}$/.test(SUBJECT_SHA), 'T4R1_GEOMETRY_EXACT_SUBJECT_REQUIRED');
  assert(CONFIG.frontier === 'T4R1_CROP_ONLY_GEOMETRY_AUTHORITY', 'T4R1_GEOMETRY_FRONTIER_MISMATCH');
  assert(CONFIG.prairie_strip_guard.strip_geometry_wkt_required === false, 'T4R1_STRIP_WKT_REQUIREMENT_MUST_BE_FALSE');
  assert(CONFIG.prairie_strip_guard.strip_geometry_may_not_be_invented === true, 'T4R1_STRIP_WKT_INVENTION_MUST_BE_FORBIDDEN');
  assert(CONFIG.resolution_policy.formal_rebind_authorized_by_this_probe === false, 'T4R1_GEOMETRY_PREMATURE_REBIND_FORBIDDEN');
  assert(CONFIG.resolution_policy.current_lifecycle_authorized_by_this_probe === false, 'T4R1_GEOMETRY_PREMATURE_LIFECYCLE_FORBIDDEN');

  const browser = await chromium.launch({ headless: true });
  try {
    const context = await browser.newContext({ userAgent: 'GEOX-MCFT-CAP09-T4R1-Crop-Geometry/1.0' });
    ...
    assert(await downloadAnchor.count() === 1, 'T4R1_GEOMETRY_DOWNLOAD_ANCHOR_REQUIRED');
    const href = await downloadAnchor.getAttribute('href');
    assert(href, 'T4R1_GEOMETRY_DOWNLOAD_HREF_REQUIRED');
    ...
    assert(resolved.protocol === 'https:'
      ...
      && !resolved.search, 'T4R1_GEOMETRY_DOWNLOAD_LOCATOR_MISMATCH');

    const csvResponse = await context.request.get(resolved.href, {
      timeout: 120_000,
      headers: { accept: 'text/csv,text/plain;q=0.9,*/*;q=0.5' },
    });
    assert(csvResponse.ok(), `T4R1_GEOMETRY_CSV_HTTP_${csvResponse.status()}`);
    const csvBytes = await csvResponse.body();
    assert(csvBytes.byteLength <= 20 * 1024 * 1024, 'T4R1_GEOMETRY_CSV_TOO_LARGE');
    const csvText = csvBytes.toString('utf8');
    assert(!/^\s*<!doctype html|^\s*<html/i.test(csvText), 'T4R1_GEOMETRY_CSV_HTML_FORBIDDEN');
    ...
    assert(matches.length === selector.expected_match_count, `T4R1_GEOMETRY_MAIN_MATCH_COUNT_${matches.length}`);
    ...
    assert(!selector.require_convex_quadrilateral || isConvex(points), 'T4R1_MAIN_PROVIDER_GEOMETRY_NOT_CONVEX');
    ...
    `T4R1_SHORT_DIMENSION_OUT_OF_RANGE:${oriented.short_dimension_m}`);
    ...
    assert(outerBoundaryMarginM >= policy.minimum_outer_boundary_margin_m,
      `T4R1_OUTER_BOUNDARY_MARGIN_INSUFFICIENT:${outerBoundaryMarginM}`);
    assert(centerStripClearanceM >= policy.minimum_center_strip_clearance_m,
      `T4R1_CENTER_STRIP_CLEARANCE_INSUFFICIENT:${centerStripClearanceM}`);
    assert(endBoundaryMarginM >= policy.minimum_end_boundary_margin_m,
      `T4R1_END_BOUNDARY_MARGIN_INSUFFICIENT:${endBoundaryMarginM}`);
    ...
    assert(derived.area_m2 >= policy.minimum_derived_area_m2 && derived.area_m2 <= policy.maximum_derived_area_m2,
      `T4R1_DERIVED_AREA_OUT_OF_RANGE:${derived.area_m2}`);

    const result = {
      schema_version: 'geox_mcft_cap09_t4r1_crop_only_geometry_result_v1',
      status: 'PASS',
      ...
      next_frontier: 'T4R1_CURRENT_LIFECYCLE_AUTHORITY',
