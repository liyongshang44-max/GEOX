// Fresh-PostgreSQL formal candidate proof for MCFT-CAP-08.S2 formal Forcing/Evidence/State/Forecast providers.
// Candidate implementation proof only; S2 effectiveness, S3, late correction, Residual, and production authority remain false.
...
if (process.env.MCFT_CAP08_S2_CANDIDATE_DESTRUCTIVE_ACCEPTANCE !== "1") throw new Error("SET_MCFT_CAP08_S2_CANDIDATE_DESTRUCTIVE_ACCEPTANCE_1");
const OUT = path.join(path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../.."), "acceptance-output/MCFT_CAP_08_S2_FORCING_STATE_FORECAST_DB_RESULT.json");
