...
async function insertCanonicalSetupV1(object: Cap05ActionFeedbackEnvelopeV1): Promise<string> {
  const id = `fact_${object.object_id}`;
  await admin.query(
    `INSERT INTO facts (fact_id,occurred_at,source,record_json)
     VALUES ($1,$2::timestamptz,'system',$3::jsonb)`,
    [id, object.logical_time, JSON.stringify({ type: object.object_type, payload: object })],
  );
  return id;
}
...
async function completionAndAuthorityCasesV1(): Promise<void> {
  ...
  const activationFactId = "fact_mcft08_s3_negative_model_activation_n22";
  ...
  await admin.query(
    `INSERT INTO facts (fact_id,occurred_at,source,record_json)
     VALUES ($1,$2::timestamptz,'system',$3::jsonb)`,
    [activationFactId, cap08TickLogicalTimeV1(23), JSON.stringify(activationRecord)],
  );
