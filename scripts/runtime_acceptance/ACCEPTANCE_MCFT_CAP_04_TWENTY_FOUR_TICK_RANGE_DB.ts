...
if (process.env.MCFT_CAP_04_RANGE_DESTRUCTIVE_ACCEPTANCE !== "1") {
  throw new Error("SET_MCFT_CAP_04_RANGE_DESTRUCTIVE_ACCEPTANCE_1");
}
const databaseUrl = process.env.DATABASE_URL;
if (!databaseUrl) throw new Error("DATABASE_URL_REQUIRED");
const databaseName = new URL(databaseUrl).pathname.replace(/^\//, "").toLowerCase();
if (!/(mcft|cap04|s7|range|acceptance|test)/.test(databaseName)) throw new Error("ISOLATED_ACCEPTANCE_DATABASE_REQUIRED");
...
function evidenceV1(input: {
  ...
  return {
    ...input.scope,
    dataset_id: "mcft_cap04_s7_postgresql_range_fixture_v1",
    source_record_id: input.source_record_id,
...
function evidenceForTickV1(scope: TwinScopeKeyV1, logicalTime: string, index: number): CanonicalReplayEvidenceRecordV1[] {
  ...
  const rainfall = evidenceV1({
    ...
    source_record_id: `rain_cap04_s7_db_${suffix}`,
    binding_id: "rainfall_c8_hourly_v1",
    origin_source_id: "weather_replay_cap04_s7_db",
  ...
  const et0 = evidenceV1({
    ...
    source_record_id: `et0_cap04_s7_db_${suffix}`,
    binding_id: "historical_et0_c8_hourly_v1",
    origin_source_id: "et0_replay_cap04_s7_db",
    role_time: { interval_start: addHoursV1(logicalTime, -1), interval_end: logicalTime, ingested_at: ingestedAt },
    canonical_payload: {
  ...
  const soil = evidenceV1({
    ...
    source_record_id: `soil_cap04_s7_db_${suffix}`,
    binding_id: "soil_obs_c8_20cm_v1",
    origin_source_id: "soil_sensor_cap04_s7_db",
  ...
  return [
    ...
    buildCap04FutureForcingSnapshotV1({
      ...
      source_record_id: `weather_cap04_s7_db_${suffix}`,
      seed: 200 + index,
    ...
    buildCap04FutureForcingSnapshotV1({
      ...
      issued_at: issuedAt,
      available_to_runtime_at: availableAt,
      source_record_id: `future_et0_cap04_s7_db_${suffix}`,
      seed: 200 + index,
      scope_override: scope,
    }),
  ];
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO twin_active_lineage_index_v1
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,'CONTROLLED_REPLAY','mcft_cap04_s7_db_fixture',NULL)`,
    ...
    await pool.query(
      `INSERT INTO twin_runtime_health_latest_index_v1
       ...
       VALUES ($1,$2,$3,$4,$5,$6,'seed_cap04_s7_health','READY',$7::timestamptz,'sha256:seed_cap04_s7_health','seed_cap04_s7_health_fact')`,
    ...
    const cropStageContext: ContinuationCropStageConfigurationContextV1 = {
      schema_version: "v1",
      dataset_id: "mcft_cap04_s7_postgresql_crop_stage_v1",
      context_class: "CONFIGURATION_DERIVED_CONTEXT",
      evidence_record: false,
      configuration_matrix_ref: "mcft_configuration_matrix_cap04_s7_db",
      configuration_matrix_hash: firstPayload.configuration_matrix_hash,
      crop_water_use_binding_ref: "crop_water_use_cap04_s7_db",
      crop_water_use_configuration_source_id: "crop_water_use_source_cap04_s7_db",
    ...
    const result = await rangeService.runContiguousRange({
      ...
      lease_owner: "mcft-cap04-s7-db-acceptance",
    ...
    const replay = await rangeService.runContiguousRange({
      scope,
      to_logical_time: TARGET_LOGICAL_TIME,
      created_at: CREATED_AT,
      runtime_config_refs_by_logical_time: configRefs,
    ...
    assert.equal(evidenceLoads, 24);
    ok("completed PostgreSQL target replay returns ALREADY_COMPLETE with zero new facts or Evidence loads");

    console.log(`MCFT-CAP-04 24-tick range DB: ${pass} PASS, 0 FAIL`);
  } finally {
    await pool.end();
  }
}
