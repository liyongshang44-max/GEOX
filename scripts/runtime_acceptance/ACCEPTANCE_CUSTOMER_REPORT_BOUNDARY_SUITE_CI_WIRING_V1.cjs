...
// Purpose: verify customer report boundary suite is documented as a CI acceptance entrypoint.
...
function main() {
  ...
  assert(
    pkg.scripts &&
      pkg.scripts[SUITE_SCRIPT_NAME] === SUITE_SCRIPT_COMMAND,
    "customer report boundary suite package script is missing or drifted",
    {
      scriptName: SUITE_SCRIPT_NAME,
      expected: SUITE_SCRIPT_COMMAND,
      actual: pkg.scripts && pkg.scripts[SUITE_SCRIPT_NAME],
    }
  );
