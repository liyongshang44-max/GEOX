...
// Purpose: execute the permanent MCFT-CAP-06 S3 D persistence/recovery acceptance in an isolated PostgreSQL database.
// Boundary: acceptance orchestration only; no production database, S5/S6 compute authority, active Config, State, checkpoint, approval, route, Web, scheduler, Model Activation or CAP-07 authority.
...
const ACCEPTANCE_PATH = 'scripts/runtime_acceptance/ACCEPTANCE_MCFT_CAP_06_S3_D_PERSISTENCE_DB.ts';
const RESULT_PATH = path.join(ROOT, 'acceptance-output', 'MCFT_CAP_06_S3_D_PERSISTENCE_RESULT.json');
const ISOLATED_DATABASE_NAME = 'mcft_cap06_s3_d_persistence_ci';
...
function persistResult(stdout) {
  const line = String(stdout || '').split(/\r?\n/).find((item) => item.startsWith('S3_RESULT_JSON:'));
  if (!line) throw new Error('MCFT_CAP_06_S3_RESULT_JSON_REQUIRED');
  const result = JSON.parse(line.slice('S3_RESULT_JSON:'.length));
...
async function main() {
  ...
  try {
    ...
    requireSuccessful(result, 'MCFT_CAP_06_S3_D_PERSISTENCE');
