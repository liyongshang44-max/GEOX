...
// Each case uses administrator-owned fault setup, invokes only the bounded pair inspector, proves zero additional mutation, and restores the exact database snapshot.
...
async function runtimeSnapshotV1(): Promise<string> {
  const value = {
    ...
    visibility: await tableRowsV1("twin_fact_visibility_index_v1", "fact_id"),
...
async function main(): Promise<void> {
  try {
    ...
    const suiteBaseline = await runtimeSnapshotV1();
...

    {
      ...
      hidden.source = "mcft_cap08_s3_completion_evidence_corrupted_v1";
      await updateFactV1(hidden);
      await expectPairRejectV1("S3-CR03", /CAP08_S3_COMPLETION_ABSENCE_WITNESS_REQUIRED/, () => updateFactV1(original));
    }
    ...
    assert.equal(await runtimeSnapshotV1(), suiteBaseline, "S3_COMPLETED_NEGATIVE_SUITE_RESTORE_DELTA_NONZERO");
