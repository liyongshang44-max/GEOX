...
// Purpose: prove one controlled irrigation Receipt becomes canonical H Action Feedback and the existing executed-irrigation adapter applies coverage exactly once while preserving time and status semantics.
...
function cloneReceipt(
  ...
  record.source_record_hash = `sha256:acceptance-receipt-${suffix}`;
...
async function main(): Promise<void> {
  ...
  ok("S5 active Approved Plan predecessor is established without supersession");

  await seedReplayEvidence(receipt);
  ...
  await seedReplayEvidence(pending);
  ...
  await seedReplayEvidence(crossHour);
  ...
  const volume = cloneReceipt("volume_unit", receipt, (record) => {
    (record.canonical_payload as any).unit = "m3";
    (record.source_payload as any).unit = "m3";
  });
  await seedReplayEvidence(volume);
  await assert.rejects(feedbackService.commitActionFeedback({
    scope,
  ...
  await seedReplayEvidence(wrongCoveredAmount);
  await assert.rejects(feedbackService.commitActionFeedback({
    scope,
