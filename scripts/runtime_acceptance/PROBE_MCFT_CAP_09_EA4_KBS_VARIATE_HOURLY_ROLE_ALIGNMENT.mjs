...
function retain(kind,id,body){
  ...
  return{kind,sha256:digest,bytes:body.length,private_retention_verified:true,raw_body_uploaded:false};
}
...
try{
  req(/^[0-9a-f]{40}$/.test(SUBJECT_SHA),'EA4RA_EXACT_SUBJECT_SHA_REQUIRED');browser=await chromium.launch({headless:true});const context=await browser.newContext({locale:'en-US',timezoneId:'UTC'});const receipts=[],endpointSeries=new Map();
  for(const id of CFG.unmapped_endpoint_ids){const url=CFG.endpoint_template.replace('{id}',String(id)),r=await context.request.get(url,{timeout:60_000,headers:{accept:'application/json,*/*;q=0.5'}});req(r.ok(),`EA4RA_ENDPOINT_HTTP:${id}:${r.status()}`);const body=await r.body(),receipt=retain('KBS_VARIATE',String(id),body);receipts.push(receipt);endpointSeries.set(id,parseEndpoint(body,id))}
  ...
  for(const id of CFG.unmapped_endpoint_ids){const ps=endpointSeries.get(id),overlap=ps.filter(p=>p.time>=referenceStart-2*HOUR&&p.time<=referenceEnd+2*HOUR),metrics=[];for(const[role,spec]of Object.entries(CFG.reference_roles))for(const method of spec.aggregation_methods)for(const offset of CFG.allowed_hour_alignment_offsets)metrics.push(candidateMetrics(overlap,refMap,role,method,offset));metrics.sort((a,b)=>b.r_squared-a.r_squared||((a.affine_nrmse??999)-(b.affine_nrmse??999))||b.pair_count-a.pair_count);const top=metrics[0],runner=metrics[1],unique=qualifies(top)&&(!runner||top.r_squared-runner.r_squared>=CFG.identity_thresholds.minimum_runner_up_r_squared_gap);endpoint_results.push({endpoint_id:id,response_sha256:receipts.find(x=>x.kind==='KBS_VARIATE'&&x===receipts.find(y=>y.sha256===y.sha256))?.sha256??null,point_count:ps.length,overlap_point_count:overlap.length,overlap_start_utc:overlap.length?new Date(overlap[0].time).toISOString():null,overlap_end_utc:overlap.length?new Date(overlap.at(-1).time).toISOString():null,top_candidates:metrics.slice(0,8),resolved_role_id:unique?top.role:null,resolved_method_id:unique?top.method:null,resolved_offset_hours:unique?top.offset_hours:null,unique_identity_threshold_passed:unique});}
  // repair response hashes deterministically from retained receipt ordering
  endpoint_results.forEach((x,i)=>{x.response_sha256=receipts[i].sha256;x.response_bytes=receipts[i].bytes});
  ...
  Object.assign(result,{status:'PASS',retrieved_at:new Date().toISOString(),raw_retention:{receipt_count:receipts.length,receipts:receipts.map(x=>({kind:x.kind,sha256:x.sha256,bytes:x.bytes,private_retention_verified:x.private_retention_verified,raw_body_uploaded:false})),all_retained_before_parse:true},raw_hourly_reference:{response_sha256:csvReceipt.sha256,response_bytes:csvReceipt.bytes,reference_start_utc:new Date(referenceStart).toISOString(),reference_end_utc:new Date(referenceEnd).toISOString(),stale_reference_used_only_for_role_identity:true,live_source_claimed:false},endpoint_results,solar_candidate_endpoint_ids:solarIds,rainfall_candidate_endpoint_ids:rainIds,decision:solarIds.length===1?'UNIQUE_KBS_HIGH_FREQUENCY_SOLAR_ROLE_DIAGNOSTIC_MATCH':rainIds.length===1?'UNIQUE_KBS_HIGH_FREQUENCY_RAIN_ROLE_DIAGNOSTIC_MATCH':'NO_UNIQUE_HIGH_FREQUENCY_SOLAR_OR_RAIN_ROLE_MATCH',endpoint_role_authority_created:false,solar_source_authority_created:false,rainfall_source_authority_created:false,timezone_authority_created:false,source_substitution_authorized:false,ea5_authorized:false,database_write_count:0,formal_evidence_write_count:0,raw_numeric_values_emitted:false,raw_payload_uploaded:false});
  write(result);console.log(JSON.stringify({status:'PASS',decision:result.decision,endpoints:endpoint_results.map(x=>({endpoint_id:x.endpoint_id,resolved_role_id:x.resolved_role_id,resolved_method_id:x.resolved_method_id,resolved_offset_hours:x.resolved_offset_hours,top_candidate:x.top_candidates[0]})),solar_candidate_endpoint_ids:solarIds,rainfall_candidate_endpoint_ids:rainIds},null,2));await context.close();
