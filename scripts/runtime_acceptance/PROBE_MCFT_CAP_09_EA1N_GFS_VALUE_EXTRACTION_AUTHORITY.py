...
USER_AGENT = "GEOX-MCFT-CAP-09-EA1N/1.0 (+exact-head qualification)"
...
def candidate_cycles(tick: datetime):
    base = tick.replace(minute=0, second=0, microsecond=0)
    out = []
...
def object_urls(cycle: datetime, lead: int):
    ...
    base = f"{PRODUCTION_ROOT}/gfs.{date}/{cc}/atmos/{stem}"
...
def prove_source_object_prior_available(url: str, tick: datetime):
    ...
    lm = parse_last_modified(headers)
    if lm > tick:
        raise RuntimeError(f"SOURCE_OBJECT_AFTER_TICK:{url}:{iso(lm)}>{iso(tick)}")
    return {
        "last_modified": lm,
    ...
    for cycle in candidate_cycles(tick):
        ...
        try:
            for lead in leads:
                ...
                lm_values.extend([grib["last_modified"], idx["last_modified"]])
            ...
            return {
                ...
                "leads": leads,
                "last_modified_min": min(lm_values),
                "last_modified_max": max(lm_values),
...
def dt_from_grib_keys(date_value, time_value) -> datetime:
    d = str(int(date_value)).zfill(8)
    t = str(int(time_value)).zfill(4)
...
def role_for(short_name: str, name: str, type_of_level: str, level: float):
    ...
    n = name.lower().strip()
    typ = type_of_level.strip()
    lev = float(level)
    if typ == "heightAboveGround" and abs(lev - 2.0) < 1e-9:
        if s in {"2t", "t"} or n == "temperature":
            return "AIR_TEMPERATURE_2M"
        if s in {"2r", "r"} or "relative humidity" in n:
            return "RELATIVE_HUMIDITY_2M"
    if typ == "heightAboveGround" and abs(lev - 10.0) < 1e-9:
        if s in {"10u", "u"} or "u component of wind" in n:
            return "U_WIND_10M"
        if s in {"10v", "v"} or "v component of wind" in n:
            return "V_WIND_10M"
    if typ == "surface":
        if s == "dswrf" or "downward short-wave radiation flux" in n or "downward shortwave radiation flux" in n:
            return "DOWNWARD_SHORTWAVE_SURFACE"
        if s == "prate" or n == "precipitation rate":
            return "PRECIPITATION_RATE_SURFACE"
    return None
            ...
            while True:
                ...
                try:
                    ...
                    if not math.isfinite(value):
                        raise RuntimeError(f"NONFINITE_DECODED_VALUE:{role}:{requested_lead}")
                    records.append({
                        "role": role,
...
def assert_raw_sanity(role: str, value: float, lead: int):
    ...
    if role == "PRECIPITATION_RATE_SURFACE" and not (0.0 <= value <= 0.05):
        raise RuntimeError(f"RAW_PRATE_SANITY_FAIL:F{lead:03d}")
...
def main():
    ...
    try:
        ...
        for lead in selected["leads"]:
            ...
            status, headers, body, final_url = request_bytes(url, method="GET", timeout=90, attempts=3)
            if status != 200:
        ...
        for lead in range(selected["lead_start"], selected["lead_end"] + 1):
            ...
            pr = find_average(records, "PRECIPITATION_RATE_SURFACE", lead)
            for rec in (temp, rh, u, v, ds, pr):
                assert_raw_sanity(rec["role"], rec["value"], lead)

            s = rolling_start(lead)
            length = lead - s
            if length == 1:
                hourly_ds = ds["value"]
                hourly_pr = pr["value"]
                ...
                ds_prev = find_average(prev_records, "DOWNWARD_SHORTWAVE_SURFACE", lead - 1)
                pr_prev = find_average(prev_records, "PRECIPITATION_RATE_SURFACE", lead - 1)
                if ds_prev["start_step"] != s or pr_prev["start_step"] != s:
                    raise RuntimeError(f"CROSS_BLOCK_DIFFERENCE_FORBIDDEN:F{lead:03d}")
                hourly_ds = length * ds["value"] - (length - 1) * ds_prev["value"]
                hourly_pr = length * pr["value"] - (length - 1) * pr_prev["value"]
                diff_dswrf += 1
                diff_prate += 1
        ...
        for name, points in series.items():
            ...
            if [p[0] for p in points] != expected_times:
        ...
        result["normalization_counts"] = {
            ...
            "prate_direct_1h": direct_prate,
            "prate_weighted_difference": diff_prate,
        }
