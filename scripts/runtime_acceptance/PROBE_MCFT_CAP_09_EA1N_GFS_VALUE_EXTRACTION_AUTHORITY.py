...
USER_AGENT = "GEOX-MCFT-CAP-09-EA1N/1.2 (+exact-head qualification)"
        ...
        with open(tmp_path, "rb") as handle:
            while True:
                ...
                try:
                    ...
                    records.append({"role":role,"short_name":short_name,"type_of_level":type_of_level,"level":level,"step_type":step_type,"start_step":start_step,"end_step":end_step,"units":units,"valid_time":valid_dt,"value":value})
                finally:
                    codes_release(gid)
    finally:
...
def find_apcp_block(records, lead: int):
    start = rolling_start(lead)
    return exact_one(apcp_block_candidates(records, lead), f"APCP_BLOCK_ACCUM_NOT_UNIQUE:S{start}:E{lead}")
...
def main():
    ...
    result = {
        ...
        "precipitation_source_candidate":"APCP_SIX_HOUR_BLOCK_CUMULATIVE_DIFFERENCE",
    ...
    try:
        ...
        by_lead, filter_hashes, metadata_for_hash = {}, [], []
        ...
        for lead in selected["leads"]:
            ...
            for rec in records:
                metadata_for_hash.append({k:(iso(rec[k]) if k=="valid_time" else rec[k]) for k in ["role","short_name","type_of_level","level","step_type","start_step","end_step","units","valid_time"]} | {"lead":lead})
            time.sleep(0.15)

        target_leads = list(range(selected["lead_start"], selected["lead_end"]+1))
        block_counts = [len(apcp_block_candidates(by_lead[lead], lead)) for lead in target_leads]
        result["apcp_block_target_count"] = POINT_COUNT
        result["apcp_block_unique_coverage_count"] = sum(c == 1 for c in block_counts)
        result["apcp_block_missing_count"] = sum(c == 0 for c in block_counts)
        result["apcp_block_ambiguous_count"] = sum(c > 1 for c in block_counts)
        if result["apcp_block_unique_coverage_count"] != POINT_COUNT:
            i = next(i for i,c in enumerate(block_counts) if c != 1)
            raise RuntimeError(f"APCP_BLOCK_72_OF_72_REQUIRED:F{target_leads[i]:03d}:COUNT={block_counts[i]}")

        series = {name:[] for name in ["temperature_c","relative_humidity_percent","wind_2m_m_s","solar_mj_m2_h","precipitation_mm"]}
        ds_direct = ds_diff = apcp_direct = apcp_diff = 0
        for lead in target_leads:
            ...
            ds = find_dswrf_average(records,lead); apcp = find_apcp_block(records,lead)
            ...
            if length == 1:
                ...
                ds_prev = find_dswrf_average(prev_records,lead-1); apcp_prev = find_apcp_block(prev_records,lead-1)
        ...
        result.update({
            ...
            "physical_sanity_pass":True,"chronology_pass":True,"single_grid_point_pass":True,"message_semantics_reconciliation_pass":True,"precipitation_source_correction_pass":True,
