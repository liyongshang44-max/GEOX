// Disposable architecture-deviation diagnostic for the outcome-preserving CAP-08 replay-dataset v2 design.
...
const OUT = "acceptance-output/MCFT_CAP_08_S5_ARCHITECTURE_V2_OUTCOME_PRESERVING_DIAGNOSTIC.json";
...
function exactMemberV1(recordSet: Cap04ARecordSetV1, objectType: string): CanonicalObjectEnvelopeV1 {
  ...
  if (matches.length !== 1) throw new Error(`CAP08_S5_V2_OUTCOME_MEMBER_CARDINALITY:${objectType}:${matches.length}`);
...
async function main(): Promise<void> {
  try {
    ...
    if (s4.status !== "COMPLETED"
      ...
      throw new Error("CAP08_S5_V2_OUTCOME_S4_CORRECTION_NOT_EXACT");
    }
    ...
    const architectureDeviationResolved = attempt.selected_parameter_value === "0.034000"
      && attempt.canonical_append_allowed;
    const result = {
      schema_version: "geox_mcft_cap08_s5_architecture_v2_outcome_preserving_diagnostic_v1",
