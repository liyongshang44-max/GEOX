...
const TOKEN = process.env.GEOX_ACCEPTANCE_TOKEN || 'admin_token';
...
async function fetchJson(path, body){ const r = await fetch(`${BASE_URL}${path}`, { method:'POST', headers:{'content-type':'application/json', authorization:`Bearer ${TOKEN}`}, body: JSON.stringify(body) }); const json = await r.json().catch(()=>({})); return { status:r.status, json }; }
async function insertFact(pool, recId, patch={}){ const payload = { version:'v1', ...scope, recommendation_id: recId, source:'ROOT_ZONE_SCENARIO_SELECTION', source_option_id:'IRRIGATE_DAY0', source_submission_id:`${RUN}_submission`, status:'CANDIDATE', human_approval_required:true, no_direct_execution:true, approval_created:false, operation_plan_created:false, task_created:false, dispatch_created:false, roi_created:false, field_memory_created:false, recommendation_kind:'IRRIGATION_CANDIDATE_FROM_SCENARIO', proposed_action:{ action_type:'IRRIGATE', total_irrigation_mm: 12 }, evidence_refs:[`${RUN}:evidence`], created_at:new Date().toISOString(), ...patch };
  const fact = `fact_${crypto.randomUUID()}`; await pool.query('INSERT INTO facts (fact_id, occurred_at, source, record_json) VALUES ($1, NOW(), $2, $3::jsonb)', [fact, RUN, JSON.stringify({ type:'decision_recommendation_v1', payload })]); return fact; }
...
async function submit(recId,key,patch={}){ return fetchJson(`/api/v1/operator/recommendations/${encodeURIComponent(recId)}/request-approval`, { ...scope, operator_id:'operator_demo', submission_reason:`${RUN} request approval`, idempotency_key:key, time_window:{ start_ts:1760000000000, end_ts:1760003600000 }, ...patch }); }
(async()=>{ const pool=new Pool({ connectionString:DATABASE_URL }); try {
  const rec = `${RUN}_rec_success`; await insertFact(pool, rec);
  const res = await submit(rec, `${RUN}_key_success`); assert(res.json.status === 'SUBMITTED_TO_APPROVAL_REQUEST', 'success response submitted', res);
  ...
  const cases = [ ['non_candidate',{status:'APPROVED'},'REJECTED_RECOMMENDATION_NOT_CANDIDATE'], ['scope_mismatch',{field_id:'other_field'},'REJECTED_SCOPE_MISMATCH'], ['approval_created',{approval_created:true},'REJECTED_DOWNSTREAM_ALREADY_CREATED'], ['operation_plan_created',{operation_plan_created:true},'REJECTED_DOWNSTREAM_ALREADY_CREATED'], ['task_created',{task_created:true},'REJECTED_DOWNSTREAM_ALREADY_CREATED'], ['direct_false',{no_direct_execution:false},'REJECTED_DIRECT_EXECUTION_FORBIDDEN'] ];
  for (const [name,patch,expected] of cases){ const id=`${RUN}_rec_${name}`; await insertFact(pool,id,patch); const before=await count(pool,'approval_request_v1'); const out=await submit(id, `${RUN}_key_${name}`); assert(out.json.status===expected, `${name} rejected as ${expected}`, out); assert(await count(pool,'approval_request_v1')===before, `${name} creates no approval_request_v1`); }
