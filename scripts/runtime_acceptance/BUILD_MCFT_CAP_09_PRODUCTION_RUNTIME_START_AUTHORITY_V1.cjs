...
const DEFAULT_ARM = path.join(ROOT, "scripts/runtime_acceptance/MCFT_CAP_09_PRODUCTION_RUNTIME_START_ARM_V1.json");
const PRE_RUNTIME_READY_PROOF_REL = "scripts/runtime_acceptance/MCFT_CAP_09_PRODUCTION_NON_GITHUB_HOST_BINDING_ARM_V1.json";
const PRE_RUNTIME_READY_CANONICAL_MAIN = "f9cdeb4eddb1801a339149a592ee41f9cf120257";
const PRE_RUNTIME_READY_HOST_PROOF_SUBJECT = "d1db5463d1363eb5f9efacc13425b75a7c8b7ee8";
...
const LEGACY_ROLLING_REFRESH_TEST_STEP = "TEST_ROLLING_REFRESH_TIME_SEMANTICS";
...
function exactHour(value, code) {
  const out = exactIso(value, code);
  req(out.endsWith(":00:00.000Z"), code);
  return out;
}
function exactSha(value, code) { req(typeof value === "string" && /^[0-9a-f]{40}$/.test(value), code); return value; }
function text(value, code) { req(typeof value === "string" && value.trim().length > 0, code); return value.trim(); }
...
function validatePreRuntimeStartReadyProof(observedHead) {
  requireAncestor(PRE_RUNTIME_READY_CANONICAL_MAIN, observedHead, "RUNTIME_START_PRE_READY_CANONICAL_MAIN_NOT_ANCESTOR");
  ...
  for (const key of ["database_write","compose_build","compose_create","compose_up","runtime_process_start","production_owner_activation","formal_v5_arm","a0_bootstrap","o00_started","secret_values_recorded"]) {
    req(proof[key] === false, "RUNTIME_START_PRE_READY_LOCAL_PROOF_FORBIDDEN_EFFECT:" + key);
  }
  return { ref: proofRef.ref, sha256: proofRef.sha256, canonical_main_sha: PRE_RUNTIME_READY_CANONICAL_MAIN, host_proof_subject_sha: PRE_RUNTIME_READY_HOST_PROOF_SUBJECT, host_id: CANONICAL_HOST_ID };
}

function validateEffectiveStageAuthorities(currentCropRef, stageArchitectureRef, scope, formalA0, adjudicationTime) {
  const currentCrop = readJson(currentCropRef.resolved);
  req(currentCrop.schema_version === "geox_mcft_cap09_t4r1_current_crop_authority_composition_result_v1" && currentCrop.status === "PASS" && currentCrop.qualification_outcome === "CURRENT_CROP_CONTEXT_AUTHORITY_CANDIDATE_RESOLVED", "RUNTIME_START_CURRENT_CROP_AUTHORITY_SCHEMA_STATUS_REQUIRED");
  req(currentCrop.architecture_effective === true && currentCrop.runtime_consumption_authorized === true, "RUNTIME_START_CURRENT_CROP_AUTHORITY_NOT_EFFECTIVE");
  for (const key of ["tenant_id","project_id","group_id","field_id","season_id","zone_id"]) {
    req(text(currentCrop.scope?.[key], "RUNTIME_START_CURRENT_CROP_SCOPE_REQUIRED:" + key) === scope[key], "RUNTIME_START_CURRENT_CROP_SCOPE_MISMATCH:" + key);
  }
  const life = currentCrop.lifecycle ?? {};
  req(life.domain_state === "ACTIVE" && life.authority_status === "RESOLVED" && life.authority_validity === "VALID" && life.authority_mode === "GOVERNED_PERSISTENT_STATE" && life.active_consumable_candidate === true, "RUNTIME_START_CURRENT_CROP_LIFECYCLE_NOT_CONSUMABLE");
  const lifecycleHorizon = exactIso(life.horizon_end_utc, "RUNTIME_START_CURRENT_CROP_LIFECYCLE_HORIZON_REQUIRED");
  req(Date.parse(formalA0) <= Date.parse(lifecycleHorizon), "RUNTIME_START_CURRENT_CROP_LIFECYCLE_HORIZON_EXPIRED");

  const biological = currentCrop.biological_stage ?? {};
  const biologicalStage = text(biological.resolved_biological_stage, "RUNTIME_START_CURRENT_CROP_BIOLOGICAL_STAGE_REQUIRED");
  if (biological.epistemic_class !== "DIRECT_OBSERVED_PHENOLOGY") req(biological.observed_biological_stage_claimed === false, "RUNTIME_START_CURRENT_CROP_DERIVED_OBSERVED_CLAIM_FORBIDDEN");
  const stageAsOf = exactHour(biological.authority_as_of, "RUNTIME_START_CURRENT_CROP_STAGE_AUTHORITY_AS_OF_REQUIRED");
  const forwardHours = Number(biological.forward_stability_hours);
  req(Number.isInteger(forwardHours) && forwardHours > 0 && forwardHours <= 48, "RUNTIME_START_CURRENT_CROP_FORWARD_STABILITY_INVALID");
  const computedStageValidUntil = new Date(Date.parse(stageAsOf) + forwardHours * 3_600_000).toISOString();
  const stageValidUntil = biological.authority_valid_until === undefined ? computedStageValidUntil : exactIso(biological.authority_valid_until, "RUNTIME_START_CURRENT_CROP_STAGE_AUTHORITY_VALID_UNTIL_REQUIRED");
  req(stageValidUntil === computedStageValidUntil, "RUNTIME_START_CURRENT_CROP_STAGE_AUTHORITY_VALID_UNTIL_MISMATCH");
  req(Date.parse(adjudicationTime) + MAX_ADJUDICATION_CLOCK_SKEW_MS >= Date.parse(stageAsOf), "RUNTIME_START_CURRENT_CROP_STAGE_AUTHORITY_FUTURE_AT_ADJUDICATION");
  req(Date.parse(adjudicationTime) <= Date.parse(stageValidUntil), "RUNTIME_START_CURRENT_CROP_STAGE_AUTHORITY_STALE_AT_ADJUDICATION");
  req(Date.parse(formalA0) >= Date.parse(stageAsOf), "RUNTIME_START_CURRENT_CROP_FUTURE_STAGE_EVIDENCE_FORBIDDEN");
  req(Date.parse(formalA0) <= Date.parse(stageValidUntil), "RUNTIME_START_CURRENT_CROP_STAGE_AUTHORITY_STALE_AT_A0");

  const waterUseStage = text(currentCrop.crop_water_use_stage, "RUNTIME_START_CURRENT_CROP_WATER_USE_STAGE_REQUIRED");
  const kc = currentCrop.crop_model_parameter ?? {};
  req(kc.parameter === "Kc" && kc.stage_code === waterUseStage && typeof kc.value === "number" && Number.isFinite(kc.value) && kc.production_effective === false, "RUNTIME_START_CURRENT_CROP_KC_AUTHORITY_INVALID");

  const stageArchitecture = readJson(stageArchitectureRef.resolved);
  req(stageArchitecture.schema_version === "geox_dt02_biological_stage_authority_effectiveness_v1" && stageArchitecture.amendment_id === "DT02-AMENDMENT-03" && stageArchitecture.status === "EFFECTIVE" && stageArchitecture.effective === true, "RUNTIME_START_BIOLOGICAL_STAGE_ARCHITECTURE_NOT_EFFECTIVE");
  const protectedMain = exactSha(stageArchitecture.protected_main_sha, "RUNTIME_START_BIOLOGICAL_STAGE_PROTECTED_MAIN_SHA_REQUIRED");
  const certificateIssuedAt = exactIso(stageArchitecture.issued_at, "RUNTIME_START_BIOLOGICAL_STAGE_CERTIFICATE_ISSUED_AT_REQUIRED");
  req(Date.parse(certificateIssuedAt) <= Date.parse(formalA0), "RUNTIME_START_BIOLOGICAL_STAGE_CERTIFICATE_FROM_FUTURE");
  for (const key of ["runtime_start_authorized","production_owner_activation_authorized","formal_v5_authorized","a0_authorized","o00_o23_authorized"]) req(stageArchitecture[key] === false, "RUNTIME_START_BIOLOGICAL_STAGE_CERTIFICATE_AUTHORITY_CEILING_DRIFT:" + key);

  const graduation = currentCrop.graduation ?? {};
  req(EFFECTIVE_CURRENT_CROP_GRADUATION_STATUSES.has(graduation.status) && graduation.amendment_id === "DT02-AMENDMENT-03", "RUNTIME_START_CURRENT_CROP_GRADUATION_REQUIRED");
  req(graduation.architecture_effectiveness_sha256 === stageArchitectureRef.sha256, "RUNTIME_START_CURRENT_CROP_ARCHITECTURE_CERTIFICATE_DIGEST_MISMATCH");
  req(graduation.protected_main_sha === protectedMain, "RUNTIME_START_CURRENT_CROP_PROTECTED_MAIN_MISMATCH");
  if (graduation.status === "EFFECTIVE_FOR_RUNTIME_CONSUMPTION") {
    req(graduation.graduated_at === stageArchitecture.issued_at, "RUNTIME_START_CURRENT_CROP_GRADUATION_TIME_MISMATCH");
  } else {
    const architectureEffectiveSince = exactIso(graduation.architecture_effective_since, "RUNTIME_START_CURRENT_CROP_ARCHITECTURE_EFFECTIVE_SINCE_REQUIRED");
    req(architectureEffectiveSince === certificateIssuedAt, "RUNTIME_START_CURRENT_CROP_ARCHITECTURE_EFFECTIVE_SINCE_MISMATCH");
    const graduatedAt = exactIso(graduation.graduated_at, "RUNTIME_START_CURRENT_CROP_REFRESH_GRADUATION_TIME_REQUIRED");
    const refreshQualificationTime = exactIso(currentCrop.refresh?.qualification_time, "RUNTIME_START_CURRENT_CROP_REFRESH_QUALIFICATION_TIME_REQUIRED");
    req(graduatedAt === refreshQualificationTime, "RUNTIME_START_CURRENT_CROP_REFRESH_GRADUATION_TIME_MISMATCH");
    req(Date.parse(graduatedAt) >= Date.parse(stageAsOf), "RUNTIME_START_CURRENT_CROP_REFRESH_GRADUATION_PRECEDES_STAGE_AUTHORITY");
    req(Date.parse(graduatedAt) <= Date.parse(stageValidUntil), "RUNTIME_START_CURRENT_CROP_REFRESH_GRADUATION_AFTER_STAGE_VALIDITY");
  }
  return { biological_stage: biologicalStage, crop_water_use_stage: waterUseStage, kc: kc.value, stage_authority_as_of: stageAsOf, stage_authority_valid_until: stageValidUntil, lifecycle_horizon_end_utc: lifecycleHorizon, protected_main_sha: protectedMain };
}
...
function runtimeModeBindingFromArm(arm) {
  const activationStep = text(arm.activation_step, "RUNTIME_START_ACTIVATION_STEP_REQUIRED");
  if (arm.runtime_mode === undefined) {
    req(
      activationStep === OWNER_CUTOVER_ACTIVATION_STEP || activationStep === LEGACY_ROLLING_REFRESH_TEST_STEP,
      "RUNTIME_START_RUNTIME_MODE_REQUIRED",
    );
    return { runtimeMode: OWNER_CUTOVER, legacyBuilderCompatibility: true };
  }
  const runtimeMode = text(arm.runtime_mode, "RUNTIME_START_RUNTIME_MODE_REQUIRED");
  req(runtimeMode === NON_OWNER_STANDBY || runtimeMode === OWNER_CUTOVER, "RUNTIME_START_RUNTIME_MODE_INVALID");
  if (runtimeMode === NON_OWNER_STANDBY) req(activationStep === NON_OWNER_ACTIVATION_STEP, "RUNTIME_START_NON_OWNER_ACTIVATION_STEP_REQUIRED");
  if (runtimeMode === OWNER_CUTOVER) req(activationStep === OWNER_CUTOVER_ACTIVATION_STEP, "RUNTIME_START_OWNER_CUTOVER_ACTIVATION_STEP_REQUIRED");
  return { runtimeMode, legacyBuilderCompatibility: false };
}
...
function build(arm, observedHead, adjudicationTime) {
  ...
  const { runtimeMode, legacyBuilderCompatibility } = runtimeModeBindingFromArm(arm);
  const preReady = validatePreRuntimeStartReadyProof(observedHead);
  const live = repoRef(arm.live_activation_authority_ref, arm.live_activation_authority_sha256, "RUNTIME_START_LIVE_ACTIVATION_AUTHORITY");
  const a0 = repoRef(arm.formal_a0_authority_ref, arm.formal_a0_authority_sha256, "RUNTIME_START_FORMAL_A0_AUTHORITY");
  const currentCrop = repoRef(arm.current_crop_authority_ref, arm.current_crop_authority_sha256, "RUNTIME_START_CURRENT_CROP_AUTHORITY");
  const stageArchitecture = repoRef(arm.biological_stage_architecture_effectiveness_ref, arm.biological_stage_architecture_effectiveness_sha256, "RUNTIME_START_BIOLOGICAL_STAGE_ARCHITECTURE_EFFECTIVENESS");

  const scope = {};
  for (const key of ["tenant_id","project_id","group_id","field_id","season_id","zone_id"]) scope[key] = text(arm.scope?.[key], "RUNTIME_START_SCOPE_REQUIRED:" + key);
  const activationFence = exactIso(arm.activation_fence_time, "RUNTIME_START_ACTIVATION_FENCE_REQUIRED");
  const adjudicationMs = Date.parse(adjudicationTime);
  if (!legacyBuilderCompatibility) {
    req(Math.abs(Date.parse(activationFence) - adjudicationMs) <= MAX_ADJUDICATION_CLOCK_SKEW_MS, "RUNTIME_START_ACTIVATION_FENCE_ADJUDICATION_CLOCK_SKEW_EXCEEDED");
  }
  const formalA0 = exactHour(arm.formal_a0_logical_time, "RUNTIME_START_FORMAL_A0_REQUIRED");
  ...
  const stageValidation = validateEffectiveStageAuthorities(
    ...
    scope,
    formalA0,
    legacyBuilderCompatibility ? activationFence : adjudicationTime,
  );
  ...
  for (const key of ["production_owner_activation_authorized","formal_v5_arm_authorized","a0_authorized","o00_authorized"]) req(arm[key] === false, "RUNTIME_START_LATER_AUTHORITY_MUST_REMAIN_FALSE:" + key);
...
try {
  ...
  const result = build(readJson(args.arm), gitHead(), new Date().toISOString());
  ...
  process.stdout.write(JSON.stringify({
    ...
    pre_runtime_start_ready_canonical_protected_main_sha: result.pre_runtime_start_ready_canonical_protected_main_sha,
