...
const EVIDENCE_TYPES = ["soil_moisture_observation_v1", "observed_rainfall_v1", "historical_et0_estimate_v1", "future_weather_assumption_v1", "future_et0_assumption_v1"] as const;

type A18BAuthority = {
  schema_version: string;
  selected_epoch_id: string;
  config_authority_created_at: string;
  prewindow_a0: { logical_time: string; crop_stage_code: "MID"; crop_stage_context_hash: string; runtime_config_ref: string; runtime_config_hash: string; parent_runtime_config_ref: null; parent_runtime_config_hash: null };
...
type A18DAuthority = {
  ...
  execution_gate: { a18d_effective_if_present_on_protected_main: boolean; execution_not_before_actual_wall_clock: string; execution_must_complete_before: string };
};
type RuntimeFootprint = {
  total_facts: number; evidence_facts: number; twin_facts: number; runtime_configs: number; active_lineage: number; state_latest: number; forecast_result_latest: number; forecast_success_latest: number; checkpoint_latest: number; scheduler_cursors: number; scheduler_slots: number; terminal_ticks: number; downstream_action_facts: number;
};
type ExpectedConfigPin = { slot_id: string; logical_time: string; ref: string; hash: string };

function requiredEnv(name: string): string { const value = process.env[name]?.trim(); if (!value) throw new Error(`A18D_ENV_REQUIRED:${name}`); return value; }
function canonicalIso(value: string, code: string): string { const parsed = Date.parse(value); if (!Number.isFinite(parsed) || new Date(parsed).toISOString() !== value) throw new Error(code); return value; }
...
function buildAndVerifyBundle(): { persistence_bundle: ExternalFormalBootstrapAuthorityBundleV1; expected_pins: ExpectedConfigPin[] } {
  ...
  const hourlyCropPins = frozen.hourly_runtime_config_pins.map(([slotId, logicalTime], index) => {
    ...
    return { slot_id: slotId, logical_time: logicalTime, crop_stage_code: "MID" as const, crop_stage_context_hash: deriveExternalFormalCropStageContextHashV2({ crop_stage_code: "MID", derivation_authority_time: logicalTime }) };
  });
  const bundle = buildExternalFormalPrewindowAuthorityBundleV2({ bootstrap_logical_time: A0, created_at: frozen.config_authority_created_at, bootstrap_crop_stage_code: "MID", hourly_crop_pins: hourlyCropPins });
  ...
  return {
    ...
    persistence_bundle: {
      scope: bundle.scope, bootstrap_logical_time: bundle.bootstrap_logical_time, window_start_utc: O00, crop_stage_code: "MID", crop_stage_context_hash: bundle.bootstrap_crop_stage_context_hash,
      geometry_semantic_hash: MCFT_CAP09_EXTERNAL_FORMAL_GEOMETRY_SEMANTIC_HASH_V1, reality_binding_snapshot: bundle.reality_binding_snapshot, bootstrap_runtime_config: bundle.bootstrap_runtime_config,
      runtime_configs: bundle.runtime_configs, hydraulic: MCFT_CAP09_EXTERNAL_FORMAL_MODEL_HYDRAULIC_PRIOR_V1,
      model_prior_ref: MCFT_CAP09_EXTERNAL_FORMAL_CONFIGURATION_MATRIX_REF_V1, model_prior_hash: MCFT_CAP09_EXTERNAL_FORMAL_CONFIGURATION_MATRIX_HASH_V1,
    },
  };
}
...
async function assertFormalDatabase(pool: Pool, databaseUrl: string): Promise<void> {
  ...
  if (String((await pool.query("SELECT current_database() AS database_name")).rows[0]?.database_name ?? "") !== FORMAL_DATABASE) throw new Error("A18D_DATABASE_SESSION_IDENTITY_REQUIRED");
}
  ...
  async loadCandidateRecords(input: { scope: TwinScopeKeyV1; logical_time: string }): Promise<readonly CanonicalReplayEvidenceRecordV1[]> {
    ...
    const result = await this.pool.query(
      `SELECT record_json->'payload' AS payload FROM public.facts
       WHERE record_json->'payload'->>'tenant_id'=$1 AND record_json->'payload'->>'project_id'=$2 AND record_json->'payload'->>'group_id'=$3
         AND record_json->'payload'->>'field_id'=$4 AND record_json->'payload'->>'season_id'=$5 AND record_json->'payload'->>'zone_id'=$6
         AND record_json->>'type'=ANY($7::text[]) ORDER BY occurred_at ASC, fact_id ASC`,
      [s.tenant_id, s.project_id, s.group_id, s.field_id, s.season_id, s.zone_id, [...EVIDENCE_TYPES]],
    );
...
function assertPreBootstrapFootprint(footprint: RuntimeFootprint, configCount: number): void {
  ...
  const allZero = tuple.every((v) => v === 0), allOne = tuple.every((v) => v === 1);
  if (!allZero && !allOne) throw new Error(`A18D_PARTIAL_BOOTSTRAP_PROJECTION_FORBIDDEN:${tuple.join(",")}`);
...
async function assertCompletedBootstrap(pool: Pool, expectedPins: ExpectedConfigPin[]): Promise<RuntimeFootprint> {
  if ((await assertExistingConfigSubsetExact(pool, expectedPins)).size !== 25) throw new Error("A18D_EXACT_25_CONFIGS_REQUIRED");
...
async function executeBootstrap(pool: Pool, expectedPins: ExpectedConfigPin[], persistenceBundle: ExternalFormalBootstrapAuthorityBundleV1): Promise<{ execution_mode: string; runtime_config_write_count: number; a0_member_write_count: number }> {
  ...
  await new ExternalFormalA0EvidenceWindowServiceV1(evidenceSource).prepare({ scope: MCFT_CAP09_EXTERNAL_FORMAL_SCOPE_V1, logical_time: A0 });
  ...
  if (before.active_lineage === 1) {
    ...
    const repo = new PostgresRuntimeRepositoryV1(pool); let writes = 0;
  ...
  const service = new ExternalFormalBootstrapPersistenceServiceV1({ runtime_config_repository: repo, bootstrap_persistence: repo, authority_snapshot_repository: new PostgresNextTickRepositoryV1(pool), evidence_source: evidenceSource });
  ...
  return { execution_mode: persisted.status === "INSERTED" ? "FRESH_A0_BOOTSTRAP_INSERTED" : "IDEMPOTENT_BOOTSTRAP_REVERIFIED", runtime_config_write_count: persisted.runtime_config_write_count, a0_member_write_count: persisted.a0_member_write_count };
}
function selftest(): void {
  loadAuthority(); const { persistence_bundle, expected_pins } = buildAndVerifyBundle();
  ...
  console.log(JSON.stringify({ status: "PASS", epoch_id: MCFT_CAP09_A18_SELECTED_EPOCH_ID_V2, prewindow_a0: A0, o00: O00, o23: O23, exact_config_pin_count: 25, a0_config_ref: expected_pins[0].ref, a0_config_hash: expected_pins[0].hash, o00_config_ref: expected_pins[1].ref, o00_config_hash: expected_pins[1].hash, execution_not_before: A0, execution_must_complete_before: O00, provider_request_count: 0, database_write_count: 0, scheduler_write_count: 0, formal_o00_started: false }));
}
...
async function main(): Promise<void> {
  ...
  const authority = loadAuthority(); const { persistence_bundle, expected_pins } = buildAndVerifyBundle();
  const databaseUrl = requiredEnv("DATABASE_URL"); const pool = new Pool({ connectionString: databaseUrl, application_name: `mcft-cap09-a18d-${mode}` });
  try {
    await assertFormalDatabase(pool, databaseUrl);
    if (mode === "preflight") {
      const proof = await readOnlyPreflight(pool);
      const result = { schema_version: "geox_mcft_cap09_a18d_prewindow_bootstrap_preflight_v1", status: "PASS", transaction_mode: "READ_ONLY", formal_database_name: FORMAL_DATABASE, epoch_id: MCFT_CAP09_A18_SELECTED_EPOCH_ID_V2, prewindow_a0: A0, o00: O00, exact_config_pin_count: expected_pins.length, ...proof, provider_request_count: 0, database_write_count: 0, scheduler_write_count: 0, canonical_runtime_tick_write_count: 0, formal_o00_started: false };
      fs.mkdirSync(OUTPUT_DIR, { recursive: true }); fs.writeFileSync(OUTPUT_PATH, JSON.stringify(result, null, 2) + "\n"); console.log(JSON.stringify(result)); return;
    }
    const subject = requiredEnv("MCFT_CAP09_SUBJECT_SHA"); assertExactProtectedMain(subject, authority);
    const executionStartedAt = new Date().toISOString(); assertExecutionWallClock(executionStartedAt);
    ...
    const executionCompletedAt = new Date().toISOString(); assertExecutionWallClock(executionCompletedAt);
    const result = {
      schema_version: "geox_mcft_cap09_a18d_prewindow_bootstrap_execution_result_v1", status: "PASS", subject_sha: subject, epoch_id: MCFT_CAP09_A18_SELECTED_EPOCH_ID_V2, formal_database_name: FORMAL_DATABASE,
      execution_started_at: executionStartedAt, execution_completed_at: executionCompletedAt, execution_not_before: A0, execution_must_complete_before: O00, execution_mode: execution.execution_mode,
      exact_config_pin_count: 25, runtime_config_write_count_this_execution: execution.runtime_config_write_count, a0_member_write_count_this_execution: execution.a0_member_write_count,
      active_runtime_config_ref: expected_pins[0].ref, active_runtime_config_hash: expected_pins[0].hash, checkpoint_next_tick_logical_time: O00,
      observed_runtime_config_count: footprint.runtime_configs, observed_active_lineage_count: footprint.active_lineage, observed_state_latest_count: footprint.state_latest, observed_checkpoint_count: footprint.checkpoint_latest,
      observed_scheduler_cursor_count: footprint.scheduler_cursors, observed_scheduler_slot_count: footprint.scheduler_slots, observed_terminal_tick_count: footprint.terminal_ticks, observed_downstream_action_fact_count: footprint.downstream_action_facts,
      provider_request_count: 0, scheduler_write_count: 0, canonical_runtime_tick_write_count: 0, formal_o00_started: false, formal_execution_count: "0/24",
    };
