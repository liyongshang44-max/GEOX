// Disposable architecture-deviation diagnostic for preserving all 16 Calibration cases while excluding FVO-10 from parameter-objective aggregation.
...
import {
  CAP08_S5_ARCHITECTURE_V2_OUTCOME_EXCEPTION_PROFILE_V1,
  establishCap08S5ArchitectureV2OutcomePreservingPredecessorV1,
} from "./mcft_cap08_s5_architecture_v2_outcome_preserving_support_v1.js";
import { runCap08S5ObjectiveEligibilityDiagnosticV1 } from "./mcft_cap08_s5_objective_eligibility_diagnostic_v1.js";

if (process.env.MCFT_CAP08_S5_DESTRUCTIVE_ACCEPTANCE !== "1") {
  throw new Error("SET_MCFT_CAP08_S5_DESTRUCTIVE_ACCEPTANCE_1");
}

const OUT = "acceptance-output/MCFT_CAP_08_S5_OBJECTIVE_ELIGIBILITY_DIAGNOSTIC.json";
...
function exactMemberV1(recordSet: Cap04ARecordSetV1, objectType: string): CanonicalObjectEnvelopeV1 {
  ...
  if (matches.length !== 1) throw new Error(`CAP08_S5_ELIGIBILITY_MEMBER_CARDINALITY:${objectType}:${matches.length}`);
...
async function main(): Promise<void> {
  try {
    const predecessor = await establishCap08S5ArchitectureV2OutcomePreservingPredecessorV1(path.resolve("."));
    ...
    if (s4.status !== "COMPLETED"
      ...
      throw new Error("CAP08_S5_ELIGIBILITY_S4_CORRECTION_NOT_EXACT");
    }
    ...
    const unfilteredAttempt = await runCap06CalibrationGridSearchV1({
      calibrationWindow: asCap06ComputeWindowV1(calibrationWindow),
      predictionPort,
    });
    const eligibilityAwareAttempt = await runCap08S5ObjectiveEligibilityDiagnosticV1({
      calibrationWindow,
      predictionPort,
      objectiveIneligibleObservationRefs: ["FVO-10"],
    });
    const architectureDeviationResolved = eligibilityAwareAttempt.selected_parameter_value === "0.034000"
      && eligibilityAwareAttempt.canonical_append_allowed;
    const result = {
      schema_version: "geox_mcft_cap08_s5_objective_eligibility_architecture_diagnostic_v1",
      ...
      windows,
      unfiltered_attempt: unfilteredAttempt,
      eligibility_aware_attempt: eligibilityAwareAttempt,
      expected_candidate_value: "0.034000",
      oracle_matches: eligibilityAwareAttempt.selected_parameter_value === "0.034000",
      candidate_append_allowed: eligibilityAwareAttempt.canonical_append_allowed,
      candidate_created: false,
      shadow_created: false,
      predecessor_effectiveness_claimed: false,
      s5_effective: false,
      final_formal_run_id: null,
      architecture_deviation_resolved: architectureDeviationResolved,
    };
