...
async function fetchBytes(params, label) {
  validateTransportConfig();
  const attempts = [];
  for (let index = 0; index < CONFIG.web_service_base_urls.length; index += 1) {
    const url = new URL(CONFIG.web_service_base_urls[index]);
    ...
    const attempt = { ...req, response_status: response.status, content_type: String(response.headers.get('content-type')||'').split(';')[0], failover_used: false };
    if (!response.ok) {
      ...
      attempts.push({ ...attempt, failover_used: mayFailover });
    ...
    return {
      ...
      evidence: {
        ...
        transport_attempts: [...attempts, { ...attempt, response_body_sha256: sha256(bytes), response_bytes: bytes.byteLength }],
      },
    };
  }
...
async function fetchJson(params, label) {
  const fetched = await fetchBytes(params, label);
  let json;
  try { json = JSON.parse(fetched.text); } catch { throw new Error(`${label}_JSON_REQUIRED`); }
  return { json, evidence: fetched.evidence };
}
function parseDelimitedLine(line, delimiter) {
  const output=[]; let value=''; let quoted=false;
  for(let i=0;i<line.length;i+=1){const ch=line[i];if(ch==='"'){if(quoted&&line[i+1]==='"'){value+='"';i+=1;}else quoted=!quoted;}else if(ch===delimiter&&!quoted){output.push(value);value='';}else value+=ch;}
  output.push(value); return output;
}
function parseInventoryText(text) {
  const trimmed = String(text || '').trim();
  if (!trimmed) throw new Error('EA1G_LIST_EMPTY_RESPONSE');
  ...
  for (const delimiter of delimiters) {
    ...
    for (const line of lines.slice(1)) {
      ...
      const record={}; headerCells.forEach((key,index)=>{record[key]=cells[index]??'';}); records.push(record);
    }
    if (records.length) return { structured: records, encoding: delimiter==='\t'?'tsv':'csv' };
  }
  throw new Error('EA1G_LIST_STRUCTURED_INVENTORY_REQUIRED');
}
async function fetchInventory(params, label) {
  const fetched=await fetchBytes(params,label);
  const parsed=parseInventoryText(fetched.text);
  return { structured:parsed.structured, evidence:{...fetched.evidence, parsed_inventory_encoding:parsed.encoding} };
}
function discoverStationCandidates(value) {
  ...
  function walk(current, p='') {
    ...
    const scalars = Object.entries(current).filter(([,v])=>['string','number'].includes(typeof v));
    const corpus = scalars.map(([,v])=>normalize(v)).join(' ');
    if (tokens.some((token)=>corpus.includes(token))) {
      const identity = {};
      for (const [key,v] of scalars) if (/(name|station|code|id|uid|state|network)/i.test(key)) identity[key] = String(v);
      if (Object.keys(identity).length) candidates.push({ object_path: p || '$', identity });
    }
...
function findRecordArray(json) {
  ...
  function walk(value,p='$') {
    if(Array.isArray(value)){
      ...
      if(objects.length>=2){const keys=new Set(objects.flatMap((x)=>Object.keys(x)));const timeKeys=[...keys].filter((key)=>/(date|time|datetime|valid|timestamp)/i.test(key));if(timeKeys.length)arrays.push({path:p,records:objects,keys:[...keys].sort(),timeKeys});}
      value.slice(0,8).forEach((x,i)=>{if(x&&typeof x==='object')walk(x,`${p}[${i}]`);});return;
    }
    if(value&&typeof value==='object')for(const[key,v]of Object.entries(value))if(v&&typeof v==='object')walk(v,`${p}.${key}`);
  }
  walk(json); arrays.sort((a,b)=>b.records.length-a.records.length); return arrays[0]||null;
}
...
function parseFlaggedNumber(raw){if(typeof raw==='number'&&Number.isFinite(raw))return{valid:true,flag:null};if(typeof raw!=='string')return{valid:false,flag:null};const match=raw.trim().match(/^[-+]?\d+(?:\.\d+)?(?:[eE][-+]?\d+)?\s*([ER]|e)?$/);return match?{valid:true,flag:match[1]||null}:{valid:false,flag:null};}
function companionFlag(record,field){const target=field.toLowerCase();for(const[key,value]of Object.entries(record)){const nk=key.toLowerCase();if(!nk.includes('flag'))continue;if(!(nk.includes(target)||nk.replace(/flag|qc|quality/g,'').includes(target)))continue;const text=String(value||'').trim();if(['E','R','e'].includes(text))return text;}return null;}
...
function analyzeRole(records,timeKeys,fields,latestRecordTime){const floor=latestRecordTime-CONFIG.freshness_and_continuity.recent_window_hours*HOUR_MS;return fields.map((field)=>{const distinctAll=new Set(),distinctUnestimated=new Set();const flagCounts={E:0,R:0,e:0,unflagged:0,missing_or_non_numeric:0};for(const record of records){const time=recordTime(record,timeKeys);if(time===null||time<floor||time>latestRecordTime)continue;const parsed=parseFlaggedNumber(record[field]);if(!parsed.valid){flagCounts.missing_or_non_numeric++;continue;}const flag=parsed.flag||companionFlag(record,field);distinctAll.add(Math.floor(time/HOUR_MS));if(flag&&Object.hasOwn(flagCounts,flag))flagCounts[flag]++;else{flagCounts.unflagged++;distinctUnestimated.add(Math.floor(time/HOUR_MS));}}return{field_name:field,recent_distinct_hours:distinctAll.size,recent_unestimated_distinct_hours:distinctUnestimated.size,flag_counts:flagCounts,raw_values_emitted:false};});}
...
try {
  ...
  for(const name of queryCandidates){try{const fetched=await fetchJson({name,productid:CONFIG.product_id,begin,end,units:CONFIG.observation_request.units,format:CONFIG.observation_request.format,tz:CONFIG.observation_request.timezone,network:CONFIG.network},'EA1G_SCQC60');const found=findRecordArray(fetched.json);if(!found){failures.push('NO_RECORD_ARRAY');continue;}chosen={name,fetched};recordArray=found;break;}catch(error){failures.push(safeError(error));}}
  ...
  const latest=times.at(-1),earliest=times[0],ageHours=(Date.now()-latest)/HOUR_MS;
  ...
  const solarFields=roleFieldCandidates(recordArray.keys,CONFIG.required_observation_roles.solar_radiation),rainFields=roleFieldCandidates(recordArray.keys,CONFIG.required_observation_roles.rainfall);
  ...
  const solar=analyzeRole(recordArray.records,recordArray.timeKeys,solarFields,latest),rainfall=analyzeRole(recordArray.records,recordArray.timeKeys,rainFields,latest);
  const solarQualified=solar.filter((x)=>x.recent_unestimated_distinct_hours>=CONFIG.freshness_and_continuity.minimum_recent_unestimated_solar_hours),rainQualified=rainfall.filter((x)=>x.recent_unestimated_distinct_hours>=CONFIG.freshness_and_continuity.minimum_recent_unestimated_rain_hours);
