...
async function assertDatabaseIdentityAndT3ZeroFreshConnection(databaseUrl: string, subjectSha: string, phase: string): Promise<void> {
  const pool = new Pool({ connectionString: databaseUrl, application_name: `mcft-cap09-t4r1-successor-guard-${phase.toLowerCase()}-${subjectSha.slice(0, 12)}`, max: 1 });
  try {
...
async function main(): Promise<void> {
  ...
  await assertDatabaseIdentityAndT3ZeroFreshConnection(databaseUrl, subjectSha, "BEFORE");
  const generated = buildSuccessorRunner();
  fs.writeFileSync(GENERATED_RUNNER, generated);
  try {
    execFileSync("pnpm", ["exec", "tsx", GENERATED_RUNNER], { stdio: "inherit", env: process.env });
    await assertDatabaseIdentityAndT3ZeroFreshConnection(databaseUrl, subjectSha, "AFTER");
