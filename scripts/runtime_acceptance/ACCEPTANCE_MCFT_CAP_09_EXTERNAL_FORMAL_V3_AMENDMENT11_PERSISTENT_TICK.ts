...
import {
  EXTERNAL_FORMAL_V3_AMENDMENT11_WATERMARK_ID_V1,
  ExternalFormalV3Amendment11PersistentTickServiceV1,
  type ExecuteExternalFormalV3Amendment11PersistentTickInputV1,
  type ExternalFormalV3Amendment11DatabaseEvidenceSourcePortV1,
...
function delayedExactRecordsV1<T extends { record_type: string; available_to_runtime_at: string; role_time: Record<string, unknown> }>(records: readonly T[]): T[] {
  ...
  for (const record of result) {
    ...
    record.available_to_runtime_at = addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 490);
    record.role_time.ingested_at = addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 491);
  }
  return result;
}
...
class FakePersistenceV1 {
  ...
  port(): Cap04ForecastScenarioPersistencePortV1 {
    const self = this;
    return {
      async lookupARecordSet(idempotencyKey) { return self.a?.idempotency_key === idempotencyKey ? structuredClone(self.a) : null; },
      ...
      async readARecordSet(recordSetId) { return self.a?.record_set_id === recordSetId ? structuredClone(self.a) : null; },
      async lookupScenarioSet(idempotencyKey) { return self.b?.idempotency_key === idempotencyKey ? structuredClone(self.b) : null; },
      ...
      async readScenarioSet(scenarioSetId) { return self.b?.scenario_set_id === scenarioSetId ? structuredClone(self.b) : null; },
      async readScenarioSetBySourceForecast(sourceForecastRef, sourceForecastHash) {
        ...
        return payload.source_forecast_ref === sourceForecastRef && payload.source_forecast_hash === sourceForecastHash ? structuredClone(self.b) : null;
      },
...
async function buildHarnessV1() {
  ...
  let evidenceCalls = 0;
  const evidenceSource: ExternalFormalV3Amendment11DatabaseEvidenceSourcePortV1 = {
    async loadCandidateRecords(input) {
      ...
      assert.equal(input.evidence_snapshot_time, addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 500));
  ...
  const configRepo = {
    async readRuntimeConfig(objectId: string) { return objectId === fixture.hourly.object_id ? structuredClone(fixture.hourly) : null; },
  ...
  const service = new ExternalFormalV3Amendment11PersistentTickServiceV1(handoffPort, evidenceSource, configRepo, persistence.port());
  const cropAuthority = fixture.hourly.payload.crop_stage_context_authority as Record<string, unknown>;
  const input: ExecuteExternalFormalV3Amendment11PersistentTickInputV1 = {
    claim: {
      boundary: {
        scope: structuredClone(fixture.scope),
        slot_id: "O00",
        logical_time: EA5B5B_LOGICAL_TIME_V1,
        scheduler_wall_clock_observed_at: addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 499),
        interval_seconds: 3600,
      },
      lease_owner: "amendment11-v3-qualification-owner",
      fencing_token: 7n,
      state: "CLAIMED",
      idempotency_key: "amendment11-v3-qualification-slot",
    },
    manifest_slot: {
      manifest_ref: "qualification://mcft-cap09/external-formal-v3/amendment11-window-manifest",
      manifest_hash: "sha256:qualification-external-formal-v3-amendment11-window-manifest",
    ...
    evidence_snapshot_time: addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 500),
    observer_started_at: addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 501),
...
async function main(): Promise<void> {
  ...
  assert.equal(inserted.provider_availability_watermark, EXTERNAL_FORMAL_V3_AMENDMENT11_WATERMARK_ID_V1);
  assert.equal(inserted.fixed_lag_authority_used, false);
  assert.equal(inserted.evidence_snapshot_time, addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 500));
  assert.equal(inserted.scheduler_to_snapshot_delay_minutes, 1);
  assert.equal(inserted.snapshot_to_observer_delay_minutes, 1);
  ...
  assert.deepEqual([inserted.runtime_provider_request_count, inserted.runtime_r2_head_count, inserted.recommendation_write_count, inserted.approval_write_count, inserted.action_write_count, inserted.dispatch_write_count, inserted.model_activation_write_count], [0, 0, 0, 0, 0, 0, 0]);
  ...
  assert.equal(positive.persistence.aWrites, 1);
  assert.equal(positive.persistence.bWrites, 1);
  assert.equal(positive.evidenceCalls(), 1, "existing canonical A must not reload Evidence");

  const recovery = await buildHarnessV1();
  recovery.persistence.failNextScenarioCommit = true;
  await assert.rejects(() => recovery.service.executeClaimedTick(recovery.input), /QUALIFICATION_INJECTED_SCENARIO_COMMIT_FAILURE/);
  ...
  assert.equal(recovery.evidenceCalls(), 1, "pending-B recovery must not reload Evidence");

  const badChronology = await buildHarnessV1();
  const badChronologyInput = structuredClone(badChronology.input);
  badChronologyInput.evidence_snapshot_time = addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 502);
  badChronologyInput.observer_started_at = addMinutesV1(EA5B5B_LOGICAL_TIME_V1, 501);
  await assert.rejects(() => badChronology.service.executeClaimedTick(badChronologyInput), /EXTERNAL_FORMAL_V3_AM11_OBSERVER_BEFORE_EVIDENCE_SNAPSHOT/);
  assert.equal(badChronology.persistence.aWrites, 0);
  assert.equal(badChronology.evidenceCalls(), 0);
  ...
  await assert.rejects(() => badConfig.service.executeClaimedTick(badConfigInput), /EXTERNAL_FORMAL_V3_AM11_RUNTIME_CONFIG_HASH_MISMATCH/);
  assert.equal(badConfig.persistence.aWrites, 0);

  const a2 = await buildHarnessV1();
  const h = a2.fixture.handoff;
  a2.persistence.a = structuredClone(buildExternalFormalCap04BlockedA2RecordSetV1({
  ...
  console.log(JSON.stringify({
    ...
    provider_availability_watermark: inserted.provider_availability_watermark,
    actual_snapshot_delay_minutes: 500,
