...
try {
  ...
  ok('changed_files_allowlist', diffAllowed(diff), { diff, base: R5_BASE_HEAD, mode: diffMode() });
  ...
  ok('readiness_outcomes_present', includesAll(doc, ['not_ready', 'blocked', 'conditional_ready', 'ready_for_human_review', 'ready_candidate', 'not pilot start', 'not pilot authorization']));
