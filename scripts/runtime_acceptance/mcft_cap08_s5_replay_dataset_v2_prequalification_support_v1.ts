// Purpose: establish the acceptance-only MCFT-CAP-08.S5 replay-dataset v2 predecessor over the exact S1-S4 provider chain.
// Boundary: fresh-PostgreSQL prequalification only; no Candidate/Shadow persistence, active Config switch, Model Activation, production Runtime source, route, scheduler, final formal closure, S6, or MCFT-CAP-09 authority.
...
export const CAP08_S5_REPLAY_DATASET_V2_ID_V1 = "mcft_cap08_stage1a_replay_v2" as const;
export const CAP08_S5_REPLAY_DATASET_V2_HIDDEN_PARAMETER_V1 = "0.034000" as const;
export const CAP08_S5_REPLAY_DATASET_V2_PROFILE_ID_V1 =
  "MULTI_REGIME_RAINFALL_PLUS_FORECAST_DERIVED_HIDDEN_0034_FVO_V1" as const;
export const CAP08_S5_REPLAY_DATASET_V2_OUTCOME_PROFILE_ID_V1 =
  "FVO10_FROZEN_BUSINESS_OUTCOME_ANCHOR_V1" as const;
export const CAP08_S5_REPLAY_DATASET_V2_CONTRACT_DIGEST_V1 =
  "sha256:85fe66eda022531dd426a0ac70624ff0c22a2662a8d5f9c661cd1891bd6a8194" as const;
...
function rewriteForcingV2(
  ...
  return records.map((source) => {
    ...
    record.dataset_id = CAP08_S5_REPLAY_DATASET_V2_ID_V1;
    record.limitations = [
...
function parseCanonicalFactV1(value: unknown, expectedType: string): CanonicalObjectEnvelopeV1 {
  const fact = recordV1(
    ...
    "CAP08_S5_V2_FACT_INVALID",
  );
  if (fact.type !== expectedType) throw new Error("CAP08_S5_V2_FACT_TYPE_MISMATCH");
  const object = recordV1(
    fact.payload,
    "CAP08_S5_V2_FACT_PAYLOAD_REQUIRED",
...
export class Cap08S5ReplayDatasetV2EvidenceSourceV1 implements ReplayEvidenceSourcePortV1 {
  constructor(
  ...
  async buildFvoFromForecastV1(input: {
    ...
    const config = new DirectCap04ExecutionConfigResolverV1()
      .resolveExecutionConfig(runtimeConfig).payload;
    ...
    const replay = executeHourlyWaterBalanceV1({
      ...
      config: {
        ...
        drainage_coefficient_per_hour: CAP08_S5_REPLAY_DATASET_V2_HIDDEN_PARAMETER_V1,
    ...
    const generatedValue = input.fvoId === "FVO-10"
      ? Number(CAP08_S3_OUTCOME_VALUE_V1)
      : Number(replay.published_state.root_zone_vwc_fraction.mean);
    const generationProfile = input.fvoId === "FVO-10"
    ...
    const canonicalPayload = {
      ...
      generation_profile_id: generationProfile,
      hidden_parameter_key: input.fvoId === "FVO-10"
        ? null
        : "dynamics_parameters.drainage_coefficient_per_hour",
      hidden_parameter_value: input.fvoId === "FVO-10"
        ? null
      ...
      business_outcome_anchor: input.fvoId === "FVO-10",
    };
    ...
    return {
      ...
      source_payload: { ...canonicalPayload, source_version: "2-prequalification" },
      canonical_payload: canonicalPayload,
  ...
  async loadCandidateRecords(input: {
    ...
    const baseRecords = rewriteForcingV2(
...
export async function establishCap08S5ReplayDatasetV2PredecessorV1(root: string) {
  ...
  const evidenceSource = new Cap08S5ReplayDatasetV2EvidenceSourceV1(
    fixture.bootstrap_evidence_source,
    runtimeRepository,
  );
  const frozenEvidence = new Cap08FrozenEvidenceSourceV1(
    new Cap08S2QualifiedEvidenceSourceV1(evidenceSource),
  );
  const deferred = new Cap08DeferredScenarioPersistenceV1(
    persistenceAdapterV1(runtimeRepository, forecastRepository, []),
  );
  ...
  const range = new Cap08S3FormalRangeServiceV1(
    handoff,
    new Cap08S3CompletionEvidenceTickServiceV1(
      baseTick,
  ...
  const runtime = new Cap08S3FormalRuntimeServiceV1(
    new A0BootstrapRuntimeServiceV1(
      runtimeRepository,
      runtimeRepository,
      fixture.bootstrap_evidence_source,
    ),
  ...
  const formalRunId = `cap08_v2pq_${semanticHashV1({
    dataset_id: CAP08_S5_REPLAY_DATASET_V2_ID_V1,
    profile_id: CAP08_S5_REPLAY_DATASET_V2_PROFILE_ID_V1,
    outcome_profile_id: CAP08_S5_REPLAY_DATASET_V2_OUTCOME_PROFILE_ID_V1,
    contract_digest: CAP08_S5_REPLAY_DATASET_V2_CONTRACT_DIGEST_V1,
    scope: fixture.scope,
  ...
  const runtimeInput = {
    ...
    lease_owner: "mcft-cap08-s5-replay-dataset-v2-prequalification",
