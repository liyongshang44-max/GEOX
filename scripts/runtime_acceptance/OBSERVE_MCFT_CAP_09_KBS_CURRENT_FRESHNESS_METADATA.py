...
def observe_current_freshness_metadata() -> dict:
    ...
    for delim in (",", "\t", ";", "|"):
        ...
        for values in rows[header_index + 1:]:
            ...
            if parsed is None or parsed > future_limit:
                continue
            parsed_row_count += 1
            normalized_time = parsed.replace(microsecond=0)
            event_times.add(normalized_time)
            event_time_row_counts[normalized_time] = event_time_row_counts.get(normalized_time, 0) + 1
    ...
    expected_latest_24h = [latest_hour - timedelta(hours=offset) for offset in range(23, -1, -1)]
    missing_latest_24h = [iso(value) for value in expected_latest_24h if value not in event_times]
    observed_latest_24h_count = len(expected_latest_24h) - len(missing_latest_24h)
    latest_24h_duplicate_rows = sum(max(0, event_time_row_counts.get(value, 0) - 1) for value in expected_latest_24h)
    proof = {
        ...
        "global_duplicate_event_time_row_count": parsed_row_count - len(event_times),
        "latest_24h_duplicate_event_time_row_count": latest_24h_duplicate_rows,
        "latest_24h_observation_start": iso(expected_latest_24h[0]),
        "latest_24h_observation_end": iso(expected_latest_24h[-1]),
        "latest_24h_expected_hour_count": 24,
        "latest_24h_observed_hour_count": observed_latest_24h_count,
        "latest_24h_missing_event_times": missing_latest_24h,
        "latest_24h_contiguous": len(missing_latest_24h) == 0,
...
def main() -> None:
    print(json.dumps(observe_current_freshness_metadata(), sort_keys=True))
