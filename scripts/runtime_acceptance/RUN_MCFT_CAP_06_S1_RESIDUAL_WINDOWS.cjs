...
// Purpose: execute the permanent MCFT-CAP-06 S1 Residual-window acceptance in an isolated PostgreSQL database.
// Boundary: acceptance orchestration only; no production database, Runtime authority, S2 math, Candidate, Evaluation, Model Activation, route, Web, scheduler, or CAP-07 authority.
...
const ACCEPTANCE_PATH = 'scripts/runtime_acceptance/ACCEPTANCE_MCFT_CAP_06_S1_RESIDUAL_WINDOWS_DB.ts';
const RESULT_PATH = path.join(ROOT, 'acceptance-output', 'MCFT_CAP_06_S1_RESULT.json');
const ISOLATED_DATABASE_NAME = 'mcft_cap06_s1_residual_windows_ci';
...
function resolveBaseDatabaseUrl() {
  ...
  if (!postgresUser || !postgresPassword || !postgresDatabase || !postgresHost || !postgresPort) {
    throw new Error('MCFT_CAP_06_S1_POSTGRESQL_ACCEPTANCE_DATABASE_CONFIG_REQUIRED');
  }
...
function persistResult(stdout) {
  const line = String(stdout || '').split(/\r?\n/).find((item) => item.startsWith('S1_RESULT_JSON:'));
  if (!line) throw new Error('MCFT_CAP_06_S1_RESULT_JSON_REQUIRED');
  const result = JSON.parse(line.slice('S1_RESULT_JSON:'.length));
  ...
  try {
    const result = run(process.platform === 'win32' ? 'pnpm.cmd' : 'pnpm', [
      ...
      env: {
        ...
        MCFT_CAP_06_S1_DESTRUCTIVE_ACCEPTANCE: '1',
      },
    });
    process.stdout.write(String(result.stdout || ''));
    process.stderr.write(String(result.stderr || ''));
    requireSuccessful(result, 'MCFT_CAP_06_S1_RESIDUAL_WINDOWS');
