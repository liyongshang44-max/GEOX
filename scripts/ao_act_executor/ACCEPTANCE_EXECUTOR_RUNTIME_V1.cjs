...
async function getTaskDetail(base, readToken, triple, actTaskId) {
  return fetchJson(
    `${base}/api/v1/ao-act/tasks/${encodeURIComponent(actTaskId)}?tenant_id=${encodeURIComponent(triple.tenant_id)}&project_id=${encodeURIComponent(triple.project_id)}&group_id=${encodeURIComponent(triple.group_id)}`,
    { method: 'GET', token: readToken }
  );
}

async function getOperationPlanDetail(base, readToken, triple, operationPlanId) {
  return fetchJson(
    `${base}/api/v1/operations/plans/${encodeURIComponent(operationPlanId)}?tenant_id=${encodeURIComponent(triple.tenant_id)}&project_id=${encodeURIComponent(triple.project_id)}&group_id=${encodeURIComponent(triple.group_id)}`,
    { method: 'GET', token: readToken }
  );
}
...
(async () => { // 主流程入口。
  ...
  const preRunTaskResp = await getTaskDetail(base, adminToken, triple, runSeed.actTaskId);
  const preRunTask = requireOk(preRunTaskResp, 'get pre-run task detail');
  const preRunPlanResp = await getOperationPlanDetail(base, adminToken, triple, runSeed.operationPlanId);
  const preRunPlan = requireOk(preRunPlanResp, 'get pre-run operation plan detail');
  const preRunPlanStatus = String(preRunPlan?.item?.plan?.record_json?.payload?.status ?? '').trim().toUpperCase();
  const preRunReceiptFactId = String(preRunPlan?.item?.plan?.record_json?.payload?.receipt_fact_id ?? '').trim();
  console.log(JSON.stringify({
    operation_plan_id: runSeed.operationPlanId,
    act_task_id: runSeed.actTaskId,
    pre_run_plan_status: preRunPlanStatus,
    pre_run_receipt_fact_id: preRunReceiptFactId || null
  }, null, 2));
  if (preRunPlanStatus === 'SUCCEEDED' || preRunPlanStatus === 'FAILED') {
    console.error('PLAN_ALREADY_TERMINAL_BEFORE_EXECUTOR', JSON.stringify({
      operation_plan_id: runSeed.operationPlanId,
      act_task_id: runSeed.actTaskId,
      pre_run_plan_status: preRunPlanStatus,
      pre_run_receipt_fact_id: preRunReceiptFactId || null,
      recommendation_id: runSeed.recommendationId,
      approval_request_id: runSeed.approvalRequestId
    }, null, 2));
    throw new Error('PLAN_ALREADY_TERMINAL_BEFORE_EXECUTOR');
  }
  assert.ok(String(preRunTask?.item?.task?.payload?.act_task_id ?? '').trim() === runSeed.actTaskId, 'PRE_RUN_TASK_ID_MISMATCH');
