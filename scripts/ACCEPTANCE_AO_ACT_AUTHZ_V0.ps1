...
$runner  = Join-Path $PSScriptRoot "ACCEPTANCE_AO_ACT_AUTHZ_V0_RUNNER.cjs"
...
try {
  ...
  $code = $LASTEXITCODE
  if ($code -ne 0) { throw "AO-ACT AuthZ v0 acceptance failed (exit_code=$code)" }
  Write-Host "[OK] AO-ACT AuthZ v0 acceptance passed"
