...
- `field_memory_v1_optional` 仅作为 export/dry-run compatibility projection rows；`--apply` 跳过 `_optional` 表，正式 field memory 必须由 `POST /api/v1/field-memory/from-acceptance` 写入 `field_memory_v1`
...
`--apply` 会先写基础表、facts 与 prescription；不会写入静态 formal Field Memory compatibility rows。随后如果传入 `--base-url`，或环境变量 `CONTROLLED_PILOT_VERIFY_API_BASE` / `BASE_URL` / `API_BASE_URL` 可用，脚本会调用：
...
API 不可达时只输出 `as-executed derivation skipped` warning，不伪装成功；`c8-formal-chain` profile 会将该跳过视为失败。`--verify-api` 会强制调用 from-receipt、`POST /api/v1/field-memory/from-acceptance`，并确认：

```text
/api/v1/as-executed/by-task/act_c8_irrigation_formal_001
/api/v1/customer/fields/field_c8_demo/memory
...
- formal Field Memory 必须是 `FORMAL_FIELD_MEMORY / FORMAL_ACCEPTED / customer_visible_memory=true / learning_eligible=true`，且正式通过来源必须是 `POST /api/v1/field-memory/from-acceptance` 后再由 `GET /api/v1/customer/fields/field_c8_demo/memory` 读回验证。
- 保留的静态 memory row 只能位于 `tables.field_memory_v1_optional`，必须带有 `compatibility_fallback=true`、`projection_support_only=true` 或等价语义；它仅用于兼容投影审查，不是唯一通过来源，治理验收原因记录在 manifest 的 `governance_acceptance.static_formal_memory_retained_reason`。
