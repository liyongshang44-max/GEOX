...
function closureRows(tenant) {
  ...
  const postWaterStateIndex = { ...common, estimate_id: postWaterStateId, source_fact_id: waterStateFactId, state: 'NORMAL', classification: 'post_irrigation_response', water_state: 'NORMAL', root_zone_soil_moisture_percent: 24.8, soil_moisture_value: 24.8, target_min_soil_moisture_percent: 22, target_max_soil_moisture_percent: 28, net_irrigation_mm: 22, gross_irrigation_requirement_mm: 22, source_sensing_window_id: postWindowId, source_sensing_window_fact_id: windowFactId, input_refs_json: json({ sensing_window_id: postWindowId }), evidence_refs_json: json([postWindowId]), calculation_inputs_json: json({ before_soil_moisture: 18.4, after_soil_moisture: 24.8 }), derivation_json: json({ source: SOURCE, mode: 'demo_post_irrigation_response' }), quality_json: json({ status: 'PASS' }), confidence_json: json({ level: 'HIGH', score: 0.95 }), confidence_level: 'HIGH', computed_at: occurredAt, created_at: occurredAt, updated_at: occurredAt, updated_ts_ms: postEndMs, created_ts_ms: postEndMs };
...
async function main() {
  ...
  const summary = { ok: true, seed: 'operator_twin_h31_h45_demo_closure_v1', tenant_id: tenant, project_id: PROJECT_ID, group_id: GROUP_ID, field_id: FIELD_ID, season_id: SEASON_ID, base_seed: 'skipped_by_default', generated_facts: rows.facts.map((row) => JSON.parse(row.record_json).type), written_index_tables: Object.keys(rows.indexes), not_written: ['roi_ledger_v1', 'field_memory_v1', 'operation_state_v1', 'customer_delivery', 'projectReportV1', 'field_report'] };
