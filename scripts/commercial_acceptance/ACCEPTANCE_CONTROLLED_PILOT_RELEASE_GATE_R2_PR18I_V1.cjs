...
const lines = ['# Controlled Pilot Readiness Report', '', `Status: ${failed.length ? 'FAIL' : 'PASS'}`, '', '## Required gates', ...results.map((r) => `- ${r.ok ? 'PASS' : 'FAIL'} ${r.id}: ${r.command}`), '', '## PR-18I formal-chain runtime contract', '- c8-formal-chain backend P0 is required.', '- c8-formal-chain seed runtime is required and cannot be replaced by full-review runtime.', '- c8-formal-chain structured verify-api is required.', '- c8-formal-chain runtime and verify-api run before full-review runtime to avoid append-only pending facts contaminating formal-only assertions.', '- full-review seed runtime remains required after formal-chain verification.', '', '## Failed gate output tails', failed.length ? failed.map((r) => `### ${r.id}\n\n\`\`\`\n${r.output_tail}\n\`\`\``).join('\n\n') : '- none', ''];
