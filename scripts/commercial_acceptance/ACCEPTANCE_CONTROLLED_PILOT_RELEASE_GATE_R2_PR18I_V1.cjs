...
const env = { ...process.env, BASE_URL: BASE, API_BASE_URL: process.env.API_BASE_URL || BASE, TENANT_ID: TENANT };
const FORMAL_ONLY = process.env.CONTROLLED_PILOT_PR18I_FORMAL_CHAIN_ONLY === '1';
const OMIT_FORMAL = process.env.CONTROLLED_PILOT_PR18I_SKIP_FORMAL_CHAIN === '1';

const seedApply = `${SEED} --apply --tenant ${TENANT} --base-url ${BASE}`;
const formalApply = `${SEED} --cleanup --apply --tenant ${TENANT} --profile c8-formal-chain && ${SEED} --apply --tenant ${TENANT} --profile c8-formal-chain --base-url ${BASE}`;
const formalVerify = `${SEED} --verify-api --tenant ${TENANT} --profile c8-formal-chain --base-url ${BASE}`;
...
const formal = [
  ['controlled_pilot_c8_formal_chain_seed_runtime', formalApply],
  ['c8_formal_chain_backend_p0', 'pnpm run ci:governance:c8-formal-chain-backend-p0'],
  ['controlled_pilot_c8_formal_chain_verify_api_structured_json', formalVerify],
];
const full = [
...
const gates = FORMAL_ONLY ? [...common, ...formal] : [...common, ...(OMIT_FORMAL ? [] : formal), ...full];
...
const lines = ['# Controlled Pilot Readiness Report', '', `Status: ${failed.length ? 'FAIL' : 'PASS'}`, '', '## Required gates', ...results.map((r) => `- ${r.ok ? 'PASS' : 'FAIL'} ${r.id}: ${r.command}`), '', '## Phase', `- FORMAL_ONLY: ${FORMAL_ONLY ? 'true' : 'false'}`, `- OMIT_FORMAL: ${OMIT_FORMAL ? 'true' : 'false'}`, '', '## Failed gate output tails', failed.length ? failed.map((r) => `### ${r.id}\n\n\`\`\`\n${r.output_tail}\n\`\`\``).join('\n\n') : '- none', ''];
fs.mkdirSync(path.dirname(REPORT), { recursive: true });
fs.writeFileSync(REPORT, `${lines.join('\n')}\n`);
console.log(JSON.stringify({ status: failed.length ? 'FAIL' : 'PASS', required_gate_count: results.length, failed_gate_ids: failed.map((r) => r.id), phase: { formal_only: FORMAL_ONLY, omit_formal: OMIT_FORMAL } }, null, 2));
