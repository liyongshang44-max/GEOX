...
const c8FormalChainApply = `${SEED} --cleanup --apply --tenant ${TENANT} --profile c8-formal-chain && ${SEED} --apply --tenant ${TENANT} --profile c8-formal-chain --base-url ${BASE}`;
const c8FormalChainVerifyApi = `${SEED} --verify-api --tenant ${TENANT} --profile c8-formal-chain --base-url ${BASE}`;
...
function tail(text) {
  ...
  return s.length > 4000 ? s.slice(s.length - 4000) : s;
}
...
const lines = [
  ...
  '- full-review seed runtime remains required after formal-chain verification.',
  '',
...
console.log(JSON.stringify({ status: failed.length ? 'FAIL' : 'PASS', required_gate_count: results.length, failed_gate_ids: failed.map((r) => r.id), formal_chain_gates: ['controlled_pilot_c8_formal_chain_seed_runtime', 'c8_formal_chain_backend_p0', 'controlled_pilot_c8_formal_chain_verify_api_structured_json'] }, null, 2));
