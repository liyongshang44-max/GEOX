...
function evaluate(manifest, source) {
  ...
  rows.push(dim('H6', 'p51_5_viewer_artifact_health', source.p51_5_closure?.acceptance?.assertion_count === 57 && source.p51_5_closure?.acceptance?.failed_assertion_count === 0 && source.present.p51_5_snapshot && snapshot?.gateway_summary?.accepted_observation_count === 21 && allPass(p515Matrix) ? 'OK' : 'BLOCKED', 'P51.5 closure, capability matrix, and checked-in viewer snapshot must be present.', [source.refs.p51_5_closure, source.refs.p51_5_capability_matrix, source.refs.p51_5_snapshot]));
